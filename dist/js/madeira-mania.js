...
🎉 Inicialização concluída com sucesso!`,"success"),j.retry.usarMutationObserver&&this.observarMudancasDOM()):this.tentativasDeCarregamento<j.retry.maxTentativas?setTimeout(()=>this.tentarCarregar(),j.retry.intervaloMs):this.log("❌ Número máximo de tentativas atingido.","error")}observarMudancasDOM(){this.log("ℹ️ MutationObserver desativado - dados carregados via dataProduct","info")}log(i,m="log",c=null){if(!j.debug)return;const h={info:"color: #2196f3; font-weight: bold",success:"color: #4caf50; font-weight: bold",warning:"color: #ff9800; font-weight: bold",error:"color: #f44336; font-weight: bold",log:"color: #666"};console.log(`%c${i}`,h[m]||h.log),c&&console.log(c)}carregarProdutos(){if(this.produtos=[],typeof dataProduct>"u"||!dataProduct.listaProdutosSugeridos)return this.log("❌ dataProduct ou listaProdutosSugeridos não encontrado","error"),!1;const i=dataProduct.listaProdutosSugeridos,m=dataProduct.produto,c=dataProduct.hostImagem||"";if(this.log(`📦 Encontrados ${i.length} produtos sugeridos + produto atual`,"info"),m&&m.complemento){const h=m.midia_path&&m.midia_arquivo_nome?`${c}/${m.midia_path}${m.midia_arquivo_nome}`:"",u={id:m.derivacao_id||m.produto_id,nomeCompleto:m.complemento.trim(),estoque:m.qtde_estoque,url:m.link?`/${m.link}`:"",imagem:h,imagemData:h,elemento:null,variacoes:{},nomeBase:"",nomeExibicao:"",isAtual:!0};this.produtoAtualId=u.id,this.extrairVariacoesDoNome(u),this.produtos.push(u),this.log(`   ✓ Produto atual: "${u.nomeCompleto}"`,"success")}return i.forEach((h,u)=>{const E=h.complemento||h.nome||"";if(!E)return;const w=h.derivacao_id||h.produto_id;if(w===this.produtoAtualId){this.log(`   ⏭️ Ignorando duplicata: "${E}"`,"info");return}const F=h.midia_path&&h.midia_arquivo_nome?`${c}/${h.midia_path}${h.midia_arquivo_nome}`:"",q={id:w||u,nomeCompleto:E.trim(),estoque:h.qtde_estoque,url:h.link?`/${h.link}`:"",imagem:F,imagemData:F,elemento:null,variacoes:{},nomeBase:"",nomeExibicao:"",isAtual:!1};this.extrairVariacoesDoNome(q),this.produtos.push(q),this.log(`   ✓ Sugerido: "${q.nomeCompleto}"`,"log")}),this.produtos.length===0?!1:(this.log("✅ Produtos carregados e processados:","success",this.produtos),!0)}normalizarSeparadores(i){const m=["–","—","−","‐","‑","⁃"];let c=i;return m.forEach(h=>{const u=new RegExp(`\\s${h}\\s`,"g");c=c.replace(u," - ")}),c}extrairVariacoesDoNome(i){const c=this.normalizarSeparadores(i.nomeCompleto).split(j.formatoNome.separador);this.log(`
📝 Processando: "${i.nomeCompleto}"`,"log"),j.formatoNome.primeiraParte==="nome_base"&&(i.nomeBase=c[0].trim(),c.shift()),c.forEach(h=>{const u=h.trim();if(u&&u.includes(j.formatoNome.separadorTipoValor)){const[E,...w]=u.split(j.formatoNome.separadorTipoValor),F=w.join(j.formatoNome.separadorTipoValor).trim(),q=this.normalizarTipo(E.trim());if(j.ignorarPalavras.includes(q))return;i.variacoes[q]=F,this.log(`   ✓ ${q}: ${F}`,"success")}}),i.nomeExibicao=j.formatoNome.exibirNomeCompleto?i.nomeCompleto:i.nomeBase||i.nomeCompleto,Object.keys(i.variacoes).length===0&&(i.variacoes.MODELO=i.nomeCompleto,i.nomeExibicao=i.nomeCompleto)}normalizarTipo(i){return i.toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").trim()}processarVariacoes(){this.log(`
🔄 Processando variações...`,"info");const i=new Set;this.produtos.forEach(m=>{Object.keys(m.variacoes).forEach(c=>i.add(c))}),i.forEach(m=>{const c=new Set,h={};this.produtos.forEach(E=>{const w=E.variacoes[m];w&&(c.add(w),h[w]||(h[w]=[]),h[w].push(E))});const u=Array.from(c).sort();this.variacoes[m]={label:j.labels[m]||m,valores:u,produtosPorValor:h,usarImagem:j.variacoesComImagem.includes(m)},this.log(`   📊 ${m}: ${u.length} valor(es) único(s) → [${u.join(", ")}]`,u.length>1?"success":"warning")}),this.log("✅ Variações processadas:","success",this.variacoes)}renderizarVariacoes(){this.log(`
🎨 Renderizando variações...`,"info");let i=b(j.selectors.areaVariacoes);if(i.length===0&&(this.criarAreaVariacoes(),i=b(j.selectors.areaVariacoes)),Object.keys(this.variacoes).length===0)return;const m=b("<div>",{class:"product-variations-pills-container"});let c=0;if(Object.keys(this.variacoes).forEach(h=>{const u=this.variacoes[h];if(u.valores.length<=1){this.log(`⏭️ Ignorando "${h}" - apenas ${u.valores.length} valor(es)`,"info");return}if(u.usarImagem){const E=this.criarGrupoSwatches(h,u);m.append(E),c++}}),Object.keys(this.variacoes).forEach(h=>{const u=this.variacoes[h];if(!(u.valores.length<=1)&&!u.usarImagem){const E=this.criarGrupoPills(h,u);m.append(E),c++}}),c===0){this.log("ℹ️ Nenhuma variação com múltiplas opções - ocultando área","info"),i.closest(".derivacoes-produto").hide(),b(j.selectors.subtituloProduto).hide();return}j.performance.useRequestAnimationFrame&&window.requestAnimationFrame?requestAnimationFrame(()=>{i.empty().append(m),this.log(`✅ ${c} variação(ões) renderizada(s)!`,"success"),this.atualizarNomeProduto()}):(i.empty().append(m),this.log(`✅ ${c} variação(ões) renderizada(s)!`,"success"),this.atualizarNomeProduto())}criarAreaVariacoes(){const i=b(j.selectors.areaProdutosSugeridos);i.length>0?i.before('<div class="derivacoes-produto"><div class="area-derivacoes"></div></div>'):b("body").prepend('<div class="derivacoes-produto"><div class="area-derivacoes"></div></div>')}criarGrupoSwatches(i,m){const c=this.obterValorAtualParaTipo(i),h=b("<div>",{class:"variation-pill-group variation-pill-group--swatches","data-variacao-tipo":i,role:"group","aria-labelledby":`pill-label-${i.toLowerCase()}`}),u=b("<div>",{class:"variation-pill-label",id:`pill-label-${i.toLowerCase()}`});u.append(b("<span>").text(m.label+":")),u.append(b("<span>",{class:"variation-pill-label-value","data-label-value":i}).text(c||""));const E=b("<div>",{class:"variation-swatches-container",role:"radiogroup","aria-labelledby":`pill-label-${i.toLowerCase()}`});return m.valores.forEach((w,F)=>{const q=m.produtosPorValor[w],M=q.some(mn=>mn.estoque===void 0||mn.estoque>0),S=w===c,J=`pill-${i.toLowerCase()}-${this.sanitizeId(w)}`,nn=this.encontrarMelhorProdutoParaSwatch(i,w,q);let R=null;nn&&(R=nn.imagemData||nn.imagem);const G=b("<input>",{type:"radio",class:"variation-pill-input",id:J,name:`variation-${i}`,value:w,"data-variacao-tipo":i,"data-produtos":JSON.stringify(q.map(mn=>({id:mn.id,url:mn.url}))),checked:S,disabled:!M,"aria-label":`${m.label}: ${w}${M?"":" (Indisponível)"}`}),O=b("<label>",{class:"variation-color-swatch-wrapper",for:J,"data-tooltip":w}),I=b("<div>",{class:"variation-color-swatch","data-valor":w,tabindex:S?0:-1});R?I.append(b("<img>",{src:R,alt:w,class:"variation-color-swatch-image",loading:"lazy"})):I.css({"background-color":"#E5E7EB",display:"flex","align-items":"center","justify-content":"center","font-size":"14px",color:"#6B7280"}).text(w.charAt(0).toUpperCase());const tn=b("<span>",{class:"variation-color-swatch-name",text:w,title:w});O.append(I).append(tn),E.append(G).append(O)}),h.append(u).append(E),h}criarGrupoPills(i,m){const c=this.obterValorAtualParaTipo(i),h=b("<div>",{class:"variation-pill-group variation-pill-group--pills","data-variacao-tipo":i,role:"group","aria-labelledby":`pill-label-${i.toLowerCase()}`}),u=b("<div>",{class:"variation-pill-label",id:`pill-label-${i.toLowerCase()}`});u.append(b("<span>").text(m.label+":")),u.append(b("<span>",{class:"variation-pill-label-value","data-label-value":i}).text(c||""));const E=b("<div>",{class:"variation-pills-container",role:"radiogroup","aria-labelledby":`pill-label-${i.toLowerCase()}`});return m.valores.forEach((w,F)=>{const q=m.produtosPorValor[w],M=q.some(O=>O.estoque===void 0||O.estoque>0),S=w===c,J=`pill-${i.toLowerCase()}-${this.sanitizeId(w)}`,nn=b("<input>",{type:"radio",class:"variation-pill-input",id:J,name:`variation-${i}`,value:w,"data-variacao-tipo":i,"data-produtos":JSON.stringify(q.map(O=>({id:O.id,url:O.url}))),checked:S,disabled:!M,"aria-label":`${m.label}: ${w}${M?"":" (Indisponível)"}`});let R=w;M||(R+=' <span class="variation-pill-badge">Indisponível</span>');const G=b("<label>",{class:"variation-pill",for:J,html:R,"data-valor":w,tabindex:S?0:-1});E.append(nn).append(G)}),h.append(u).append(E),h}sanitizeId(i){return i.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}encontrarMelhorProdutoParaSwatch(i,m,c){const h=this.produtos.find(F=>F.isAtual||F.id===this.produtoAtualId);if(!h||c.length===0)return c[0]||null;if(c.length===1)return c[0];const u=h.variacoes;let E=null,w=-1;return c.forEach(F=>{let q=0;Object.keys(u).forEach(M=>{M!==i&&F.variacoes[M]===u[M]&&q++}),(F.imagemData||F.imagem)&&(q+=.5),q>w&&(w=q,E=F)}),this.log(`   🎯 Melhor produto para ${i}="${m}": score=${w}`,"log"),E||c[0]}obterValorAtualParaTipo(i){const m=this.produtos.find(c=>c.isAtual||c.id===this.produtoAtualId);return m?m.variacoes[i]:null}atualizarNomeProduto(){const i=this.produtos.find(c=>c.isAtual||c.id===this.produtoAtualId);if(!i)return;["h1.nome-produto",".product-name h1",".info-produto h1",'h1[itemprop="name"]',".box-info-produto h1"].forEach(c=>{const h=b(c);h.length>0&&h.text(i.nomeExibicao)})}bindEventos(){this.log(`
🔗 Vinculando eventos...`,"info"),b(document).on("change",".variation-pill-input",i=>{this.aoMudarVariacao(i)}),b(document).on("keydown",".variation-pills-container, .variation-swatches-container",i=>{this.handleKeyboardNavigation(i)}),b(document).on("click",".variation-pill, .variation-color-swatch-wrapper",function(){const i=b(this).is("label")?b("#"+b(this).attr("for")):b(this).closest("label").prev(".variation-pill-input");i.length&&!i.prop("disabled")&&b(this).closest(".variation-pill-group").addClass("is-loading")}),this.log("✅ Eventos vinculados","success")}handleKeyboardNavigation(i){const c=b(i.currentTarget).find(".variation-pill-input:not(:disabled)"),h=b(document.activeElement);if(!h.hasClass("variation-pill-input"))return;const u=c.index(h);let E=u;switch(i.key){case"ArrowRight":case"ArrowDown":i.preventDefault(),E=(u+1)%c.length;break;case"ArrowLeft":case"ArrowUp":i.preventDefault(),E=u-1<0?c.length-1:u-1;break;case"Home":i.preventDefault(),E=0;break;case"End":i.preventDefault(),E=c.length-1;break;default:return}c.eq(E).focus().prop("checked",!0).trigger("change")}aoMudarVariacao(i){const m=b(i.target),c=m.data("variacao-tipo"),h=m.val();this.log(`
🔄 Variação selecionada: ${c} = ${h}`,"info"),b(`.variation-pill-label-value[data-label-value="${c}"]`).text(h);const u={};b(".variation-pill-input:checked").each(function(){const w=b(this).data("variacao-tipo"),F=b(this).val();F&&(u[w]=F)}),this.log("📋 Seleção atual:","info",u);const E=this.encontrarProdutoPorVariacoes(u);if(E)this.log("✅ Produto encontrado!","success",E),this.navegarParaProduto(E);else{this.log("⚠️ Produto exato não encontrado, buscando melhor correspondência...","warning");const w=this.encontrarMelhorCorrespondencia(u);w?(this.log("✅ Melhor correspondência encontrada!","success",w),this.navegarParaProduto(w)):(this.log("❌ Nenhum produto correspondente encontrado","error"),b(".variation-pill-group").removeClass("is-loading"))}}encontrarProdutoPorVariacoes(i){return this.produtos.find(m=>Object.keys(i).every(c=>m.variacoes[c]===i[c]))}encontrarMelhorCorrespondencia(i){let m=null,c=0;return this.produtos.forEach(h=>{let u=0;Object.keys(i).forEach(E=>{h.variacoes[E]===i[E]&&u++}),u>c&&(c=u,m=h)}),c>0?m:null}navegarParaProduto(i){this.log(`
🚀 Navegando para: ${i.url}`,"info"),i.url?window.location.href=i.url:(this.log("❌ URL não encontrada para navegação","error"),b(".variation-pill-group").removeClass("is-loading"))}}setTimeout(function(){b(".product-variations-pills-container").remove(),b(".derivacoes-produto").remove();const f=new B;f.init(),window.GerenciadorVariacoesPillsMagazord=f},100)})(typeof jQuery<"u"?jQuery:window.jQuery||window.$),(function(){"use strict";function b(){if(!(typeof jQuery>"u"&&typeof $>"u")){var o=typeof jQuery<"u"?jQuery:$;o(document).ajaxComplete(function(l,p,v){v.url&&v.url.indexOf("checkout/cart")!==-1&&setTimeout(function(){window.dispatchEvent(new CustomEvent("reactItemAddedToCart"))},100)})}}function P(){var o=document.querySelector("#cart-preview-area .border-t.border-solid");if(!(!o||o.querySelector(".installment-total"))){var l=0,p=document.querySelectorAll("#cart-preview-area .cart-item");if(p.forEach(function(L){var _=parseFloat(L.getAttribute("data-item-price"))||0,Q=parseInt(L.getAttribute("data-item-quantity"))||1;l+=_*Q}),!(l<=0)){var v=(l/12).toFixed(2).replace(".",","),A=document.createElement("div");A.className="installment-total",A.textContent="ou 12x de R$ "+v;var y=o.querySelector(".valor-pix");y&&y.parentNode&&y.parentNode.insertBefore(A,y.nextSibling)}}}var j='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><path d="M5 12h14"/></svg>',B='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg>',f='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v2"/></svg>';function i(){P();var o=document.querySelectorAll("#cart-preview-area .cart-item, .content-cart .cart-item");o.forEach(function(l){if(!l.querySelector(".qty-btn-minus")){var p=l.querySelector(".qtd-value");if(p){var v=l.querySelector(".cart-remove-item"),A=v?v.getAttribute("data-id"):null;if(A){var y=l.querySelector(".prod-remove");y&&!p.contains(v)&&(p.appendChild(v),y.style.display="none");var L=p.parentElement,_=null;if(L)for(var Q=0;Q<L.children.length;Q++){var V=L.children[Q];if(V!==p&&V.classList&&V.classList.contains("valor")){_=V;break}}_&&!p.contains(_)&&p.appendChild(_);var Y=parseInt(l.getAttribute("data-item-quantity"));if(!Y||isNaN(Y)){var t=p.textContent.match(/(\d+)/);Y=t?parseInt(t[1]):1}var a=document.createElement("button");a.className="qty-btn-minus",a.type="button",a.setAttribute("aria-label","Diminuir quantidade"),a.innerHTML=j,a.addEventListener("click",function(s){s.preventDefault(),s.stopPropagation();var d=parseInt(n.textContent)||1;if(d<=1){var g=l.querySelector(".cart-remove-item");g&&g.click();return}ft(l,A,-1,n,a,e)});var n=document.createElement("span");n.className="qty-display",n.textContent=Y;var e=document.createElement("button");e.className="qty-btn-plus",e.type="button",e.setAttribute("aria-label","Aumentar quantidade"),e.innerHTML=B,e.addEventListener("click",function(s){s.preventDefault(),s.stopPropagation(),ft(l,A,1,n,a,e)});var r=document.createElement("div");r.className="mm-qty-wrap",r.appendChild(a),r.appendChild(n),r.appendChild(e),p.insertBefore(r,p.firstChild),v&&(v.innerHTML=f,v.setAttribute("aria-label","Remover produto"))}}}})}function m(){document.addEventListener("click",function(o){var l=o.target.closest(".cart-remove-item");if(!(!l||!l.closest("#cart-preview-area"))){o.preventDefault(),o.stopPropagation(),o.stopImmediatePropagation();var p=l.getAttribute("data-id");if(p){var v=l.closest(".cart-item"),A=v&&v.querySelector(".prod-nome")?.textContent?.trim()||"este produto",y=A.length>50?A.substring(0,50)+"…":A,L=document.getElementById("mm-confirm-overlay");L&&L.remove();var _=document.createElement("div");_.id="mm-confirm-overlay",_.className="mm-confirm-overlay",_.innerHTML='<div class="mm-confirm-card"><p class="mm-confirm-title">Remover produto?</p><p class="mm-confirm-desc">'+y.replace(/&/g,"&amp;").replace(/</g,"&lt;")+'</p><div class="mm-confirm-actions"><button class="mm-confirm-btn mm-confirm-btn-cancel">Manter</button><button class="mm-confirm-btn mm-confirm-btn-delete">Remover</button></div></div>',document.body.appendChild(_),_.querySelector(".mm-confirm-btn-cancel").addEventListener("click",function(){_.remove()}),_.querySelector(".mm-confirm-btn-delete").addEventListener("click",function(){_.remove(),window.__mmDeleteItem&&v?window.__mmDeleteItem(v,p):typeof Zord<"u"&&Zord.checkout&&Zord.checkout.deleteItem(parseInt(p))}),_.addEventListener("click",function(Q){Q.target===_&&_.remove()})}}},!0)}function c(){document.addEventListener("click",function(o){var l=o.target.closest(".finalizar-compra");l&&l.closest("#cart-preview-area")&&(o.preventDefault(),o.stopPropagation(),window.location.href="/checkout/identify")},!0)}function h(o,l){var p="cep=&cupom-desconto=&nenhumCreditoSelecionado=true&id="+encodeURIComponent(String(l))+"&area=main-cart";return fetch("/checkout/cart?operation="+encodeURIComponent(o),{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/x-www-form-urlencoded",Accept:"text/html,application/json,*/*","X-Requested-With":"XMLHttpRequest"},body:p}).then(function(v){if(!v.ok)throw new Error("HTTP "+v.status);return v.text()})}var u=1500,E=2e3,w="mm_cep",F="mm_cart_snapshot",q=1800*1e3;function M(){try{var o=localStorage.getItem(w)||"",l=o.replace(/\D/g,"");if(l.length===8)return l}catch{}return null}function S(o){return!o||o.length!==8?"":o.slice(0,5)+"-"+o.slice(5)}function J(o){if(!o||o.length!==8)return E;var l=parseInt(o.slice(0,2),10);return isNaN(l)?E:l>=1&&l<=39||l>=80&&l<=99?u:E}function nn(){try{var o=localStorage.getItem(F);if(!o)return null;var l=JSON.parse(o);return!l||!l.ts||Date.now()-l.ts>q?null:l}catch{return null}}function R(o){var l=[];return o.forEach(function(p){var v=(p.querySelector(".prod-nome a, .prod-nome")||{}).textContent||"",A=p.querySelector(".qty-display"),y=A?parseInt(A.textContent):parseInt(p.getAttribute("data-item-quantity"))||1;l.push(v.trim().slice(0,30)+"x"+y)}),l.sort().join("|")}function G(o){if(!o||!Array.isArray(o.items))return"";var l=o.items.map(function(p){return(p.name||"").trim().slice(0,30)+"x"+(p.quantity||1)});return l.sort().join("|")}var O='<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><polyline points="20 6 9 17 4 12"/></svg>',I='<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M18 6 6 18M6 6l12 12"/></svg>',tn=["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"];function mn(o){if(!o)return null;var l=String(o).match(/\d+/g);return!l||!l.length?null:Math.max.apply(null,l.map(Number))}function dn(o,l){for(var p=new Date(o.getTime()),v=0;v<l;){p.setDate(p.getDate()+1);var A=p.getDay();A!==0&&A!==6&&v++}return p}function un(o){var l=new Date,p="dia "+o.getDate()+" de "+tn[o.getMonth()];return o.getFullYear()!==l.getFullYear()&&(p+=" de "+o.getFullYear()),p}function nt(o){var l=mn(o);if(!l||l<1)return null;var p=dn(new Date,l);return"Receba até "+un(p)}var hn={},Wn=4e3,_n={};function wn(o,l){if(!o||o.length!==8)return Promise.resolve(null);if(hn[o])return hn[o];if(!l){var p=_n[o]||0;if(Date.now()-p<Wn)return Promise.resolve(null)}var v="cep="+encodeURIComponent(o.slice(0,5)+"-"+o.slice(5))+"&cupom-desconto=&nenhumCreditoSelecionado=true&area=main-cart",A=fetch("/checkout/cart?operation=atualizaValoresCarrinho",{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/x-www-form-urlencoded",Accept:"text/html,application/json,*/*","X-Requested-With":"XMLHttpRequest"},body:v}).then(function(y){if(!y.ok)throw new Error("HTTP "+y.status);return y.text()}).then(function(y){return _n[o]=Date.now(),Zn(y)}).catch(function(){return null}).then(function(y){return delete hn[o],y});return hn[o]=A,A}function Zn(o){try{var l=new DOMParser().parseFromString(o,"text/html"),p=l.querySelector("#resumo-compra .frete-calculado")||l.querySelector(".frete-calculado");if(!p)return null;var v="",A=p.querySelector(".frete-location .city");A&&(v=A.textContent.trim());var y=null,L="",_="",Q=p.querySelector(".info-frete-selec");if(Q){var V=Q.querySelector(".dias-entrega"),Y=Q.querySelector(".info-title span, .info-title");V&&(L=(V.textContent||"").trim()),Y&&(_=(Y.textContent||"").trim())}var t=p.querySelector(".line.valor-frete .value, .value.valor-frete")||p.querySelector(".valor-compra-frete .value");if(t){var a=(t.textContent||"").trim();if(/gr[áa]tis/i.test(a))y=0;else{var n=a.match(/[\d.,]+/);if(n){var e=parseFloat(n[0].replace(/\./g,"").replace(",","."));isNaN(e)||(y=e)}}}if(y==null){var r=p.querySelector(".servico-frete");if(r){var s=parseFloat(r.getAttribute("data-valor-frete")||"0");if(isNaN(s)||(y=s),_||(_=r.getAttribute("data-servico-frete")||""),!L){var d=r.querySelector(".dias-entrega");d&&(L=(d.textContent||"").trim())}}}if(y==null)return null;var g=null,x=l.querySelector(".total-boleto .value, .totais-valor.total-boleto .value");if(x){var k=(x.textContent||"").trim(),N=k.match(/[\d.,]+/);if(N){var T=parseFloat(N[0].replace(/\./g,"").replace(",","."));isNaN(T)||(g=T)}}return{city:v,shipping:y,shippingDeadline:L,shippingName:_,totalPix:g}}catch{return null}}function In(o,l,p){if(!(!o||!p)){Vn(o);try{let Q=function(V){for(var Y=0;Y<A.length;Y++)if(A[Y]&&A[Y].name===V)return A[Y];return null};var v=nn()||{};v.ts=Date.now(),v.cepValue=l.slice(0,5)+"-"+l.slice(5),v.shipping=p.shipping,v.shippingDeadline=p.shippingDeadline,v.shippingName=p.shippingName,v.shippingCity=p.city,p.totalPix!=null&&(v.subtotalPix=p.totalPix);var A=v.items&&v.items.length?v.items:[],y=o.querySelectorAll(".cart-item:not(.mm-removing)");v.items=Array.prototype.map.call(y,function(V){var Y=V.querySelector(".prod-nome a, .prod-nome"),t=(Y&&Y.textContent||"").trim(),a=V.querySelector(".qty-display"),n=a?parseInt(a.textContent):parseInt(V.getAttribute("data-item-quantity"))||1,e=Q(t);return e&&e.quantity===n&&(e.lineTotalPix>0||e.lineTotal>0||e.imgSrc)?e:{name:t,quantity:n}}),localStorage.setItem(F,JSON.stringify(v))}catch{}var L=o.querySelectorAll(".cart-item:not(.mm-removing)"),_=0;L.forEach(function(Q){var V=Q.querySelector(".valor");if(V){var Y=qn(V.textContent);isNaN(Y)||(_+=Y)}}),o.querySelector(".box-total-btn")?Hn(o):An(o)}}function Ln(o){var l=M();if(l){var p=nn(),v=G(p),A=R(o.querySelectorAll(".cart-item:not(.mm-removing)")),y=p&&p.cepValue&&p.cepValue.replace(/\D/g,"")===l,L=p&&p.shipping!=null&&!isNaN(p.shipping);p&&v===A&&y&&L||wn(l).then(function(_){_&&In(o,l,_)})}}function kn(o){return String(o||"").replace(/[&<>"']/g,function(l){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[l]})}function Pn(o){if(!o)return null;var l=o.querySelector(".box-total-btn");if(l)return{host:l,before:l.querySelector(".total")};var p=o.querySelector(".area-finalizar-compra");if(p)return{host:p,before:p.firstElementChild};var v=o.querySelector(".finalizar-compra");if(v&&v.parentElement){var A=v.parentElement;return{host:A,before:A.firstElementChild}}return null}function it(o){if(!o)return null;var l=o.closest(".carrinho-rapido-ctn");return l||(o.closest("#cart-preview-area")?ht():null)}function zn(o,l,p,v){if(o){var A=Pn(o);if(A){var y=A.host;o.classList.add("mm-ship-scope");var L=M(),_=nn(),Q=R(o.querySelectorAll(".cart-item:not(.mm-removing)")),V=G(_),Y=_&&V===Q,t=J(L),a=l>=t,n=Math.max(0,t-l),e=Math.max(0,Math.min(100,l/t*100)),r=y.querySelector(".mm-cart-ship");if(!r){r=document.createElement("div"),r.className="mm-cart-ship",r.setAttribute("role","group"),r.setAttribute("aria-label","Informações de frete");var s=A.before;s&&s.parentNode===y?y.insertBefore(r,s):y.insertBefore(r,y.firstChild)}if(r.classList.toggle("is-free",a),vt(r),r.dataset.mmEditing!=="1"){var d=_&&_.cepValue&&_.cepValue.replace(/\D/g,"")===L,g=L&&Y&&d&&_.shippingCity,x=g?nt(_.shippingDeadline):null,k="";if(k+='<div class="mm-cart-ship-location">',L){var N=S(L);if(g&&(N+=" · "+kn(_.shippingCity)),k+='<span class="mm-cart-ship-label">Envio para</span><div class="mm-cart-ship-location-value"><span>'+N+'</span><button class="mm-cart-ship-edit" type="button" data-mm-ship="edit">Trocar</button></div>',x){var T=kn(x);v&&p>0?T+=" · <strong>"+kn(bn(p))+"</strong>":v&&p===0&&(T+=" · <strong>Grátis</strong>"),k+='<span class="mm-cart-ship-deadline">'+T+"</span>"}}else k+='<span class="mm-cart-ship-label">Calcule o frete</span><div class="mm-cart-ship-location-value"><span style="color:#9CA3AF;font-weight:500;">Informe seu CEP</span><button class="mm-cart-ship-edit" type="button" data-mm-ship="edit">Calcular</button></div>';k+="</div>";var W=a?"Frete grátis desbloqueado":"Faltam "+bn(n)+" para frete grátis",D=parseFloat(o.dataset.mmShipPct||"0")||0;k+='<div class="mm-cart-ship-progress">',k+='<div class="mm-cart-ship-bar" role="progressbar" aria-valuenow="'+Math.round(l)+'" aria-valuemin="0" aria-valuemax="'+Math.round(t)+'" aria-valuetext="'+kn(W)+'" aria-label="Progresso para frete grátis"><div class="mm-cart-ship-bar-fill" style="width:'+D.toFixed(1)+'%"></div></div>',k+='<p class="mm-cart-ship-nudge'+(a?" is-free":"")+'">',a?k+=O+"Frete grátis garantido":k+="Faltam <strong>"+kn(bn(n))+"</strong> para frete grátis",k+="</p>",k+="</div>",r.innerHTML=k;var X=r.querySelector(".mm-cart-ship-bar-fill");X&&requestAnimationFrame(function(){X.style.width=e.toFixed(1)+"%"});var U=o.dataset.mmShipWasFree==="1";a&&!U&&D>0&&(r.classList.remove("mm-just-unlocked"),r.offsetWidth,r.classList.add("mm-just-unlocked"),setTimeout(function(){r.classList.remove("mm-just-unlocked")},1400)),o.dataset.mmShipWasFree=a?"1":"",o.dataset.mmShipPct=e.toFixed(1)}}}}var mt='<svg class="mm-cart-ship-spinner" width="14" height="14" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><style>.mm-sp{animation:mm-spin 1.2s linear infinite;transform-origin:center}.mm-sp circle{stroke:#4B664A;stroke-width:3;fill:none;stroke-linecap:round;stroke-dasharray:44;stroke-dashoffset:16;animation:mm-sp-dash 1.2s ease-in-out infinite}@keyframes mm-spin{to{transform:rotate(360deg)}}@keyframes mm-sp-dash{0%{stroke-dashoffset:44}50%{stroke-dashoffset:8}100%{stroke-dashoffset:44}}</style><g class="mm-sp"><circle cx="12" cy="12" r="9.5"/></g></svg>';function tt(o){if(o){var l=o.querySelector(".mm-cart-ship-deadline");if(l)l.innerHTML="Recalculando frete "+mt;else{var p=o.querySelector(".mm-cart-ship-location");if(p){var v=document.createElement("span");v.className="mm-cart-ship-deadline",v.innerHTML="Recalculando frete "+mt,p.appendChild(v)}}var A=Pn(o);A&&A.host.classList.add("mm-ship-loading")}}function Vn(o){if(o){var l=Pn(o);l&&l.host.classList.remove("mm-ship-loading")}}function vt(o){!o||o.dataset.mmShipBound||(o.dataset.mmShipBound="1",o.addEventListener("click",function(l){var p=l.target.closest('[data-mm-ship="edit"]');if(p){l.preventDefault(),l.stopPropagation(),ut(o);return}var v=l.target.closest('[data-mm-ship="cancel"]');if(v){l.preventDefault(),l.stopPropagation(),st(o);return}l.target.closest(".mm-cart-ship-cep-form")&&l.stopPropagation()},!0))}function ut(o){var l=o.querySelector(".mm-cart-ship-location");if(l){o.dataset.mmEditing="1";var p=M()||"";l.innerHTML='<span class="mm-cart-ship-label">Digite seu CEP</span><form class="mm-cart-ship-cep-form" data-mm-ship-form="1" novalidate><input class="mm-cart-ship-cep-input" type="text" inputmode="numeric" autocomplete="postal-code" placeholder="00000-000" maxlength="9" value="'+kn(S(p))+'" aria-label="CEP" /><button class="mm-cart-ship-cep-save" type="submit">OK</button><button class="mm-cart-ship-cep-cancel" type="button" data-mm-ship="cancel" aria-label="Cancelar">'+I+"</button></form>";var v=l.querySelector(".mm-cart-ship-cep-input"),A=l.querySelector("form");v&&(setTimeout(function(){try{v.focus(),v.select()}catch{}},10),v.addEventListener("input",function(){v.classList.remove("is-invalid");var y=v.value.replace(/\D/g,"").slice(0,8);v.value=y.length>5?y.slice(0,5)+"-"+y.slice(5):y}),v.addEventListener("keydown",function(y){y.key==="Escape"&&(y.preventDefault(),st(o))})),A&&A.addEventListener("submit",function(y){y.preventDefault(),y.stopPropagation(),wt(o)})}}function st(o){o.dataset.mmEditing="";var l=it(o);if(l){var p=l.querySelectorAll(".cart-item:not(.mm-removing)"),v=0;p.forEach(function(A){var y=A.querySelector(".valor");if(y){var L=qn(y.textContent);isNaN(L)||(v+=L)}}),zn(l,v)}}function wt(o){var l=o.querySelector(".mm-cart-ship-cep-input");if(l){var p=l.value.replace(/\D/g,"");if(p.length!==8){l.classList.add("is-invalid"),l.focus();return}try{localStorage.setItem(w,p)}catch{}var v=o.querySelector(".mm-cart-ship-cep-save");v&&(v.disabled=!0,v.textContent="...");var A=it(o);try{var y=nn();y&&(y.cepValue="",localStorage.setItem(F,JSON.stringify(y)))}catch{}st(o),wn(p).then(function(L){L&&A&&In(A,p,L)})}}function qn(o){if(!o)return NaN;var l=String(o).replace(/\s/g,"").match(/([\d.,]+)/);return l?parseFloat(l[1].replace(/\./g,"").replace(",",".")):NaN}function bn(o){return isNaN(o)?"":"R$ "+o.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}function rn(o){if(isNaN(o))return"";var l=o.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),p=l.split(",");return"R$&nbsp;"+p[0]+'<span class="mm-cents">,'+(p[1]||"00")+"</span>"}function et(o){var l=0;return Array.prototype.forEach.call(o,function(p){var v=parseFloat(p.getAttribute("data-item-price"))||0,A=parseInt(p.getAttribute("data-item-quantity"));if(!A||isNaN(A)){var y=p.querySelector(".qty-display");y?A=parseInt(y.textContent)||1:A=1}l+=v*A}),l}function lt(o){var l=o.querySelectorAll(".cart-item:not(.mm-removing)");l.forEach(function(p){var v=parseFloat(p.getAttribute("data-item-price"))||0,A=parseInt(p.getAttribute("data-item-quantity"));if(!A||isNaN(A)){var y=p.querySelector(".qty-display");y?A=parseInt(y.textContent)||1:A=1}var L=p.querySelector(".valor");L&&v>0&&(L.innerHTML=rn(v*A))})}function Qn(o){if(!(!o||o.dataset.mmTotalRatio)){var l=o.querySelectorAll(".cart-item");if(l.length){var p=et(l),v=o.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||o.querySelector(".box-total-btn .linha-total .valor-final strong");if(v&&p>.01){var A=qn(v.textContent);isNaN(A)||(o.dataset.mmTotalRatio=String(A/p))}}}}function pt(o,l){var p=o.querySelector(".box-total-btn .linha-total");if(p){var v=p.parentElement.querySelector(".mm-cart-savings");if(v&&v.remove(),!(!l||l<.01)){var A=document.createElement("span");A.className="mm-cart-savings",A.textContent="Você economiza "+bn(l)+" com PIX",p.nextSibling?p.parentElement.insertBefore(A,p.nextSibling):p.parentElement.appendChild(A)}}}function Hn(o,l){if(o){Qn(o);var p=o.querySelectorAll(".cart-item:not(.mm-removing)"),v=et(p),A=o.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||o.querySelector(".box-total-btn .linha-total .valor-final strong"),y=parseFloat(o.dataset.mmTotalRatio||"0.95")||.95,L=v*y,_=v-L,Q=M(),V=nn(),Y=G(V),t=R(p),a=V&&V.cepValue&&V.cepValue.replace(/\D/g,"")===Q,n=!!(Q&&V&&a&&V.shipping!=null&&!isNaN(V.shipping)),e=o.dataset.mmShipPendingFetch==="1";!n&&e&&Q&&V&&V.shipping!=null&&(n=!0);var r=n?parseFloat(V.shipping):0,s=L+r,d=v+r;if(A){var g=qn(A.textContent);if(l)(isNaN(g)||Math.abs(s-g)>.005)&&(A.innerHTML=rn(s));else if(!isNaN(g)&&Math.abs(s-g)>.005){var x=o.querySelector(".box-total-btn .linha-total .valor-final");x&&(x.classList.remove("mm-pop"),x.offsetWidth,x.classList.add("mm-pop"),setTimeout(function(){x.classList.remove("mm-pop")},450)),ct(A,g,s)}else A.innerHTML=rn(s)}var k=o.querySelector(".box-total-btn .valor-final.card");if(k){var N=d/12;k.innerHTML="<span>ou em até <strong>12x</strong> de <strong>"+bn(N)+"</strong> no cartão</span>"}var T=o.querySelector(".box-total-btn .linha-total .valor-final .tipo-pagto");T&&(T.textContent="No PIX"),pt(o,_),zn(o,v,r,n);try{var W=0;p.forEach(function(X){var U=X.querySelector(".qty-display");U&&(W+=parseInt(U.textContent)||0)});var D=document.getElementById("mm-h-cart-count");D&&(W>0?(D.textContent=W>99?"99+":String(W),D.hidden=!1):D.hidden=!0)}catch{}}}function Gn(){var o=document.querySelector(".carrinho-rapido-ctn");if(!(!o||!o.querySelector(".box-total-btn"))){var l=o.querySelectorAll(".cart-item:not(.mm-removing)");if(l.length){var p=o.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||o.querySelector(".box-total-btn .linha-total .valor-final strong");if(p){var v=et(l);if(v>.01){var A=parseFloat(o.dataset.mmTotalRatio||"0.95")||.95,y=M(),L=nn(),_=L&&L.cepValue&&L.cepValue.replace(/\D/g,"")===y,Q=!!(y&&L&&_&&L.shipping!=null&&!isNaN(L.shipping));!Q&&o.dataset.mmShipPendingFetch==="1"&&y&&L&&L.shipping!=null&&(Q=!0);var V=Q?parseFloat(L.shipping):0,Y=v*A+V,t=qn(p.textContent);(isNaN(t)||Math.abs(Y-t)>.01)&&Hn(o,!0)}}}}}function ht(){var o=document.querySelector('#cart-preview-area > div[class*="z-[9999]"]');if(o)return o;var l=document.querySelector(".carrinho-rapido-ctn");return l&&!l.querySelector(".box-total-btn")&&l.querySelector(".valor-pix")?l:null}function zt(o,l){var p=o.querySelector(".installment-total");if(!(!p||!p.parentElement)){var v=p.parentElement,A=v.querySelector(".mm-cart-savings-mobile");if(!l||l<.01){A&&A.remove();return}var y="Você economiza "+bn(l)+" com PIX";if(A){A.textContent!==y&&(A.textContent=y);return}var L=document.createElement("span");L.className="mm-cart-savings-mobile",L.textContent=y,p.nextSibling?v.insertBefore(L,p.nextSibling):v.appendChild(L)}}function Sn(o,l,p){if(!(!o||!o.classList||!o.classList.contains("carrinho-rapido-ctn"))&&!o.querySelector(".box-total-btn")){var v=o.querySelector(".area-finalizar-compra");if(!(!v||!(l>0))){var A=v.querySelector(".forma-pix"),y=A?A.parentElement:null;if(y){var L=l*p,_=l/12,Q=l-L;y.classList.add("mm-native-pay-hidden");var V=v.querySelector(".mm-cart-total-b");V||(V=document.createElement("div"),V.className="mm-cart-total-b",V.innerHTML='<span class="mm-tb-label">Total</span><span class="mm-tb-row"><strong class="mm-tb-value"></strong><span class="mm-tb-pix">No PIX</span></span><span class="mm-tb-savings"></span><span class="mm-tb-parcela"></span>',y.nextSibling?v.insertBefore(V,y.nextSibling):v.appendChild(V));var Y=V.querySelector(".mm-tb-value"),t=bn(L);Y&&Y.textContent!==t&&(Y.textContent=t);var a=V.querySelector(".mm-tb-savings");if(a)if(Q>=.01){var n="Você economiza "+bn(Q)+" com PIX";a.textContent!==n&&(a.textContent=n),a.style.display=""}else a.style.display="none";var e=V.querySelector(".mm-tb-parcela");if(e){var r="ou em até 12x de "+bn(_)+" no cartão";e.textContent!==r&&(e.textContent=r)}}}}}function An(o){if(o=o||ht(),!(!o||o.querySelector(".box-total-btn")||!o.querySelector(".valor-pix"))){var l=o.querySelectorAll(".cart-item:not(.mm-removing)"),p=l.length,v=document.getElementById("mm-h-cart-count");if(v&&p>0){var A=p>99?"99+":String(p);(v.textContent!==A||v.hidden)&&(v.textContent=A,v.hidden=!1)}if(l.length){var y=et(l);if(y>0){Array.prototype.forEach.call(l,function(D){var X=parseFloat(D.getAttribute("data-item-price"))||0;if(X>0){var U=parseInt(D.getAttribute("data-item-quantity"));if(!U||isNaN(U)){var an=D.querySelector(".qty-display");U=an&&parseInt(an.textContent)||1}var en=D.querySelector(".valor");if(en){var pn=X*U,cn=en.querySelector("span")||en,yn=qn(cn.textContent);(isNaN(yn)||Math.abs(yn-pn)>.005)&&(cn.textContent=bn(pn))}}});var L=o.querySelector(".valor-pix"),_=L?L.querySelector("span")||L:null;if(!o.dataset.mmMobileRatio&&_){var Q=qn(_.textContent);if(!isNaN(Q)&&Q>0){var V=Q/y;V>.8&&V<=1.0001&&(o.dataset.mmMobileRatio=String(V))}}var Y=parseFloat(o.dataset.mmMobileRatio||"0.95");if(Y>.8&&Y<=1.0001||(Y=.95),_){var t=y*Y,a=qn(_.textContent);(isNaN(a)||Math.abs(a-t)>.005)&&(_.textContent=bn(t))}var n=o.querySelector(".installment-total");if(n){var e=y/12,r=qn(n.textContent);(isNaN(r)||Math.abs(r-e)>.005)&&(n.textContent="ou 12x de "+bn(e))}var s=M(),d=nn(),g=d&&d.cepValue&&d.cepValue.replace(/\D/g,"")===s,x=!!(s&&d&&g&&d.shipping!=null&&!isNaN(d.shipping)),k=x?parseFloat(d.shipping):0,N=(s||"")+"|"+y.toFixed(2)+"|"+(x?1:0)+"|"+k,T=o.querySelector(".mm-cart-ship"),W=T&&T.dataset.mmEditing==="1";!W&&(!T||o.dataset.mmMobShipSig!==N)&&(o.dataset.mmMobShipSig=N,zn(o,y,k,x)),Ln(o),zt(o,y-y*Y),Sn(o,y,Y)}}}}function ct(o,l,p){if(!o||isNaN(l)||isNaN(p))return;var v=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;if(v){o.innerHTML=rn(p);return}var A=420,y=performance.now();function L(_){return 1-Math.pow(1-_,3)}(function _(Q){var V=Math.min(1,(Q-y)/A);o.innerHTML=rn(l+(p-l)*L(V)),V<1&&requestAnimationFrame(_)})(y)}function ft(o,l,p,v,A,y){if(!(!o||!l)){var L=parseInt(v.textContent)||1,_=L+p;if(!(_<1)){var t=o.closest(".carrinho-rapido-ctn");Qn(t),A.disabled=!0,y.disabled=!0;var Q=parseFloat(o.getAttribute("data-item-price"))||0;v.textContent=_,o.setAttribute("data-item-quantity",_);var V=o.querySelector(".valor");if(V){var Y=Q*_;V.textContent=bn(Y),V.classList.remove("mm-pop"),V.offsetWidth,V.classList.add("mm-pop"),setTimeout(function(){V.classList.remove("mm-pop")},450)}var t=o.closest(".carrinho-rapido-ctn"),a=p>0?"adicionaItem":"removeItem";M()&&t&&(t.dataset.mmShipPendingFetch="1"),Hn(t),An(),M()&&tt(t),h(a,l).catch(function(){v.textContent=L,o.setAttribute("data-item-quantity",L),V&&(V.innerHTML=rn(Q*L)),t&&(t.dataset.mmShipPendingFetch=""),Hn(t),An()}).then(function(){A.disabled=!1,y.disabled=!1;var n=M();n&&t?(tt(t),wn(n,!0).then(function(e){t.dataset.mmShipPendingFetch="",e?In(t,n,e):Vn(t)})):t&&(t.dataset.mmShipPendingFetch="")})}}}function bt(o,l){if(!(!o||!l)){var p=o.closest(".carrinho-rapido-ctn");Qn(p);var v=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches,A=v?0:360;v||o.classList.add("mm-removing"),setTimeout(function(){o.parentNode&&o.parentNode.removeChild(o);var y=p?p.querySelectorAll(".cart-item:not(.mm-removing)"):[],L=y.length===0;if(L){if(p){p.dataset.mmShipPendingFetch="";var _=p.querySelector(".mm-cart-ship");_&&_.remove();var Q=p.querySelector(".box-total-btn, .area-finalizar-compra");Q&&(Q.style.display="none")}typeof window.__mmForceEmptyCartState=="function"&&window.__mmForceEmptyCartState(p)}else Hn(p),M()&&tt(p);var V=document.getElementById("mm-h-cart-count"),Y=document.getElementById("mm-h-cart"),t=y.length;V&&(V.textContent=t>99?"99+":String(t),V.hidden=t===0),Y&&Y.setAttribute("aria-label","Carrinho, "+t+" "+(t===1?"item":"itens")),An()},A),h("deleteItem",l).catch(function(){}).then(function(){var y=p?p.querySelectorAll(".cart-item:not(.mm-removing)"):[],L=document.getElementById("mm-h-cart-count");if(L&&(L.textContent=y.length>99?"99+":String(y.length),L.hidden=y.length===0),y.length===0){p&&(p.dataset.mmShipPendingFetch="");return}var _=M();_&&p?(p.dataset.mmShipPendingFetch="1",tt(p),wn(_,!0).then(function(Q){p.dataset.mmShipPendingFetch="",Q?In(p,_,Q):Vn(p)})):p&&(p.dataset.mmShipPendingFetch="")})}}window.__mmDeleteItem=bt;var qt=null,gt=new Set,Yn=null;function Cn(o){if(!o)return NaN;var l=String(o).replace(/\s/g,"").match(/([\d.,]+)/);if(!l)return NaN;var p=l[1].replace(/\./g,"").replace(",","."),v=parseFloat(p);return isNaN(v)?NaN:v}function Xn(o){return isNaN(o)?"":"R$ "+o.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}function At(o,l,p){if(!o||isNaN(l)||isNaN(p))return;Yn&&cancelAnimationFrame(Yn);var v=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;if(v){o.textContent=Xn(p);return}var A=420,y=performance.now();function L(Q){return 1-Math.pow(1-Q,3)}function _(Q){var V=Q-y,Y=Math.min(1,V/A),t=l+(p-l)*L(Y);o.textContent=Xn(t),Y<1?Yn=requestAnimationFrame(_):Yn=null}Yn=requestAnimationFrame(_)}function at(o){return o?o.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||o.querySelector(".box-total-btn .linha-total .valor-final strong")||o.querySelector(".valor-pix strong")||o.querySelector(".valor-pix"):null}function xt(o){return o?o.querySelector(".box-total-btn .linha-total .valor-final"):null}function Fn(o){var l=xt(o);l&&(l.classList.remove("mm-pop"),l.offsetWidth,l.classList.add("mm-pop"),setTimeout(function(){l.classList.remove("mm-pop")},450))}function On(){var o=document.querySelector(".carrinho-rapido-ctn");if(o){var l=o.querySelectorAll(".cart-item:not(.mm-removing)");if(l.length>0){var p=o.querySelector(".box-total-btn");p&&p.dataset.mmTotalEnhanced!=="1"&&(Qn(o),lt(o),Hn(o),p.dataset.mmTotalEnhanced="1",o.dataset.mmShipRendered="1",Ln(o))}o.querySelector(".box-total-btn")||An(o);var v=o.querySelectorAll(".cart-item"),A=new Set;v.forEach(function(y){var L=y.id||y.getAttribute("data-item-id")||"";L&&(A.add(L),!gt.has(L)&&gt.size>0&&(y.classList.add("mm-added"),setTimeout(function(){y.classList.remove("mm-added")},500)))}),gt=A}}function Un(){var o=document.querySelector(".carrinho-rapido-ctn");if(!(!o||o.dataset.mmAnimObserved)){o.dataset.mmAnimObserved="1",On();var l=new MutationObserver(function(){clearTimeout(Un._t),Un._t=setTimeout(On,60)});l.observe(o,{childList:!0,subtree:!0,characterData:!0})}}function rt(){b(),m(),c();var o=document.getElementById("cart-preview-area");if(o){var l=new MutationObserver(function(){setTimeout(i,100),setTimeout(Un,150),setTimeout(An,180),setTimeout(Gn,220)});l.observe(o,{childList:!0,subtree:!0})}setInterval(i,800),setInterval(Un,800),setInterval(An,800),setInterval(Gn,800),i(),Un(),An(),Gn()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",rt):rt()})(),(function b(){"use strict";var P="mm_cep",j="mm_cart_snapshot",B=18e5,f="mm_onepage_draft",i=1440*60*1e3,m=2e3,c="https://magazord-public.s3.sa-east-1.amazonaws.com/madeiramania/resources/Design%20sem%20nome%20(1).svg",h="https://api.whatsapp.com/send?phone=5511915299488&text="+encodeURIComponent("Olá! Estou no checkout e gostaria de tirar uma dúvida sobre meu pedido."),u=location.pathname,E=u.indexOf("/checkout/cart")!==-1,w=u.indexOf("/checkout/identify")!==-1,F=u.indexOf("/checkout/onepage")!==-1,q=u.indexOf("/checkout/payment")!==-1,M=u.indexOf("/checkout/done")!==-1;if(M){try{localStorage.removeItem("mm_onepage_draft")}catch{}return}if(!E&&!w&&!F&&!q)return;b._retries=(b._retries||0)+1;var S=document.querySelector("#checkout-main-area");if(!S){try{var J=document.body&&document.body.textContent||"",nn=/muito tempo inativo|realize login novamente/i.test(J);if((F||w)&&(nn||b._retries>=40)&&!sessionStorage.getItem("mm_checkout_recovery")){sessionStorage.setItem("mm_checkout_recovery","1"),location.href="/checkout/cart";return}}catch{}b._retries<40&&setTimeout(b,400);return}try{sessionStorage.removeItem("mm_checkout_recovery")}catch{}function R(t){return isNaN(t)||t<0?"R$ 0,00":"R$ "+t.toFixed(2).replace(".",",").replace(/(\d)(?=(\d{3})+,)/g,"$1.")}function G(t){if(!t)return 0;var a=String(t).match(/(-?[\d.]+,\d{2})/);return a&&parseFloat(a[1].replace(/\./g,"").replace(",","."))||0}function O(t){return String(t||"").replace(/[&<>"']/g,function(a){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[a]})}var I={truck:'<svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2a3 3 0 0 0 6 0h6a3 3 0 0 0 6 0h2v-5l-3-4zM6 18.5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm12 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm.5-7H17V9.5h2.04l1.46 2-.04 0z"/></svg>',check:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>',checkCircle:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>',bolt:'<svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>',shield:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 1 3 5v6c0 5.5 3.8 10.7 9 12 5.2-1.3 9-6.5 9-12V5l-9-4zm-1.4 16L6 12.4l1.4-1.4 3.2 3.2 6.8-6.8L18.8 8.8 10.6 17z"/></svg>',lock:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM9 6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9V6z"/></svg>',card:'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>',rotate:'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.74 9.74 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>',minus:'<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M3 7h8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',plus:'<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 3v8M3 7h8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',trash:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',close:'<svg width="12" height="12" viewBox="0 0 14 14" fill="none"><path d="M10.5 3.5l-7 7M3.5 3.5l7 7" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',arrow:'<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>',tag:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.59 13.41 13.42 20.58a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>',whats:'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0 0 20.464 3.488"/></svg>',box:'<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>'},tn={get:function(t){try{return localStorage.getItem(t)}catch{return null}},set:function(t,a){try{localStorage.setItem(t,a)}catch{}},remove:function(t){try{localStorage.removeItem(t)}catch{}}};function mn(t){try{var a={ts:Date.now(),items:t.items.map(function(n){return{name:n.name,variant:n.variant,imgSrc:n.imgSrc,quantity:n.quantity,lineTotal:n.lineTotal,lineTotalPix:n.lineTotalPix,isPix:n.isPix,deposito:n.deposito}}),subtotalPix:t.subtotalPix,subtotalFull:t.subtotalFull,discount:t.discount,couponCode:t.couponCode,shipping:t.shipping,shippingDeadline:t.shippingDeadline,shippingName:t.shippingName,shippingCity:t.shippingCity,shippingOptions:t.shippingOptions,cepValue:t.cepValue};tn.set(j,JSON.stringify(a))}catch{}}function dn(){try{var t=tn.get(j);if(!t)return null;var a=JSON.parse(t);return!a||!a.ts||Date.now()-a.ts>B?null:a}catch{return null}}function un(){try{for(var t=["mm-op-email","mm-op-nome","mm-op-cpf","mm-op-tel","mm-op-cep","mm-op-rua","mm-op-num","mm-op-comp","mm-op-bairro","mm-op-cidade","mm-op-uf"],a={ts:Date.now()},n=0,e=0;e<t.length;e++){var r=document.getElementById(t[e]);r&&r.value&&(a[t[e]]=r.value,n++)}if(n===0){window._mmDraftDebug&&console.log("[mm-draft] skip save (0 fields)");return}tn.set(f,JSON.stringify(a)),window._mmDraftDebug&&console.log("[mm-draft] saved",n,"fields",a)}catch(s){window._mmDraftDebug&&console.warn("[mm-draft] save failed",s)}}function nt(){try{var t=tn.get(f);if(!t)return null;var a=JSON.parse(t);return!a||!a.ts?null:Date.now()-a.ts>i?(tn.remove(f),null):a}catch{return null}}function hn(){try{tn.remove(f)}catch{}}function Wn(){var t=nt();if(!t)return window._mmDraftDebug&&console.log("[mm-draft] no draft to restore"),null;for(var a=["mm-op-email","mm-op-nome","mm-op-cpf","mm-op-tel","mm-op-cep","mm-op-rua","mm-op-num","mm-op-comp","mm-op-bairro","mm-op-cidade","mm-op-uf"],n=0,e=0;e<a.length;e++){var r=document.getElementById(a[e]);if(r&&t[a[e]]){try{var s=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;s.call(r,t[a[e]])}catch{r.value=t[a[e]]}r.dispatchEvent(new Event("input",{bubbles:!0})),/^mm-op-(rua|bairro|cidade|uf)$/.test(a[e])&&(r.dataset.mmCepFill="1"),n++}}return window._mmDraftDebug&&console.log("[mm-draft] restored",n,"fields from draft",t),t}function _n(){for(var t={items:[],subtotalPix:0,subtotalFull:0,discount:0,shipping:null,shippingRaw:"",shippingDeadline:"",shippingName:"",shippingCity:"",shippingOptions:[],couponCode:"",cepValue:"",canFinalize:!1,hasFinalizar:!1},a=S.querySelectorAll(".cart-item"),n=0;n<a.length;n++){var e=a[n],r=e.querySelector('.qtd-item[id^="item_carrinho_"]'),s=r&&r.id.match(/item_carrinho_(\d+)/),d=s?parseInt(s[1],10):null,g=e.querySelector("figure img")||e.querySelector("#product-img")||e.querySelector("img"),x=e.querySelector(".nome-produto .link")||e.querySelector("figure a"),k=e.querySelector(".column-valor-produto .valor"),N=k?k.textContent.trim():"",T=!!e.querySelector(".column-valor-produto .sub");t.items.push({dataId:d,sku:e.getAttribute("data-item-id")||"",name:e.getAttribute("data-item-name")||e.getAttribute("data-name")||"",variant:e.getAttribute("data-item-variant")||"",brand:e.getAttribute("data-item-brand")||"",category:e.getAttribute("data-item-category")||"",priceUnit:parseFloat(e.getAttribute("data-item-price")||"0"),lineTotal:parseFloat(e.getAttribute("data-valor")||"0"),quantity:parseInt(e.getAttribute("data-item-quantity")||"1",10),deposito:e.getAttribute("data-item-deposito")==="1",imgSrc:g?g.getAttribute("src")||g.currentSrc:"",href:x?x.getAttribute("href"):"",lineTotalPix:G(N),isPix:T}),t.subtotalFull+=parseFloat(e.getAttribute("data-valor")||"0")}var W=S.querySelector("#resumo-compra .resumo-valores .value");W&&(t.subtotalPix=G(W.textContent)),t.subtotalPix<=0&&(t.subtotalPix=t.items.reduce(function(yt,Ft){return yt+(Ft.lineTotalPix||0)},0));var D=S.querySelector("#resumo-compra .discount-value");D&&(t.discount=G(D.textContent));var X=S.querySelector("#resumo-compra .txt-cupom, #resumo-compra .alert.alert-type-1");if(X){var U=X.textContent.match(/([A-Z0-9]{3,})/);U&&(t.couponCode=U[1])}var an=S.querySelector("#resumo-compra .frete-calculado, .frete-calculado");if(an&&an.textContent.trim()){t.shippingRaw=an.textContent.trim();var en=an.querySelector(".frete-location .city");en&&(t.shippingCity=en.textContent.trim());for(var pn=an.querySelectorAll(".servico-frete"),cn=0;cn<pn.length;cn++){var yn=pn[cn],$n=yn.querySelector('input[type="radio"]'),C=yn.querySelector(".dias-entrega"),z=parseFloat(yn.getAttribute("data-valor-frete")||"0"),H=yn.getAttribute("data-servico-frete")||"",K=C?C.textContent.trim().replace(/\s+/g," ").replace(/\s*-\s*$/,"").trim():"",Z=K.match(/(\d+(?:\s*[aà]\s*\d+)?\s*dias?(?:\s*[úu]teis)?)/i),sn=Z?Z[1].replace(/\s+/g," "):K;t.shippingOptions.push({id:$n?$n.value:"",name:H,deadline:sn,value:z,isFree:z===0,isSelected:$n?$n.checked:!1})}var ln=t.shippingOptions.filter(function(yt){return yt.isSelected})[0];if(!ln&&t.shippingOptions.length>0&&(ln=t.shippingOptions[0]),ln)t.shipping=ln.value,t.shippingName=ln.name,t.shippingDeadline=ln.deadline;else{var gn=an.querySelector(".info-frete-selec"),vn=an.querySelector(".dias-entrega, .info-frete-selec .dias-entrega"),xn=an.querySelector(".valor-frete .value, .value.valor-frete"),Bn=an.querySelector(".info-frete-selec .info-title span, .info-title span");if(xn){var En=xn.textContent.trim();if(/gr[aá]tis/i.test(En))t.shipping=0;else{var Dn=G(En);Dn>0&&(t.shipping=Dn)}}if(vn){var Rn=vn.textContent.match(/(\d+(?:\s*[aà]\s*\d+)?\s*dias?(?:\s*\([^)]+\))?(?:\s*[úu]teis)?)/i);Rn&&(t.shippingDeadline=Rn[1].replace(/\s+/g," ").replace(/\(s\)/,"").trim())}if(Bn&&(t.shippingName=Bn.textContent.trim()),t.shipping===null)if(/gr[aá]tis/i.test(t.shippingRaw))t.shipping=0;else{var ot=G(t.shippingRaw);ot>0&&(t.shipping=ot)}if(!t.shippingDeadline){var fn=t.shippingRaw.match(/(\d+(?:\s*[aà]\s*\d+)?)\s*dias?/i);fn&&(t.shippingDeadline=fn[1]+" dias úteis")}}}var dt=S.querySelector("#cep, .input-cep");return dt&&(t.cepValue=dt.value||""),t.hasFinalizar=!!S.querySelector("#finalizar-compra"),t.canFinalize=t.items.length>0,t}function wn(t){t=t||"cart";var a='<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 1 3 5v6c0 5.5 3.8 10.7 9 12 5.2-1.3 9-6.5 9-12V5l-9-4zm-1.4 16L6 12.4l1.4-1.4 3.2 3.2 6.8-6.8L18.8 8.8 10.6 17z"/></svg>';function n(e,r){var s=e===t,d="mm-checkout-step"+(s?" is-active":""),g=s?' aria-current="step"':"";return'<li class="'+d+'"'+g+'><span class="mm-checkout-step-label">'+r+"</span></li>"}return'<header class="mm-checkout-header"><a class="mm-checkout-header-logo" href="/" aria-label="Madeira Mania - voltar à home"><img src="'+c+'" alt="Madeira Mania" width="180" height="44"></a><nav class="mm-checkout-steps" aria-label="Etapas do checkout"><ol>'+n("cart","Carrinho")+'<li class="mm-checkout-step-sep" aria-hidden="true">›</li>'+n("delivery","Entrega")+'<li class="mm-checkout-step-sep" aria-hidden="true">›</li>'+n("payment","Pagamento")+'</ol></nav><div class="mm-checkout-secure">'+a+'<span class="mm-checkout-secure-text"><strong>Compra 100% segura</strong><small>Site protegido por SSL</small></span></div></header>'}function Zn(t){var a=t.imgSrc?'<img src="'+O(t.imgSrc)+'" alt="'+O(t.name)+'" loading="lazy">':"",n=t.href?'<a class="mm-item-name" href="'+O(t.href)+'">'+O(t.name)+"</a>":'<span class="mm-item-name">'+O(t.name)+"</span>",e=t.variant?'<p class="mm-item-variant">'+O(t.variant)+"</p>":"",r=t.deposito?'<span class="mm-item-badge">'+I.bolt+"<span>Pronta entrega · Envio em 24h</span></span>":"",s=t.quantity<=1?' disabled aria-disabled="true"':"",d;if(t.lineTotalPix>0&&t.isPix){var g='<span class="mm-item-price-sub">no PIX</span>',x=t.quantity>1?R(t.lineTotalPix/t.quantity)+" cada":"";d='<div class="mm-item-price"><span class="mm-item-price-value">'+R(t.lineTotalPix)+"</span>"+g+(x?'<span class="mm-item-price-unit">'+x+"</span>":"")+"</div>"}else{var k=t.quantity>1?R(t.priceUnit)+" cada":"";d='<div class="mm-item-price"><span class="mm-item-price-value">'+R(t.lineTotal)+"</span>"+(k?'<span class="mm-item-price-unit">'+k+"</span>":"")+"</div>"}return'<div class="mm-item" data-mm-id="'+t.dataId+'"><div class="mm-item-thumb">'+a+'</div><div class="mm-item-body">'+n+e+r+"</div>"+d+'<div class="mm-item-controls"><div class="mm-qty" role="group" aria-label="Quantidade"><button type="button" class="mm-qty-btn" data-mm-act="dec"'+s+' aria-label="Diminuir quantidade">'+I.minus+'</button><span class="mm-qty-value">'+t.quantity+'</span><button type="button" class="mm-qty-btn" data-mm-act="inc" aria-label="Aumentar quantidade">'+I.plus+'</button></div><button type="button" class="mm-item-remove" data-mm-act="remove" aria-label="Remover item" title="Remover">'+I.trash+"</button></div></div>"}function In(t){return t.items.length?t.items.map(Zn).join(""):'<div class="mm-empty"><div class="mm-empty-icon">'+I.box+'</div><h3 class="mm-empty-title">Seu carrinho está vazio</h3><p class="mm-empty-desc">Explore nossos móveis e encontre a peça certa para sua casa.</p><a class="mm-empty-cta" href="/">Explorar produtos '+I.arrow+'</a><p class="mm-empty-perks">Frete grátis acima de R$ 2.000 · 12x sem juros · 7% off no PIX</p></div>'}function Ln(t){for(var a="",n=0;n<t;n++)a+='<div class="mm-skel-item"><div class="mm-skel mm-skel-thumb"></div><div class="mm-skel-lines"><div class="mm-skel mm-skel-line w-3-4"></div><div class="mm-skel mm-skel-line w-1-2"></div><div class="mm-skel mm-skel-line w-1-3"></div></div><div class="mm-skel-lines"><div class="mm-skel mm-skel-line w-1-2"></div></div></div>';return a}function kn(t){var a=t.subtotalFull>0?t.subtotalFull:t.subtotalPix,n='<div class="mm-row"><span class="mm-row-label">Subtotal</span><span class="mm-row-value">'+R(a)+"</span></div>";if(t.shipping!==null){var e;t.shipping===0?e='<span class="mm-row-value is-free">'+I.check+" Grátis</span>":e='<span class="mm-row-value">'+R(t.shipping)+"</span>";var r='<span class="mm-row-label">Frete';t.shippingName&&(r+=' <span class="mm-row-sub">· '+O(t.shippingName)+"</span>"),t.shippingDeadline&&(r+=' <span class="mm-row-sub">· '+O(t.shippingDeadline)+"</span>"),r+="</span>",n+='<div class="mm-row">'+r+e+"</div>"}t.discount>0&&(n+='<div class="mm-row"><span class="mm-row-label">Desconto</span><span class="mm-row-value is-discount">− '+R(t.discount)+"</span></div>");var s="";if(t.shipping!==null){var d=Math.max(0,a+t.shipping-t.discount),g=Math.max(0,t.subtotalPix+t.shipping-t.discount),x=d-g,k=d/12;s='<div class="mm-total"><div class="mm-total-label">Total</div><div class="mm-total-value">'+R(d)+'</div><div class="mm-total-pix"><span>'+R(g)+" à vista no PIX</span>"+(x>0?'<span class="mm-total-pix-save">economia de '+R(x)+"</span>":"")+'</div><div class="mm-total-parcela">ou 12x de '+R(k)+" sem juros no cartão</div></div>"}else s='<div class="mm-total"><div class="mm-total-label">Subtotal</div><div class="mm-total-value">'+R(t.subtotalPix)+'</div><div class="mm-total-pix"><span>à vista no PIX</span></div><div class="mm-total-pending">Informe seu CEP para ver o frete e o total final.</div></div>';var N;return t.couponCode?N='<div class="mm-coupon-applied"><span class="mm-coupon-applied-left">'+I.tag+"<span>"+O(t.couponCode)+'</span></span><button type="button" data-mm-act="coupon-remove" aria-label="Remover cupom">'+I.close+"</button></div>":N='<div class="mm-coupon"><button type="button" class="mm-coupon-toggle" data-mm-act="coupon-toggle">'+I.tag+'<span>Tenho um cupom</span></button><form class="mm-coupon-form" data-mm-act="coupon-submit"><input type="text" class="mm-input" name="mm-coupon-code" placeholder="CUPOM" autocomplete="off" autocapitalize="characters" spellcheck="false" /><button type="submit" class="mm-btn-secondary">Aplicar</button></form></div>','<div class="mm-sum-stack"><div class="mm-rows">'+n+"</div>"+N+s+"</div>"}function Pn(){var t=document.getElementById("mm-layout");if(t)return t.parentElement!==S&&S.insertBefore(t,S.firstChild),t;var a=document.createElement("div");return a.id="mm-layout",a.innerHTML=wn("cart")+'<div class="mm-grid"><section class="mm-items"><h2 class="mm-h">Carrinho</h2><div class="mm-items-card"><div id="mm-item-list">'+Ln(2)+'</div></div></section><aside class="mm-sum"><h2 class="mm-h">Resumo</h2><div class="mm-sum-card"><div class="mm-cep"><div class="mm-cep-label"><span class="mm-cep-label-text">Calcular frete</span><a class="mm-cep-label-link" href="https://buscacepinter.correios.com.br/app/endereco/index.php" target="_blank" rel="noopener">Não sei meu CEP</a></div><div class="mm-cep-row"><input type="text" class="mm-input" id="mm-cep-input" name="mm_cep_calc" inputmode="numeric" maxlength="9" placeholder="00000-000" autocomplete="off" data-lpignore="true" data-1p-ignore="true" data-bwignore="true" data-form-type="other" aria-label="CEP" /><button type="button" class="mm-btn-secondary" data-mm-act="calc-cep">Calcular</button></div></div><div id="mm-sum-dynamic"></div><button type="button" class="mm-cta" data-mm-act="finalizar">Finalizar compra'+I.arrow+'</button><div class="mm-trust"><span class="mm-trust-item">'+I.lock+'<span>Pagamento seguro</span></span><span class="mm-trust-item">'+I.rotate+'<span>7 dias para troca</span></span><span class="mm-trust-item">'+I.card+'<span>12x sem juros</span></span></div><a class="mm-help" href="'+h+'" target="_blank" rel="noopener" data-mm-track="help-whats">'+I.whats+"<span><strong>Ficou com alguma dúvida?</strong> Fale com a gente no WhatsApp</span></a></div></aside></div>",S.insertBefore(a,S.firstChild),S.classList.add("mm-shadow-mode"),document.body.classList.add("mm-checkout-rebuild"),document.documentElement.classList.remove("mm-cart-loading"),a}function it(){var t=_n(),a=document.getElementById("mm-item-list");a&&(a.innerHTML=In(t));var n=document.getElementById("mm-sum-dynamic");n&&(n.innerHTML=kn(t));var e=document.querySelector(".mm-cta");e&&(e.disabled=!t.canFinalize,e.style.opacity=t.canFinalize?"1":"0.5",e.style.pointerEvents=t.canFinalize?"auto":"none");var r=document.getElementById("mm-cep-input");if(r&&!r.matches(":focus")){var s=tn.get(P),d=t.cepValue||s||"";d&&(r.value=zn(d))}return t.items&&t.items.length>0&&mn(t),t}function zn(t){var a=String(t||"").replace(/\D/g,"").slice(0,8);return a.length<=5?a:a.slice(0,5)+"-"+a.slice(5)}function mt(t){var a=String(t||"").replace(/\D/g,"");a.length===8&&tn.set(P,a)}function tt(t){t=t||0;var a=tn.get(P);if(!(!a||a.length!==8)){var n=S.querySelector("#cep, .input-cep");if(!n){t<12&&setTimeout(function(){tt(t+1)},350);return}var e=S.querySelector("#resumo-compra .frete-calculado");if(e&&e.textContent.trim()){var r=document.getElementById("mm-cep-input");r&&!r.value&&(r.value=zn(a));return}var s=document.getElementById("mm-cep-input");s&&!s.value&&(s.value=zn(a)),n.value=zn(a),Vn(n),setTimeout(function(){vt()},200)}}function Vn(t){try{var a=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;a.call(t,t.value)}catch{}t.dispatchEvent(new Event("input",{bubbles:!0})),t.dispatchEvent(new Event("change",{bubbles:!0}))}function vt(){try{if(window.Zord&&window.Zord.Cart&&typeof window.Zord.Cart.calculaFreteCarrinho=="function"){window.Zord.Cart.calculaFreteCarrinho();return}}catch{}var t=S.querySelector("#resumo-compra .calcula-frete > button, .area-frete button");t&&t.click()}function ut(){var t=document.getElementById("mm-layout");!t||t._mmBound||(t._mmBound=!0,t.addEventListener("click",function(a){var n=a.target.closest("[data-mm-act]");if(n){var e=n.getAttribute("data-mm-act"),r=n.closest(".mm-item"),s=r?parseInt(r.getAttribute("data-mm-id"),10):null;switch(e){case"inc":st(s,r,"inc");break;case"dec":st(s,r,"dec");break;case"remove":wt(s,r);break;case"calc-cep":qn();break;case"coupon-toggle":var d=n.closest(".mm-coupon");if(d){d.classList.add("is-open");var g=d.querySelector("input");g&&setTimeout(function(){g.focus()},100)}break;case"coupon-remove":rn();break;case"finalizar":et();break}}}),t.addEventListener("submit",function(a){var n=a.target.closest('[data-mm-act="coupon-submit"]');if(n){a.preventDefault();var e=n.querySelector("input");e&&bn(e.value.trim())}}),t.addEventListener("input",function(a){a.target&&a.target.id==="mm-cep-input"&&(a.target.value=zn(a.target.value))}),t.addEventListener("keydown",function(a){a.key==="Enter"&&a.target&&a.target.id==="mm-cep-input"&&(a.preventDefault(),qn())}))}function st(t,a,n){if(!(!t||!a)&&!(!window.Zord||!window.Zord.checkout)){a.classList.add("is-updating");try{n==="inc"?window.Zord.checkout.adicionaQuantidade(t):window.Zord.checkout.removeQuantidade(t)}catch(e){console.warn("[mm-cart] qty change failed",e),a.classList.remove("is-updating")}}}function wt(t,a){if(!(!t||!a)&&!(!window.Zord||!window.Zord.checkout)){a.classList.add("is-updating"),a.style.transition="opacity 200ms, transform 200ms",a.style.opacity="0",a.style.transform="translateX(-12px)";try{typeof window.Zord.checkout.deleteItem=="function"?window.Zord.checkout.deleteItem(t):window.Zord.checkout.removeQuantidade(t)}catch(n){console.warn("[mm-cart] remove failed",n),a.classList.remove("is-updating"),a.style.opacity="1",a.style.transform=""}}}function qn(){var t=document.getElementById("mm-cep-input");if(t){var a=(t.value||"").replace(/\D/g,"");if(a.length!==8){t.focus(),t.classList.add("mm-input-error"),setTimeout(function(){t.classList.remove("mm-input-error")},1200);return}mt(a);var n=S.querySelector("#cep, .input-cep");n&&(n.value=zn(a),Vn(n)),vt()}}function bn(t){if(t&&!(!window.Zord||!window.Zord.checkout)){var a=S.querySelector("#cupom-desconto");a&&(a.value=t.toUpperCase(),Vn(a));try{window.Zord.checkout.addCupomDesconto()}catch(n){console.warn("[mm-cart] coupon apply failed",n)}}}function rn(){if(!(!window.Zord||!window.Zord.checkout))try{window.Zord.checkout.removeCupomDesconto()}catch(t){console.warn("[mm-cart] coupon remove failed",t)}}function et(){try{var t=_n();t.items&&t.items.length>0&&mn(t)}catch{}var a=document.getElementById("finalizar-compra");if(a){a.click();return}location.href="/checkout/identify"}if(E){let t=function(n){if(n=n||0,n>30){a();return}var e=S.querySelectorAll(".cart-item").length>0,r=S.querySelector("#resumo-compra");e||r||n>8?a():setTimeout(function(){t(n+1)},250)},a=function(){Pn(),ut(),it(),tt(),typeof jQuery<"u"&&jQuery(document).ajaxComplete(function(r,s,d){if(!(!d||!d.url)){var g=d.url;(g.indexOf("checkout/cart")!==-1||g.indexOf("atualiza")!==-1||g.indexOf("cupom")!==-1||g.indexOf("frete")!==-1||g.indexOf("removeItem")!==-1||g.indexOf("adicionaItem")!==-1)&&(setTimeout(it,120),setTimeout(function(){var x=_n();x.shipping!==null&&x.cepValue&&mt(x.cepValue)},200))}});try{var n=new MutationObserver(function(r){a._mutTimer&&clearTimeout(a._mutTimer),a._mutTimer=setTimeout(it,200)}),e=[S.querySelector("#cart-area"),S.querySelector(".cart-area"),S.querySelector("#resumo-compra")].filter(Boolean);e.forEach(function(r){n.observe(r,{childList:!0,subtree:!0,characterData:!0})})}catch{}};if(!document.getElementById("mm-checkout-cro-done")){var lt=document.createElement("div");lt.id="mm-checkout-cro-done",lt.style.display="none",document.body.appendChild(lt)}t()}function Qn(t){if(!t||!t.items||!t.items.length)return'<aside class="mm-id-sum mm-sum"><h2 class="mm-h">Resumo</h2><div class="mm-sum-card"><div class="mm-sum-empty"><p>Não conseguimos carregar o resumo do seu pedido.</p><a class="mm-btn-secondary" href="/checkout/cart">Voltar ao carrinho</a></div></div></aside>';var a=3,n=t.items.slice(0,a),e=t.items.length-a,r=n.map(function(U){var an=U.quantity>1?'<strong class="mm-id-thumb-qty">'+U.quantity+"×</strong> ":"",en=!U.imgSrc&&!(U.lineTotalPix>0)&&!(U.lineTotal>0);if(en)return'<div class="mm-id-thumb"><div class="mm-id-thumb-img"><span class="mm-skel" style="display:block;width:100%;height:100%"></span></div><div class="mm-id-thumb-body"><p class="mm-id-thumb-name">'+an+O(U.name)+"</p>"+(U.variant?'<p class="mm-id-thumb-variant">'+O(U.variant)+"</p>":"")+'</div><div class="mm-id-thumb-price"><span class="mm-skel" style="display:inline-block;width:56px;height:15px"></span></div></div>';var pn=U.imgSrc?'<img src="'+O(U.imgSrc)+'" alt="'+O(U.name)+'" loading="lazy">':'<div class="mm-id-thumb-placeholder">'+I.box+"</div>",cn=U.lineTotalPix>0?U.lineTotalPix:U.lineTotal;return'<div class="mm-id-thumb"><div class="mm-id-thumb-img">'+pn+'</div><div class="mm-id-thumb-body"><p class="mm-id-thumb-name">'+an+O(U.name)+"</p>"+(U.variant?'<p class="mm-id-thumb-variant">'+O(U.variant)+"</p>":"")+'</div><div class="mm-id-thumb-price">'+R(cn)+"</div></div>"}).join("");e>0&&(r+='<div class="mm-id-thumb-more">+ '+e+" "+(e===1?"item":"itens")+" a mais</div>");var s=t.subtotalFull>0?t.subtotalFull:t.subtotalPix,d='<div class="mm-row"><span class="mm-row-label">Subtotal</span><span class="mm-row-value">'+R(s)+"</span></div>";if(t.shipping!==null&&t.shipping!==void 0){var g;t.shipping===0?g='<span class="mm-row-value is-free">'+I.check+" Grátis</span>":g='<span class="mm-row-value">'+R(t.shipping)+"</span>",d+='<div class="mm-row"><span class="mm-row-label">Frete'+(t.shippingDeadline?' <span class="mm-row-sub">· '+O(t.shippingDeadline)+"</span>":"")+"</span>"+g+"</div>"}t.discount>0&&(d+='<div class="mm-row"><span class="mm-row-label">Desconto'+(t.couponCode?' <span class="mm-row-sub">· '+O(t.couponCode)+"</span>":"")+'</span><span class="mm-row-value is-discount">− '+R(t.discount)+"</span></div>");var x,k=t.shipping!==null&&t.shipping!==void 0?t.shipping:0;if(t.shipping!==null&&t.shipping!==void 0){var N=Math.max(0,s+k-(t.discount||0)),T=Math.max(0,t.subtotalPix+k-(t.discount||0)),W=N-T,D=N/12;x='<div class="mm-total"><div class="mm-total-label">Total</div><div class="mm-total-value">'+R(N)+'</div><div class="mm-total-pix"><span>'+R(T)+" à vista no PIX</span>"+(W>0?'<span class="mm-total-pix-save">economia de '+R(W)+"</span>":"")+'</div><div class="mm-total-parcela">ou 12x de '+R(D)+" sem juros</div></div>"}else{var X=s/12;x='<div class="mm-total"><div class="mm-total-label">Subtotal</div><div class="mm-total-value">'+R(t.subtotalPix)+'</div><div class="mm-total-pix"><span>à vista no PIX</span></div><div class="mm-total-parcela">ou 12x de '+R(X)+" sem juros</div></div>"}return'<aside class="mm-id-sum mm-sum"><h2 class="mm-h">Resumo do pedido</h2><div class="mm-sum-card"><div class="mm-id-thumbs">'+r+'</div><div class="mm-rows">'+d+"</div>"+x+'<a class="mm-id-edit-cart" href="/checkout/cart"><span>Editar carrinho</span></a></div><a class="mm-help mm-sum-help" href="'+h+'" target="_blank" rel="noopener" data-mm-track="help-whats-sum">'+I.whats+"<span><strong>Ficou com alguma dúvida?</strong> Fale com a gente no WhatsApp</span></a></aside>"}function pt(){var t='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>',a='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',n='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><line x1="7" y1="9" x2="17" y2="9"/><line x1="7" y1="13" x2="17" y2="13"/><line x1="7" y1="17" x2="13" y2="17"/></svg>',e='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>';return'<section class="mm-id-form-col"><h2 class="mm-id-h2">Quase lá! Identifique-se</h2><p class="mm-id-sub">Informe seu e-mail para finalizar a compra de forma rápida e segura.</p><form class="mm-id-form" data-mm-act="identify-submit" novalidate><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+t+'</span><input type="email" id="mm-id-email" name="mm-email" class="mm-input" placeholder="seu@email.com" autocomplete="email" inputmode="email" required></div><p class="mm-id-microcopy">'+I.lock+'<span>Seu e-mail é seguro · Não compartilhamos com terceiros</span></p><button type="submit" class="mm-cta">Continuar'+I.arrow+'</button></form><div class="mm-id-divider"><span>ou</span></div><div class="mm-id-google-slot"></div><button type="button" class="mm-id-guest-toggle" data-mm-act="guest-go" aria-label="Continuar como visitante"><span class="mm-id-guest-icon" aria-hidden="true">'+a+'</span><span class="mm-id-guest-label">Continuar como visitante (sem criar conta)</span><span class="mm-id-guest-arrow" aria-hidden="true">'+I.arrow+'</span></button><div class="mm-trust mm-id-trust"><span class="mm-trust-item">'+I.lock+'<span>Pagamento seguro</span></span><span class="mm-trust-item">'+I.rotate+'<span>7 dias para troca</span></span><span class="mm-trust-item">'+I.shield+'<span>Garantia 12 meses</span></span></div><p class="mm-id-lgpd">Ao continuar, você concorda com nossa <a href="/politica-de-privacidade" target="_blank" rel="noopener">Política de Privacidade</a></p></section>'}function Hn(t){var a=document.getElementById("mm-layout");if(a)return a.parentElement!==S&&S.insertBefore(a,S.firstChild),a;var n=document.createElement("div");return n.id="mm-layout",n.classList.add("mm-id-layout"),n.innerHTML=wn("delivery")+'<div class="mm-grid mm-id-grid">'+pt()+Qn(t)+"</div>",S.insertBefore(n,S.firstChild),document.body.classList.add("mm-checkout-rebuild"),Gn(),S.classList.add("mm-shadow-mode"),document.documentElement.classList.remove("mm-cart-loading"),n}function Gn(){var t=document.querySelector(".mm-id-google-slot"),a=S.querySelector(".social-login-area");if(!(!t||!a)&&!t.contains(a))try{t.appendChild(a),t.classList.add("is-loaded")}catch{}}function ht(t){tn.set("mm_user_email",t);var a=S.querySelector("#login");if(!a)return!1;a.value=t,Vn(a);var n=a.closest("form"),e=n?n.querySelector('button.button-send, button[type="submit"]'):null;return e?(e.click(),!0):n?(n.submit(),!0):!1}function zt(){var t=document.getElementById("mm-layout");!t||t._mmBound||(t._mmBound=!0,t.addEventListener("submit",function(a){var n=a.target.closest('[data-mm-act="identify-submit"]');if(n){a.preventDefault();var e=n.querySelector("#mm-id-email"),r=e?e.value.trim():"";if(!r||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(r)){e&&(e.classList.add("mm-input-error"),e.focus(),setTimeout(function(){e.classList.remove("mm-input-error")},1500));return}var s=ht(r);if(s){var d=n.querySelector(".mm-cta");d&&d.classList.add("is-loading")}return}}),t.addEventListener("click",function(a){var n=a.target.closest("[data-mm-act]");if(n){var e=n.getAttribute("data-mm-act");e==="guest-go"&&(tn.set("mm_checkout_mode","guest"),n.classList.add("is-loading"),location.href="/checkout/onepage")}}))}function Sn(t){try{var a=new DOMParser().parseFromString(t,"text/html"),n=a.querySelector("#checkout-main-area");if(!n)return null;for(var e=[],r=n.querySelectorAll(".cart-item"),s=0,d=0;d<r.length;d++){var g=r[d],x=g.querySelector("figure img")||g.querySelector("#product-img")||g.querySelector("img"),k=g.querySelector(".column-valor-produto .valor"),N=parseFloat(g.getAttribute("data-valor")||"0"),T=k?G(k.textContent):0;e.push({name:g.getAttribute("data-item-name")||g.getAttribute("data-name")||"",variant:g.getAttribute("data-item-variant")||"",imgSrc:x&&x.getAttribute("src")||"",quantity:parseInt(g.getAttribute("data-item-quantity")||"1",10),lineTotal:N,lineTotalPix:T,isPix:!!g.querySelector(".column-valor-produto .sub"),deposito:g.getAttribute("data-item-deposito")==="1"}),s+=N}if(e.length===0)return null;var W=n.querySelector("#resumo-compra .resumo-valores .value"),D=W?G(W.textContent):0;D<=0&&(D=e.reduce(function(K,Z){return K+(Z.lineTotalPix||0)},0));var X=n.querySelector("#resumo-compra .discount-value"),U=X?G(X.textContent):0,an=n.querySelector("#resumo-compra .txt-cupom, #resumo-compra .alert.alert-type-1"),en="";if(an){var pn=an.textContent.match(/([A-Z0-9]{3,})/);pn&&(en=pn[1])}var cn=n.querySelector("#resumo-compra .frete-calculado"),yn=null,$n="";if(cn&&cn.textContent.trim()){var C=cn.textContent.trim();if(/gr[aá]tis/i.test(C))yn=0;else{var z=G(C);z>0&&(yn=z)}var H=C.match(/(\d+)\s*dias?/i);H&&($n=H[1]+" dias úteis")}return{ts:Date.now(),items:e,subtotalPix:D,subtotalFull:s,discount:U,couponCode:en,shipping:yn,shippingDeadline:$n,cepValue:""}}catch{return null}}function An(t){try{fetch("/checkout/cart",{credentials:"include",headers:{"X-Requested-With":"XMLHttpRequest"}}).then(function(a){return a.text()}).then(function(a){var n=Sn(a);n&&tn.set(j,JSON.stringify(n)),t(n)}).catch(function(){t(null)})}catch{t(null)}}function ct(t){var a=document.querySelector("#mm-layout .mm-id-sum");if(a){var n=a.parentNode;if(n){var e=document.createElement("div");e.innerHTML=Qn(t);var r=e.firstChild;r&&n.replaceChild(r,a)}}}function ft(){An(function(t){t&&t.items&&t.items.length>0&&ct(t)})}if(w){let t=function(n){if(n=n||0,n>30){a();return}var e=S.querySelector("#login, #login-form-etapa-01");e||n>8?a():setTimeout(function(){t(n+1)},250)},a=function(){var n=dn();Hn(n),zt(),Gn(),setTimeout(Gn,600),setTimeout(Gn,1500),ft(),setTimeout(function(){var e=document.getElementById("mm-id-email");e&&!("ontouchstart"in window)&&e.focus()},250)};t()}function bt(t){var a=String(t||"").replace(/\D/g,"").slice(0,11);return a.length<=3?a:a.length<=6?a.slice(0,3)+"."+a.slice(3):a.length<=9?a.slice(0,3)+"."+a.slice(3,6)+"."+a.slice(6):a.slice(0,3)+"."+a.slice(3,6)+"."+a.slice(6,9)+"-"+a.slice(9)}function qt(t){var a=String(t||"").replace(/\D/g,"").slice(0,11);return a.length<=2?a.length?"("+a:"":a.length<=6?"("+a.slice(0,2)+") "+a.slice(2):a.length<=10?"("+a.slice(0,2)+") "+a.slice(2,6)+"-"+a.slice(6):"("+a.slice(0,2)+") "+a.slice(2,7)+"-"+a.slice(7)}function gt(t){var a=String(t||"").replace(/\D/g,"").slice(0,8);return a.length<=5?a:a.slice(0,5)+"-"+a.slice(5)}function Yn(t,a){var n=String(t||"").replace(/\D/g,"");if(n.length!==8){a(null);return}try{fetch("https://viacep.com.br/ws/"+n+"/json/",{headers:{Accept:"application/json"}}).then(function(e){return e.json()}).then(function(e){if(!e||e.erro){a(null);return}a({logradouro:e.logradouro||"",bairro:e.bairro||"",cidade:e.localidade||"",estado:e.uf||""})}).catch(function(){a(null)})}catch{a(null)}}var Cn={mail:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4-8 5-8-5V6l8 5 8-5v2z"/></svg>',user:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>',doc:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M14 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6zm4 18H6V4h7v5h5v11zM8 12h8v2H8zm0 4h5v2H8z"/></svg>',phone:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z"/></svg>',pin:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>',home:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>',pix:'<svg width="24" height="24" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M11.917 11.71a2.046 2.046 0 0 1-1.454-.602l-2.1-2.1a.4.4 0 0 0-.551 0l-2.108 2.108a2.044 2.044 0 0 1-1.454.602h-.414l2.66 2.66c.83.83 2.177.83 3.007 0l2.667-2.668h-.253zM4.25 4.282c.55 0 1.066.214 1.454.602l2.108 2.108a.39.39 0 0 0 .552 0l2.1-2.1a2.044 2.044 0 0 1 1.453-.602h.253L9.503 1.623a2.127 2.127 0 0 0-3.007 0l-2.66 2.66h.414z"/><path d="m14.377 6.496-1.612-1.612a.307.307 0 0 1-.114.023h-.733c-.379 0-.75.154-1.017.422l-2.1 2.1a1.005 1.005 0 0 1-1.425 0L5.268 5.32a1.448 1.448 0 0 0-1.018-.422h-.9a.306.306 0 0 1-.109-.021L1.623 6.496c-.83.83-.83 2.177 0 3.008l1.618 1.618a.305.305 0 0 1 .108-.022h.901c.38 0 .75-.153 1.018-.421L7.375 8.57a1.034 1.034 0 0 1 1.426 0l2.1 2.1c.267.268.638.421 1.017.421h.733c.04 0 .079.01.114.024l1.612-1.612c.83-.83.83-2.178 0-3.008z"/></svg>',cardBig:'<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 4H4c-1.11 0-1.99.89-1.99 2L2 18c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V6c0-1.11-.89-2-2-2zm0 14H4v-6h16v6zm0-10H4V6h16v2z"/></svg>',barcode:'<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M2 6h2v12H2zm3 0h1v12H5zm2 0h3v12H7zm4 0h1v12h-1zm3 0h2v12h-2zm3 0h1v12h-1zm2 0h3v12h-3z"/></svg>',editPencil:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a.996.996 0 0 0 0-1.41l-2.34-2.34a.996.996 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>'};function Xn(t){var a=t?' value="'+O(t)+'"':"";return'<section class="mm-op-form-col"><h2 class="mm-id-h2">Falta pouco. Onde entregar?</h2><p class="mm-id-sub">Preencha seus dados e o endereço de entrega — o frete é calculado automaticamente.</p><form class="mm-op-form" data-mm-act="onepage-submit" novalidate><div class="mm-op-card"><h3 class="mm-op-card-title">'+Cn.user+'<span>Quem vai receber</span></h3><div class="mm-op-grid-2"><div class="mm-input-wrap mm-op-col-2"><span class="mm-input-icon" aria-hidden="true">'+Cn.mail+'</span><input type="email" id="mm-op-email" class="mm-input" placeholder="seu@email.com" autocomplete="email" inputmode="email" required'+a+'></div><div class="mm-input-wrap mm-op-col-2"><span class="mm-input-icon" aria-hidden="true">'+Cn.user+'</span><input type="text" id="mm-op-nome" class="mm-input" placeholder="Nome completo" autocomplete="name" required></div><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+Cn.doc+'</span><input type="tel" id="mm-op-cpf" class="mm-input" placeholder="CPF" inputmode="numeric" autocomplete="off" maxlength="14" required></div><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+Cn.phone+'</span><input type="tel" id="mm-op-tel" class="mm-input" placeholder="(11) 91234-5678" inputmode="tel" autocomplete="tel" maxlength="15" required></div></div><p class="mm-op-microcopy-soft">Usamos seus dados só pra emitir nota fiscal e te avisar da entrega.</p></div><div class="mm-op-card"><h3 class="mm-op-card-title">'+Cn.pin+'<span>Endereço de entrega</span></h3><div class="mm-op-grid-2"><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+Cn.pin+'</span><input type="tel" id="mm-op-cep" class="mm-input" placeholder="CEP — 00000-000" inputmode="numeric" autocomplete="postal-code" maxlength="9" required><span class="mm-input-status" id="mm-op-cep-status" aria-live="polite"></span></div><a class="mm-op-cep-help" href="https://buscacepinter.correios.com.br/app/endereco/index.php" target="_blank" rel="noopener">Não sei meu CEP</a><div class="mm-input-wrap mm-op-col-2"><span class="mm-input-icon" aria-hidden="true">'+Cn.home+'</span><input type="text" id="mm-op-rua" class="mm-input" placeholder="Rua, Av, Travessa..." autocomplete="address-line1" required></div><div class="mm-input-wrap"><input type="tel" id="mm-op-num" class="mm-input mm-input-noicon" placeholder="Número" inputmode="numeric" required></div><div class="mm-input-wrap"><input type="text" id="mm-op-comp" class="mm-input mm-input-noicon" placeholder="Complemento (opcional)" autocomplete="address-line2"></div><div class="mm-input-wrap mm-op-col-2"><input type="text" id="mm-op-bairro" class="mm-input mm-input-noicon" placeholder="Bairro" autocomplete="address-level3" required></div><div class="mm-input-wrap"><input type="text" id="mm-op-cidade" class="mm-input mm-input-noicon" placeholder="Cidade" autocomplete="address-level2" required></div><div class="mm-input-wrap"><input type="text" id="mm-op-uf" class="mm-input mm-input-noicon" placeholder="UF" maxlength="2" autocomplete="address-level1" required></div></div><div class="mm-op-frete" id="mm-op-frete-slot"></div></div><button type="submit" class="mm-cta mm-op-cta">Última etapa: pagamento'+I.arrow+'</button><p class="mm-id-microcopy mm-op-cta-sub">'+I.lock+'<span>Você revisa tudo antes de finalizar</span></p></form><div class="mm-trust mm-id-trust"><span class="mm-trust-item">'+I.lock+'<span>Pagamento seguro</span></span><span class="mm-trust-item">'+I.rotate+'<span>7 dias para troca</span></span><span class="mm-trust-item">'+I.shield+"<span>Garantia 12 meses</span></span></div></section>"}function At(t,a){var n=document.getElementById("mm-layout");if(n)return n.parentElement!==S&&S.insertBefore(n,S.firstChild),n;var e=document.createElement("div");return e.id="mm-layout",e.classList.add("mm-id-layout"),e.classList.add("mm-op-layout"),e.innerHTML=wn("delivery")+'<div class="mm-grid mm-id-grid mm-op-grid">'+Xn(a)+Qn(t)+"</div>",S.insertBefore(e,S.firstChild),document.body.classList.add("mm-checkout-rebuild"),S.classList.add("mm-shadow-mode"),document.documentElement.classList.remove("mm-cart-loading"),e}function at(t){var a=document.getElementById("mm-op-frete-slot");if(a){if(t==="loading"){a.innerHTML='<div class="mm-op-frete-loading"><div class="mm-op-frete-spinner"></div><span>Calculando frete...</span></div>';return}if(t==="error"){a.innerHTML='<div class="mm-op-frete-error"><span>Não conseguimos calcular o frete. Confira o CEP e tente novamente.</span></div>';return}var n=t.value===0,e=n?'<strong class="mm-op-frete-value is-free">Grátis</strong>':'<strong class="mm-op-frete-value">'+R(t.value)+"</strong>",r=t.name?'<span class="mm-op-frete-name">'+O(t.name)+"</span>":"",s=t.deadline?'<span class="mm-op-frete-deadline">Entrega em '+O(t.deadline)+"</span>":"",d=t.city?'<span class="mm-op-frete-city">para '+O(t.city)+"</span>":"",g="";if(t.options&&t.options.length>1){g='<div class="mm-op-frete-options"><button type="button" class="mm-op-frete-toggle" data-mm-act="toggle-frete-opts" aria-expanded="false">Ver outras opções ('+t.options.length+')</button><div class="mm-op-frete-options-list" hidden>';for(var x=0;x<t.options.length;x++){var k=t.options[x],N=k.isSelected?" is-selected":"",T=k.isFree?'<span class="mm-op-frete-opt-value is-free">Grátis</span>':'<span class="mm-op-frete-opt-value">'+R(k.value)+"</span>";g+='<button type="button" class="mm-op-frete-opt'+N+'" data-mm-act="op-ship-select" data-ship-id="'+O(k.id)+'" aria-pressed="'+(k.isSelected?"true":"false")+'"><span class="mm-op-frete-opt-radio" aria-hidden="true"><span></span></span><span class="mm-op-frete-opt-body"><span class="mm-op-frete-opt-name">'+O(k.name||"Padrão")+"</span>"+(k.deadline?'<span class="mm-op-frete-opt-deadline">'+O(k.deadline)+"</span>":"")+"</span>"+T+"</button>"}g+="</div></div>"}a.innerHTML='<div class="mm-op-frete-card'+(n?" is-free":"")+'"><div class="mm-op-frete-icon">'+I.truck+'</div><div class="mm-op-frete-body"><div class="mm-op-frete-row">'+r+e+"</div>"+s+d+"</div></div>"+g}}function xt(){function t(K){if(!K)return"";var Z=K.match(/(\d+(?:\s*[aà]\s*\d+)?)\s*dias?(?:\s*\([^)]+\))?\s*([úu]teis)?/i);return Z?Z[1].replace(/\s+/g," ")+" dias úteis":""}function a(K){for(var Z=[],sn=K.querySelectorAll(".servico-frete"),ln=0;ln<sn.length;ln++){var gn=sn[ln],vn=gn.querySelector('input[type="radio"]'),xn=gn.querySelector(".dias-entrega"),Bn=parseFloat(gn.getAttribute("data-valor-frete")||"0"),En=gn.getAttribute("data-servico-frete")||"",Dn=xn?xn.textContent.trim():"",Rn=Dn.match(/(\d+(?:\s*[aà]\s*\d+)?\s*dias?(?:\s*[úu]teis)?)/i);Z.push({id:vn?vn.value:"",name:En,deadline:Rn?Rn[1].replace(/\s+/g," "):Dn,value:Bn,isFree:Bn===0,isSelected:vn?vn.checked:!1})}return Z}var n=S.querySelector(".frete-calculado");if(n&&n.textContent.trim()){var e=a(n),r=n.querySelector(".frete-location .city"),s=r?r.textContent.trim():"",d=e.filter(function(K){return K.isSelected})[0]||e[0];if(d)return{value:d.value,name:d.name,deadline:d.deadline,city:s,options:e};var g=n.querySelector(".info-frete-selec .info-title span, .info-title span"),x=n.querySelector(".info-frete-selec .dias-entrega, .dias-entrega"),k=n.querySelector(".value.valor-frete, .valor-frete .value"),N=n.textContent,T=null;if(k&&(/gr[aá]tis/i.test(k.textContent)?T=0:T=G(k.textContent)),T===null&&(/gr[aá]tis/i.test(N)?T=0:T=G(N)||null),T!==null)return{value:T,name:g?g.textContent.trim():"",deadline:t(x?x.textContent:N),city:s,options:[]}}var W=S.querySelector(".line-entrega"),D=S.querySelector(".value.valor-frete, .valor-frete .value");if(W||D){var X=((W||D).textContent||"").trim(),U=dn(),an=U&&U.shippingName||"",en=U&&U.shippingDeadline||"",pn=U&&U.shippingCity||"",cn=U?U.shippingOptions||[]:[];if(X){var yn=(S.querySelector(".nome-servico-frete, .info-frete-selec .info-title span")||{}).textContent||"",$n=(S.querySelector(".info-entrega, .prazo-entrega, .line-entrega .sub, .info-frete-selec .dias-entrega")||{}).textContent||"",C=t($n)||t(X)||en,z=yn.trim()||an;if(/gr[aá]tis/i.test(X))return{value:0,deadline:C,name:z,city:pn,options:cn};var H=G(X);if(H>0)return{value:H,deadline:C,name:z,city:pn,options:cn}}if(U&&U.shipping!==null&&U.shipping!==void 0)return{value:U.shipping,deadline:en,name:an,city:pn,options:cn}}return null}function Fn(){var t=document.getElementById("mm-op-cep");if(!t)return;var a=(t.value||"").replace(/\D/g,"");if(a.length!==8)return;if(Fn._lastCep===a){var n=document.getElementById("mm-op-frete-slot");if(n&&n.querySelector(".mm-op-frete-card"))return}Fn._lastCep=a;var e=(Fn._token||0)+1;Fn._token=e,mt(a);var r=S.querySelector("#cep, .input-cep");r&&(r.value=zn(a),Vn(r)),at("loading");try{window.Zord&&window.Zord.Cart&&typeof window.Zord.Cart.calculaFreteCarrinho=="function"&&window.Zord.Cart.calculaFreteCarrinho()}catch{}var s=0,d=18;function g(){if(Fn._token===e){s++;var x=xt();if(x){at(x);var k=dn();k&&(k.shipping=x.value,k.shippingDeadline=x.deadline,k.shippingName=x.name||"",k.shippingCity=x.city||"",k.shippingOptions=x.options||[],tn.set(j,JSON.stringify(k)),ct(k));return}s<d?setTimeout(g,350):at("error")}}setTimeout(g,400)}function On(){var t=document.getElementById("mm-layout");if(!t||t._mmOpBound)return;t._mmOpBound=!0,t.addEventListener("click",function(n){var e=n.target.closest('[data-mm-act="toggle-frete-opts"]');if(e){n.preventDefault();var r=e.parentElement.querySelector(".mm-op-frete-options-list");if(r){var s=r.hasAttribute("hidden");s?r.removeAttribute("hidden"):r.setAttribute("hidden",""),e.setAttribute("aria-expanded",s?"true":"false"),e.textContent=s?"Ocultar opções":"Ver outras opções"}return}var d=n.target.closest('[data-mm-act="op-ship-select"]');if(d){n.preventDefault();var g=d.getAttribute("data-ship-id");if(!g)return;var x=S.querySelector('.servico-frete input[type="radio"][value="'+g+'"]');if(!x){console.warn("[mm-op] modalidade não encontrada no DOM:",g);return}for(var k=t.querySelectorAll(".mm-op-frete-opt"),N=0;N<k.length;N++){var T=k[N],W=T.getAttribute("data-ship-id")===g;T.classList.toggle("is-selected",W),T.setAttribute("aria-pressed",W?"true":"false")}x.checked=!0,x.click(),setTimeout(function(){var D=xt();if(D){at(D);var X=dn();X&&(X.shipping=D.value,X.shippingDeadline=D.deadline,X.shippingName=D.name||"",X.shippingOptions=D.options||[],tn.set(j,JSON.stringify(X)),ct(X))}},700);return}}),t.addEventListener("submit",function(n){var e=n.target.closest('[data-mm-act="onepage-submit"]');if(e){n.preventDefault();var r={email:(document.getElementById("mm-op-email")||{}).value||"",nome:(document.getElementById("mm-op-nome")||{}).value||"",cpf:(document.getElementById("mm-op-cpf")||{}).value||"",tel:(document.getElementById("mm-op-tel")||{}).value||"",cep:(document.getElementById("mm-op-cep")||{}).value||"",rua:(document.getElementById("mm-op-rua")||{}).value||"",num:(document.getElementById("mm-op-num")||{}).value||"",comp:(document.getElementById("mm-op-comp")||{}).value||"",bairro:(document.getElementById("mm-op-bairro")||{}).value||"",cidade:(document.getElementById("mm-op-cidade")||{}).value||"",uf:(document.getElementById("mm-op-uf")||{}).value||""},s=[];if(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(r.email.trim())||s.push("mm-op-email"),r.nome.trim().split(/\s+/).length<2&&s.push("mm-op-nome"),r.cpf.replace(/\D/g,"").length!==11&&s.push("mm-op-cpf"),r.tel.replace(/\D/g,"").length<10&&s.push("mm-op-tel"),r.cep.replace(/\D/g,"").length!==8&&s.push("mm-op-cep"),r.rua.trim()||s.push("mm-op-rua"),r.num.trim()||s.push("mm-op-num"),r.bairro.trim()||s.push("mm-op-bairro"),r.cidade.trim()||s.push("mm-op-cidade"),r.uf.trim()||s.push("mm-op-uf"),s.length){s.forEach(function(x){var k=document.getElementById(x);k&&(k.classList.add("mm-input-error"),setTimeout(function(){k.classList.remove("mm-input-error")},1800))});var d=document.getElementById(s[0]);d&&(d.focus(),d.scrollIntoView({block:"center",behavior:"smooth"}));return}var g=e.querySelector(".mm-cta");g&&g.classList.add("is-loading"),tn.set("mm_user_email",r.email.trim()),o(r)}}),t.addEventListener("input",function(n){var e=n.target;if(e){if(e.dataset&&e.dataset.mmCepFill==="1"&&delete e.dataset.mmCepFill,e.id==="mm-op-cpf")e.value=bt(e.value);else if(e.id==="mm-op-tel")e.value=qt(e.value);else if(e.id==="mm-op-cep"){e.value=gt(e.value);var r=e.value.replace(/\D/g,"");r.length===8&&Un(r)}else e.id==="mm-op-uf"&&(e.value=(e.value||"").replace(/[^A-Za-z]/g,"").toUpperCase().slice(0,2));e.id&&e.id.indexOf("mm-op-")===0&&(On._draftTimer&&clearTimeout(On._draftTimer),On._draftTimer=setTimeout(un,400))}});function a(){On._draftTimer&&(clearTimeout(On._draftTimer),On._draftTimer=null),un()}t.addEventListener("blur",function(n){var e=n.target;e&&e.id&&e.id.indexOf("mm-op-")===0&&a()},!0),window.addEventListener("beforeunload",a)}function Un(t){var a=document.getElementById("mm-op-cep-status");a&&(a.className="mm-input-status is-loading",a.textContent="Buscando..."),Yn(t,function(n){if(a&&(a.className="mm-input-status"),!n){a&&(a.className="mm-input-status is-error",a.textContent="CEP não encontrado",setTimeout(function(){a.className="mm-input-status",a.textContent=""},2500));return}a&&(a.className="mm-input-status is-ok",a.innerHTML=I.check,setTimeout(function(){a.className="mm-input-status",a.innerHTML=""},1800));var e=[["mm-op-rua",n.logradouro],["mm-op-bairro",n.bairro],["mm-op-cidade",n.cidade],["mm-op-uf",n.estado]];e.forEach(function(s){var d=document.getElementById(s[0]);!d||!s[1]||(!d.value||d.dataset.mmCepFill==="1")&&(d.value=s[1],d.dataset.mmCepFill="1")});var r=document.getElementById("mm-op-num");r&&setTimeout(function(){r.focus()},100),Fn._t&&clearTimeout(Fn._t),Fn._t=setTimeout(Fn,200)})}function rt(t){if(!document.getElementById("mm-op-overlay")){var a=document.createElement("div");a.id="mm-op-overlay",a.innerHTML='<div class="mm-op-overlay-card"><div class="mm-op-overlay-spinner"></div><p class="mm-op-overlay-text">'+O(t||"Processando...")+"</p></div>",document.body.appendChild(a)}}function o(t){var a=t.nome.trim(),n=t.email.trim(),e=t.rua.trim(),r=t.bairro.trim(),s=t.cidade.trim(),d=t.uf.trim(),g=zn(t.cep.replace(/\D/g,""));tn.set("mm_user_email",n);var x=function(k,N){var T=S.querySelector(k);if(!T)return!1;try{var W=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;W.call(T,N)}catch{T.value=N}return T.dispatchEvent(new Event("input",{bubbles:!0})),T.dispatchEvent(new Event("change",{bubbles:!0})),T.dispatchEvent(new Event("blur",{bubbles:!0})),!0};x("#nome-completo_2",a),x("#cpf_2",t.cpf),x("#email_3",n),x("#telefone_2",t.tel),x("#cep",g),x("#destinatario",a),x("#logradouro",e),x("#numero",t.num),x("#complemento",t.comp),x("#bairro",r),x("#cidade",s),x("#estado",d),rt("Indo para o pagamento..."),setTimeout(function(){var k=document.getElementById("mm-layout");k&&(k.style.display="none"),S.classList.remove("mm-shadow-mode");function N(D){var X=S.querySelector("#"+D);return X?X.closest("form"):null}function T(){var D=N("nome-completo_2");return D&&typeof D.requestSubmit=="function"?(D.requestSubmit(),!0):D?(D.submit(),!0):!1}function W(){var D=N("cep");return D&&typeof D.requestSubmit=="function"?(D.requestSubmit(),!0):D?(D.submit(),!0):!1}setTimeout(function(){T(),setTimeout(function(){W();var D=0;function X(){D++;var U=document.querySelector('input[name="forma-pagto"], #forma-pagto-pix, #forma-pagto-cartao, #forma-pagto-boleto');if(U&&U.offsetParent!==null){try{p(t)}catch{var an=document.getElementById("mm-op-overlay");an&&an.remove();var en=document.getElementById("mm-layout");en&&(en.style.display="none")}return}if(D<30)setTimeout(X,250);else{var pn=document.getElementById("mm-op-overlay");pn&&pn.remove()}}setTimeout(X,800)},1500)},150)},100)}function l(){var t={pix:null,cartao:null,boleto:null},a=[];try{a=window.Zord&&window.Zord.Calculo&&window.Zord.Calculo.FormasPagamentoPedido||[]}catch{}a.forEach(function(T){var W=T.formaPagamento&&T.formaPagamento.id,D=T.formaPagamento&&T.formaPagamento.isCartao,X=T.condicoes||[];!X.length||!D||(!t.cartao||X.length>t.cartao.condicoes.length)&&(t.cartao={formaId:W,valorTotal:X[0].valorTotal,condicoes:X.map(function(U){return{nome:U.condicaoPagamento&&U.condicaoPagamento.nome,numParcelas:U.condicaoPagamento&&U.condicaoPagamento.numeroParcelas,valorParcela:U.valorParcela,valorTotal:U.valorTotal}})})});function n(T){if(!T)return 0;var W=T.getAttribute&&T.getAttribute("data-valor-total");if(W){var D=parseFloat(W);if(D>0)return D}var X=(T.textContent||"").replace(/[^\d,.]/g,"");return X.indexOf(",")!==-1&&(X=X.replace(/\./g,"").replace(",",".")),parseFloat(X)||0}var e=document.querySelector("#valor-total-pedido-pix, .valor-total-pix[data-valor-total]"),r=n(e);if(r>0)t.pix={formaId:17,valorTotal:r};else{var s=a.find&&a.find(function(T){return T.formaPagamento&&T.formaPagamento.id===17});s&&s.condicoes&&s.condicoes[0]&&(t.pix={formaId:17,valorTotal:s.condicoes[0].valorTotal})}var d=document.querySelector("#valor-total-pedido-boleto, .valor-total-boleto[data-valor-total]"),g=n(d);if(g>0)t.boleto={valorTotal:g};else{var x=a.find&&a.find(function(T){var W=T.formaPagamento&&T.formaPagamento.id,D=T.formaPagamento&&T.formaPagamento.isCartao;return!D&&W!==17&&T.condicoes&&T.condicoes.length});x&&(t.boleto={formaId:x.formaPagamento.id,valorTotal:x.condicoes[0].valorTotal})}if(!t.cartao){var k=document.querySelector(".valor-parcela-cartao");if(k){var N=n(k);N>0&&(t.cartao={valorTotal:N*12,condicoes:[]})}}return t}function p(t){var a=dn(),n=l();S.classList.add("mm-shadow-mode"),document.body.classList.add("mm-checkout-rebuild");var e=document.getElementById("mm-layout");(!e||e.parentElement!==S)&&(e&&e.parentElement&&e.parentElement.removeChild(e),e=document.createElement("div"),e.id="mm-layout",S.insertBefore(e,S.firstChild)),e.className="mm-op-layout mm-op-step3-layout",e.style.display="",e.innerHTML=v(a,t,n),document.documentElement.classList.remove("mm-cart-loading");var r=document.getElementById("mm-op-overlay");r&&r.remove();try{window.scrollTo({top:0,behavior:"smooth"})}catch{window.scrollTo(0,0)}Q(t,n)}function v(t,a,n){var e=typeof renderGlobalFooter=="function"?renderGlobalFooter():"";return wn("payment")+'<main class="mm-op-main"><div class="mm-op-step3-grid"><section class="mm-op-step3-left">'+y(n)+L()+'</section><aside class="mm-op-step3-sum-wrap">'+A(a)+_(t,n,"pix")+"</aside></div></main>"+e}function A(t){var a=t||{},n=O(a.nome||""),e=O(a.email||""),r=O(a.cpf||""),s=O(a.tel||""),d=O(a.rua||""),g=O(a.num||""),x=a.comp?", "+O(a.comp):"",k=O(a.bairro||""),N=O(a.cidade||""),T=O(a.uf||""),W=O(a.cep||"");return'<div class="mm-op-step3-completed"><div class="mm-op-completed-card" data-section="dados"><div class="mm-op-completed-head"><span class="mm-op-completed-check">'+I.check+'</span><h3 class="mm-op-completed-title">Dados pessoais</h3><a href="/checkout/onepage" class="mm-op-completed-edit" data-mm-act="edit-dados" aria-label="Editar dados">'+Cn.editPencil+' Editar</a></div><dl class="mm-op-completed-body">'+(n?"<div><dt>Nome</dt><dd>"+n+"</dd></div>":"")+(e?"<div><dt>E-mail</dt><dd>"+e+"</dd></div>":"")+(r?"<div><dt>CPF</dt><dd>"+r+"</dd></div>":"")+(s?"<div><dt>Telefone</dt><dd>"+s+"</dd></div>":"")+'</dl></div><div class="mm-op-completed-card" data-section="endereco"><div class="mm-op-completed-head"><span class="mm-op-completed-check">'+I.check+'</span><h3 class="mm-op-completed-title">Endereço de entrega</h3><a href="/checkout/onepage" class="mm-op-completed-edit" data-mm-act="edit-endereco" aria-label="Editar endereço">'+Cn.editPencil+' Editar</a></div><address class="mm-op-completed-address">'+d+", "+g+x+"<br>"+k+" — "+N+"/"+T+"<br>"+(W?"CEP "+W:"")+"</address></div></div>"}function y(t){var a=t.pix?t.pix.valorTotal:0,n=t.cartao?t.cartao.valorTotal:0,e=t.boleto?t.boleto.valorTotal:0,r=n>a?n-a:0,s=null;t.cartao&&t.cartao.condicoes&&t.cartao.condicoes.length&&(s=t.cartao.condicoes[t.cartao.condicoes.length-1]);var d=s?"até "+s.numParcelas+"x de "+R(s.valorParcela)+" sem juros":n>0?"até 12x sem juros":"Cartão de crédito",g='<label class="mm-op-pay-radio is-active" data-forma="pix"><input type="radio" name="mm-pay" value="pix" checked><div class="mm-op-pay-head"><span class="mm-op-pay-radio-dot" aria-hidden="true"></span><span class="mm-op-pay-icon">'+Cn.pix+'</span><div class="mm-op-pay-body"><span class="mm-op-pay-title">PIX</span><span class="mm-op-pay-sub">Aprovação instantânea · em 1 minuto</span></div><div class="mm-op-pay-price">'+(r>0?'<span class="mm-op-pay-badge-save">Economize '+R(r)+"</span>":"")+'<span class="mm-op-pay-amount">'+R(a)+'</span></div></div><div class="mm-op-pay-detail"><ul class="mm-op-pay-benefits"><li>'+I.check+"<span>QR Code e Copia-e-Cola após confirmar</span></li><li>"+I.check+"<span>Pedido aprovado em até 1 minuto</span></li><li>"+I.check+"<span>Pagamento 100% seguro · sem dados de cartão</span></li></ul></div></label>",x='<div class="mm-op-pay-brands"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-visa.svg" alt="Visa" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-mastercard.svg" alt="Mastercard" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-american-express.svg" alt="American Express" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-elo.svg" alt="Elo" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-hipercard.svg" alt="Hipercard" width="32" height="20"></div>',k='<span class="mm-op-req" aria-hidden="true">*</span>';function N(X){return'<span class="mm-op-field-err" id="'+X+'-err" role="alert" aria-live="polite"></span>'}var T='<div class="mm-op-card-form"><div class="mm-op-card-field mm-op-card-field-full"><label for="mm-pay-card-num">Número do cartão'+k+'</label><div class="mm-input-wrap mm-input-wrap-card"><span class="mm-input-icon" aria-hidden="true">'+I.card+'</span><input id="mm-pay-card-num" type="tel" class="mm-input" inputmode="numeric" autocomplete="off" data-mmac="cc-number" placeholder="0000 0000 0000 0000" maxlength="23" aria-describedby="mm-pay-card-num-err" aria-required="true" disabled><span class="mm-op-card-brand-detected" aria-live="polite"></span></div>'+N("mm-pay-card-num")+'</div><div class="mm-op-card-field mm-op-card-field-full"><label for="mm-pay-card-name">Nome impresso no cartão'+k+'</label><input id="mm-pay-card-name" type="text" class="mm-input mm-input-noicon" autocomplete="off" data-mmac="cc-name" placeholder="Como aparece no cartão" maxlength="100" aria-describedby="mm-pay-card-name-err" aria-required="true" disabled>'+N("mm-pay-card-name")+'</div><div class="mm-op-card-field mm-op-card-field-half"><label for="mm-pay-card-exp">Validade'+k+'</label><input id="mm-pay-card-exp" type="tel" class="mm-input mm-input-noicon" inputmode="numeric" autocomplete="off" data-mmac="cc-exp" placeholder="MM/AA" maxlength="5" aria-describedby="mm-pay-card-exp-err" aria-required="true" disabled>'+N("mm-pay-card-exp")+'</div><div class="mm-op-card-field mm-op-card-field-half"><label for="mm-pay-card-cvv">CVV'+k+'</label><input id="mm-pay-card-cvv" type="tel" class="mm-input mm-input-noicon" inputmode="numeric" autocomplete="off" data-mmac="cc-csc" placeholder="000" maxlength="4" aria-describedby="mm-pay-card-cvv-err" aria-required="true" disabled>'+N("mm-pay-card-cvv")+'</div><div class="mm-op-card-field mm-op-card-field-full"><label for="mm-pay-card-installments">Condições de pagamento'+k+'</label><select id="mm-pay-card-installments" class="mm-input mm-input-noicon mm-op-card-installments" aria-describedby="mm-pay-card-installments-err" aria-required="true" disabled><option value="">Digite o cartão pra ver as condições</option></select>'+N("mm-pay-card-installments")+"</div></div>",W='<label class="mm-op-pay-radio" data-forma="cartao"><input type="radio" name="mm-pay" value="cartao"><div class="mm-op-pay-head"><span class="mm-op-pay-radio-dot" aria-hidden="true"></span><span class="mm-op-pay-icon">'+Cn.cardBig+'</span><div class="mm-op-pay-body"><span class="mm-op-pay-title">Cartão de Crédito</span><span class="mm-op-pay-sub">'+O(d)+'</span></div><div class="mm-op-pay-price"><span class="mm-op-pay-amount">'+R(n)+'</span></div></div><div class="mm-op-pay-detail">'+x+T+"</div></label>",D='<label class="mm-op-pay-radio" data-forma="boleto"><input type="radio" name="mm-pay" value="boleto"><div class="mm-op-pay-head"><span class="mm-op-pay-radio-dot" aria-hidden="true"></span><span class="mm-op-pay-icon">'+Cn.barcode+'</span><div class="mm-op-pay-body"><span class="mm-op-pay-title">Boleto Bancário</span><span class="mm-op-pay-sub">Aprovação em 1 a 3 dias úteis</span></div><div class="mm-op-pay-price"><span class="mm-op-pay-amount">'+R(e)+'</span></div></div><div class="mm-op-pay-detail"><ul class="mm-op-pay-benefits"><li>'+I.check+"<span>Código de barras enviado por e-mail</span></li><li>"+I.check+"<span>Pagamento em banco, lotérica ou app</span></li><li>"+I.check+"<span>Vencimento em 3 dias úteis</span></li></ul></div></label>";return'<section class="mm-op-step3-card" aria-labelledby="mm-op-step3-h"><div class="mm-op-step3-heading"><h2 id="mm-op-step3-h" class="mm-h">Como você quer pagar?</h2><p class="mm-op-step3-sub">Última etapa · escolha sua forma de pagamento '+(r>0?"· PIX tem desconto de "+R(r):"")+'</p></div><div class="mm-op-pay-radios" role="radiogroup" aria-label="Forma de pagamento">'+g+W+D+'</div><button type="button" class="mm-cta mm-op-cta mm-op-finalizar" data-mm-act="finalizar-compra">'+I.lock+'<span class="mm-op-finalizar-label">Finalizar compra · '+R(a)+'</span></button><p class="mm-op-finalizar-sub">'+I.shield+"<span>Pagamento seguro · criptografia SSL-256 · seus dados não são armazenados</span></p></section>"}function L(){return'<div class="mm-op-trust-payment"><div class="mm-op-trust-payment-row"><span class="mm-trust-item">'+I.lock+'<span>Site 100% seguro</span></span><span class="mm-trust-item">'+I.shield+'<span>Garantia 12 meses</span></span><span class="mm-trust-item">'+I.rotate+'<span>7 dias pra trocar</span></span><span class="mm-trust-item">'+I.whats+'<span>Atendimento humano</span></span></div><p class="mm-op-trust-payment-note">Ao finalizar, você concorda com os <a href="/termos-de-uso" target="_blank" rel="noopener">termos de compra</a> e a <a href="/politica-privacidade" target="_blank" rel="noopener">política de privacidade</a>.</p></div>'}function _(t,a,n){if(!t||!t.items||!t.items.length)return'<aside class="mm-id-sum mm-sum mm-op-step3-sum"><h2 class="mm-h">Resumo</h2><div class="mm-sum-card"><p class="mm-sum-empty">Não conseguimos carregar o resumo do seu pedido.</p></div></aside>';var e=a.pix?a.pix.valorTotal:0,r=a.cartao?a.cartao.valorTotal:0,s=a.boleto?a.boleto.valorTotal:0,d=r>e?r-e:0,g=n==="pix"?e:n==="boleto"?s:r,x=n==="pix"?"no PIX":n==="boleto"?"no boleto":"no cartão",k=3,N=t.items.slice(0,k),T=t.items.length-k,W=N.map(function(en){var pn=en.imgSrc?'<img src="'+O(en.imgSrc)+'" alt="'+O(en.name)+'" loading="lazy">':'<div class="mm-id-thumb-placeholder">'+I.box+"</div>",cn=en.quantity>1?'<strong class="mm-id-thumb-qty">'+en.quantity+"×</strong> ":"",yn=en.lineTotalPix>0?en.lineTotalPix:en.lineTotal;return'<div class="mm-id-thumb"><div class="mm-id-thumb-img">'+pn+'</div><div class="mm-id-thumb-body"><p class="mm-id-thumb-name">'+cn+O(en.name)+"</p>"+(en.variant?'<p class="mm-id-thumb-variant">'+O(en.variant)+"</p>":"")+'</div><div class="mm-id-thumb-price">'+R(yn)+"</div></div>"}).join("");T>0&&(W+='<div class="mm-id-thumb-more">+ '+T+" "+(T===1?"item":"itens")+" a mais</div>");var D=t.subtotalFull>0?t.subtotalFull:t.subtotalPix,X='<div class="mm-row"><span class="mm-row-label">Subtotal</span><span class="mm-row-value">'+R(D)+"</span></div>";if(t.shipping!==null&&t.shipping!==void 0){var U=t.shipping===0?'<span class="mm-row-value is-free">'+I.check+" Grátis</span>":'<span class="mm-row-value">'+R(t.shipping)+"</span>";X+='<div class="mm-row"><span class="mm-row-label">Frete'+(t.shippingDeadline?' <span class="mm-row-sub">· '+O(t.shippingDeadline)+"</span>":"")+"</span>"+U+"</div>"}t.discount>0&&(X+='<div class="mm-row"><span class="mm-row-label">Cupom'+(t.couponCode?' <span class="mm-row-sub">· '+O(t.couponCode)+"</span>":"")+'</span><span class="mm-row-value is-discount">− '+R(t.discount)+"</span></div>"),d>0&&n==="pix"&&(X+='<div class="mm-row mm-row-pix-discount"><span class="mm-row-label">'+I.bolt+' Desconto PIX</span><span class="mm-row-value is-discount">− '+R(d)+"</span></div>");var an='<div class="mm-total"><div class="mm-total-label">Total</div><div class="mm-total-value" data-mm-total>'+R(g)+'</div><div class="mm-total-pix" data-mm-total-sub><span>'+x+"</span></div></div>";return'<aside class="mm-id-sum mm-sum mm-op-step3-sum" data-active-forma="'+O(n)+'"><h2 class="mm-h">Resumo do pedido</h2><div class="mm-sum-card"><div class="mm-id-thumbs">'+W+'</div><div class="mm-sum-rows">'+X+"</div>"+an+"</div></aside>"}function Q(t,a){var n=document.getElementById("mm-layout");if(!n||n._mmStep3Bound)return;n._mmStep3Bound=!0;var e=dn(),r={activeForma:"pix",cardNumValid:!1,cardBrand:null,cardInstallments:null,submitting:!1};n.addEventListener("click",function(C){var z=C.target.closest(".mm-op-pay-radio");if(z){var H=z.querySelector('input[type="radio"]');H&&!H.checked&&(C.preventDefault(),H.checked=!0,k(z.getAttribute("data-forma")));return}if(C.target.closest('[data-mm-act="finalizar-compra"]')){C.preventDefault(),$n();return}var K=C.target.closest('[data-mm-act="edit-dados"], [data-mm-act="edit-endereco"]');if(K){C.preventDefault(),location.reload();return}}),n.addEventListener("input",function(C){var z=C.target;!z||!z.id||(z.id==="mm-pay-card-num"?X(z):z.id==="mm-pay-card-exp"?U(z):z.id==="mm-pay-card-cvv"&&(z.value=(z.value||"").replace(/\D/g,"").slice(0,4)))}),n.addEventListener("change",function(C){if(C.target&&C.target.id==="mm-pay-card-installments"){var z=C.target,H=z.options[z.selectedIndex];H&&H.value&&(r.cardInstallments={numero:parseInt(H.value,10),label:H.textContent||""},pn(H.value),d("mm-pay-card-installments"))}}),n.addEventListener("blur",function(C){var z=C.target;if(!(!z||!z.id)){var H=["mm-pay-card-num","mm-pay-card-name","mm-pay-card-exp","mm-pay-card-cvv"];H.indexOf(z.id)!==-1&&g(z.id)}},!0),n.addEventListener("focus",function(C){var z=C.target;!z||!z.id||/^mm-pay-card-/.test(z.id)&&d(z.id)},!0);function s(C,z){var H=document.getElementById(C),K=document.getElementById(C+"-err");H&&(H.classList.add("mm-input-error"),H.setAttribute("aria-invalid","true")),K&&(K.textContent=z,K.classList.add("is-visible"))}function d(C){var z=document.getElementById(C),H=document.getElementById(C+"-err");z&&(z.classList.remove("mm-input-error"),z.removeAttribute("aria-invalid")),H&&(H.textContent="",H.classList.remove("is-visible"))}function g(C){var z=document.getElementById(C);if(!z)return!0;var H=(z.value||"").trim();if(C==="mm-pay-card-num"){var K=H.replace(/\D/g,"");return K?K.length<13?(s(C,"Número do cartão incompleto"),!1):x(K)?(d(C),!0):(s(C,"Número do cartão inválido — confira os dígitos"),!1):(s(C,"Informe o número do cartão"),!1)}if(C==="mm-pay-card-name")return H?H.split(/\s+/).length<2?(s(C,"Use o nome completo como aparece no cartão"),!1):(d(C),!0):(s(C,"Informe o nome impresso no cartão"),!1);if(C==="mm-pay-card-exp"){var Z=H.replace(/\D/g,"");if(Z.length!==4)return s(C,"Informe a validade no formato MM/AA"),!1;var sn=parseInt(Z.slice(0,2),10),ln=parseInt(Z.slice(2),10);if(sn<1||sn>12)return s(C,"Mês inválido (01 a 12)"),!1;var gn=new Date,vn=gn.getFullYear()%100,xn=gn.getMonth()+1;return ln<vn||ln===vn&&sn<xn?(s(C,"Cartão expirado"),!1):(d(C),!0)}if(C==="mm-pay-card-cvv"){var Bn=H.replace(/\D/g,"");return Bn.length<3?(s(C,"CVV deve ter 3 ou 4 dígitos"),!1):(d(C),!0)}return C==="mm-pay-card-installments"?H?(d(C),!0):(s(C,"Selecione o número de parcelas"),!1):!0}function x(C){for(var z=0,H=!1,K=C.length-1;K>=0;K--){var Z=parseInt(C.charAt(K),10);H&&(Z*=2,Z>9&&(Z-=9)),z+=Z,H=!H}return z%10===0}function k(C){if(!(!C||r.activeForma===C)){r.activeForma=C,n.querySelectorAll(".mm-op-pay-radio").forEach(function(Z){Z.classList.toggle("is-active",Z.getAttribute("data-forma")===C)});var z=document.getElementById("forma-pagto-"+C);if(z&&!z.checked)try{z.click()}catch{}var H=n.querySelectorAll(".mm-op-card-form input, .mm-op-card-form select"),K=C==="cartao";H.forEach(function(Z){Z.disabled=!K,K?Z.dataset.mmac&&Z.setAttribute("autocomplete",Z.dataset.mmac):Z.setAttribute("autocomplete","off")}),N(C),C==="cartao"&&setTimeout(function(){var Z=document.getElementById("mm-pay-card-num");Z&&!Z.value&&Z.focus()},250)}}function N(C){var z=n.querySelector(".mm-op-step3-sum");if(z){z.setAttribute("data-active-forma",C);var H=a.pix?a.pix.valorTotal:0,K=a.cartao?a.cartao.valorTotal:0,Z=a.boleto?a.boleto.valorTotal:0,sn=K>H?K-H:0,ln=C==="pix"?H:C==="boleto"?Z:K,gn=C==="pix"?"no PIX":C==="boleto"?"no boleto":"no cartão",vn=z.querySelector("[data-mm-total]");if(vn){var xn=vn.textContent||"",Bn=G(xn);Bn!==ln?T(vn,Bn,ln,360):vn.textContent=R(ln)}var En=z.querySelector("[data-mm-total-sub] span");En&&En.textContent!==gn&&(En.textContent=gn);var Dn=z.querySelector(".mm-sum-rows"),Rn=Dn?Dn.querySelector(".mm-row-pix-discount"):null;if(C==="pix"&&sn>0){if(!Rn&&Dn){var ot=document.createElement("div");ot.innerHTML='<div class="mm-row mm-row-pix-discount"><span class="mm-row-label">'+I.bolt+' Desconto PIX</span><span class="mm-row-value is-discount">− '+R(sn)+"</span></div>",Dn.appendChild(ot.firstChild)}}else Rn&&Rn.remove();W(C)}}function T(C,z,H,K){C._mmAnimToken&&cancelAnimationFrame(C._mmAnimToken);var Z=null,sn=H-z;function ln(gn){Z||(Z=gn);var vn=gn-Z,xn=Math.min(1,vn/K),Bn=1-Math.pow(1-xn,3),En=z+sn*Bn;C.textContent=R(En),xn<1?C._mmAnimToken=requestAnimationFrame(ln):(C.textContent=R(H),C._mmAnimToken=null)}C._mmAnimToken=requestAnimationFrame(ln)}function W(C){var z=n.querySelector(".mm-op-finalizar-label");if(z){var H=C==="pix"?a.pix&&a.pix.valorTotal:C==="boleto"?a.boleto&&a.boleto.valorTotal:a.cartao&&a.cartao.valorTotal;z.textContent="Finalizar compra · "+R(H||0)}}function D(C){var z=(C||"").replace(/\D/g,"");return z?/^4/.test(z)?"visa":/^(5[1-5]|22[2-9]|2[3-6]|27[01]|2720)/.test(z)?"mastercard":/^3[47]/.test(z)?"amex":/^(4011|4312|4389|4514|4573|5041|5066|5067|509|6277|6362|6363|650|6516|6550)/.test(z)?"elo":/^(606282|384100|384140|384160|606|637095|637568|637599|637609|637612)/.test(z)?"hipercard":null:null}function X(C){var z=(C.value||"").replace(/\D/g,"").slice(0,19),H=z.replace(/(\d{4})(?=\d)/g,"$1 ");if(H!==C.value){var K=C.selectionStart;C.value=H;try{C.setSelectionRange(K+1,K+1)}catch{}}var Z=D(z);r.cardBrand=Z,r.cardNumValid=z.length>=13;var sn=n.querySelector(".mm-op-card-brand-detected");sn&&(sn.className="mm-op-card-brand-detected"+(Z?" is-"+Z:""),sn.textContent=Z?Z.toUpperCase():""),z.length>=6&&(an(z),yn())}function U(C){var z=(C.value||"").replace(/\D/g,"").slice(0,4),H=z.length>2?z.slice(0,2)+"/"+z.slice(2):z;if(C.value=H,z.length===4){var K=z.slice(0,2),Z="20"+z.slice(2);en("pag-cartao-mes-validade",String(parseInt(K,10))),en("pag-cartao-ano-validade",Z)}}function an(C){var z=document.getElementById("pag-cartao-numero");if(z){try{var H=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;H.call(z,C)}catch{z.value=C}z.dispatchEvent(new Event("input",{bubbles:!0})),z.dispatchEvent(new Event("change",{bubbles:!0})),z.dispatchEvent(new Event("blur",{bubbles:!0}))}}function en(C,z){var H=document.getElementById(C);if(H){try{var K=Object.getOwnPropertyDescriptor(window.HTMLSelectElement.prototype,"value").set;K.call(H,z)}catch{H.value=z}H.dispatchEvent(new Event("change",{bubbles:!0}))}}function pn(C){en("pag-cartao-parcela",C)}var cn=null;function yn(){if(cn)return;var C=document.getElementById("pag-cartao-parcela");if(!C)return;var z=document.getElementById("mm-pay-card-installments");if(!z)return;function H(){var K=C.querySelectorAll("option");if(!(K.length<=1)){var Z="";K.forEach(function(sn){if(!sn.value){Z+='<option value="">Selecione as parcelas</option>';return}Z+='<option value="'+O(sn.value)+'">'+O(sn.textContent.trim())+"</option>"}),z.innerHTML=Z,z.options.length>1&&(z.selectedIndex=1,r.cardInstallments={numero:parseInt(z.options[1].value,10)||1,label:z.options[1].textContent},pn(z.options[1].value))}}H(),cn=new MutationObserver(H),cn.observe(C,{childList:!0,subtree:!0})}function $n(){if(console.log("[mm-checkout] submitFinalizar() entry",{submitting:r.submitting,activeForma:r.activeForma}),r.submitting)return;var C=r.activeForma,z=n.querySelector(".mm-op-finalizar");if(!z){console.log("[mm-checkout] submitFinalizar: no .mm-op-finalizar btn");return}if(C==="cartao"){var H=["mm-pay-card-num","mm-pay-card-name","mm-pay-card-exp","mm-pay-card-cvv","mm-pay-card-installments"],K=H.filter(function(on){return!g(on)});if(console.log("[mm-checkout] validation",{errorCount:K.length,errors:K}),K.length){var Z=document.getElementById(K[0]);if(Z){Z.focus();try{Z.scrollIntoView({block:"center",behavior:"smooth"})}catch{}}return}var sn=document.getElementById("mm-pay-card-name"),ln=document.getElementById("mm-pay-card-cvv"),gn=document.getElementById("pag-cartao-titular");if(gn){try{var vn=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;vn.call(gn,sn.value.trim())}catch{gn.value=sn.value.trim()}gn.dispatchEvent(new Event("input",{bubbles:!0})),gn.dispatchEvent(new Event("blur",{bubbles:!0}))}var xn=document.getElementById("pag-cartao-vericacao");if(xn){try{var Bn=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;Bn.call(xn,ln.value.replace(/\D/g,""))}catch{xn.value=ln.value.replace(/\D/g,"")}xn.dispatchEvent(new Event("input",{bubbles:!0})),xn.dispatchEvent(new Event("blur",{bubbles:!0}))}var En=document.getElementById("pag-cartao-cpf");if(En&&t&&t.cpf){try{var Dn=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;Dn.call(En,t.cpf)}catch{En.value=t.cpf}En.dispatchEvent(new Event("input",{bubbles:!0})),En.dispatchEvent(new Event("blur",{bubbles:!0}))}}r.submitting=!0,z.classList.add("is-loading"),z.setAttribute("aria-busy","true");var Rn=z.querySelector(".mm-op-finalizar-label");if(Rn&&(Rn.textContent="Processando pagamento..."),rt("Finalizando seu pedido..."),C==="cartao"){var ot=document.getElementById("mm-pay-card-installments");ot&&ot.value&&pn(ot.value)}window.__mmCheckoutDebug=window.__mmCheckoutDebug||[];function fn(on,Tn){var Mn={t:new Date().toISOString(),msg:on,data:Tn};window.__mmCheckoutDebug.push(Mn),console.log("[mm-checkout]",on,Tn||"")}function dt(){if(fn("doSubmit() called",{forma:C}),C==="cartao"){var on=document.getElementById("aceito-termos-bcash-one-card");on&&!on.checked&&(on.checked=!0,on.dispatchEvent(new Event("change",{bubbles:!0}))),fn("terms",{checked:on?.checked})}var Tn=C==="pix"?"form-pag-pix":C==="boleto"?"form-pag-boleto":"form-pag-cartao",Mn=document.getElementById(Tn);if(!Mn){fn("ERROR: form not found",{formId:Tn}),alert("Erro interno: formulário de pagamento não encontrado. Recarregue a página."),r.submitting=!1,z.classList.remove("is-loading");var Nn=document.getElementById("mm-op-overlay");Nn&&Nn.remove();return}C==="cartao"&&fn("pre-submit cartao state",{numero:document.getElementById("pag-cartao-numero")?.value,bandeira:document.getElementById("pag-cartao-bandeira")?.value,parcela:document.getElementById("pag-cartao-parcela")?.value,titular:document.getElementById("pag-cartao-titular")?.value,mes:document.getElementById("pag-cartao-mes-validade")?.value,ano:document.getElementById("pag-cartao-ano-validade")?.value,cvv:document.getElementById("pag-cartao-vericacao")?.value?.length,cpf:document.getElementById("pag-cartao-cpf")?.value,tokenMp:document.getElementById("pag-cartao-token-mp")?.value?.substring(0,20)});var jn=Mn.querySelector('button.button-success, button[type="submit"], input[type="submit"]');if(jn)fn("clicking native button",{text:jn.textContent?.trim()}),jn.click();else if(typeof Mn.requestSubmit=="function"){fn("no native btn, using requestSubmit");try{Mn.requestSubmit()}catch(Kn){fn("requestSubmit error",Kn.message),Mn.submit()}}else fn("no native btn, using form.submit()"),Mn.submit();setTimeout(function(){if(r.submitting&&location.pathname.indexOf("/onepage")!==-1){fn("8s failsafe: still on /onepage, removing overlay"),r.submitting=!1,z.classList.remove("is-loading");var Kn=document.getElementById("mm-op-overlay");Kn&&Kn.remove(),S.classList.remove("mm-shadow-mode"),n&&(n.style.display="none")}},8e3),setTimeout(function(){S.classList.remove("mm-shadow-mode"),n&&(n.style.display="none")},600)}function yt(){r.submitting=!1,z.classList.remove("is-loading"),z.removeAttribute("aria-busy");var on=z.querySelector(".mm-op-finalizar-label");on&&(on.textContent="Finalizar compra");var Tn=document.getElementById("mm-op-overlay");Tn&&Tn.remove()}function Ft(){var on=Date.now(),Tn=1e4;(function Mn(){var Nn=document.getElementById("pag-cartao-token-mp"),jn=Nn?(Nn.value||"").trim():"",Kn=jn&&jn!=="loading..."&&jn.length>10;if(Kn){fn("fallback: token ready"),dt();return}if(Date.now()-on>=Tn){fn("fallback: timeout",{lastVal:jn}),dt();return}setTimeout(Mn,200)})()}function It(){if(fn("generateMpTokenAndSubmit() start"),typeof Mercadopago>"u"){fn("Mercadopago global missing, falling back to wait strategy"),Ft();return}var on=document.getElementById("pag-cartao-token-mp"),Tn=on?(on.value||"").trim():"";if(Tn&&Tn!=="loading..."&&Tn.length>10){fn("token already present, submitting",{len:Tn.length}),dt();return}var Mn=(document.getElementById("mm-pay-card-num")?.value||"").replace(/\D/g,""),Nn=(document.getElementById("mm-pay-card-exp")?.value||"").replace(/\D/g,""),jn=(document.getElementById("mm-pay-card-cvv")?.value||"").replace(/\D/g,""),Kn=(document.getElementById("mm-pay-card-name")?.value||"").trim(),Tt=(t&&t.cpf||document.getElementById("mm-op-cpf")?.value||"").replace(/\D/g,"");if(!Mn||!Nn||!jn||!Kn||!Tt){fn("missing card fields",{num:Mn.length,exp:Nn.length,cvv:jn.length,name:!!Kn,doc:Tt.length}),alert("Preencha todos os dados do cartão antes de finalizar."),yt();return}var Mt=Nn.slice(0,2),Bt=Nn.length===4?"20"+Nn.slice(2):Nn.slice(2);fn("calling Mercadopago.createToken",{numLen:Mn.length,month:Mt,year:Bt});try{Mercadopago.createToken({cardNumber:Mn,securityCode:jn,cardExpirationMonth:Mt,cardExpirationYear:Bt,cardholderName:Kn,docType:"CPF",docNumber:Tt},function(kt,Jn){if(fn("createToken callback",{status:kt,hasId:!!(Jn&&Jn.id),err:Jn&&Jn.error}),kt===200||kt===201){if(on){var Pt=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;Pt.call(on,Jn.id),on.dispatchEvent(new Event("input",{bubbles:!0})),on.dispatchEvent(new Event("change",{bubbles:!0}))}dt()}else{var Lt="Não foi possível validar os dados do cartão.";Jn&&Jn.cause&&Jn.cause[0]&&Jn.cause[0].description&&(Lt=Jn.cause[0].description),alert(Lt),yt()}})}catch(kt){fn("createToken exception",kt.message),Ft()}}C==="cartao"?It():setTimeout(dt,500)}}if(F){let t=function(n){if(n=n||0,n>30){a();return}var e=S.querySelector("#cep, .box-area-dados, #nome-completo_2");e||n>8?a():setTimeout(function(){t(n+1)},250)},a=function(){var n=dn(),e=tn.get("mm_user_email")||"";tn.remove("mm_checkout_mode"),typeof window._mmDraftDebug>"u"&&(window._mmDraftDebug=!1),At(n,e),On();var r=Wn(),s=tn.get(P),d=s&&String(s).replace(/\D/g,"").length===8;d?at("loading"):n&&typeof n.shipping=="number"&&n.shipping>0&&at({value:n.shipping,name:n.shippingName||"",deadline:n.shippingDeadline||"",city:n.shippingCity||"",options:n.shippingOptions||[]});try{var g=Array.from(S.querySelectorAll("a, button")).find(function(N){var T=(N.textContent||"").toLowerCase();return T.indexOf("sem cadastro")!==-1&&N.offsetParent!==null});g&&!g.classList.contains("active")&&g.click()}catch{}ft();var x=tn.get(P);if(x&&x.length===8){var k=document.getElementById("mm-op-cep");k&&(k.value=zn(x),setTimeout(function(){Un(x)},400))}setTimeout(function(){if(!("ontouchstart"in window))for(var N=["mm-op-email","mm-op-nome","mm-op-cpf","mm-op-tel","mm-op-cep"],T=0;T<N.length;T++){var W=document.getElementById(N[T]);if(W&&!W.value){W.focus();break}}},350)};t()}if(q){document.documentElement.classList.remove("mm-cart-loading");var V=S.querySelector('input[placeholder*="numero do cart" i]');V&&(V.inputMode="numeric");var Y=S.querySelector('input[placeholder*="000" i]');Y&&(!Y.maxLength||Y.maxLength<=4)&&(Y.inputMode="numeric")}setTimeout(function(){document.documentElement.classList.contains("mm-cart-loading")&&(console.warn("[mm-cart] failsafe: removing mm-cart-loading after 2s timeout"),document.documentElement.classList.remove("mm-cart-loading"))},2e3)})()})();
