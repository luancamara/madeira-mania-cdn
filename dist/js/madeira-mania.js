...
🎉 Inicialização concluída com sucesso!`,"success"),O.retry.usarMutationObserver&&this.observarMudancasDOM()):this.tentativasDeCarregamento<O.retry.maxTentativas?setTimeout(()=>this.tentarCarregar(),O.retry.intervaloMs):this.log("❌ Número máximo de tentativas atingido.","error")}observarMudancasDOM(){this.log("ℹ️ MutationObserver desativado - dados carregados via dataProduct","info")}log(o,m="log",p=null){if(!O.debug)return;const d={info:"color: #2196f3; font-weight: bold",success:"color: #4caf50; font-weight: bold",warning:"color: #ff9800; font-weight: bold",error:"color: #f44336; font-weight: bold",log:"color: #666"};console.log(`%c${o}`,d[m]||d.log),p&&console.log(p)}carregarProdutos(){if(this.produtos=[],typeof dataProduct>"u"||!dataProduct.listaProdutosSugeridos)return this.log("❌ dataProduct ou listaProdutosSugeridos não encontrado","error"),!1;const o=dataProduct.listaProdutosSugeridos,m=dataProduct.produto,p=dataProduct.hostImagem||"";if(this.log(`📦 Encontrados ${o.length} produtos sugeridos + produto atual`,"info"),m&&m.complemento){const d=m.midia_path&&m.midia_arquivo_nome?`${p}/${m.midia_path}${m.midia_arquivo_nome}`:"",c={id:m.derivacao_id||m.produto_id,nomeCompleto:m.complemento.trim(),estoque:m.qtde_estoque,url:m.link?`/${m.link}`:"",imagem:d,imagemData:d,elemento:null,variacoes:{},nomeBase:"",nomeExibicao:"",isAtual:!0};this.produtoAtualId=c.id,this.extrairVariacoesDoNome(c),this.produtos.push(c),this.log(`   ✓ Produto atual: "${c.nomeCompleto}"`,"success")}return o.forEach((d,c)=>{const k=d.complemento||d.nome||"";if(!k)return;const b=d.derivacao_id||d.produto_id;if(b===this.produtoAtualId){this.log(`   ⏭️ Ignorando duplicata: "${k}"`,"info");return}const q=d.midia_path&&d.midia_arquivo_nome?`${p}/${d.midia_path}${d.midia_arquivo_nome}`:"",C={id:b||c,nomeCompleto:k.trim(),estoque:d.qtde_estoque,url:d.link?`/${d.link}`:"",imagem:q,imagemData:q,elemento:null,variacoes:{},nomeBase:"",nomeExibicao:"",isAtual:!1};this.extrairVariacoesDoNome(C),this.produtos.push(C),this.log(`   ✓ Sugerido: "${C.nomeCompleto}"`,"log")}),this.produtos.length===0?!1:(this.log("✅ Produtos carregados e processados:","success",this.produtos),!0)}normalizarSeparadores(o){const m=["–","—","−","‐","‑","⁃"];let p=o;return m.forEach(d=>{const c=new RegExp(`\\s${d}\\s`,"g");p=p.replace(c," - ")}),p}extrairVariacoesDoNome(o){const p=this.normalizarSeparadores(o.nomeCompleto).split(O.formatoNome.separador);this.log(`
📝 Processando: "${o.nomeCompleto}"`,"log"),O.formatoNome.primeiraParte==="nome_base"&&(o.nomeBase=p[0].trim(),p.shift()),p.forEach(d=>{const c=d.trim();if(c&&c.includes(O.formatoNome.separadorTipoValor)){const[k,...b]=c.split(O.formatoNome.separadorTipoValor),q=b.join(O.formatoNome.separadorTipoValor).trim(),C=this.normalizarTipo(k.trim());if(O.ignorarPalavras.includes(C))return;o.variacoes[C]=q,this.log(`   ✓ ${C}: ${q}`,"success")}}),o.nomeExibicao=O.formatoNome.exibirNomeCompleto?o.nomeCompleto:o.nomeBase||o.nomeCompleto,Object.keys(o.variacoes).length===0&&(o.variacoes.MODELO=o.nomeCompleto,o.nomeExibicao=o.nomeCompleto)}normalizarTipo(o){return o.toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").trim()}processarVariacoes(){this.log(`
🔄 Processando variações...`,"info");const o=new Set;this.produtos.forEach(m=>{Object.keys(m.variacoes).forEach(p=>o.add(p))}),o.forEach(m=>{const p=new Set,d={};this.produtos.forEach(k=>{const b=k.variacoes[m];b&&(p.add(b),d[b]||(d[b]=[]),d[b].push(k))});const c=Array.from(p).sort();this.variacoes[m]={label:O.labels[m]||m,valores:c,produtosPorValor:d,usarImagem:O.variacoesComImagem.includes(m)},this.log(`   📊 ${m}: ${c.length} valor(es) único(s) → [${c.join(", ")}]`,c.length>1?"success":"warning")}),this.log("✅ Variações processadas:","success",this.variacoes)}renderizarVariacoes(){this.log(`
🎨 Renderizando variações...`,"info");let o=g(O.selectors.areaVariacoes);if(o.length===0&&(this.criarAreaVariacoes(),o=g(O.selectors.areaVariacoes)),Object.keys(this.variacoes).length===0)return;const m=g("<div>",{class:"product-variations-pills-container"});let p=0;if(Object.keys(this.variacoes).forEach(d=>{const c=this.variacoes[d];if(c.valores.length<=1){this.log(`⏭️ Ignorando "${d}" - apenas ${c.valores.length} valor(es)`,"info");return}if(c.usarImagem){const k=this.criarGrupoSwatches(d,c);m.append(k),p++}}),Object.keys(this.variacoes).forEach(d=>{const c=this.variacoes[d];if(!(c.valores.length<=1)&&!c.usarImagem){const k=this.criarGrupoPills(d,c);m.append(k),p++}}),p===0){this.log("ℹ️ Nenhuma variação com múltiplas opções - ocultando área","info"),o.closest(".derivacoes-produto").hide(),g(O.selectors.subtituloProduto).hide();return}O.performance.useRequestAnimationFrame&&window.requestAnimationFrame?requestAnimationFrame(()=>{o.empty().append(m),this.log(`✅ ${p} variação(ões) renderizada(s)!`,"success"),this.atualizarNomeProduto()}):(o.empty().append(m),this.log(`✅ ${p} variação(ões) renderizada(s)!`,"success"),this.atualizarNomeProduto())}criarAreaVariacoes(){const o=g(O.selectors.areaProdutosSugeridos);o.length>0?o.before('<div class="derivacoes-produto"><div class="area-derivacoes"></div></div>'):g("body").prepend('<div class="derivacoes-produto"><div class="area-derivacoes"></div></div>')}criarGrupoSwatches(o,m){const p=this.obterValorAtualParaTipo(o),d=g("<div>",{class:"variation-pill-group variation-pill-group--swatches","data-variacao-tipo":o,role:"group","aria-labelledby":`pill-label-${o.toLowerCase()}`}),c=g("<div>",{class:"variation-pill-label",id:`pill-label-${o.toLowerCase()}`});c.append(g("<span>").text(m.label+":")),c.append(g("<span>",{class:"variation-pill-label-value","data-label-value":o}).text(p||""));const k=g("<div>",{class:"variation-swatches-container",role:"radiogroup","aria-labelledby":`pill-label-${o.toLowerCase()}`});return m.valores.forEach((b,q)=>{const C=m.produtosPorValor[b],A=C.some(sn=>sn.estoque===void 0||sn.estoque>0),x=b===p,F=`pill-${o.toLowerCase()}-${this.sanitizeId(b)}`,V=this.encontrarMelhorProdutoParaSwatch(o,b,C);let I=null;V&&(I=V.imagemData||V.imagem);const z=g("<input>",{type:"radio",class:"variation-pill-input",id:F,name:`variation-${o}`,value:b,"data-variacao-tipo":o,"data-produtos":JSON.stringify(C.map(sn=>({id:sn.id,url:sn.url}))),checked:x,disabled:!A,"aria-label":`${m.label}: ${b}${A?"":" (Indisponível)"}`}),Z=g("<label>",{class:"variation-color-swatch-wrapper",for:F,"data-tooltip":b}),tn=g("<div>",{class:"variation-color-swatch","data-valor":b,tabindex:x?0:-1});I?tn.append(g("<img>",{src:I,alt:b,class:"variation-color-swatch-image",loading:"lazy"})):tn.css({"background-color":"#E5E7EB",display:"flex","align-items":"center","justify-content":"center","font-size":"14px",color:"#6B7280"}).text(b.charAt(0).toUpperCase());const en=g("<span>",{class:"variation-color-swatch-name",text:b,title:b});Z.append(tn).append(en),k.append(z).append(Z)}),d.append(c).append(k),d}criarGrupoPills(o,m){const p=this.obterValorAtualParaTipo(o),d=g("<div>",{class:"variation-pill-group variation-pill-group--pills","data-variacao-tipo":o,role:"group","aria-labelledby":`pill-label-${o.toLowerCase()}`}),c=g("<div>",{class:"variation-pill-label",id:`pill-label-${o.toLowerCase()}`});c.append(g("<span>").text(m.label+":")),c.append(g("<span>",{class:"variation-pill-label-value","data-label-value":o}).text(p||""));const k=g("<div>",{class:"variation-pills-container",role:"radiogroup","aria-labelledby":`pill-label-${o.toLowerCase()}`});return m.valores.forEach((b,q)=>{const C=m.produtosPorValor[b],A=C.some(Z=>Z.estoque===void 0||Z.estoque>0),x=b===p,F=`pill-${o.toLowerCase()}-${this.sanitizeId(b)}`,V=g("<input>",{type:"radio",class:"variation-pill-input",id:F,name:`variation-${o}`,value:b,"data-variacao-tipo":o,"data-produtos":JSON.stringify(C.map(Z=>({id:Z.id,url:Z.url}))),checked:x,disabled:!A,"aria-label":`${m.label}: ${b}${A?"":" (Indisponível)"}`});let I=b;A||(I+=' <span class="variation-pill-badge">Indisponível</span>');const z=g("<label>",{class:"variation-pill",for:F,html:I,"data-valor":b,tabindex:x?0:-1});k.append(V).append(z)}),d.append(c).append(k),d}sanitizeId(o){return o.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}encontrarMelhorProdutoParaSwatch(o,m,p){const d=this.produtos.find(q=>q.isAtual||q.id===this.produtoAtualId);if(!d||p.length===0)return p[0]||null;if(p.length===1)return p[0];const c=d.variacoes;let k=null,b=-1;return p.forEach(q=>{let C=0;Object.keys(c).forEach(A=>{A!==o&&q.variacoes[A]===c[A]&&C++}),(q.imagemData||q.imagem)&&(C+=.5),C>b&&(b=C,k=q)}),this.log(`   🎯 Melhor produto para ${o}="${m}": score=${b}`,"log"),k||p[0]}obterValorAtualParaTipo(o){const m=this.produtos.find(p=>p.isAtual||p.id===this.produtoAtualId);return m?m.variacoes[o]:null}atualizarNomeProduto(){const o=this.produtos.find(p=>p.isAtual||p.id===this.produtoAtualId);if(!o)return;["h1.nome-produto",".product-name h1",".info-produto h1",'h1[itemprop="name"]',".box-info-produto h1"].forEach(p=>{const d=g(p);d.length>0&&d.text(o.nomeExibicao)})}bindEventos(){this.log(`
🔗 Vinculando eventos...`,"info"),g(document).on("change",".variation-pill-input",o=>{this.aoMudarVariacao(o)}),g(document).on("keydown",".variation-pills-container, .variation-swatches-container",o=>{this.handleKeyboardNavigation(o)}),g(document).on("click",".variation-pill, .variation-color-swatch-wrapper",function(){const o=g(this).is("label")?g("#"+g(this).attr("for")):g(this).closest("label").prev(".variation-pill-input");o.length&&!o.prop("disabled")&&g(this).closest(".variation-pill-group").addClass("is-loading")}),this.log("✅ Eventos vinculados","success")}handleKeyboardNavigation(o){const p=g(o.currentTarget).find(".variation-pill-input:not(:disabled)"),d=g(document.activeElement);if(!d.hasClass("variation-pill-input"))return;const c=p.index(d);let k=c;switch(o.key){case"ArrowRight":case"ArrowDown":o.preventDefault(),k=(c+1)%p.length;break;case"ArrowLeft":case"ArrowUp":o.preventDefault(),k=c-1<0?p.length-1:c-1;break;case"Home":o.preventDefault(),k=0;break;case"End":o.preventDefault(),k=p.length-1;break;default:return}p.eq(k).focus().prop("checked",!0).trigger("change")}aoMudarVariacao(o){const m=g(o.target),p=m.data("variacao-tipo"),d=m.val();this.log(`
🔄 Variação selecionada: ${p} = ${d}`,"info"),g(`.variation-pill-label-value[data-label-value="${p}"]`).text(d);const c={};g(".variation-pill-input:checked").each(function(){const b=g(this).data("variacao-tipo"),q=g(this).val();q&&(c[b]=q)}),this.log("📋 Seleção atual:","info",c);const k=this.encontrarProdutoPorVariacoes(c);if(k)this.log("✅ Produto encontrado!","success",k),this.navegarParaProduto(k);else{this.log("⚠️ Produto exato não encontrado, buscando melhor correspondência...","warning");const b=this.encontrarMelhorCorrespondencia(c);b?(this.log("✅ Melhor correspondência encontrada!","success",b),this.navegarParaProduto(b)):(this.log("❌ Nenhum produto correspondente encontrado","error"),g(".variation-pill-group").removeClass("is-loading"))}}encontrarProdutoPorVariacoes(o){return this.produtos.find(m=>Object.keys(o).every(p=>m.variacoes[p]===o[p]))}encontrarMelhorCorrespondencia(o){let m=null,p=0;return this.produtos.forEach(d=>{let c=0;Object.keys(o).forEach(k=>{d.variacoes[k]===o[k]&&c++}),c>p&&(p=c,m=d)}),p>0?m:null}navegarParaProduto(o){this.log(`
🚀 Navegando para: ${o.url}`,"info"),o.url?window.location.href=o.url:(this.log("❌ URL não encontrada para navegação","error"),g(".variation-pill-group").removeClass("is-loading"))}}setTimeout(function(){g(".product-variations-pills-container").remove(),g(".derivacoes-produto").remove();const h=new B;h.init(),window.GerenciadorVariacoesPillsMagazord=h},100)})(typeof jQuery<"u"?jQuery:window.jQuery||window.$),(function(){"use strict";function g(){if(!(typeof jQuery>"u"&&typeof $>"u")){var i=typeof jQuery<"u"?jQuery:$;i(document).ajaxComplete(function(l,u,E){E.url&&E.url.indexOf("checkout/cart")!==-1&&setTimeout(function(){window.dispatchEvent(new CustomEvent("reactItemAddedToCart"))},100)})}}function P(){var i=document.querySelector("#cart-preview-area .border-t.border-solid");if(!(!i||i.querySelector(".installment-total"))){var l=0,u=document.querySelectorAll("#cart-preview-area .cart-item");if(u.forEach(function(_){var R=parseFloat(_.getAttribute("data-item-price"))||0,U=parseInt(_.getAttribute("data-item-quantity"))||1;l+=R*U}),!(l<=0)){var E=(l/12).toFixed(2).replace(".",","),M=document.createElement("div");M.className="installment-total",M.textContent="ou 12x de R$ "+E;var L=i.querySelector(".valor-pix");L&&L.parentNode&&L.parentNode.insertBefore(M,L.nextSibling)}}}var O='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><path d="M5 12h14"/></svg>',B='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg>',h='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v2"/></svg>';function o(){P();var i=document.querySelectorAll("#cart-preview-area .cart-item, .content-cart .cart-item");i.forEach(function(l){if(!l.querySelector(".qty-btn-minus")){var u=l.querySelector(".qtd-value");if(u){var E=l.querySelector(".cart-remove-item"),M=E?E.getAttribute("data-id"):null;if(M){var L=l.querySelector(".prod-remove");L&&!u.contains(E)&&(u.appendChild(E),L.style.display="none");var _=u.parentElement,R=null;if(_)for(var U=0;U<_.children.length;U++){var K=_.children[U];if(K!==u&&K.classList&&K.classList.contains("valor")){R=K;break}}R&&!u.contains(R)&&u.appendChild(R);var an=parseInt(l.getAttribute("data-item-quantity"));if(!an||isNaN(an)){var Q=u.textContent.match(/(\d+)/);an=Q?parseInt(Q[1]):1}var on=document.createElement("button");on.className="qty-btn-minus",on.type="button",on.setAttribute("aria-label","Diminuir quantidade"),on.innerHTML=O,on.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation();var n=parseInt(pn.textContent)||1;if(n<=1){var e=l.querySelector(".cart-remove-item");e&&e.click();return}st(l,M,-1,pn,on,Y)});var pn=document.createElement("span");pn.className="qty-display",pn.textContent=an;var Y=document.createElement("button");Y.className="qty-btn-plus",Y.type="button",Y.setAttribute("aria-label","Aumentar quantidade"),Y.innerHTML=B,Y.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation(),st(l,M,1,pn,on,Y)});var t=document.createElement("div");t.className="mm-qty-wrap",t.appendChild(on),t.appendChild(pn),t.appendChild(Y),u.insertBefore(t,u.firstChild),E&&(E.innerHTML=h,E.setAttribute("aria-label","Remover produto"))}}}})}function m(){document.addEventListener("click",function(i){var l=i.target.closest(".cart-remove-item");if(!(!l||!l.closest("#cart-preview-area"))){i.preventDefault(),i.stopPropagation(),i.stopImmediatePropagation();var u=l.getAttribute("data-id");if(u){var E=l.closest(".cart-item"),M=E&&E.querySelector(".prod-nome")?.textContent?.trim()||"este produto",L=M.length>50?M.substring(0,50)+"…":M,_=document.getElementById("mm-confirm-overlay");_&&_.remove();var R=document.createElement("div");R.id="mm-confirm-overlay",R.className="mm-confirm-overlay",R.innerHTML='<div class="mm-confirm-card"><p class="mm-confirm-title">Remover produto?</p><p class="mm-confirm-desc">'+L.replace(/&/g,"&amp;").replace(/</g,"&lt;")+'</p><div class="mm-confirm-actions"><button class="mm-confirm-btn mm-confirm-btn-cancel">Manter</button><button class="mm-confirm-btn mm-confirm-btn-delete">Remover</button></div></div>',document.body.appendChild(R),R.querySelector(".mm-confirm-btn-cancel").addEventListener("click",function(){R.remove()}),R.querySelector(".mm-confirm-btn-delete").addEventListener("click",function(){R.remove(),window.__mmDeleteItem&&E?window.__mmDeleteItem(E,u):typeof Zord<"u"&&Zord.checkout&&Zord.checkout.deleteItem(parseInt(u))}),R.addEventListener("click",function(U){U.target===R&&R.remove()})}}},!0)}function p(){document.addEventListener("click",function(i){var l=i.target.closest(".finalizar-compra");l&&l.closest("#cart-preview-area")&&(i.preventDefault(),i.stopPropagation(),window.location.href="/checkout/identify")},!0)}function d(i,l){var u="cep=&cupom-desconto=&nenhumCreditoSelecionado=true&id="+encodeURIComponent(String(l))+"&area=main-cart";return fetch("/checkout/cart?operation="+encodeURIComponent(i),{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/x-www-form-urlencoded",Accept:"text/html,application/json,*/*","X-Requested-With":"XMLHttpRequest"},body:u}).then(function(E){if(!E.ok)throw new Error("HTTP "+E.status);return E.text()})}var c=1500,k=2e3,b="mm_cep",q="mm_cart_snapshot",C=1800*1e3;function A(){try{var i=localStorage.getItem(b)||"",l=i.replace(/\D/g,"");if(l.length===8)return l}catch{}return null}function x(i){return!i||i.length!==8?"":i.slice(0,5)+"-"+i.slice(5)}function F(i){if(!i||i.length!==8)return k;var l=parseInt(i.slice(0,2),10);return isNaN(l)?k:l>=1&&l<=39||l>=80&&l<=99?c:k}function V(){try{var i=localStorage.getItem(q);if(!i)return null;var l=JSON.parse(i);return!l||!l.ts||Date.now()-l.ts>C?null:l}catch{return null}}function I(i){var l=[];return i.forEach(function(u){var E=(u.querySelector(".prod-nome a, .prod-nome")||{}).textContent||"",M=u.querySelector(".qty-display"),L=M?parseInt(M.textContent):parseInt(u.getAttribute("data-item-quantity"))||1;l.push(E.trim().slice(0,30)+"x"+L)}),l.sort().join("|")}function z(i){if(!i||!Array.isArray(i.items))return"";var l=i.items.map(function(u){return(u.name||"").trim().slice(0,30)+"x"+(u.quantity||1)});return l.sort().join("|")}var Z='<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><polyline points="20 6 9 17 4 12"/></svg>',tn='<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M18 6 6 18M6 6l12 12"/></svg>',en=["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"];function sn(i){if(!i)return null;var l=String(i).match(/\d+/g);return!l||!l.length?null:Math.max.apply(null,l.map(Number))}function zn(i,l){for(var u=new Date(i.getTime()),E=0;E<l;){u.setDate(u.getDate()+1);var M=u.getDay();M!==0&&M!==6&&E++}return u}function xn(i){var l=new Date,u="dia "+i.getDate()+" de "+en[i.getMonth()];return i.getFullYear()!==l.getFullYear()&&(u+=" de "+i.getFullYear()),u}function Yn(i){var l=sn(i);if(!l||l<1)return null;var u=zn(new Date,l);return"Receba até "+xn(u)}var ln={},Ln=4e3,Jn={};function Mn(i,l){if(!i||i.length!==8)return Promise.resolve(null);if(ln[i])return ln[i];if(!l){var u=Jn[i]||0;if(Date.now()-u<Ln)return Promise.resolve(null)}var E="cep="+encodeURIComponent(i.slice(0,5)+"-"+i.slice(5))+"&cupom-desconto=&nenhumCreditoSelecionado=true&area=main-cart",M=fetch("/checkout/cart?operation=atualizaValoresCarrinho",{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/x-www-form-urlencoded",Accept:"text/html,application/json,*/*","X-Requested-With":"XMLHttpRequest"},body:E}).then(function(L){if(!L.ok)throw new Error("HTTP "+L.status);return L.text()}).then(function(L){return Jn[i]=Date.now(),Un(L)}).catch(function(){return null}).then(function(L){return delete ln[i],L});return ln[i]=M,M}function Un(i){try{var l=new DOMParser().parseFromString(i,"text/html"),u=l.querySelector("#resumo-compra .frete-calculado")||l.querySelector(".frete-calculado");if(!u)return null;var E="",M=u.querySelector(".frete-location .city");M&&(E=M.textContent.trim());var L=null,_="",R="",U=u.querySelector(".info-frete-selec");if(U){var K=U.querySelector(".dias-entrega"),an=U.querySelector(".info-title span, .info-title");K&&(_=(K.textContent||"").trim()),an&&(R=(an.textContent||"").trim())}var Q=u.querySelector(".line.valor-frete .value, .value.valor-frete")||u.querySelector(".valor-compra-frete .value");if(Q){var on=(Q.textContent||"").trim();if(/gr[áa]tis/i.test(on))L=0;else{var pn=on.match(/[\d.,]+/);if(pn){var Y=parseFloat(pn[0].replace(/\./g,"").replace(",","."));isNaN(Y)||(L=Y)}}}if(L==null){var t=u.querySelector(".servico-frete");if(t){var a=parseFloat(t.getAttribute("data-valor-frete")||"0");if(isNaN(a)||(L=a),R||(R=t.getAttribute("data-servico-frete")||""),!_){var n=t.querySelector(".dias-entrega");n&&(_=(n.textContent||"").trim())}}}if(L==null)return null;var e=null,r=l.querySelector(".total-boleto .value, .totais-valor.total-boleto .value");if(r){var s=(r.textContent||"").trim(),f=s.match(/[\d.,]+/);if(f){var v=parseFloat(f[0].replace(/\./g,"").replace(",","."));isNaN(v)||(e=v)}}return{city:E,shipping:L,shippingDeadline:_,shippingName:R,totalPix:e}}catch{return null}}function In(i,l,u){if(!(!i||!u)){_n(i);try{var E=V()||{};E.ts=Date.now(),E.cepValue=l.slice(0,5)+"-"+l.slice(5),E.shipping=u.shipping,E.shippingDeadline=u.shippingDeadline,E.shippingName=u.shippingName,E.shippingCity=u.city,u.totalPix!=null&&(E.subtotalPix=u.totalPix);var M=i.querySelectorAll(".cart-item:not(.mm-removing)");E.items=Array.prototype.map.call(M,function(R){var U=R.querySelector(".prod-nome a, .prod-nome"),K=(U&&U.textContent||"").trim(),an=R.querySelector(".qty-display"),Q=an?parseInt(an.textContent):parseInt(R.getAttribute("data-item-quantity"))||1;return{name:K,quantity:Q}}),localStorage.setItem(q,JSON.stringify(E))}catch{}var L=i.querySelectorAll(".cart-item:not(.mm-removing)"),_=0;L.forEach(function(R){var U=R.querySelector(".valor");if(U){var K=nt(U.textContent);isNaN(K)||(_+=K)}}),tt(i)}}function On(i){var l=A();if(l){var u=V(),E=z(u),M=I(i.querySelectorAll(".cart-item:not(.mm-removing)")),L=u&&u.cepValue&&u.cepValue.replace(/\D/g,"")===l,_=u&&u.shipping!=null&&!isNaN(u.shipping);u&&E===M&&L&&_||Mn(l).then(function(R){R&&In(i,l,R)})}}function kn(i){return String(i||"").replace(/[&<>"']/g,function(l){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[l]})}function yn(i,l,u,E){if(i){var M=i.querySelector(".box-total-btn");if(M){var L=A(),_=V(),R=I(i.querySelectorAll(".cart-item:not(.mm-removing)")),U=z(_),K=_&&U===R,an=F(L),Q=l>=an,on=Math.max(0,an-l),pn=Math.max(0,Math.min(100,l/an*100)),Y=M.querySelector(".mm-cart-ship");if(!Y){Y=document.createElement("div"),Y.className="mm-cart-ship",Y.setAttribute("role","group"),Y.setAttribute("aria-label","Informações de frete");var t=M.querySelector(".total");t?M.insertBefore(Y,t):M.insertBefore(Y,M.firstChild)}if(Y.classList.toggle("is-free",Q),ut(Y),Y.dataset.mmEditing!=="1"){var a=_&&_.cepValue&&_.cepValue.replace(/\D/g,"")===L,n=L&&K&&a&&_.shippingCity,e=n?Yn(_.shippingDeadline):null,r="";if(r+='<div class="mm-cart-ship-location">',L){var s=x(L);if(n&&(s+=" · "+kn(_.shippingCity)),r+='<span class="mm-cart-ship-label">Envio para</span><div class="mm-cart-ship-location-value"><span>'+s+'</span><button class="mm-cart-ship-edit" type="button" data-mm-ship="edit">Trocar</button></div>',e){var f=kn(e);E&&u>0?f+=" · <strong>"+kn($n(u))+"</strong>":E&&u===0&&(f+=" · <strong>Grátis</strong>"),r+='<span class="mm-cart-ship-deadline">'+f+"</span>"}}else r+='<span class="mm-cart-ship-label">Calcule o frete</span><div class="mm-cart-ship-location-value"><span style="color:#9CA3AF;font-weight:500;">Informe seu CEP</span><button class="mm-cart-ship-edit" type="button" data-mm-ship="edit">Calcular</button></div>';r+="</div>";var v=Q?"Frete grátis desbloqueado":"Faltam "+$n(on)+" para frete grátis",y=parseFloat(i.dataset.mmShipPct||"0")||0;r+='<div class="mm-cart-ship-progress">',r+='<div class="mm-cart-ship-bar" role="progressbar" aria-valuenow="'+Math.round(l)+'" aria-valuemin="0" aria-valuemax="'+Math.round(an)+'" aria-valuetext="'+kn(v)+'" aria-label="Progresso para frete grátis"><div class="mm-cart-ship-bar-fill" style="width:'+y.toFixed(1)+'%"></div></div>',r+='<p class="mm-cart-ship-nudge'+(Q?" is-free":"")+'">',Q?r+=Z+"Frete grátis garantido":r+="Faltam <strong>"+kn($n(on))+"</strong> para frete grátis",r+="</p>",r+="</div>",Y.innerHTML=r;var T=Y.querySelector(".mm-cart-ship-bar-fill");T&&requestAnimationFrame(function(){T.style.width=pn.toFixed(1)+"%"});var N=i.dataset.mmShipWasFree==="1";Q&&!N&&y>0&&(Y.classList.remove("mm-just-unlocked"),Y.offsetWidth,Y.classList.add("mm-just-unlocked"),setTimeout(function(){Y.classList.remove("mm-just-unlocked")},1400)),i.dataset.mmShipWasFree=Q?"1":"",i.dataset.mmShipPct=pn.toFixed(1)}}}}var at='<svg class="mm-cart-ship-spinner" width="14" height="14" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><style>.mm-sp{animation:mm-spin 1.2s linear infinite;transform-origin:center}.mm-sp circle{stroke:#4B664A;stroke-width:3;fill:none;stroke-linecap:round;stroke-dasharray:44;stroke-dashoffset:16;animation:mm-sp-dash 1.2s ease-in-out infinite}@keyframes mm-spin{to{transform:rotate(360deg)}}@keyframes mm-sp-dash{0%{stroke-dashoffset:44}50%{stroke-dashoffset:8}100%{stroke-dashoffset:44}}</style><g class="mm-sp"><circle cx="12" cy="12" r="9.5"/></g></svg>';function Kn(i){if(i){var l=i.querySelector(".mm-cart-ship-deadline");if(l)l.innerHTML="Recalculando frete "+at;else{var u=i.querySelector(".mm-cart-ship-location");if(u){var E=document.createElement("span");E.className="mm-cart-ship-deadline",E.innerHTML="Recalculando frete "+at,u.appendChild(E)}}var M=i.querySelector(".box-total-btn");M&&M.classList.add("mm-ship-loading")}}function _n(i){if(i){var l=i.querySelector(".box-total-btn");l&&l.classList.remove("mm-ship-loading")}}function ut(i){!i||i.dataset.mmShipBound||(i.dataset.mmShipBound="1",i.addEventListener("click",function(l){var u=l.target.closest('[data-mm-ship="edit"]');if(u){l.preventDefault(),l.stopPropagation(),vt(i);return}var E=l.target.closest('[data-mm-ship="cancel"]');if(E){l.preventDefault(),l.stopPropagation(),rt(i);return}l.target.closest(".mm-cart-ship-cep-form")&&l.stopPropagation()},!0))}function vt(i){var l=i.querySelector(".mm-cart-ship-location");if(l){i.dataset.mmEditing="1";var u=A()||"";l.innerHTML='<span class="mm-cart-ship-label">Digite seu CEP</span><form class="mm-cart-ship-cep-form" data-mm-ship-form="1" novalidate><input class="mm-cart-ship-cep-input" type="text" inputmode="numeric" autocomplete="postal-code" placeholder="00000-000" maxlength="9" value="'+kn(x(u))+'" aria-label="CEP" /><button class="mm-cart-ship-cep-save" type="submit">OK</button><button class="mm-cart-ship-cep-cancel" type="button" data-mm-ship="cancel" aria-label="Cancelar">'+tn+"</button></form>";var E=l.querySelector(".mm-cart-ship-cep-input"),M=l.querySelector("form");E&&(setTimeout(function(){try{E.focus(),E.select()}catch{}},10),E.addEventListener("input",function(){E.classList.remove("is-invalid");var L=E.value.replace(/\D/g,"").slice(0,8);E.value=L.length>5?L.slice(0,5)+"-"+L.slice(5):L}),E.addEventListener("keydown",function(L){L.key==="Escape"&&(L.preventDefault(),rt(i))})),M&&M.addEventListener("submit",function(L){L.preventDefault(),L.stopPropagation(),it(i)})}}function rt(i){i.dataset.mmEditing="";var l=i.closest(".carrinho-rapido-ctn");if(l){var u=l.querySelectorAll(".cart-item:not(.mm-removing)"),E=0;u.forEach(function(M){var L=M.querySelector(".valor");if(L){var _=nt(L.textContent);isNaN(_)||(E+=_)}}),yn(l,E)}}function it(i){var l=i.querySelector(".mm-cart-ship-cep-input");if(l){var u=l.value.replace(/\D/g,"");if(u.length!==8){l.classList.add("is-invalid"),l.focus();return}try{localStorage.setItem(b,u)}catch{}var E=i.querySelector(".mm-cart-ship-cep-save");E&&(E.disabled=!0,E.textContent="...");var M=i.closest(".carrinho-rapido-ctn");try{var L=V();L&&(L.cepValue="",localStorage.setItem(q,JSON.stringify(L)))}catch{}rt(i),Mn(u).then(function(_){_&&M&&In(M,u,_)})}}function nt(i){if(!i)return NaN;var l=String(i).replace(/\s/g,"").match(/([\d.,]+)/);return l?parseFloat(l[1].replace(/\./g,"").replace(",",".")):NaN}function $n(i){return isNaN(i)?"":"R$ "+i.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}function Wn(i){if(isNaN(i))return"";var l=i.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),u=l.split(",");return"R$&nbsp;"+u[0]+'<span class="mm-cents">,'+(u[1]||"00")+"</span>"}function mt(i){var l=0;return Array.prototype.forEach.call(i,function(u){var E=parseFloat(u.getAttribute("data-item-price"))||0,M=parseInt(u.getAttribute("data-item-quantity"));if(!M||isNaN(M)){var L=u.querySelector(".qty-display");L?M=parseInt(L.textContent)||1:M=1}l+=E*M}),l}function hn(i){var l=i.querySelectorAll(".cart-item:not(.mm-removing)");l.forEach(function(u){var E=parseFloat(u.getAttribute("data-item-price"))||0,M=parseInt(u.getAttribute("data-item-quantity"));if(!M||isNaN(M)){var L=u.querySelector(".qty-display");L?M=parseInt(L.textContent)||1:M=1}var _=u.querySelector(".valor");_&&E>0&&(_.innerHTML=Wn(E*M))})}function Vn(i){if(!(!i||i.dataset.mmTotalRatio)){var l=i.querySelectorAll(".cart-item");if(l.length){var u=mt(l),E=i.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||i.querySelector(".box-total-btn .linha-total .valor-final strong");if(E&&u>.01){var M=nt(E.textContent);isNaN(M)||(i.dataset.mmTotalRatio=String(M/u))}}}}function ht(i,l){var u=i.querySelector(".box-total-btn .linha-total");if(u){var E=u.parentElement.querySelector(".mm-cart-savings");if(E&&E.remove(),!(!l||l<.01)){var M=document.createElement("span");M.className="mm-cart-savings",M.textContent="Você economiza "+$n(l)+" com PIX",u.nextSibling?u.parentElement.insertBefore(M,u.nextSibling):u.parentElement.appendChild(M)}}}function tt(i){if(i){Vn(i);var l=i.querySelectorAll(".cart-item:not(.mm-removing)"),u=mt(l),E=i.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||i.querySelector(".box-total-btn .linha-total .valor-final strong"),M=parseFloat(i.dataset.mmTotalRatio||"0.95")||.95,L=u*M,_=u-L,R=A(),U=V(),K=z(U),an=I(l),Q=U&&U.cepValue&&U.cepValue.replace(/\D/g,"")===R,on=!!(R&&U&&Q&&U.shipping!=null&&!isNaN(U.shipping)),pn=i.dataset.mmShipPendingFetch==="1";!on&&pn&&R&&U&&U.shipping!=null&&(on=!0);var Y=on?parseFloat(U.shipping):0,t=L+Y,a=u+Y;if(E){var n=nt(E.textContent);if(!isNaN(n)&&Math.abs(t-n)>.005){var e=i.querySelector(".box-total-btn .linha-total .valor-final");e&&(e.classList.remove("mm-pop"),e.offsetWidth,e.classList.add("mm-pop"),setTimeout(function(){e.classList.remove("mm-pop")},450)),Hn(E,n,t)}else E.innerHTML=Wn(t)}var r=i.querySelector(".box-total-btn .valor-final.card");if(r){var s=a/12;r.innerHTML="<span>ou em até <strong>12x</strong> de <strong>"+$n(s)+"</strong> no cartão</span>"}var f=i.querySelector(".box-total-btn .linha-total .valor-final .tipo-pagto");f&&(f.textContent="No PIX"),ht(i,_),yn(i,u,Y,on);try{var v=0;l.forEach(function(T){var N=T.querySelector(".qty-display");N&&(v+=parseInt(N.textContent)||0)});var y=document.getElementById("mm-h-cart-count");y&&(v>0?(y.textContent=v>99?"99+":String(v),y.hidden=!1):y.hidden=!0)}catch{}}}function Hn(i,l,u){if(!i||isNaN(l)||isNaN(u))return;var E=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;if(E){i.innerHTML=Wn(u);return}var M=420,L=performance.now();function _(R){return 1-Math.pow(1-R,3)}(function R(U){var K=Math.min(1,(U-L)/M);i.innerHTML=Wn(l+(u-l)*_(K)),K<1&&requestAnimationFrame(R)})(L)}function st(i,l,u,E,M,L){if(!(!i||!l)){var _=parseInt(E.textContent)||1,R=_+u;if(!(R<1)){var Q=i.closest(".carrinho-rapido-ctn");Vn(Q),M.disabled=!0,L.disabled=!0;var U=parseFloat(i.getAttribute("data-item-price"))||0;E.textContent=R,i.setAttribute("data-item-quantity",R);var K=i.querySelector(".valor");if(K){var an=U*R;K.textContent=$n(an),K.classList.remove("mm-pop"),K.offsetWidth,K.classList.add("mm-pop"),setTimeout(function(){K.classList.remove("mm-pop")},450)}var Q=i.closest(".carrinho-rapido-ctn"),on=u>0?"adicionaItem":"removeItem";A()&&(Q.dataset.mmShipPendingFetch="1"),tt(Q),A()&&Kn(Q),d(on,l).catch(function(){E.textContent=_,i.setAttribute("data-item-quantity",_),K&&(K.innerHTML=Wn(U*_)),Q.dataset.mmShipPendingFetch="",tt(Q)}).then(function(){M.disabled=!1,L.disabled=!1;var pn=A();pn?(Kn(Q),Mn(pn,!0).then(function(Y){Q.dataset.mmShipPendingFetch="",Y?In(Q,pn,Y):_n(Q)})):Q.dataset.mmShipPendingFetch=""})}}}function ft(i,l){if(!(!i||!l)){var u=i.closest(".carrinho-rapido-ctn");Vn(u);var E=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches,M=E?0:360;E||i.classList.add("mm-removing"),setTimeout(function(){i.parentNode&&i.parentNode.removeChild(i);var L=u?u.querySelectorAll(".cart-item:not(.mm-removing)"):[],_=L.length===0;if(_){u.dataset.mmShipPendingFetch="";var R=u.querySelector(".mm-cart-ship");R&&R.remove();var U=u.querySelector(".box-total-btn, .area-finalizar-compra");U&&(U.style.display="none")}else tt(u),A()&&Kn(u);var K=document.getElementById("mm-h-cart-count"),an=document.getElementById("mm-h-cart"),Q=L.length;K&&(K.textContent=Q>99?"99+":String(Q),K.hidden=Q===0),an&&an.setAttribute("aria-label","Carrinho, "+Q+" "+(Q===1?"item":"itens"))},M),d("deleteItem",l).catch(function(){}).then(function(){var L=u?u.querySelectorAll(".cart-item:not(.mm-removing)"):[],_=document.getElementById("mm-h-cart-count");if(_&&(_.textContent=L.length>99?"99+":String(L.length),_.hidden=L.length===0),L.length===0){u.dataset.mmShipPendingFetch="";return}var R=A();R?(u.dataset.mmShipPendingFetch="1",Kn(u),Mn(R,!0).then(function(U){u.dataset.mmShipPendingFetch="",U?In(u,R,U):_n(u)})):u.dataset.mmShipPendingFetch=""})}}window.__mmDeleteItem=ft;var bt=null,lt=new Set,fn=null;function pt(i){if(!i)return NaN;var l=String(i).replace(/\s/g,"").match(/([\d.,]+)/);if(!l)return NaN;var u=l[1].replace(/\./g,"").replace(",","."),E=parseFloat(u);return isNaN(E)?NaN:E}function ct(i){return isNaN(i)?"":"R$ "+i.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}function Et(i,l,u){if(!i||isNaN(l)||isNaN(u))return;fn&&cancelAnimationFrame(fn);var E=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;if(E){i.textContent=ct(u);return}var M=420,L=performance.now();function _(U){return 1-Math.pow(1-U,3)}function R(U){var K=U-L,an=Math.min(1,K/M),Q=l+(u-l)*_(an);i.textContent=ct(Q),an<1?fn=requestAnimationFrame(R):fn=null}fn=requestAnimationFrame(R)}function xt(i){return i?i.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||i.querySelector(".box-total-btn .linha-total .valor-final strong")||i.querySelector(".valor-pix strong")||i.querySelector(".valor-pix"):null}function Cn(i){return i?i.querySelector(".box-total-btn .linha-total .valor-final"):null}function Ct(i){var l=Cn(i);l&&(l.classList.remove("mm-pop"),l.offsetWidth,l.classList.add("mm-pop"),setTimeout(function(){l.classList.remove("mm-pop")},450))}function dt(){var i=document.querySelector(".carrinho-rapido-ctn");if(i){if(!i.dataset.mmShipRendered){var l=i.querySelectorAll(".cart-item:not(.mm-removing)");l.length>0&&(Vn(i),hn(i),tt(i),i.dataset.mmShipRendered="1",On(i))}var l=i.querySelectorAll(".cart-item"),u=new Set;l.forEach(function(E){var M=E.id||E.getAttribute("data-item-id")||"";M&&(u.add(M),!lt.has(M)&&lt.size>0&&(E.classList.add("mm-added"),setTimeout(function(){E.classList.remove("mm-added")},500)))}),lt=u}}function Pn(){var i=document.querySelector(".carrinho-rapido-ctn");if(!(!i||i.dataset.mmAnimObserved)){i.dataset.mmAnimObserved="1",dt();var l=new MutationObserver(function(){clearTimeout(Pn._t),Pn._t=setTimeout(dt,60)});l.observe(i,{childList:!0,subtree:!0,characterData:!0})}}function Xn(){g(),m(),p();var i=document.getElementById("cart-preview-area");if(i){var l=new MutationObserver(function(){setTimeout(o,100),setTimeout(Pn,150)});l.observe(i,{childList:!0,subtree:!0})}setInterval(o,800),setInterval(Pn,800),o(),Pn()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Xn):Xn()})(),(function g(){"use strict";var P="mm_cep",O="mm_cart_snapshot",B=18e5,h="mm_onepage_draft",o=1440*60*1e3,m=2e3,p="https://magazord-public.s3.sa-east-1.amazonaws.com/madeiramania/resources/Design%20sem%20nome%20(1).svg",d="https://api.whatsapp.com/send?phone=5511915299488&text="+encodeURIComponent("Olá! Estou no checkout e gostaria de tirar uma dúvida sobre meu pedido."),c=location.pathname,k=c.indexOf("/checkout/cart")!==-1,b=c.indexOf("/checkout/identify")!==-1,q=c.indexOf("/checkout/onepage")!==-1,C=c.indexOf("/checkout/payment")!==-1,A=c.indexOf("/checkout/done")!==-1;if(A){try{localStorage.removeItem("mm_onepage_draft")}catch{}return}if(!k&&!b&&!q&&!C)return;g._retries=(g._retries||0)+1;var x=document.querySelector("#checkout-main-area");if(!x){g._retries<40&&setTimeout(g,400);return}function F(t){return isNaN(t)||t<0?"R$ 0,00":"R$ "+t.toFixed(2).replace(".",",").replace(/(\d)(?=(\d{3})+,)/g,"$1.")}function V(t){if(!t)return 0;var a=String(t).match(/(-?[\d.]+,\d{2})/);return a&&parseFloat(a[1].replace(/\./g,"").replace(",","."))||0}function I(t){return String(t||"").replace(/[&<>"']/g,function(a){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[a]})}var z={truck:'<svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2a3 3 0 0 0 6 0h6a3 3 0 0 0 6 0h2v-5l-3-4zM6 18.5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm12 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm.5-7H17V9.5h2.04l1.46 2-.04 0z"/></svg>',check:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>',checkCircle:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>',bolt:'<svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>',shield:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 1 3 5v6c0 5.5 3.8 10.7 9 12 5.2-1.3 9-6.5 9-12V5l-9-4zm-1.4 16L6 12.4l1.4-1.4 3.2 3.2 6.8-6.8L18.8 8.8 10.6 17z"/></svg>',lock:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM9 6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9V6z"/></svg>',card:'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>',rotate:'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.74 9.74 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>',minus:'<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M3 7h8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',plus:'<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 3v8M3 7h8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',trash:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',close:'<svg width="12" height="12" viewBox="0 0 14 14" fill="none"><path d="M10.5 3.5l-7 7M3.5 3.5l7 7" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',arrow:'<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>',tag:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.59 13.41 13.42 20.58a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>',whats:'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0 0 20.464 3.488"/></svg>',box:'<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>'},Z={get:function(t){try{return localStorage.getItem(t)}catch{return null}},set:function(t,a){try{localStorage.setItem(t,a)}catch{}},remove:function(t){try{localStorage.removeItem(t)}catch{}}};function tn(t){try{var a={ts:Date.now(),items:t.items.map(function(n){return{name:n.name,variant:n.variant,imgSrc:n.imgSrc,quantity:n.quantity,lineTotal:n.lineTotal,lineTotalPix:n.lineTotalPix,isPix:n.isPix,deposito:n.deposito}}),subtotalPix:t.subtotalPix,subtotalFull:t.subtotalFull,discount:t.discount,couponCode:t.couponCode,shipping:t.shipping,shippingDeadline:t.shippingDeadline,shippingName:t.shippingName,shippingCity:t.shippingCity,shippingOptions:t.shippingOptions,cepValue:t.cepValue};Z.set(O,JSON.stringify(a))}catch{}}function en(){try{var t=Z.get(O);if(!t)return null;var a=JSON.parse(t);return!a||!a.ts||Date.now()-a.ts>B?null:a}catch{return null}}function sn(){try{for(var t=["mm-op-email","mm-op-nome","mm-op-cpf","mm-op-tel","mm-op-cep","mm-op-rua","mm-op-num","mm-op-comp","mm-op-bairro","mm-op-cidade","mm-op-uf"],a={ts:Date.now()},n=0,e=0;e<t.length;e++){var r=document.getElementById(t[e]);r&&r.value&&(a[t[e]]=r.value,n++)}if(n===0){window._mmDraftDebug&&console.log("[mm-draft] skip save (0 fields)");return}Z.set(h,JSON.stringify(a)),window._mmDraftDebug&&console.log("[mm-draft] saved",n,"fields",a)}catch(s){window._mmDraftDebug&&console.warn("[mm-draft] save failed",s)}}function zn(){try{var t=Z.get(h);if(!t)return null;var a=JSON.parse(t);return!a||!a.ts?null:Date.now()-a.ts>o?(Z.remove(h),null):a}catch{return null}}function xn(){try{Z.remove(h)}catch{}}function Yn(){var t=zn();if(!t)return window._mmDraftDebug&&console.log("[mm-draft] no draft to restore"),null;for(var a=["mm-op-email","mm-op-nome","mm-op-cpf","mm-op-tel","mm-op-cep","mm-op-rua","mm-op-num","mm-op-comp","mm-op-bairro","mm-op-cidade","mm-op-uf"],n=0,e=0;e<a.length;e++){var r=document.getElementById(a[e]);if(r&&t[a[e]]){try{var s=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;s.call(r,t[a[e]])}catch{r.value=t[a[e]]}r.dispatchEvent(new Event("input",{bubbles:!0})),n++}}return window._mmDraftDebug&&console.log("[mm-draft] restored",n,"fields from draft",t),t}function ln(){for(var t={items:[],subtotalPix:0,subtotalFull:0,discount:0,shipping:null,shippingRaw:"",shippingDeadline:"",shippingName:"",shippingCity:"",shippingOptions:[],couponCode:"",cepValue:"",canFinalize:!1,hasFinalizar:!1},a=x.querySelectorAll(".cart-item"),n=0;n<a.length;n++){var e=a[n],r=e.querySelector('.qtd-item[id^="item_carrinho_"]'),s=r&&r.id.match(/item_carrinho_(\d+)/),f=s?parseInt(s[1],10):null,v=e.querySelector("figure img")||e.querySelector("#product-img")||e.querySelector("img"),y=e.querySelector(".nome-produto .link")||e.querySelector("figure a"),T=e.querySelector(".column-valor-produto .valor"),N=T?T.textContent.trim():"",D=!!e.querySelector(".column-valor-produto .sub");t.items.push({dataId:f,sku:e.getAttribute("data-item-id")||"",name:e.getAttribute("data-item-name")||e.getAttribute("data-name")||"",variant:e.getAttribute("data-item-variant")||"",brand:e.getAttribute("data-item-brand")||"",category:e.getAttribute("data-item-category")||"",priceUnit:parseFloat(e.getAttribute("data-item-price")||"0"),lineTotal:parseFloat(e.getAttribute("data-valor")||"0"),quantity:parseInt(e.getAttribute("data-item-quantity")||"1",10),deposito:e.getAttribute("data-item-deposito")==="1",imgSrc:v?v.getAttribute("src")||v.currentSrc:"",href:y?y.getAttribute("href"):"",lineTotalPix:V(N),isPix:D}),t.subtotalFull+=parseFloat(e.getAttribute("data-valor")||"0")}var nn=x.querySelector("#resumo-compra .resumo-valores .value");nn&&(t.subtotalPix=V(nn.textContent)),t.subtotalPix<=0&&(t.subtotalPix=t.items.reduce(function(gt,zt){return gt+(zt.lineTotalPix||0)},0));var X=x.querySelector("#resumo-compra .discount-value");X&&(t.discount=V(X.textContent));var W=x.querySelector("#resumo-compra .txt-cupom, #resumo-compra .alert.alert-type-1");if(W){var J=W.textContent.match(/([A-Z0-9]{3,})/);J&&(t.couponCode=J[1])}var cn=x.querySelector("#resumo-compra .frete-calculado, .frete-calculado");if(cn&&cn.textContent.trim()){t.shippingRaw=cn.textContent.trim();var rn=cn.querySelector(".frete-location .city");rn&&(t.shippingCity=rn.textContent.trim());for(var vn=cn.querySelectorAll(".servico-frete"),Sn=0;Sn<vn.length;Sn++){var Tn=vn[Sn],Gn=Tn.querySelector('input[type="radio"]'),w=Tn.querySelector(".dias-entrega"),S=parseFloat(Tn.getAttribute("data-valor-frete")||"0"),j=Tn.getAttribute("data-servico-frete")||"",G=w?w.textContent.trim().replace(/\s+/g," ").replace(/\s*-\s*$/,"").trim():"",H=G.match(/(\d+(?:\s*[aà]\s*\d+)?\s*dias?(?:\s*[úu]teis)?)/i),dn=H?H[1].replace(/\s+/g," "):G;t.shippingOptions.push({id:Gn?Gn.value:"",name:j,deadline:dn,value:S,isFree:S===0,isSelected:Gn?Gn.checked:!1})}var un=t.shippingOptions.filter(function(gt){return gt.isSelected})[0];if(!un&&t.shippingOptions.length>0&&(un=t.shippingOptions[0]),un)t.shipping=un.value,t.shippingName=un.name,t.shippingDeadline=un.deadline;else{var bn=cn.querySelector(".info-frete-selec"),wn=cn.querySelector(".dias-entrega, .info-frete-selec .dias-entrega"),En=cn.querySelector(".valor-frete .value, .value.valor-frete"),Bn=cn.querySelector(".info-frete-selec .info-title span, .info-title span");if(En){var qn=En.textContent.trim();if(/gr[aá]tis/i.test(qn))t.shipping=0;else{var Dn=V(qn);Dn>0&&(t.shipping=Dn)}}if(wn){var Rn=wn.textContent.match(/(\d+(?:\s*[aà]\s*\d+)?\s*dias?(?:\s*\([^)]+\))?(?:\s*[úu]teis)?)/i);Rn&&(t.shippingDeadline=Rn[1].replace(/\s+/g," ").replace(/\(s\)/,"").trim())}if(Bn&&(t.shippingName=Bn.textContent.trim()),t.shipping===null)if(/gr[aá]tis/i.test(t.shippingRaw))t.shipping=0;else{var et=V(t.shippingRaw);et>0&&(t.shipping=et)}if(!t.shippingDeadline){var gn=t.shippingRaw.match(/(\d+(?:\s*[aà]\s*\d+)?)\s*dias?/i);gn&&(t.shippingDeadline=gn[1]+" dias úteis")}}}var ot=x.querySelector("#cep, .input-cep");return ot&&(t.cepValue=ot.value||""),t.hasFinalizar=!!x.querySelector("#finalizar-compra"),t.canFinalize=t.items.length>0,t}function Ln(t){t=t||"cart";var a='<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 1 3 5v6c0 5.5 3.8 10.7 9 12 5.2-1.3 9-6.5 9-12V5l-9-4zm-1.4 16L6 12.4l1.4-1.4 3.2 3.2 6.8-6.8L18.8 8.8 10.6 17z"/></svg>';function n(e,r){var s=e===t,f="mm-checkout-step"+(s?" is-active":""),v=s?' aria-current="step"':"";return'<li class="'+f+'"'+v+'><span class="mm-checkout-step-label">'+r+"</span></li>"}return'<header class="mm-checkout-header"><a class="mm-checkout-header-logo" href="/" aria-label="Madeira Mania - voltar à home"><img src="'+p+'" alt="Madeira Mania" width="180" height="44"></a><nav class="mm-checkout-steps" aria-label="Etapas do checkout"><ol>'+n("cart","Carrinho")+'<li class="mm-checkout-step-sep" aria-hidden="true">›</li>'+n("delivery","Entrega")+'<li class="mm-checkout-step-sep" aria-hidden="true">›</li>'+n("payment","Pagamento")+'</ol></nav><div class="mm-checkout-secure">'+a+'<span class="mm-checkout-secure-text"><strong>Compra 100% segura</strong><small>Site protegido por SSL</small></span></div></header>'}function Jn(t){var a=t.imgSrc?'<img src="'+I(t.imgSrc)+'" alt="'+I(t.name)+'" loading="lazy">':"",n=t.href?'<a class="mm-item-name" href="'+I(t.href)+'">'+I(t.name)+"</a>":'<span class="mm-item-name">'+I(t.name)+"</span>",e=t.variant?'<p class="mm-item-variant">'+I(t.variant)+"</p>":"",r=t.deposito?'<span class="mm-item-badge">'+z.bolt+"<span>Pronta entrega · Envio em 24h</span></span>":"",s=t.quantity<=1?' disabled aria-disabled="true"':"",f;if(t.lineTotalPix>0&&t.isPix){var v='<span class="mm-item-price-sub">no PIX</span>',y=t.quantity>1?F(t.lineTotalPix/t.quantity)+" cada":"";f='<div class="mm-item-price"><span class="mm-item-price-value">'+F(t.lineTotalPix)+"</span>"+v+(y?'<span class="mm-item-price-unit">'+y+"</span>":"")+"</div>"}else{var T=t.quantity>1?F(t.priceUnit)+" cada":"";f='<div class="mm-item-price"><span class="mm-item-price-value">'+F(t.lineTotal)+"</span>"+(T?'<span class="mm-item-price-unit">'+T+"</span>":"")+"</div>"}return'<div class="mm-item" data-mm-id="'+t.dataId+'"><div class="mm-item-thumb">'+a+'</div><div class="mm-item-body">'+n+e+r+"</div>"+f+'<div class="mm-item-controls"><div class="mm-qty" role="group" aria-label="Quantidade"><button type="button" class="mm-qty-btn" data-mm-act="dec"'+s+' aria-label="Diminuir quantidade">'+z.minus+'</button><span class="mm-qty-value">'+t.quantity+'</span><button type="button" class="mm-qty-btn" data-mm-act="inc" aria-label="Aumentar quantidade">'+z.plus+'</button></div><button type="button" class="mm-item-remove" data-mm-act="remove" aria-label="Remover item" title="Remover">'+z.trash+"</button></div></div>"}function Mn(t){return t.items.length?t.items.map(Jn).join(""):'<div class="mm-empty"><div class="mm-empty-icon">'+z.box+'</div><h3 class="mm-empty-title">Seu carrinho está vazio</h3><p class="mm-empty-desc">Explore nossos móveis e encontre a peça certa para sua casa.</p><a class="mm-empty-cta" href="/">Explorar produtos '+z.arrow+'</a><p class="mm-empty-perks">Frete grátis acima de R$ 2.000 · 12x sem juros · 7% off no PIX</p></div>'}function Un(t){for(var a="",n=0;n<t;n++)a+='<div class="mm-skel-item"><div class="mm-skel mm-skel-thumb"></div><div class="mm-skel-lines"><div class="mm-skel mm-skel-line w-3-4"></div><div class="mm-skel mm-skel-line w-1-2"></div><div class="mm-skel mm-skel-line w-1-3"></div></div><div class="mm-skel-lines"><div class="mm-skel mm-skel-line w-1-2"></div></div></div>';return a}function In(t){var a=t.subtotalFull>0?t.subtotalFull:t.subtotalPix,n='<div class="mm-row"><span class="mm-row-label">Subtotal</span><span class="mm-row-value">'+F(a)+"</span></div>";if(t.shipping!==null){var e;t.shipping===0?e='<span class="mm-row-value is-free">'+z.check+" Grátis</span>":e='<span class="mm-row-value">'+F(t.shipping)+"</span>";var r='<span class="mm-row-label">Frete';t.shippingName&&(r+=' <span class="mm-row-sub">· '+I(t.shippingName)+"</span>"),t.shippingDeadline&&(r+=' <span class="mm-row-sub">· '+I(t.shippingDeadline)+"</span>"),r+="</span>",n+='<div class="mm-row">'+r+e+"</div>"}t.discount>0&&(n+='<div class="mm-row"><span class="mm-row-label">Desconto</span><span class="mm-row-value is-discount">− '+F(t.discount)+"</span></div>");var s="";if(t.shipping!==null){var f=Math.max(0,a+t.shipping-t.discount),v=Math.max(0,t.subtotalPix+t.shipping-t.discount),y=f-v,T=f/12;s='<div class="mm-total"><div class="mm-total-label">Total</div><div class="mm-total-value">'+F(f)+'</div><div class="mm-total-pix"><span>'+F(v)+" à vista no PIX</span>"+(y>0?'<span class="mm-total-pix-save">economia de '+F(y)+"</span>":"")+'</div><div class="mm-total-parcela">ou 12x de '+F(T)+" sem juros no cartão</div></div>"}else s='<div class="mm-total"><div class="mm-total-label">Subtotal</div><div class="mm-total-value">'+F(t.subtotalPix)+'</div><div class="mm-total-pix"><span>à vista no PIX</span></div><div class="mm-total-pending">Informe seu CEP para ver o frete e o total final.</div></div>';var N;return t.couponCode?N='<div class="mm-coupon-applied"><span class="mm-coupon-applied-left">'+z.tag+"<span>"+I(t.couponCode)+'</span></span><button type="button" data-mm-act="coupon-remove" aria-label="Remover cupom">'+z.close+"</button></div>":N='<div class="mm-coupon"><button type="button" class="mm-coupon-toggle" data-mm-act="coupon-toggle">'+z.tag+'<span>Tenho um cupom</span></button><form class="mm-coupon-form" data-mm-act="coupon-submit"><input type="text" class="mm-input" name="mm-coupon-code" placeholder="CUPOM" autocomplete="off" autocapitalize="characters" spellcheck="false" /><button type="submit" class="mm-btn-secondary">Aplicar</button></form></div>','<div class="mm-sum-stack"><div class="mm-rows">'+n+"</div>"+N+s+"</div>"}function On(){var t=document.getElementById("mm-layout");if(t)return t.parentElement!==x&&x.insertBefore(t,x.firstChild),t;var a=document.createElement("div");return a.id="mm-layout",a.innerHTML=Ln("cart")+'<div class="mm-grid"><section class="mm-items"><h2 class="mm-h">Carrinho</h2><div class="mm-items-card"><div id="mm-item-list">'+Un(2)+'</div></div></section><aside class="mm-sum"><h2 class="mm-h">Resumo</h2><div class="mm-sum-card"><div class="mm-cep"><div class="mm-cep-label"><span class="mm-cep-label-text">Calcular frete</span><a class="mm-cep-label-link" href="https://buscacepinter.correios.com.br/app/endereco/index.php" target="_blank" rel="noopener">Não sei meu CEP</a></div><div class="mm-cep-row"><input type="text" class="mm-input" id="mm-cep-input" name="mm_cep_calc" inputmode="numeric" maxlength="9" placeholder="00000-000" autocomplete="off" data-lpignore="true" data-1p-ignore="true" data-bwignore="true" data-form-type="other" aria-label="CEP" /><button type="button" class="mm-btn-secondary" data-mm-act="calc-cep">Calcular</button></div></div><div id="mm-sum-dynamic"></div><button type="button" class="mm-cta" data-mm-act="finalizar">Finalizar compra'+z.arrow+'</button><div class="mm-trust"><span class="mm-trust-item">'+z.lock+'<span>Pagamento seguro</span></span><span class="mm-trust-item">'+z.rotate+'<span>7 dias para troca</span></span><span class="mm-trust-item">'+z.card+'<span>12x sem juros</span></span></div><a class="mm-help" href="'+d+'" target="_blank" rel="noopener" data-mm-track="help-whats">'+z.whats+"<span><strong>Ficou com alguma dúvida?</strong> Fale com a gente no WhatsApp</span></a></div></aside></div>",x.insertBefore(a,x.firstChild),x.classList.add("mm-shadow-mode"),document.body.classList.add("mm-checkout-rebuild"),document.documentElement.classList.remove("mm-cart-loading"),a}function kn(){var t=ln(),a=document.getElementById("mm-item-list");a&&(a.innerHTML=Mn(t));var n=document.getElementById("mm-sum-dynamic");n&&(n.innerHTML=In(t));var e=document.querySelector(".mm-cta");e&&(e.disabled=!t.canFinalize,e.style.opacity=t.canFinalize?"1":"0.5",e.style.pointerEvents=t.canFinalize?"auto":"none");var r=document.getElementById("mm-cep-input");if(r&&!r.matches(":focus")){var s=Z.get(P),f=t.cepValue||s||"";f&&(r.value=yn(f))}return t.items&&t.items.length>0&&tn(t),t}function yn(t){var a=String(t||"").replace(/\D/g,"").slice(0,8);return a.length<=5?a:a.slice(0,5)+"-"+a.slice(5)}function at(t){var a=String(t||"").replace(/\D/g,"");a.length===8&&Z.set(P,a)}function Kn(t){t=t||0;var a=Z.get(P);if(!(!a||a.length!==8)){var n=x.querySelector("#cep, .input-cep");if(!n){t<12&&setTimeout(function(){Kn(t+1)},350);return}var e=x.querySelector("#resumo-compra .frete-calculado");if(e&&e.textContent.trim()){var r=document.getElementById("mm-cep-input");r&&!r.value&&(r.value=yn(a));return}var s=document.getElementById("mm-cep-input");s&&!s.value&&(s.value=yn(a)),n.value=yn(a),_n(n),setTimeout(function(){ut()},200)}}function _n(t){try{var a=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;a.call(t,t.value)}catch{}t.dispatchEvent(new Event("input",{bubbles:!0})),t.dispatchEvent(new Event("change",{bubbles:!0}))}function ut(){try{if(window.Zord&&window.Zord.Cart&&typeof window.Zord.Cart.calculaFreteCarrinho=="function"){window.Zord.Cart.calculaFreteCarrinho();return}}catch{}var t=x.querySelector("#resumo-compra .calcula-frete > button, .area-frete button");t&&t.click()}function vt(){var t=document.getElementById("mm-layout");!t||t._mmBound||(t._mmBound=!0,t.addEventListener("click",function(a){var n=a.target.closest("[data-mm-act]");if(n){var e=n.getAttribute("data-mm-act"),r=n.closest(".mm-item"),s=r?parseInt(r.getAttribute("data-mm-id"),10):null;switch(e){case"inc":rt(s,r,"inc");break;case"dec":rt(s,r,"dec");break;case"remove":it(s,r);break;case"calc-cep":nt();break;case"coupon-toggle":var f=n.closest(".mm-coupon");if(f){f.classList.add("is-open");var v=f.querySelector("input");v&&setTimeout(function(){v.focus()},100)}break;case"coupon-remove":Wn();break;case"finalizar":mt();break}}}),t.addEventListener("submit",function(a){var n=a.target.closest('[data-mm-act="coupon-submit"]');if(n){a.preventDefault();var e=n.querySelector("input");e&&$n(e.value.trim())}}),t.addEventListener("input",function(a){a.target&&a.target.id==="mm-cep-input"&&(a.target.value=yn(a.target.value))}),t.addEventListener("keydown",function(a){a.key==="Enter"&&a.target&&a.target.id==="mm-cep-input"&&(a.preventDefault(),nt())}))}function rt(t,a,n){if(!(!t||!a)&&!(!window.Zord||!window.Zord.checkout)){a.classList.add("is-updating");try{n==="inc"?window.Zord.checkout.adicionaQuantidade(t):window.Zord.checkout.removeQuantidade(t)}catch(e){console.warn("[mm-cart] qty change failed",e),a.classList.remove("is-updating")}}}function it(t,a){if(!(!t||!a)&&!(!window.Zord||!window.Zord.checkout)){a.classList.add("is-updating"),a.style.transition="opacity 200ms, transform 200ms",a.style.opacity="0",a.style.transform="translateX(-12px)";try{typeof window.Zord.checkout.deleteItem=="function"?window.Zord.checkout.deleteItem(t):window.Zord.checkout.removeQuantidade(t)}catch(n){console.warn("[mm-cart] remove failed",n),a.classList.remove("is-updating"),a.style.opacity="1",a.style.transform=""}}}function nt(){var t=document.getElementById("mm-cep-input");if(t){var a=(t.value||"").replace(/\D/g,"");if(a.length!==8){t.focus(),t.classList.add("mm-input-error"),setTimeout(function(){t.classList.remove("mm-input-error")},1200);return}at(a);var n=x.querySelector("#cep, .input-cep");n&&(n.value=yn(a),_n(n)),ut()}}function $n(t){if(t&&!(!window.Zord||!window.Zord.checkout)){var a=x.querySelector("#cupom-desconto");a&&(a.value=t.toUpperCase(),_n(a));try{window.Zord.checkout.addCupomDesconto()}catch(n){console.warn("[mm-cart] coupon apply failed",n)}}}function Wn(){if(!(!window.Zord||!window.Zord.checkout))try{window.Zord.checkout.removeCupomDesconto()}catch(t){console.warn("[mm-cart] coupon remove failed",t)}}function mt(){try{var t=ln();t.items&&t.items.length>0&&tn(t)}catch{}var a=document.getElementById("finalizar-compra");if(a){a.click();return}location.href="/checkout/identify"}if(k){let t=function(n){if(n=n||0,n>30){a();return}var e=x.querySelectorAll(".cart-item").length>0,r=x.querySelector("#resumo-compra");e||r||n>8?a():setTimeout(function(){t(n+1)},250)},a=function(){On(),vt(),kn(),Kn(),typeof jQuery<"u"&&jQuery(document).ajaxComplete(function(r,s,f){if(!(!f||!f.url)){var v=f.url;(v.indexOf("checkout/cart")!==-1||v.indexOf("atualiza")!==-1||v.indexOf("cupom")!==-1||v.indexOf("frete")!==-1||v.indexOf("removeItem")!==-1||v.indexOf("adicionaItem")!==-1)&&(setTimeout(kn,120),setTimeout(function(){var y=ln();y.shipping!==null&&y.cepValue&&at(y.cepValue)},200))}});try{var n=new MutationObserver(function(r){a._mutTimer&&clearTimeout(a._mutTimer),a._mutTimer=setTimeout(kn,200)}),e=[x.querySelector("#cart-area"),x.querySelector(".cart-area"),x.querySelector("#resumo-compra")].filter(Boolean);e.forEach(function(r){n.observe(r,{childList:!0,subtree:!0,characterData:!0})})}catch{}};if(!document.getElementById("mm-checkout-cro-done")){var hn=document.createElement("div");hn.id="mm-checkout-cro-done",hn.style.display="none",document.body.appendChild(hn)}t()}function Vn(t){if(!t||!t.items||!t.items.length)return'<aside class="mm-id-sum mm-sum"><h2 class="mm-h">Resumo</h2><div class="mm-sum-card"><div class="mm-sum-empty"><p>Não conseguimos carregar o resumo do seu pedido.</p><a class="mm-btn-secondary" href="/checkout/cart">Voltar ao carrinho</a></div></div></aside>';var a=3,n=t.items.slice(0,a),e=t.items.length-a,r=n.map(function(J){var cn=J.imgSrc?'<img src="'+I(J.imgSrc)+'" alt="'+I(J.name)+'" loading="lazy">':'<div class="mm-id-thumb-placeholder">'+z.box+"</div>",rn=J.quantity>1?'<strong class="mm-id-thumb-qty">'+J.quantity+"×</strong> ":"",vn=J.lineTotalPix>0?J.lineTotalPix:J.lineTotal;return'<div class="mm-id-thumb"><div class="mm-id-thumb-img">'+cn+'</div><div class="mm-id-thumb-body"><p class="mm-id-thumb-name">'+rn+I(J.name)+"</p>"+(J.variant?'<p class="mm-id-thumb-variant">'+I(J.variant)+"</p>":"")+'</div><div class="mm-id-thumb-price">'+F(vn)+"</div></div>"}).join("");e>0&&(r+='<div class="mm-id-thumb-more">+ '+e+" "+(e===1?"item":"itens")+" a mais</div>");var s=t.subtotalFull>0?t.subtotalFull:t.subtotalPix,f='<div class="mm-row"><span class="mm-row-label">Subtotal</span><span class="mm-row-value">'+F(s)+"</span></div>";if(t.shipping!==null&&t.shipping!==void 0){var v;t.shipping===0?v='<span class="mm-row-value is-free">'+z.check+" Grátis</span>":v='<span class="mm-row-value">'+F(t.shipping)+"</span>",f+='<div class="mm-row"><span class="mm-row-label">Frete'+(t.shippingDeadline?' <span class="mm-row-sub">· '+I(t.shippingDeadline)+"</span>":"")+"</span>"+v+"</div>"}t.discount>0&&(f+='<div class="mm-row"><span class="mm-row-label">Desconto'+(t.couponCode?' <span class="mm-row-sub">· '+I(t.couponCode)+"</span>":"")+'</span><span class="mm-row-value is-discount">− '+F(t.discount)+"</span></div>");var y,T=t.shipping!==null&&t.shipping!==void 0?t.shipping:0;if(t.shipping!==null&&t.shipping!==void 0){var N=Math.max(0,s+T-(t.discount||0)),D=Math.max(0,t.subtotalPix+T-(t.discount||0)),nn=N-D,X=N/12;y='<div class="mm-total"><div class="mm-total-label">Total</div><div class="mm-total-value">'+F(N)+'</div><div class="mm-total-pix"><span>'+F(D)+" à vista no PIX</span>"+(nn>0?'<span class="mm-total-pix-save">economia de '+F(nn)+"</span>":"")+'</div><div class="mm-total-parcela">ou 12x de '+F(X)+" sem juros</div></div>"}else{var W=s/12;y='<div class="mm-total"><div class="mm-total-label">Subtotal</div><div class="mm-total-value">'+F(t.subtotalPix)+'</div><div class="mm-total-pix"><span>à vista no PIX</span></div><div class="mm-total-parcela">ou 12x de '+F(W)+" sem juros</div></div>"}return'<aside class="mm-id-sum mm-sum"><h2 class="mm-h">Resumo do pedido</h2><div class="mm-sum-card"><div class="mm-id-thumbs">'+r+'</div><div class="mm-rows">'+f+"</div>"+y+'<a class="mm-id-edit-cart" href="/checkout/cart"><span>Editar carrinho</span></a></div><a class="mm-help mm-sum-help" href="'+d+'" target="_blank" rel="noopener" data-mm-track="help-whats-sum">'+z.whats+"<span><strong>Ficou com alguma dúvida?</strong> Fale com a gente no WhatsApp</span></a></aside>"}function ht(){var t='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>',a='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',n='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><line x1="7" y1="9" x2="17" y2="9"/><line x1="7" y1="13" x2="17" y2="13"/><line x1="7" y1="17" x2="13" y2="17"/></svg>',e='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>';return'<section class="mm-id-form-col"><h2 class="mm-id-h2">Quase lá! Identifique-se</h2><p class="mm-id-sub">Informe seu e-mail para finalizar a compra de forma rápida e segura.</p><form class="mm-id-form" data-mm-act="identify-submit" novalidate><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+t+'</span><input type="email" id="mm-id-email" name="mm-email" class="mm-input" placeholder="seu@email.com" autocomplete="email" inputmode="email" required></div><p class="mm-id-microcopy">'+z.lock+'<span>Seu e-mail é seguro · Não compartilhamos com terceiros</span></p><button type="submit" class="mm-cta">Continuar'+z.arrow+'</button></form><div class="mm-id-divider"><span>ou</span></div><div class="mm-id-google-slot"></div><button type="button" class="mm-id-guest-toggle" data-mm-act="guest-go" aria-label="Continuar como visitante"><span class="mm-id-guest-icon" aria-hidden="true">'+a+'</span><span class="mm-id-guest-label">Continuar como visitante (sem criar conta)</span><span class="mm-id-guest-arrow" aria-hidden="true">'+z.arrow+'</span></button><div class="mm-trust mm-id-trust"><span class="mm-trust-item">'+z.lock+'<span>Pagamento seguro</span></span><span class="mm-trust-item">'+z.rotate+'<span>7 dias para troca</span></span><span class="mm-trust-item">'+z.shield+'<span>Garantia 12 meses</span></span></div><p class="mm-id-lgpd">Ao continuar, você concorda com nossa <a href="/politica-de-privacidade" target="_blank" rel="noopener">Política de Privacidade</a></p></section>'}function tt(t){var a=document.getElementById("mm-layout");if(a)return a.parentElement!==x&&x.insertBefore(a,x.firstChild),a;var n=document.createElement("div");return n.id="mm-layout",n.classList.add("mm-id-layout"),n.innerHTML=Ln("delivery")+'<div class="mm-grid mm-id-grid">'+ht()+Vn(t)+"</div>",x.insertBefore(n,x.firstChild),document.body.classList.add("mm-checkout-rebuild"),Hn(),x.classList.add("mm-shadow-mode"),document.documentElement.classList.remove("mm-cart-loading"),n}function Hn(){var t=document.querySelector(".mm-id-google-slot"),a=x.querySelector(".social-login-area");if(!(!t||!a)&&!t.contains(a))try{t.appendChild(a),t.classList.add("is-loaded")}catch{}}function st(t){Z.set("mm_user_email",t);var a=x.querySelector("#login");if(!a)return!1;a.value=t,_n(a);var n=a.closest("form"),e=n?n.querySelector('button.button-send, button[type="submit"]'):null;return e?(e.click(),!0):n?(n.submit(),!0):!1}function ft(){var t=document.getElementById("mm-layout");!t||t._mmBound||(t._mmBound=!0,t.addEventListener("submit",function(a){var n=a.target.closest('[data-mm-act="identify-submit"]');if(n){a.preventDefault();var e=n.querySelector("#mm-id-email"),r=e?e.value.trim():"";if(!r||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(r)){e&&(e.classList.add("mm-input-error"),e.focus(),setTimeout(function(){e.classList.remove("mm-input-error")},1500));return}var s=st(r);if(s){var f=n.querySelector(".mm-cta");f&&f.classList.add("is-loading")}return}}),t.addEventListener("click",function(a){var n=a.target.closest("[data-mm-act]");if(n){var e=n.getAttribute("data-mm-act");e==="guest-go"&&(Z.set("mm_checkout_mode","guest"),n.classList.add("is-loading"),location.href="/checkout/onepage")}}))}function bt(t){try{var a=new DOMParser().parseFromString(t,"text/html"),n=a.querySelector("#checkout-main-area");if(!n)return null;for(var e=[],r=n.querySelectorAll(".cart-item"),s=0,f=0;f<r.length;f++){var v=r[f],y=v.querySelector("figure img")||v.querySelector("#product-img")||v.querySelector("img"),T=v.querySelector(".column-valor-produto .valor"),N=parseFloat(v.getAttribute("data-valor")||"0"),D=T?V(T.textContent):0;e.push({name:v.getAttribute("data-item-name")||v.getAttribute("data-name")||"",variant:v.getAttribute("data-item-variant")||"",imgSrc:y&&y.getAttribute("src")||"",quantity:parseInt(v.getAttribute("data-item-quantity")||"1",10),lineTotal:N,lineTotalPix:D,isPix:!!v.querySelector(".column-valor-produto .sub"),deposito:v.getAttribute("data-item-deposito")==="1"}),s+=N}if(e.length===0)return null;var nn=n.querySelector("#resumo-compra .resumo-valores .value"),X=nn?V(nn.textContent):0;X<=0&&(X=e.reduce(function(G,H){return G+(H.lineTotalPix||0)},0));var W=n.querySelector("#resumo-compra .discount-value"),J=W?V(W.textContent):0,cn=n.querySelector("#resumo-compra .txt-cupom, #resumo-compra .alert.alert-type-1"),rn="";if(cn){var vn=cn.textContent.match(/([A-Z0-9]{3,})/);vn&&(rn=vn[1])}var Sn=n.querySelector("#resumo-compra .frete-calculado"),Tn=null,Gn="";if(Sn&&Sn.textContent.trim()){var w=Sn.textContent.trim();if(/gr[aá]tis/i.test(w))Tn=0;else{var S=V(w);S>0&&(Tn=S)}var j=w.match(/(\d+)\s*dias?/i);j&&(Gn=j[1]+" dias úteis")}return{ts:Date.now(),items:e,subtotalPix:X,subtotalFull:s,discount:J,couponCode:rn,shipping:Tn,shippingDeadline:Gn,cepValue:""}}catch{return null}}function lt(t){try{fetch("/checkout/cart",{credentials:"include",headers:{"X-Requested-With":"XMLHttpRequest"}}).then(function(a){return a.text()}).then(function(a){var n=bt(a);n&&Z.set(O,JSON.stringify(n)),t(n)}).catch(function(){t(null)})}catch{t(null)}}function fn(t){var a=document.querySelector("#mm-layout .mm-id-sum");if(a){var n=a.parentNode;if(n){var e=document.createElement("div");e.innerHTML=Vn(t);var r=e.firstChild;r&&n.replaceChild(r,a)}}}if(b){let t=function(n){if(n=n||0,n>30){a();return}var e=x.querySelector("#login, #login-form-etapa-01");e||n>8?a():setTimeout(function(){t(n+1)},250)},a=function(){var n=en();tt(n),ft(),Hn(),setTimeout(Hn,600),setTimeout(Hn,1500),(!n||!n.items||n.items.length===0)&&lt(function(e){e&&e.items&&e.items.length>0&&fn(e)}),setTimeout(function(){var e=document.getElementById("mm-id-email");e&&!("ontouchstart"in window)&&e.focus()},250)};t()}function pt(t){var a=String(t||"").replace(/\D/g,"").slice(0,11);return a.length<=3?a:a.length<=6?a.slice(0,3)+"."+a.slice(3):a.length<=9?a.slice(0,3)+"."+a.slice(3,6)+"."+a.slice(6):a.slice(0,3)+"."+a.slice(3,6)+"."+a.slice(6,9)+"-"+a.slice(9)}function ct(t){var a=String(t||"").replace(/\D/g,"").slice(0,11);return a.length<=2?a.length?"("+a:"":a.length<=6?"("+a.slice(0,2)+") "+a.slice(2):a.length<=10?"("+a.slice(0,2)+") "+a.slice(2,6)+"-"+a.slice(6):"("+a.slice(0,2)+") "+a.slice(2,7)+"-"+a.slice(7)}function Et(t){var a=String(t||"").replace(/\D/g,"").slice(0,8);return a.length<=5?a:a.slice(0,5)+"-"+a.slice(5)}function xt(t,a){var n=String(t||"").replace(/\D/g,"");if(n.length!==8){a(null);return}try{fetch("https://viacep.com.br/ws/"+n+"/json/",{headers:{Accept:"application/json"}}).then(function(e){return e.json()}).then(function(e){if(!e||e.erro){a(null);return}a({logradouro:e.logradouro||"",bairro:e.bairro||"",cidade:e.localidade||"",estado:e.uf||""})}).catch(function(){a(null)})}catch{a(null)}}var Cn={mail:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4-8 5-8-5V6l8 5 8-5v2z"/></svg>',user:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>',doc:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M14 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6zm4 18H6V4h7v5h5v11zM8 12h8v2H8zm0 4h5v2H8z"/></svg>',phone:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z"/></svg>',pin:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>',home:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>',pix:'<svg width="24" height="24" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M11.917 11.71a2.046 2.046 0 0 1-1.454-.602l-2.1-2.1a.4.4 0 0 0-.551 0l-2.108 2.108a2.044 2.044 0 0 1-1.454.602h-.414l2.66 2.66c.83.83 2.177.83 3.007 0l2.667-2.668h-.253zM4.25 4.282c.55 0 1.066.214 1.454.602l2.108 2.108a.39.39 0 0 0 .552 0l2.1-2.1a2.044 2.044 0 0 1 1.453-.602h.253L9.503 1.623a2.127 2.127 0 0 0-3.007 0l-2.66 2.66h.414z"/><path d="m14.377 6.496-1.612-1.612a.307.307 0 0 1-.114.023h-.733c-.379 0-.75.154-1.017.422l-2.1 2.1a1.005 1.005 0 0 1-1.425 0L5.268 5.32a1.448 1.448 0 0 0-1.018-.422h-.9a.306.306 0 0 1-.109-.021L1.623 6.496c-.83.83-.83 2.177 0 3.008l1.618 1.618a.305.305 0 0 1 .108-.022h.901c.38 0 .75-.153 1.018-.421L7.375 8.57a1.034 1.034 0 0 1 1.426 0l2.1 2.1c.267.268.638.421 1.017.421h.733c.04 0 .079.01.114.024l1.612-1.612c.83-.83.83-2.178 0-3.008z"/></svg>',cardBig:'<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 4H4c-1.11 0-1.99.89-1.99 2L2 18c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V6c0-1.11-.89-2-2-2zm0 14H4v-6h16v6zm0-10H4V6h16v2z"/></svg>',barcode:'<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M2 6h2v12H2zm3 0h1v12H5zm2 0h3v12H7zm4 0h1v12h-1zm3 0h2v12h-2zm3 0h1v12h-1zm2 0h3v12h-3z"/></svg>',editPencil:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a.996.996 0 0 0 0-1.41l-2.34-2.34a.996.996 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>'};function Ct(t){var a=t?' value="'+I(t)+'"':"";return'<section class="mm-op-form-col"><h2 class="mm-id-h2">Falta pouco. Onde entregar?</h2><p class="mm-id-sub">Preencha seus dados e o endereço de entrega — o frete é calculado automaticamente.</p><form class="mm-op-form" data-mm-act="onepage-submit" novalidate><div class="mm-op-card"><h3 class="mm-op-card-title">'+Cn.user+'<span>Quem vai receber</span></h3><div class="mm-op-grid-2"><div class="mm-input-wrap mm-op-col-2"><span class="mm-input-icon" aria-hidden="true">'+Cn.mail+'</span><input type="email" id="mm-op-email" class="mm-input" placeholder="seu@email.com" autocomplete="email" inputmode="email" required'+a+'></div><div class="mm-input-wrap mm-op-col-2"><span class="mm-input-icon" aria-hidden="true">'+Cn.user+'</span><input type="text" id="mm-op-nome" class="mm-input" placeholder="Nome completo" autocomplete="name" required></div><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+Cn.doc+'</span><input type="tel" id="mm-op-cpf" class="mm-input" placeholder="CPF" inputmode="numeric" autocomplete="off" maxlength="14" required></div><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+Cn.phone+'</span><input type="tel" id="mm-op-tel" class="mm-input" placeholder="(11) 91234-5678" inputmode="tel" autocomplete="tel" maxlength="15" required></div></div><p class="mm-op-microcopy-soft">Usamos seus dados só pra emitir nota fiscal e te avisar da entrega.</p></div><div class="mm-op-card"><h3 class="mm-op-card-title">'+Cn.pin+'<span>Endereço de entrega</span></h3><div class="mm-op-grid-2"><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+Cn.pin+'</span><input type="tel" id="mm-op-cep" class="mm-input" placeholder="CEP — 00000-000" inputmode="numeric" autocomplete="postal-code" maxlength="9" required><span class="mm-input-status" id="mm-op-cep-status" aria-live="polite"></span></div><a class="mm-op-cep-help" href="https://buscacepinter.correios.com.br/app/endereco/index.php" target="_blank" rel="noopener">Não sei meu CEP</a><div class="mm-input-wrap mm-op-col-2"><span class="mm-input-icon" aria-hidden="true">'+Cn.home+'</span><input type="text" id="mm-op-rua" class="mm-input" placeholder="Rua, Av, Travessa..." autocomplete="address-line1" required></div><div class="mm-input-wrap"><input type="tel" id="mm-op-num" class="mm-input mm-input-noicon" placeholder="Número" inputmode="numeric" required></div><div class="mm-input-wrap"><input type="text" id="mm-op-comp" class="mm-input mm-input-noicon" placeholder="Complemento (opcional)" autocomplete="address-line2"></div><div class="mm-input-wrap mm-op-col-2"><input type="text" id="mm-op-bairro" class="mm-input mm-input-noicon" placeholder="Bairro" autocomplete="address-level3" required></div><div class="mm-input-wrap"><input type="text" id="mm-op-cidade" class="mm-input mm-input-noicon" placeholder="Cidade" autocomplete="address-level2" required></div><div class="mm-input-wrap"><input type="text" id="mm-op-uf" class="mm-input mm-input-noicon" placeholder="UF" maxlength="2" autocomplete="address-level1" required></div></div><div class="mm-op-frete" id="mm-op-frete-slot"></div></div><button type="submit" class="mm-cta mm-op-cta">Última etapa: pagamento'+z.arrow+'</button><p class="mm-id-microcopy mm-op-cta-sub">'+z.lock+'<span>Você revisa tudo antes de finalizar</span></p></form><div class="mm-trust mm-id-trust"><span class="mm-trust-item">'+z.lock+'<span>Pagamento seguro</span></span><span class="mm-trust-item">'+z.rotate+'<span>7 dias para troca</span></span><span class="mm-trust-item">'+z.shield+"<span>Garantia 12 meses</span></span></div></section>"}function dt(t,a){var n=document.getElementById("mm-layout");if(n)return n.parentElement!==x&&x.insertBefore(n,x.firstChild),n;var e=document.createElement("div");return e.id="mm-layout",e.classList.add("mm-id-layout"),e.classList.add("mm-op-layout"),e.innerHTML=Ln("delivery")+'<div class="mm-grid mm-id-grid mm-op-grid">'+Ct(a)+Vn(t)+"</div>",x.insertBefore(e,x.firstChild),document.body.classList.add("mm-checkout-rebuild"),x.classList.add("mm-shadow-mode"),document.documentElement.classList.remove("mm-cart-loading"),e}function Pn(t){var a=document.getElementById("mm-op-frete-slot");if(a){if(t==="loading"){a.innerHTML='<div class="mm-op-frete-loading"><div class="mm-op-frete-spinner"></div><span>Calculando frete...</span></div>';return}if(t==="error"){a.innerHTML='<div class="mm-op-frete-error"><span>Não conseguimos calcular o frete. Confira o CEP e tente novamente.</span></div>';return}var n=t.value===0,e=n?'<strong class="mm-op-frete-value is-free">Grátis</strong>':'<strong class="mm-op-frete-value">'+F(t.value)+"</strong>",r=t.name?'<span class="mm-op-frete-name">'+I(t.name)+"</span>":"",s=t.deadline?'<span class="mm-op-frete-deadline">Entrega em '+I(t.deadline)+"</span>":"",f=t.city?'<span class="mm-op-frete-city">para '+I(t.city)+"</span>":"",v="";if(t.options&&t.options.length>1){v='<div class="mm-op-frete-options"><button type="button" class="mm-op-frete-toggle" data-mm-act="toggle-frete-opts" aria-expanded="false">Ver outras opções ('+t.options.length+')</button><div class="mm-op-frete-options-list" hidden>';for(var y=0;y<t.options.length;y++){var T=t.options[y],N=T.isSelected?" is-selected":"",D=T.isFree?'<span class="mm-op-frete-opt-value is-free">Grátis</span>':'<span class="mm-op-frete-opt-value">'+F(T.value)+"</span>";v+='<button type="button" class="mm-op-frete-opt'+N+'" data-mm-act="op-ship-select" data-ship-id="'+I(T.id)+'" aria-pressed="'+(T.isSelected?"true":"false")+'"><span class="mm-op-frete-opt-radio" aria-hidden="true"><span></span></span><span class="mm-op-frete-opt-body"><span class="mm-op-frete-opt-name">'+I(T.name||"Padrão")+"</span>"+(T.deadline?'<span class="mm-op-frete-opt-deadline">'+I(T.deadline)+"</span>":"")+"</span>"+D+"</button>"}v+="</div></div>"}a.innerHTML='<div class="mm-op-frete-card'+(n?" is-free":"")+'"><div class="mm-op-frete-icon">'+z.truck+'</div><div class="mm-op-frete-body"><div class="mm-op-frete-row">'+r+e+"</div>"+s+f+"</div></div>"+v}}function Xn(){function t(G){if(!G)return"";var H=G.match(/(\d+(?:\s*[aà]\s*\d+)?)\s*dias?(?:\s*\([^)]+\))?\s*([úu]teis)?/i);return H?H[1].replace(/\s+/g," ")+" dias úteis":""}function a(G){for(var H=[],dn=G.querySelectorAll(".servico-frete"),un=0;un<dn.length;un++){var bn=dn[un],wn=bn.querySelector('input[type="radio"]'),En=bn.querySelector(".dias-entrega"),Bn=parseFloat(bn.getAttribute("data-valor-frete")||"0"),qn=bn.getAttribute("data-servico-frete")||"",Dn=En?En.textContent.trim():"",Rn=Dn.match(/(\d+(?:\s*[aà]\s*\d+)?\s*dias?(?:\s*[úu]teis)?)/i);H.push({id:wn?wn.value:"",name:qn,deadline:Rn?Rn[1].replace(/\s+/g," "):Dn,value:Bn,isFree:Bn===0,isSelected:wn?wn.checked:!1})}return H}var n=x.querySelector(".frete-calculado");if(n&&n.textContent.trim()){var e=a(n),r=n.querySelector(".frete-location .city"),s=r?r.textContent.trim():"",f=e.filter(function(G){return G.isSelected})[0]||e[0];if(f)return{value:f.value,name:f.name,deadline:f.deadline,city:s,options:e};var v=n.querySelector(".info-frete-selec .info-title span, .info-title span"),y=n.querySelector(".info-frete-selec .dias-entrega, .dias-entrega"),T=n.querySelector(".value.valor-frete, .valor-frete .value"),N=n.textContent,D=null;if(T&&(/gr[aá]tis/i.test(T.textContent)?D=0:D=V(T.textContent)),D===null&&(/gr[aá]tis/i.test(N)?D=0:D=V(N)||null),D!==null)return{value:D,name:v?v.textContent.trim():"",deadline:t(y?y.textContent:N),city:s,options:[]}}var nn=x.querySelector(".line-entrega"),X=x.querySelector(".value.valor-frete, .valor-frete .value");if(nn||X){var W=((nn||X).textContent||"").trim(),J=en(),cn=J&&J.shippingName||"",rn=J&&J.shippingDeadline||"",vn=J&&J.shippingCity||"",Sn=J?J.shippingOptions||[]:[];if(W){var Tn=(x.querySelector(".nome-servico-frete, .info-frete-selec .info-title span")||{}).textContent||"",Gn=(x.querySelector(".info-entrega, .prazo-entrega, .line-entrega .sub, .info-frete-selec .dias-entrega")||{}).textContent||"",w=t(Gn)||t(W)||rn,S=Tn.trim()||cn;if(/gr[aá]tis/i.test(W))return{value:0,deadline:w,name:S,city:vn,options:Sn};var j=V(W);if(j>0)return{value:j,deadline:w,name:S,city:vn,options:Sn}}if(J&&J.shipping!==null&&J.shipping!==void 0)return{value:J.shipping,deadline:rn,name:cn,city:vn,options:Sn}}return null}function i(){var t=document.getElementById("mm-op-cep");if(!t)return;var a=(t.value||"").replace(/\D/g,"");if(a.length!==8)return;if(i._lastCep===a){var n=document.getElementById("mm-op-frete-slot");if(n&&n.querySelector(".mm-op-frete-card"))return}i._lastCep=a;var e=(i._token||0)+1;i._token=e,at(a);var r=x.querySelector("#cep, .input-cep");r&&(r.value=yn(a),_n(r)),Pn("loading");try{window.Zord&&window.Zord.Cart&&typeof window.Zord.Cart.calculaFreteCarrinho=="function"&&window.Zord.Cart.calculaFreteCarrinho()}catch{}var s=0,f=18;function v(){if(i._token===e){s++;var y=Xn();if(y){Pn(y);var T=en();T&&(T.shipping=y.value,T.shippingDeadline=y.deadline,T.shippingName=y.name||"",T.shippingCity=y.city||"",T.shippingOptions=y.options||[],Z.set(O,JSON.stringify(T)),fn(T));return}s<f?setTimeout(v,350):Pn("error")}}setTimeout(v,400)}function l(){var t=document.getElementById("mm-layout");if(!t||t._mmOpBound)return;t._mmOpBound=!0,t.addEventListener("click",function(n){var e=n.target.closest('[data-mm-act="toggle-frete-opts"]');if(e){n.preventDefault();var r=e.parentElement.querySelector(".mm-op-frete-options-list");if(r){var s=r.hasAttribute("hidden");s?r.removeAttribute("hidden"):r.setAttribute("hidden",""),e.setAttribute("aria-expanded",s?"true":"false"),e.textContent=s?"Ocultar opções":"Ver outras opções"}return}var f=n.target.closest('[data-mm-act="op-ship-select"]');if(f){n.preventDefault();var v=f.getAttribute("data-ship-id");if(!v)return;var y=x.querySelector('.servico-frete input[type="radio"][value="'+v+'"]');if(!y){console.warn("[mm-op] modalidade não encontrada no DOM:",v);return}for(var T=t.querySelectorAll(".mm-op-frete-opt"),N=0;N<T.length;N++){var D=T[N],nn=D.getAttribute("data-ship-id")===v;D.classList.toggle("is-selected",nn),D.setAttribute("aria-pressed",nn?"true":"false")}y.checked=!0,y.click(),setTimeout(function(){var X=Xn();if(X){Pn(X);var W=en();W&&(W.shipping=X.value,W.shippingDeadline=X.deadline,W.shippingName=X.name||"",W.shippingOptions=X.options||[],Z.set(O,JSON.stringify(W)),fn(W))}},700);return}}),t.addEventListener("submit",function(n){var e=n.target.closest('[data-mm-act="onepage-submit"]');if(e){n.preventDefault();var r={email:(document.getElementById("mm-op-email")||{}).value||"",nome:(document.getElementById("mm-op-nome")||{}).value||"",cpf:(document.getElementById("mm-op-cpf")||{}).value||"",tel:(document.getElementById("mm-op-tel")||{}).value||"",cep:(document.getElementById("mm-op-cep")||{}).value||"",rua:(document.getElementById("mm-op-rua")||{}).value||"",num:(document.getElementById("mm-op-num")||{}).value||"",comp:(document.getElementById("mm-op-comp")||{}).value||"",bairro:(document.getElementById("mm-op-bairro")||{}).value||"",cidade:(document.getElementById("mm-op-cidade")||{}).value||"",uf:(document.getElementById("mm-op-uf")||{}).value||""},s=[];if(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(r.email.trim())||s.push("mm-op-email"),r.nome.trim().split(/\s+/).length<2&&s.push("mm-op-nome"),r.cpf.replace(/\D/g,"").length!==11&&s.push("mm-op-cpf"),r.tel.replace(/\D/g,"").length<10&&s.push("mm-op-tel"),r.cep.replace(/\D/g,"").length!==8&&s.push("mm-op-cep"),r.rua.trim()||s.push("mm-op-rua"),r.num.trim()||s.push("mm-op-num"),r.bairro.trim()||s.push("mm-op-bairro"),r.cidade.trim()||s.push("mm-op-cidade"),r.uf.trim()||s.push("mm-op-uf"),s.length){s.forEach(function(y){var T=document.getElementById(y);T&&(T.classList.add("mm-input-error"),setTimeout(function(){T.classList.remove("mm-input-error")},1800))});var f=document.getElementById(s[0]);f&&(f.focus(),f.scrollIntoView({block:"center",behavior:"smooth"}));return}var v=e.querySelector(".mm-cta");v&&v.classList.add("is-loading"),Z.set("mm_user_email",r.email.trim()),M(r)}}),t.addEventListener("input",function(n){var e=n.target;if(e){if(e.id==="mm-op-cpf")e.value=pt(e.value);else if(e.id==="mm-op-tel")e.value=ct(e.value);else if(e.id==="mm-op-cep"){e.value=Et(e.value);var r=e.value.replace(/\D/g,"");r.length===8&&u(r)}else e.id==="mm-op-uf"&&(e.value=(e.value||"").replace(/[^A-Za-z]/g,"").toUpperCase().slice(0,2));e.id&&e.id.indexOf("mm-op-")===0&&(l._draftTimer&&clearTimeout(l._draftTimer),l._draftTimer=setTimeout(sn,400))}});function a(){l._draftTimer&&(clearTimeout(l._draftTimer),l._draftTimer=null),sn()}t.addEventListener("blur",function(n){var e=n.target;e&&e.id&&e.id.indexOf("mm-op-")===0&&a()},!0),window.addEventListener("beforeunload",a)}function u(t){var a=document.getElementById("mm-op-cep-status");a&&(a.className="mm-input-status is-loading",a.textContent="Buscando..."),xt(t,function(n){if(a&&(a.className="mm-input-status"),!n){a&&(a.className="mm-input-status is-error",a.textContent="CEP não encontrado",setTimeout(function(){a.className="mm-input-status",a.textContent=""},2500));return}a&&(a.className="mm-input-status is-ok",a.innerHTML=z.check,setTimeout(function(){a.className="mm-input-status",a.innerHTML=""},1800));var e=[["mm-op-rua",n.logradouro],["mm-op-bairro",n.bairro],["mm-op-cidade",n.cidade],["mm-op-uf",n.estado]];e.forEach(function(s){var f=document.getElementById(s[0]);f&&s[1]&&!f.value&&(f.value=s[1])});var r=document.getElementById("mm-op-num");r&&setTimeout(function(){r.focus()},100),i._t&&clearTimeout(i._t),i._t=setTimeout(i,200)})}function E(t){if(!document.getElementById("mm-op-overlay")){var a=document.createElement("div");a.id="mm-op-overlay",a.innerHTML='<div class="mm-op-overlay-card"><div class="mm-op-overlay-spinner"></div><p class="mm-op-overlay-text">'+I(t||"Processando...")+"</p></div>",document.body.appendChild(a)}}function M(t){var a=t.nome.trim(),n=t.email.trim(),e=t.rua.trim(),r=t.bairro.trim(),s=t.cidade.trim(),f=t.uf.trim(),v=yn(t.cep.replace(/\D/g,""));Z.set("mm_user_email",n);var y=function(T,N){var D=x.querySelector(T);if(!D)return!1;try{var nn=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;nn.call(D,N)}catch{D.value=N}return D.dispatchEvent(new Event("input",{bubbles:!0})),D.dispatchEvent(new Event("change",{bubbles:!0})),D.dispatchEvent(new Event("blur",{bubbles:!0})),!0};y("#nome-completo_2",a),y("#cpf_2",t.cpf),y("#email_3",n),y("#telefone_2",t.tel),y("#cep",v),y("#destinatario",a),y("#logradouro",e),y("#numero",t.num),y("#complemento",t.comp),y("#bairro",r),y("#cidade",s),y("#estado",f),E("Indo para o pagamento..."),setTimeout(function(){var T=document.getElementById("mm-layout");T&&(T.style.display="none"),x.classList.remove("mm-shadow-mode");function N(X){var W=x.querySelector("#"+X);return W?W.closest("form"):null}function D(){var X=N("nome-completo_2");return X&&typeof X.requestSubmit=="function"?(X.requestSubmit(),!0):X?(X.submit(),!0):!1}function nn(){var X=N("cep");return X&&typeof X.requestSubmit=="function"?(X.requestSubmit(),!0):X?(X.submit(),!0):!1}setTimeout(function(){D(),setTimeout(function(){nn();var X=0;function W(){X++;var J=document.querySelector('input[name="forma-pagto"], #forma-pagto-pix, #forma-pagto-cartao, #forma-pagto-boleto');if(J&&J.offsetParent!==null){try{_(t)}catch{var cn=document.getElementById("mm-op-overlay");cn&&cn.remove();var rn=document.getElementById("mm-layout");rn&&(rn.style.display="none")}return}if(X<30)setTimeout(W,250);else{var vn=document.getElementById("mm-op-overlay");vn&&vn.remove()}}setTimeout(W,800)},1500)},150)},100)}function L(){var t={pix:null,cartao:null,boleto:null},a=[];try{a=window.Zord&&window.Zord.Calculo&&window.Zord.Calculo.FormasPagamentoPedido||[]}catch{}a.forEach(function(D){var nn=D.formaPagamento&&D.formaPagamento.id,X=D.formaPagamento&&D.formaPagamento.isCartao,W=D.condicoes||[];!W.length||!X||(!t.cartao||W.length>t.cartao.condicoes.length)&&(t.cartao={formaId:nn,valorTotal:W[0].valorTotal,condicoes:W.map(function(J){return{nome:J.condicaoPagamento&&J.condicaoPagamento.nome,numParcelas:J.condicaoPagamento&&J.condicaoPagamento.numeroParcelas,valorParcela:J.valorParcela,valorTotal:J.valorTotal}})})});function n(D){if(!D)return 0;var nn=D.getAttribute&&D.getAttribute("data-valor-total");if(nn){var X=parseFloat(nn);if(X>0)return X}var W=(D.textContent||"").replace(/[^\d,.]/g,"");return W.indexOf(",")!==-1&&(W=W.replace(/\./g,"").replace(",",".")),parseFloat(W)||0}var e=document.querySelector("#valor-total-pedido-pix, .valor-total-pix[data-valor-total]"),r=n(e);if(r>0)t.pix={formaId:17,valorTotal:r};else{var s=a.find&&a.find(function(D){return D.formaPagamento&&D.formaPagamento.id===17});s&&s.condicoes&&s.condicoes[0]&&(t.pix={formaId:17,valorTotal:s.condicoes[0].valorTotal})}var f=document.querySelector("#valor-total-pedido-boleto, .valor-total-boleto[data-valor-total]"),v=n(f);if(v>0)t.boleto={valorTotal:v};else{var y=a.find&&a.find(function(D){var nn=D.formaPagamento&&D.formaPagamento.id,X=D.formaPagamento&&D.formaPagamento.isCartao;return!X&&nn!==17&&D.condicoes&&D.condicoes.length});y&&(t.boleto={formaId:y.formaPagamento.id,valorTotal:y.condicoes[0].valorTotal})}if(!t.cartao){var T=document.querySelector(".valor-parcela-cartao");if(T){var N=n(T);N>0&&(t.cartao={valorTotal:N*12,condicoes:[]})}}return t}function _(t){var a=en(),n=L();x.classList.add("mm-shadow-mode"),document.body.classList.add("mm-checkout-rebuild");var e=document.getElementById("mm-layout");(!e||e.parentElement!==x)&&(e&&e.parentElement&&e.parentElement.removeChild(e),e=document.createElement("div"),e.id="mm-layout",x.insertBefore(e,x.firstChild)),e.className="mm-op-layout mm-op-step3-layout",e.style.display="",e.innerHTML=R(a,t,n),document.documentElement.classList.remove("mm-cart-loading");var r=document.getElementById("mm-op-overlay");r&&r.remove();try{window.scrollTo({top:0,behavior:"smooth"})}catch{window.scrollTo(0,0)}on(t,n)}function R(t,a,n){var e=typeof renderGlobalFooter=="function"?renderGlobalFooter():"";return Ln("payment")+'<main class="mm-op-main"><div class="mm-op-step3-grid"><section class="mm-op-step3-left">'+K(n)+an()+'</section><aside class="mm-op-step3-sum-wrap">'+U(a)+Q(t,n,"pix")+"</aside></div></main>"+e}function U(t){var a=t||{},n=I(a.nome||""),e=I(a.email||""),r=I(a.cpf||""),s=I(a.tel||""),f=I(a.rua||""),v=I(a.num||""),y=a.comp?", "+I(a.comp):"",T=I(a.bairro||""),N=I(a.cidade||""),D=I(a.uf||""),nn=I(a.cep||"");return'<div class="mm-op-step3-completed"><div class="mm-op-completed-card" data-section="dados"><div class="mm-op-completed-head"><span class="mm-op-completed-check">'+z.check+'</span><h3 class="mm-op-completed-title">Dados pessoais</h3><a href="/checkout/onepage" class="mm-op-completed-edit" data-mm-act="edit-dados" aria-label="Editar dados">'+Cn.editPencil+' Editar</a></div><dl class="mm-op-completed-body">'+(n?"<div><dt>Nome</dt><dd>"+n+"</dd></div>":"")+(e?"<div><dt>E-mail</dt><dd>"+e+"</dd></div>":"")+(r?"<div><dt>CPF</dt><dd>"+r+"</dd></div>":"")+(s?"<div><dt>Telefone</dt><dd>"+s+"</dd></div>":"")+'</dl></div><div class="mm-op-completed-card" data-section="endereco"><div class="mm-op-completed-head"><span class="mm-op-completed-check">'+z.check+'</span><h3 class="mm-op-completed-title">Endereço de entrega</h3><a href="/checkout/onepage" class="mm-op-completed-edit" data-mm-act="edit-endereco" aria-label="Editar endereço">'+Cn.editPencil+' Editar</a></div><address class="mm-op-completed-address">'+f+", "+v+y+"<br>"+T+" — "+N+"/"+D+"<br>"+(nn?"CEP "+nn:"")+"</address></div></div>"}function K(t){var a=t.pix?t.pix.valorTotal:0,n=t.cartao?t.cartao.valorTotal:0,e=t.boleto?t.boleto.valorTotal:0,r=n>a?n-a:0,s=null;t.cartao&&t.cartao.condicoes&&t.cartao.condicoes.length&&(s=t.cartao.condicoes[t.cartao.condicoes.length-1]);var f=s?"até "+s.numParcelas+"x de "+F(s.valorParcela)+" sem juros":n>0?"até 12x sem juros":"Cartão de crédito",v='<label class="mm-op-pay-radio is-active" data-forma="pix"><input type="radio" name="mm-pay" value="pix" checked><div class="mm-op-pay-head"><span class="mm-op-pay-radio-dot" aria-hidden="true"></span><span class="mm-op-pay-icon">'+Cn.pix+'</span><div class="mm-op-pay-body"><span class="mm-op-pay-title">PIX</span><span class="mm-op-pay-sub">Aprovação instantânea · em 1 minuto</span></div><div class="mm-op-pay-price">'+(r>0?'<span class="mm-op-pay-badge-save">Economize '+F(r)+"</span>":"")+'<span class="mm-op-pay-amount">'+F(a)+'</span></div></div><div class="mm-op-pay-detail"><ul class="mm-op-pay-benefits"><li>'+z.check+"<span>QR Code e Copia-e-Cola após confirmar</span></li><li>"+z.check+"<span>Pedido aprovado em até 1 minuto</span></li><li>"+z.check+"<span>Pagamento 100% seguro · sem dados de cartão</span></li></ul></div></label>",y='<div class="mm-op-pay-brands"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-visa.svg" alt="Visa" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-mastercard.svg" alt="Mastercard" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-american-express.svg" alt="American Express" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-elo.svg" alt="Elo" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-hipercard.svg" alt="Hipercard" width="32" height="20"></div>',T='<span class="mm-op-req" aria-hidden="true">*</span>';function N(W){return'<span class="mm-op-field-err" id="'+W+'-err" role="alert" aria-live="polite"></span>'}var D='<div class="mm-op-card-form"><div class="mm-op-card-field mm-op-card-field-full"><label for="mm-pay-card-num">Número do cartão'+T+'</label><div class="mm-input-wrap mm-input-wrap-card"><span class="mm-input-icon" aria-hidden="true">'+z.card+'</span><input id="mm-pay-card-num" type="tel" class="mm-input" inputmode="numeric" autocomplete="off" data-mmac="cc-number" placeholder="0000 0000 0000 0000" maxlength="23" aria-describedby="mm-pay-card-num-err" aria-required="true" disabled><span class="mm-op-card-brand-detected" aria-live="polite"></span></div>'+N("mm-pay-card-num")+'</div><div class="mm-op-card-field mm-op-card-field-full"><label for="mm-pay-card-name">Nome impresso no cartão'+T+'</label><input id="mm-pay-card-name" type="text" class="mm-input mm-input-noicon" autocomplete="off" data-mmac="cc-name" placeholder="Como aparece no cartão" maxlength="100" aria-describedby="mm-pay-card-name-err" aria-required="true" disabled>'+N("mm-pay-card-name")+'</div><div class="mm-op-card-field mm-op-card-field-half"><label for="mm-pay-card-exp">Validade'+T+'</label><input id="mm-pay-card-exp" type="tel" class="mm-input mm-input-noicon" inputmode="numeric" autocomplete="off" data-mmac="cc-exp" placeholder="MM/AA" maxlength="5" aria-describedby="mm-pay-card-exp-err" aria-required="true" disabled>'+N("mm-pay-card-exp")+'</div><div class="mm-op-card-field mm-op-card-field-half"><label for="mm-pay-card-cvv">CVV'+T+'</label><input id="mm-pay-card-cvv" type="tel" class="mm-input mm-input-noicon" inputmode="numeric" autocomplete="off" data-mmac="cc-csc" placeholder="000" maxlength="4" aria-describedby="mm-pay-card-cvv-err" aria-required="true" disabled>'+N("mm-pay-card-cvv")+'</div><div class="mm-op-card-field mm-op-card-field-full"><label for="mm-pay-card-installments">Condições de pagamento'+T+'</label><select id="mm-pay-card-installments" class="mm-input mm-input-noicon mm-op-card-installments" aria-describedby="mm-pay-card-installments-err" aria-required="true" disabled><option value="">Digite o cartão pra ver as condições</option></select>'+N("mm-pay-card-installments")+"</div></div>",nn='<label class="mm-op-pay-radio" data-forma="cartao"><input type="radio" name="mm-pay" value="cartao"><div class="mm-op-pay-head"><span class="mm-op-pay-radio-dot" aria-hidden="true"></span><span class="mm-op-pay-icon">'+Cn.cardBig+'</span><div class="mm-op-pay-body"><span class="mm-op-pay-title">Cartão de Crédito</span><span class="mm-op-pay-sub">'+I(f)+'</span></div><div class="mm-op-pay-price"><span class="mm-op-pay-amount">'+F(n)+'</span></div></div><div class="mm-op-pay-detail">'+y+D+"</div></label>",X='<label class="mm-op-pay-radio" data-forma="boleto"><input type="radio" name="mm-pay" value="boleto"><div class="mm-op-pay-head"><span class="mm-op-pay-radio-dot" aria-hidden="true"></span><span class="mm-op-pay-icon">'+Cn.barcode+'</span><div class="mm-op-pay-body"><span class="mm-op-pay-title">Boleto Bancário</span><span class="mm-op-pay-sub">Aprovação em 1 a 3 dias úteis</span></div><div class="mm-op-pay-price"><span class="mm-op-pay-amount">'+F(e)+'</span></div></div><div class="mm-op-pay-detail"><ul class="mm-op-pay-benefits"><li>'+z.check+"<span>Código de barras enviado por e-mail</span></li><li>"+z.check+"<span>Pagamento em banco, lotérica ou app</span></li><li>"+z.check+"<span>Vencimento em 3 dias úteis</span></li></ul></div></label>";return'<section class="mm-op-step3-card" aria-labelledby="mm-op-step3-h"><div class="mm-op-step3-heading"><h2 id="mm-op-step3-h" class="mm-h">Como você quer pagar?</h2><p class="mm-op-step3-sub">Última etapa · escolha sua forma de pagamento '+(r>0?"· PIX tem desconto de "+F(r):"")+'</p></div><div class="mm-op-pay-radios" role="radiogroup" aria-label="Forma de pagamento">'+v+nn+X+'</div><button type="button" class="mm-cta mm-op-cta mm-op-finalizar" data-mm-act="finalizar-compra">'+z.lock+'<span class="mm-op-finalizar-label">Finalizar compra · '+F(a)+'</span></button><p class="mm-op-finalizar-sub">'+z.shield+"<span>Pagamento seguro · criptografia SSL-256 · seus dados não são armazenados</span></p></section>"}function an(){return'<div class="mm-op-trust-payment"><div class="mm-op-trust-payment-row"><span class="mm-trust-item">'+z.lock+'<span>Site 100% seguro</span></span><span class="mm-trust-item">'+z.shield+'<span>Garantia 12 meses</span></span><span class="mm-trust-item">'+z.rotate+'<span>7 dias pra trocar</span></span><span class="mm-trust-item">'+z.whats+'<span>Atendimento humano</span></span></div><p class="mm-op-trust-payment-note">Ao finalizar, você concorda com os <a href="/termos-de-uso" target="_blank" rel="noopener">termos de compra</a> e a <a href="/politica-privacidade" target="_blank" rel="noopener">política de privacidade</a>.</p></div>'}function Q(t,a,n){if(!t||!t.items||!t.items.length)return'<aside class="mm-id-sum mm-sum mm-op-step3-sum"><h2 class="mm-h">Resumo</h2><div class="mm-sum-card"><p class="mm-sum-empty">Não conseguimos carregar o resumo do seu pedido.</p></div></aside>';var e=a.pix?a.pix.valorTotal:0,r=a.cartao?a.cartao.valorTotal:0,s=a.boleto?a.boleto.valorTotal:0,f=r>e?r-e:0,v=n==="pix"?e:n==="boleto"?s:r,y=n==="pix"?"no PIX":n==="boleto"?"no boleto":"no cartão",T=3,N=t.items.slice(0,T),D=t.items.length-T,nn=N.map(function(rn){var vn=rn.imgSrc?'<img src="'+I(rn.imgSrc)+'" alt="'+I(rn.name)+'" loading="lazy">':'<div class="mm-id-thumb-placeholder">'+z.box+"</div>",Sn=rn.quantity>1?'<strong class="mm-id-thumb-qty">'+rn.quantity+"×</strong> ":"",Tn=rn.lineTotalPix>0?rn.lineTotalPix:rn.lineTotal;return'<div class="mm-id-thumb"><div class="mm-id-thumb-img">'+vn+'</div><div class="mm-id-thumb-body"><p class="mm-id-thumb-name">'+Sn+I(rn.name)+"</p>"+(rn.variant?'<p class="mm-id-thumb-variant">'+I(rn.variant)+"</p>":"")+'</div><div class="mm-id-thumb-price">'+F(Tn)+"</div></div>"}).join("");D>0&&(nn+='<div class="mm-id-thumb-more">+ '+D+" "+(D===1?"item":"itens")+" a mais</div>");var X=t.subtotalFull>0?t.subtotalFull:t.subtotalPix,W='<div class="mm-row"><span class="mm-row-label">Subtotal</span><span class="mm-row-value">'+F(X)+"</span></div>";if(t.shipping!==null&&t.shipping!==void 0){var J=t.shipping===0?'<span class="mm-row-value is-free">'+z.check+" Grátis</span>":'<span class="mm-row-value">'+F(t.shipping)+"</span>";W+='<div class="mm-row"><span class="mm-row-label">Frete'+(t.shippingDeadline?' <span class="mm-row-sub">· '+I(t.shippingDeadline)+"</span>":"")+"</span>"+J+"</div>"}t.discount>0&&(W+='<div class="mm-row"><span class="mm-row-label">Cupom'+(t.couponCode?' <span class="mm-row-sub">· '+I(t.couponCode)+"</span>":"")+'</span><span class="mm-row-value is-discount">− '+F(t.discount)+"</span></div>"),f>0&&n==="pix"&&(W+='<div class="mm-row mm-row-pix-discount"><span class="mm-row-label">'+z.bolt+' Desconto PIX</span><span class="mm-row-value is-discount">− '+F(f)+"</span></div>");var cn='<div class="mm-total"><div class="mm-total-label">Total</div><div class="mm-total-value" data-mm-total>'+F(v)+'</div><div class="mm-total-pix" data-mm-total-sub><span>'+y+"</span></div></div>";return'<aside class="mm-id-sum mm-sum mm-op-step3-sum" data-active-forma="'+I(n)+'"><h2 class="mm-h">Resumo do pedido</h2><div class="mm-sum-card"><div class="mm-id-thumbs">'+nn+'</div><div class="mm-sum-rows">'+W+"</div>"+cn+"</div></aside>"}function on(t,a){var n=document.getElementById("mm-layout");if(!n||n._mmStep3Bound)return;n._mmStep3Bound=!0;var e=en(),r={activeForma:"pix",cardNumValid:!1,cardBrand:null,cardInstallments:null,submitting:!1};n.addEventListener("click",function(w){var S=w.target.closest(".mm-op-pay-radio");if(S){var j=S.querySelector('input[type="radio"]');j&&!j.checked&&(w.preventDefault(),j.checked=!0,T(S.getAttribute("data-forma")));return}if(w.target.closest('[data-mm-act="finalizar-compra"]')){w.preventDefault(),Gn();return}var G=w.target.closest('[data-mm-act="edit-dados"], [data-mm-act="edit-endereco"]');if(G){w.preventDefault(),location.reload();return}}),n.addEventListener("input",function(w){var S=w.target;!S||!S.id||(S.id==="mm-pay-card-num"?W(S):S.id==="mm-pay-card-exp"?J(S):S.id==="mm-pay-card-cvv"&&(S.value=(S.value||"").replace(/\D/g,"").slice(0,4)))}),n.addEventListener("change",function(w){if(w.target&&w.target.id==="mm-pay-card-installments"){var S=w.target,j=S.options[S.selectedIndex];j&&j.value&&(r.cardInstallments={numero:parseInt(j.value,10),label:j.textContent||""},vn(j.value),f("mm-pay-card-installments"))}}),n.addEventListener("blur",function(w){var S=w.target;if(!(!S||!S.id)){var j=["mm-pay-card-num","mm-pay-card-name","mm-pay-card-exp","mm-pay-card-cvv"];j.indexOf(S.id)!==-1&&v(S.id)}},!0),n.addEventListener("focus",function(w){var S=w.target;!S||!S.id||/^mm-pay-card-/.test(S.id)&&f(S.id)},!0);function s(w,S){var j=document.getElementById(w),G=document.getElementById(w+"-err");j&&(j.classList.add("mm-input-error"),j.setAttribute("aria-invalid","true")),G&&(G.textContent=S,G.classList.add("is-visible"))}function f(w){var S=document.getElementById(w),j=document.getElementById(w+"-err");S&&(S.classList.remove("mm-input-error"),S.removeAttribute("aria-invalid")),j&&(j.textContent="",j.classList.remove("is-visible"))}function v(w){var S=document.getElementById(w);if(!S)return!0;var j=(S.value||"").trim();if(w==="mm-pay-card-num"){var G=j.replace(/\D/g,"");return G?G.length<13?(s(w,"Número do cartão incompleto"),!1):y(G)?(f(w),!0):(s(w,"Número do cartão inválido — confira os dígitos"),!1):(s(w,"Informe o número do cartão"),!1)}if(w==="mm-pay-card-name")return j?j.split(/\s+/).length<2?(s(w,"Use o nome completo como aparece no cartão"),!1):(f(w),!0):(s(w,"Informe o nome impresso no cartão"),!1);if(w==="mm-pay-card-exp"){var H=j.replace(/\D/g,"");if(H.length!==4)return s(w,"Informe a validade no formato MM/AA"),!1;var dn=parseInt(H.slice(0,2),10),un=parseInt(H.slice(2),10);if(dn<1||dn>12)return s(w,"Mês inválido (01 a 12)"),!1;var bn=new Date,wn=bn.getFullYear()%100,En=bn.getMonth()+1;return un<wn||un===wn&&dn<En?(s(w,"Cartão expirado"),!1):(f(w),!0)}if(w==="mm-pay-card-cvv"){var Bn=j.replace(/\D/g,"");return Bn.length<3?(s(w,"CVV deve ter 3 ou 4 dígitos"),!1):(f(w),!0)}return w==="mm-pay-card-installments"?j?(f(w),!0):(s(w,"Selecione o número de parcelas"),!1):!0}function y(w){for(var S=0,j=!1,G=w.length-1;G>=0;G--){var H=parseInt(w.charAt(G),10);j&&(H*=2,H>9&&(H-=9)),S+=H,j=!j}return S%10===0}function T(w){if(!(!w||r.activeForma===w)){r.activeForma=w,n.querySelectorAll(".mm-op-pay-radio").forEach(function(H){H.classList.toggle("is-active",H.getAttribute("data-forma")===w)});var S=document.getElementById("forma-pagto-"+w);if(S&&!S.checked)try{S.click()}catch{}var j=n.querySelectorAll(".mm-op-card-form input, .mm-op-card-form select"),G=w==="cartao";j.forEach(function(H){H.disabled=!G,G?H.dataset.mmac&&H.setAttribute("autocomplete",H.dataset.mmac):H.setAttribute("autocomplete","off")}),N(w),w==="cartao"&&setTimeout(function(){var H=document.getElementById("mm-pay-card-num");H&&!H.value&&H.focus()},250)}}function N(w){var S=n.querySelector(".mm-op-step3-sum");if(S){S.setAttribute("data-active-forma",w);var j=a.pix?a.pix.valorTotal:0,G=a.cartao?a.cartao.valorTotal:0,H=a.boleto?a.boleto.valorTotal:0,dn=G>j?G-j:0,un=w==="pix"?j:w==="boleto"?H:G,bn=w==="pix"?"no PIX":w==="boleto"?"no boleto":"no cartão",wn=S.querySelector("[data-mm-total]");if(wn){var En=wn.textContent||"",Bn=V(En);Bn!==un?D(wn,Bn,un,360):wn.textContent=F(un)}var qn=S.querySelector("[data-mm-total-sub] span");qn&&qn.textContent!==bn&&(qn.textContent=bn);var Dn=S.querySelector(".mm-sum-rows"),Rn=Dn?Dn.querySelector(".mm-row-pix-discount"):null;if(w==="pix"&&dn>0){if(!Rn&&Dn){var et=document.createElement("div");et.innerHTML='<div class="mm-row mm-row-pix-discount"><span class="mm-row-label">'+z.bolt+' Desconto PIX</span><span class="mm-row-value is-discount">− '+F(dn)+"</span></div>",Dn.appendChild(et.firstChild)}}else Rn&&Rn.remove();nn(w)}}function D(w,S,j,G){w._mmAnimToken&&cancelAnimationFrame(w._mmAnimToken);var H=null,dn=j-S;function un(bn){H||(H=bn);var wn=bn-H,En=Math.min(1,wn/G),Bn=1-Math.pow(1-En,3),qn=S+dn*Bn;w.textContent=F(qn),En<1?w._mmAnimToken=requestAnimationFrame(un):(w.textContent=F(j),w._mmAnimToken=null)}w._mmAnimToken=requestAnimationFrame(un)}function nn(w){var S=n.querySelector(".mm-op-finalizar-label");if(S){var j=w==="pix"?a.pix&&a.pix.valorTotal:w==="boleto"?a.boleto&&a.boleto.valorTotal:a.cartao&&a.cartao.valorTotal;S.textContent="Finalizar compra · "+F(j||0)}}function X(w){var S=(w||"").replace(/\D/g,"");return S?/^4/.test(S)?"visa":/^(5[1-5]|22[2-9]|2[3-6]|27[01]|2720)/.test(S)?"mastercard":/^3[47]/.test(S)?"amex":/^(4011|4312|4389|4514|4573|5041|5066|5067|509|6277|6362|6363|650|6516|6550)/.test(S)?"elo":/^(606282|384100|384140|384160|606|637095|637568|637599|637609|637612)/.test(S)?"hipercard":null:null}function W(w){var S=(w.value||"").replace(/\D/g,"").slice(0,19),j=S.replace(/(\d{4})(?=\d)/g,"$1 ");if(j!==w.value){var G=w.selectionStart;w.value=j;try{w.setSelectionRange(G+1,G+1)}catch{}}var H=X(S);r.cardBrand=H,r.cardNumValid=S.length>=13;var dn=n.querySelector(".mm-op-card-brand-detected");dn&&(dn.className="mm-op-card-brand-detected"+(H?" is-"+H:""),dn.textContent=H?H.toUpperCase():""),S.length>=6&&(cn(S),Tn())}function J(w){var S=(w.value||"").replace(/\D/g,"").slice(0,4),j=S.length>2?S.slice(0,2)+"/"+S.slice(2):S;if(w.value=j,S.length===4){var G=S.slice(0,2),H="20"+S.slice(2);rn("pag-cartao-mes-validade",String(parseInt(G,10))),rn("pag-cartao-ano-validade",H)}}function cn(w){var S=document.getElementById("pag-cartao-numero");if(S){try{var j=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;j.call(S,w)}catch{S.value=w}S.dispatchEvent(new Event("input",{bubbles:!0})),S.dispatchEvent(new Event("change",{bubbles:!0})),S.dispatchEvent(new Event("blur",{bubbles:!0}))}}function rn(w,S){var j=document.getElementById(w);if(j){try{var G=Object.getOwnPropertyDescriptor(window.HTMLSelectElement.prototype,"value").set;G.call(j,S)}catch{j.value=S}j.dispatchEvent(new Event("change",{bubbles:!0}))}}function vn(w){rn("pag-cartao-parcela",w)}var Sn=null;function Tn(){if(Sn)return;var w=document.getElementById("pag-cartao-parcela");if(!w)return;var S=document.getElementById("mm-pay-card-installments");if(!S)return;function j(){var G=w.querySelectorAll("option");if(!(G.length<=1)){var H="";G.forEach(function(dn){if(!dn.value){H+='<option value="">Selecione as parcelas</option>';return}H+='<option value="'+I(dn.value)+'">'+I(dn.textContent.trim())+"</option>"}),S.innerHTML=H,S.options.length>1&&(S.selectedIndex=1,r.cardInstallments={numero:parseInt(S.options[1].value,10)||1,label:S.options[1].textContent},vn(S.options[1].value))}}j(),Sn=new MutationObserver(j),Sn.observe(w,{childList:!0,subtree:!0})}function Gn(){if(console.log("[mm-checkout] submitFinalizar() entry",{submitting:r.submitting,activeForma:r.activeForma}),r.submitting)return;var w=r.activeForma,S=n.querySelector(".mm-op-finalizar");if(!S){console.log("[mm-checkout] submitFinalizar: no .mm-op-finalizar btn");return}if(w==="cartao"){var j=["mm-pay-card-num","mm-pay-card-name","mm-pay-card-exp","mm-pay-card-cvv","mm-pay-card-installments"],G=j.filter(function(mn){return!v(mn)});if(console.log("[mm-checkout] validation",{errorCount:G.length,errors:G}),G.length){var H=document.getElementById(G[0]);if(H){H.focus();try{H.scrollIntoView({block:"center",behavior:"smooth"})}catch{}}return}var dn=document.getElementById("mm-pay-card-name"),un=document.getElementById("mm-pay-card-cvv"),bn=document.getElementById("pag-cartao-titular");if(bn){try{var wn=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;wn.call(bn,dn.value.trim())}catch{bn.value=dn.value.trim()}bn.dispatchEvent(new Event("input",{bubbles:!0})),bn.dispatchEvent(new Event("blur",{bubbles:!0}))}var En=document.getElementById("pag-cartao-vericacao");if(En){try{var Bn=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;Bn.call(En,un.value.replace(/\D/g,""))}catch{En.value=un.value.replace(/\D/g,"")}En.dispatchEvent(new Event("input",{bubbles:!0})),En.dispatchEvent(new Event("blur",{bubbles:!0}))}var qn=document.getElementById("pag-cartao-cpf");if(qn&&t&&t.cpf){try{var Dn=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;Dn.call(qn,t.cpf)}catch{qn.value=t.cpf}qn.dispatchEvent(new Event("input",{bubbles:!0})),qn.dispatchEvent(new Event("blur",{bubbles:!0}))}}r.submitting=!0,S.classList.add("is-loading"),S.setAttribute("aria-busy","true");var Rn=S.querySelector(".mm-op-finalizar-label");if(Rn&&(Rn.textContent="Processando pagamento..."),E("Finalizando seu pedido..."),w==="cartao"){var et=document.getElementById("mm-pay-card-installments");et&&et.value&&vn(et.value)}window.__mmCheckoutDebug=window.__mmCheckoutDebug||[];function gn(mn,An){var Fn={t:new Date().toISOString(),msg:mn,data:An};window.__mmCheckoutDebug.push(Fn),console.log("[mm-checkout]",mn,An||"")}function ot(){if(gn("doSubmit() called",{forma:w}),w==="cartao"){var mn=document.getElementById("aceito-termos-bcash-one-card");mn&&!mn.checked&&(mn.checked=!0,mn.dispatchEvent(new Event("change",{bubbles:!0}))),gn("terms",{checked:mn?.checked})}var An=w==="pix"?"form-pag-pix":w==="boleto"?"form-pag-boleto":"form-pag-cartao",Fn=document.getElementById(An);if(!Fn){gn("ERROR: form not found",{formId:An}),alert("Erro interno: formulário de pagamento não encontrado. Recarregue a página."),r.submitting=!1,S.classList.remove("is-loading");var jn=document.getElementById("mm-op-overlay");jn&&jn.remove();return}w==="cartao"&&gn("pre-submit cartao state",{numero:document.getElementById("pag-cartao-numero")?.value,bandeira:document.getElementById("pag-cartao-bandeira")?.value,parcela:document.getElementById("pag-cartao-parcela")?.value,titular:document.getElementById("pag-cartao-titular")?.value,mes:document.getElementById("pag-cartao-mes-validade")?.value,ano:document.getElementById("pag-cartao-ano-validade")?.value,cvv:document.getElementById("pag-cartao-vericacao")?.value?.length,cpf:document.getElementById("pag-cartao-cpf")?.value,tokenMp:document.getElementById("pag-cartao-token-mp")?.value?.substring(0,20)});var Nn=Fn.querySelector('button.button-success, button[type="submit"], input[type="submit"]');if(Nn)gn("clicking native button",{text:Nn.textContent?.trim()}),Nn.click();else if(typeof Fn.requestSubmit=="function"){gn("no native btn, using requestSubmit");try{Fn.requestSubmit()}catch(Zn){gn("requestSubmit error",Zn.message),Fn.submit()}}else gn("no native btn, using form.submit()"),Fn.submit();setTimeout(function(){if(r.submitting&&location.pathname.indexOf("/onepage")!==-1){gn("8s failsafe: still on /onepage, removing overlay"),r.submitting=!1,S.classList.remove("is-loading");var Zn=document.getElementById("mm-op-overlay");Zn&&Zn.remove(),x.classList.remove("mm-shadow-mode"),n&&(n.style.display="none")}},8e3),setTimeout(function(){x.classList.remove("mm-shadow-mode"),n&&(n.style.display="none")},600)}function gt(){r.submitting=!1,S.classList.remove("is-loading"),S.removeAttribute("aria-busy");var mn=S.querySelector(".mm-op-finalizar-label");mn&&(mn.textContent="Finalizar compra");var An=document.getElementById("mm-op-overlay");An&&An.remove()}function zt(){var mn=Date.now(),An=1e4;(function Fn(){var jn=document.getElementById("pag-cartao-token-mp"),Nn=jn?(jn.value||"").trim():"",Zn=Nn&&Nn!=="loading..."&&Nn.length>10;if(Zn){gn("fallback: token ready"),ot();return}if(Date.now()-mn>=An){gn("fallback: timeout",{lastVal:Nn}),ot();return}setTimeout(Fn,200)})()}function Bt(){if(gn("generateMpTokenAndSubmit() start"),typeof Mercadopago>"u"){gn("Mercadopago global missing, falling back to wait strategy"),zt();return}var mn=document.getElementById("pag-cartao-token-mp"),An=mn?(mn.value||"").trim():"";if(An&&An!=="loading..."&&An.length>10){gn("token already present, submitting",{len:An.length}),ot();return}var Fn=(document.getElementById("mm-pay-card-num")?.value||"").replace(/\D/g,""),jn=(document.getElementById("mm-pay-card-exp")?.value||"").replace(/\D/g,""),Nn=(document.getElementById("mm-pay-card-cvv")?.value||"").replace(/\D/g,""),Zn=(document.getElementById("mm-pay-card-name")?.value||"").trim(),qt=(t&&t.cpf||document.getElementById("mm-op-cpf")?.value||"").replace(/\D/g,"");if(!Fn||!jn||!Nn||!Zn||!qt){gn("missing card fields",{num:Fn.length,exp:jn.length,cvv:Nn.length,name:!!Zn,doc:qt.length}),alert("Preencha todos os dados do cartão antes de finalizar."),gt();return}var At=jn.slice(0,2),Ft=jn.length===4?"20"+jn.slice(2):jn.slice(2);gn("calling Mercadopago.createToken",{numLen:Fn.length,month:At,year:Ft});try{Mercadopago.createToken({cardNumber:Fn,securityCode:Nn,cardExpirationMonth:At,cardExpirationYear:Ft,cardholderName:Zn,docType:"CPF",docNumber:qt},function(yt,Qn){if(gn("createToken callback",{status:yt,hasId:!!(Qn&&Qn.id),err:Qn&&Qn.error}),yt===200||yt===201){if(mn){var Lt=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;Lt.call(mn,Qn.id),mn.dispatchEvent(new Event("input",{bubbles:!0})),mn.dispatchEvent(new Event("change",{bubbles:!0}))}ot()}else{var Tt="Não foi possível validar os dados do cartão.";Qn&&Qn.cause&&Qn.cause[0]&&Qn.cause[0].description&&(Tt=Qn.cause[0].description),alert(Tt),gt()}})}catch(yt){gn("createToken exception",yt.message),zt()}}w==="cartao"?Bt():setTimeout(ot,500)}}if(q){let t=function(n){if(n=n||0,n>30){a();return}var e=x.querySelector("#cep, .box-area-dados, #nome-completo_2");e||n>8?a():setTimeout(function(){t(n+1)},250)},a=function(){var n=en(),e=Z.get("mm_user_email")||"";Z.remove("mm_checkout_mode"),typeof window._mmDraftDebug>"u"&&(window._mmDraftDebug=!0),dt(n,e),l();var r=Yn();n&&n.shipping!==null&&n.shipping!==void 0&&Pn({value:n.shipping,name:n.shippingName||"",deadline:n.shippingDeadline||"",city:n.shippingCity||"",options:n.shippingOptions||[]});try{var s=Array.from(x.querySelectorAll("a, button")).find(function(y){var T=(y.textContent||"").toLowerCase();return T.indexOf("sem cadastro")!==-1&&y.offsetParent!==null});s&&!s.classList.contains("active")&&s.click()}catch{}(!n||!n.items||n.items.length===0)&&lt(function(y){y&&y.items&&y.items.length>0&&fn(y)});var f=Z.get(P);if(f&&f.length===8){var v=document.getElementById("mm-op-cep");v&&(v.value=yn(f),setTimeout(function(){u(f)},400))}setTimeout(function(){if(!("ontouchstart"in window))for(var y=["mm-op-email","mm-op-nome","mm-op-cpf","mm-op-tel","mm-op-cep"],T=0;T<y.length;T++){var N=document.getElementById(y[T]);if(N&&!N.value){N.focus();break}}},350)};t()}if(C){document.documentElement.classList.remove("mm-cart-loading");var pn=x.querySelector('input[placeholder*="numero do cart" i]');pn&&(pn.inputMode="numeric");var Y=x.querySelector('input[placeholder*="000" i]');Y&&(!Y.maxLength||Y.maxLength<=4)&&(Y.inputMode="numeric")}setTimeout(function(){document.documentElement.classList.contains("mm-cart-loading")&&(console.warn("[mm-cart] failsafe: removing mm-cart-loading after 2s timeout"),document.documentElement.classList.remove("mm-cart-loading"))},2e3)})()})();
