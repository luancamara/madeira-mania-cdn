...
🎉 Inicialização concluída com sucesso!`,"success"),R.retry.usarMutationObserver&&this.observarMudancasDOM()):this.tentativasDeCarregamento<R.retry.maxTentativas?setTimeout(()=>this.tentarCarregar(),R.retry.intervaloMs):this.log("❌ Número máximo de tentativas atingido.","error")}observarMudancasDOM(){this.log("ℹ️ MutationObserver desativado - dados carregados via dataProduct","info")}log(i,m="log",d=null){if(!R.debug)return;const f={info:"color: #2196f3; font-weight: bold",success:"color: #4caf50; font-weight: bold",warning:"color: #ff9800; font-weight: bold",error:"color: #f44336; font-weight: bold",log:"color: #666"};console.log(`%c${i}`,f[m]||f.log),d&&console.log(d)}carregarProdutos(){if(this.produtos=[],typeof dataProduct>"u"||!dataProduct.listaProdutosSugeridos)return this.log("❌ dataProduct ou listaProdutosSugeridos não encontrado","error"),!1;const i=dataProduct.listaProdutosSugeridos,m=dataProduct.produto,d=dataProduct.hostImagem||"";if(this.log(`📦 Encontrados ${i.length} produtos sugeridos + produto atual`,"info"),m&&m.complemento){const f=m.midia_path&&m.midia_arquivo_nome?`${d}/${m.midia_path}${m.midia_arquivo_nome}`:"",u={id:m.derivacao_id||m.produto_id,nomeCompleto:m.complemento.trim(),estoque:m.qtde_estoque,url:m.link?`/${m.link}`:"",imagem:f,imagemData:f,elemento:null,variacoes:{},nomeBase:"",nomeExibicao:"",isAtual:!0};this.produtoAtualId=u.id,this.extrairVariacoesDoNome(u),this.produtos.push(u),this.log(`   ✓ Produto atual: "${u.nomeCompleto}"`,"success")}return i.forEach((f,u)=>{const z=f.complemento||f.nome||"";if(!z)return;const v=f.derivacao_id||f.produto_id;if(v===this.produtoAtualId){this.log(`   ⏭️ Ignorando duplicata: "${z}"`,"info");return}const T=f.midia_path&&f.midia_arquivo_nome?`${d}/${f.midia_path}${f.midia_arquivo_nome}`:"",A={id:v||u,nomeCompleto:z.trim(),estoque:f.qtde_estoque,url:f.link?`/${f.link}`:"",imagem:T,imagemData:T,elemento:null,variacoes:{},nomeBase:"",nomeExibicao:"",isAtual:!1};this.extrairVariacoesDoNome(A),this.produtos.push(A),this.log(`   ✓ Sugerido: "${A.nomeCompleto}"`,"log")}),this.produtos.length===0?!1:(this.log("✅ Produtos carregados e processados:","success",this.produtos),!0)}normalizarSeparadores(i){const m=["–","—","−","‐","‑","⁃"];let d=i;return m.forEach(f=>{const u=new RegExp(`\\s${f}\\s`,"g");d=d.replace(u," - ")}),d}extrairVariacoesDoNome(i){const d=this.normalizarSeparadores(i.nomeCompleto).split(R.formatoNome.separador);this.log(`
📝 Processando: "${i.nomeCompleto}"`,"log"),R.formatoNome.primeiraParte==="nome_base"&&(i.nomeBase=d[0].trim(),d.shift()),d.forEach(f=>{const u=f.trim();if(u&&u.includes(R.formatoNome.separadorTipoValor)){const[z,...v]=u.split(R.formatoNome.separadorTipoValor),T=v.join(R.formatoNome.separadorTipoValor).trim(),A=this.normalizarTipo(z.trim());if(R.ignorarPalavras.includes(A))return;i.variacoes[A]=T,this.log(`   ✓ ${A}: ${T}`,"success")}}),i.nomeExibicao=R.formatoNome.exibirNomeCompleto?i.nomeCompleto:i.nomeBase||i.nomeCompleto,Object.keys(i.variacoes).length===0&&(i.variacoes.MODELO=i.nomeCompleto,i.nomeExibicao=i.nomeCompleto)}normalizarTipo(i){return i.toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").trim()}processarVariacoes(){this.log(`
🔄 Processando variações...`,"info");const i=new Set;this.produtos.forEach(m=>{Object.keys(m.variacoes).forEach(d=>i.add(d))}),i.forEach(m=>{const d=new Set,f={};this.produtos.forEach(z=>{const v=z.variacoes[m];v&&(d.add(v),f[v]||(f[v]=[]),f[v].push(z))});const u=Array.from(d).sort();this.variacoes[m]={label:R.labels[m]||m,valores:u,produtosPorValor:f,usarImagem:R.variacoesComImagem.includes(m)},this.log(`   📊 ${m}: ${u.length} valor(es) único(s) → [${u.join(", ")}]`,u.length>1?"success":"warning")}),this.log("✅ Variações processadas:","success",this.variacoes)}renderizarVariacoes(){this.log(`
🎨 Renderizando variações...`,"info");let i=x(R.selectors.areaVariacoes);if(i.length===0&&(this.criarAreaVariacoes(),i=x(R.selectors.areaVariacoes)),Object.keys(this.variacoes).length===0)return;const m=x("<div>",{class:"product-variations-pills-container"});let d=0;if(Object.keys(this.variacoes).forEach(f=>{const u=this.variacoes[f];if(u.valores.length<=1){this.log(`⏭️ Ignorando "${f}" - apenas ${u.valores.length} valor(es)`,"info");return}if(u.usarImagem){const z=this.criarGrupoSwatches(f,u);m.append(z),d++}}),Object.keys(this.variacoes).forEach(f=>{const u=this.variacoes[f];if(!(u.valores.length<=1)&&!u.usarImagem){const z=this.criarGrupoPills(f,u);m.append(z),d++}}),d===0){this.log("ℹ️ Nenhuma variação com múltiplas opções - ocultando área","info"),i.closest(".derivacoes-produto").hide(),x(R.selectors.subtituloProduto).hide();return}R.performance.useRequestAnimationFrame&&window.requestAnimationFrame?requestAnimationFrame(()=>{i.empty().append(m),this.log(`✅ ${d} variação(ões) renderizada(s)!`,"success"),this.atualizarNomeProduto()}):(i.empty().append(m),this.log(`✅ ${d} variação(ões) renderizada(s)!`,"success"),this.atualizarNomeProduto())}criarAreaVariacoes(){const i=x(R.selectors.areaProdutosSugeridos);i.length>0?i.before('<div class="derivacoes-produto"><div class="area-derivacoes"></div></div>'):x("body").prepend('<div class="derivacoes-produto"><div class="area-derivacoes"></div></div>')}criarGrupoSwatches(i,m){const d=this.obterValorAtualParaTipo(i),f=x("<div>",{class:"variation-pill-group variation-pill-group--swatches","data-variacao-tipo":i,role:"group","aria-labelledby":`pill-label-${i.toLowerCase()}`}),u=x("<div>",{class:"variation-pill-label",id:`pill-label-${i.toLowerCase()}`});u.append(x("<span>").text(m.label+":")),u.append(x("<span>",{class:"variation-pill-label-value","data-label-value":i}).text(d||""));const z=x("<div>",{class:"variation-swatches-container",role:"radiogroup","aria-labelledby":`pill-label-${i.toLowerCase()}`});return m.valores.forEach((v,T)=>{const A=m.produtosPorValor[v],q=A.some(rn=>rn.estoque===void 0||rn.estoque>0),c=v===d,U=`pill-${i.toLowerCase()}-${this.sanitizeId(v)}`,Q=this.encontrarMelhorProdutoParaSwatch(i,v,A);let D=null;Q&&(D=Q.imagemData||Q.imagem);const V=x("<input>",{type:"radio",class:"variation-pill-input",id:U,name:`variation-${i}`,value:v,"data-variacao-tipo":i,"data-produtos":JSON.stringify(A.map(rn=>({id:rn.id,url:rn.url}))),checked:c,disabled:!q,"aria-label":`${m.label}: ${v}${q?"":" (Indisponível)"}`}),O=x("<label>",{class:"variation-color-swatch-wrapper",for:U,"data-tooltip":v}),B=x("<div>",{class:"variation-color-swatch","data-valor":v,tabindex:c?0:-1});D?B.append(x("<img>",{src:D,alt:v,class:"variation-color-swatch-image",loading:"lazy"})):B.css({"background-color":"#E5E7EB",display:"flex","align-items":"center","justify-content":"center","font-size":"14px",color:"#6B7280"}).text(v.charAt(0).toUpperCase());const tn=x("<span>",{class:"variation-color-swatch-name",text:v,title:v});O.append(B).append(tn),z.append(V).append(O)}),f.append(u).append(z),f}criarGrupoPills(i,m){const d=this.obterValorAtualParaTipo(i),f=x("<div>",{class:"variation-pill-group variation-pill-group--pills","data-variacao-tipo":i,role:"group","aria-labelledby":`pill-label-${i.toLowerCase()}`}),u=x("<div>",{class:"variation-pill-label",id:`pill-label-${i.toLowerCase()}`});u.append(x("<span>").text(m.label+":")),u.append(x("<span>",{class:"variation-pill-label-value","data-label-value":i}).text(d||""));const z=x("<div>",{class:"variation-pills-container",role:"radiogroup","aria-labelledby":`pill-label-${i.toLowerCase()}`});return m.valores.forEach((v,T)=>{const A=m.produtosPorValor[v],q=A.some(O=>O.estoque===void 0||O.estoque>0),c=v===d,U=`pill-${i.toLowerCase()}-${this.sanitizeId(v)}`,Q=x("<input>",{type:"radio",class:"variation-pill-input",id:U,name:`variation-${i}`,value:v,"data-variacao-tipo":i,"data-produtos":JSON.stringify(A.map(O=>({id:O.id,url:O.url}))),checked:c,disabled:!q,"aria-label":`${m.label}: ${v}${q?"":" (Indisponível)"}`});let D=v;q||(D+=' <span class="variation-pill-badge">Indisponível</span>');const V=x("<label>",{class:"variation-pill",for:U,html:D,"data-valor":v,tabindex:c?0:-1});z.append(Q).append(V)}),f.append(u).append(z),f}sanitizeId(i){return i.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}encontrarMelhorProdutoParaSwatch(i,m,d){const f=this.produtos.find(T=>T.isAtual||T.id===this.produtoAtualId);if(!f||d.length===0)return d[0]||null;if(d.length===1)return d[0];const u=f.variacoes;let z=null,v=-1;return d.forEach(T=>{let A=0;Object.keys(u).forEach(q=>{q!==i&&T.variacoes[q]===u[q]&&A++}),(T.imagemData||T.imagem)&&(A+=.5),A>v&&(v=A,z=T)}),this.log(`   🎯 Melhor produto para ${i}="${m}": score=${v}`,"log"),z||d[0]}obterValorAtualParaTipo(i){const m=this.produtos.find(d=>d.isAtual||d.id===this.produtoAtualId);return m?m.variacoes[i]:null}atualizarNomeProduto(){const i=this.produtos.find(d=>d.isAtual||d.id===this.produtoAtualId);if(!i)return;["h1.nome-produto",".product-name h1",".info-produto h1",'h1[itemprop="name"]',".box-info-produto h1"].forEach(d=>{const f=x(d);f.length>0&&f.text(i.nomeExibicao)})}bindEventos(){this.log(`
🔗 Vinculando eventos...`,"info"),x(document).on("change",".variation-pill-input",i=>{this.aoMudarVariacao(i)}),x(document).on("keydown",".variation-pills-container, .variation-swatches-container",i=>{this.handleKeyboardNavigation(i)}),x(document).on("click",".variation-pill, .variation-color-swatch-wrapper",function(){const i=x(this).is("label")?x("#"+x(this).attr("for")):x(this).closest("label").prev(".variation-pill-input");i.length&&!i.prop("disabled")&&x(this).closest(".variation-pill-group").addClass("is-loading")}),this.log("✅ Eventos vinculados","success")}handleKeyboardNavigation(i){const d=x(i.currentTarget).find(".variation-pill-input:not(:disabled)"),f=x(document.activeElement);if(!f.hasClass("variation-pill-input"))return;const u=d.index(f);let z=u;switch(i.key){case"ArrowRight":case"ArrowDown":i.preventDefault(),z=(u+1)%d.length;break;case"ArrowLeft":case"ArrowUp":i.preventDefault(),z=u-1<0?d.length-1:u-1;break;case"Home":i.preventDefault(),z=0;break;case"End":i.preventDefault(),z=d.length-1;break;default:return}d.eq(z).focus().prop("checked",!0).trigger("change")}aoMudarVariacao(i){const m=x(i.target),d=m.data("variacao-tipo"),f=m.val();this.log(`
🔄 Variação selecionada: ${d} = ${f}`,"info"),x(`.variation-pill-label-value[data-label-value="${d}"]`).text(f);const u={};x(".variation-pill-input:checked").each(function(){const v=x(this).data("variacao-tipo"),T=x(this).val();T&&(u[v]=T)}),this.log("📋 Seleção atual:","info",u);const z=this.encontrarProdutoPorVariacoes(u);if(z)this.log("✅ Produto encontrado!","success",z),this.navegarParaProduto(z);else{this.log("⚠️ Produto exato não encontrado, buscando melhor correspondência...","warning");const v=this.encontrarMelhorCorrespondencia(u);v?(this.log("✅ Melhor correspondência encontrada!","success",v),this.navegarParaProduto(v)):(this.log("❌ Nenhum produto correspondente encontrado","error"),x(".variation-pill-group").removeClass("is-loading"))}}encontrarProdutoPorVariacoes(i){return this.produtos.find(m=>Object.keys(i).every(d=>m.variacoes[d]===i[d]))}encontrarMelhorCorrespondencia(i){let m=null,d=0;return this.produtos.forEach(f=>{let u=0;Object.keys(i).forEach(z=>{f.variacoes[z]===i[z]&&u++}),u>d&&(d=u,m=f)}),d>0?m:null}navegarParaProduto(i){this.log(`
🚀 Navegando para: ${i.url}`,"info"),i.url?window.location.href=i.url:(this.log("❌ URL não encontrada para navegação","error"),x(".variation-pill-group").removeClass("is-loading"))}}setTimeout(function(){x(".product-variations-pills-container").remove(),x(".derivacoes-produto").remove();const h=new M;h.init(),window.GerenciadorVariacoesPillsMagazord=h},100)})(typeof jQuery<"u"?jQuery:window.jQuery||window.$),(function(){"use strict";function x(){if(!(typeof jQuery>"u"&&typeof $>"u")){var o=typeof jQuery<"u"?jQuery:$;o(document).ajaxComplete(function(l,p,y){y.url&&y.url.indexOf("checkout/cart")!==-1&&setTimeout(function(){window.dispatchEvent(new CustomEvent("reactItemAddedToCart"))},100)})}}function P(){var o=document.querySelector("#cart-preview-area .border-t.border-solid");if(!(!o||o.querySelector(".installment-total"))){var l=0,p=document.querySelectorAll("#cart-preview-area .cart-item");if(p.forEach(function(I){var _=parseFloat(I.getAttribute("data-item-price"))||0,J=parseInt(I.getAttribute("data-item-quantity"))||1;l+=_*J}),!(l<=0)){var y=(l/12).toFixed(2).replace(".",","),F=document.createElement("div");F.className="installment-total",F.textContent="ou 12x de R$ "+y;var k=o.querySelector(".valor-pix");k&&k.parentNode&&k.parentNode.insertBefore(F,k.nextSibling)}}}var R='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><path d="M5 12h14"/></svg>',M='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg>',h='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v2"/></svg>';function i(){P();var o=document.querySelectorAll("#cart-preview-area .cart-item, .content-cart .cart-item");o.forEach(function(l){if(!l.querySelector(".qty-btn-minus")){var p=l.querySelector(".qtd-value");if(p){var y=l.querySelector(".cart-remove-item"),F=y?y.getAttribute("data-id"):null;if(F){var k=l.querySelector(".prod-remove");k&&!p.contains(y)&&(p.appendChild(y),k.style.display="none");var I=p.parentElement,_=null;if(I)for(var J=0;J<I.children.length;J++){var G=I.children[J];if(G!==p&&G.classList&&G.classList.contains("valor")){_=G;break}}_&&!p.contains(_)&&p.appendChild(_);var K=parseInt(l.getAttribute("data-item-quantity"));if(!K||isNaN(K)){var t=p.textContent.match(/(\d+)/);K=t?parseInt(t[1]):1}var a=document.createElement("button");a.className="qty-btn-minus",a.type="button",a.setAttribute("aria-label","Diminuir quantidade"),a.innerHTML=R,a.addEventListener("click",function(s){s.preventDefault(),s.stopPropagation();var g=parseInt(n.textContent)||1;if(g<=1){var b=l.querySelector(".cart-remove-item");b&&b.click();return}vt(l,F,-1,n,a,e)});var n=document.createElement("span");n.className="qty-display",n.textContent=K;var e=document.createElement("button");e.className="qty-btn-plus",e.type="button",e.setAttribute("aria-label","Aumentar quantidade"),e.innerHTML=M,e.addEventListener("click",function(s){s.preventDefault(),s.stopPropagation(),vt(l,F,1,n,a,e)});var r=document.createElement("div");r.className="mm-qty-wrap",r.appendChild(a),r.appendChild(n),r.appendChild(e),p.insertBefore(r,p.firstChild),y&&(y.innerHTML=h,y.setAttribute("aria-label","Remover produto"))}}}})}function m(){document.addEventListener("click",function(o){var l=o.target.closest(".cart-remove-item");if(!(!l||!l.closest("#cart-preview-area"))){o.preventDefault(),o.stopPropagation(),o.stopImmediatePropagation();var p=l.getAttribute("data-id");if(p){var y=l.closest(".cart-item"),F=y&&y.querySelector(".prod-nome")?.textContent?.trim()||"este produto",k=F.length>50?F.substring(0,50)+"…":F,I=document.getElementById("mm-confirm-overlay");I&&I.remove();var _=document.createElement("div");_.id="mm-confirm-overlay",_.className="mm-confirm-overlay",_.innerHTML='<div class="mm-confirm-card"><p class="mm-confirm-title">Remover produto?</p><p class="mm-confirm-desc">'+k.replace(/&/g,"&amp;").replace(/</g,"&lt;")+'</p><div class="mm-confirm-actions"><button class="mm-confirm-btn mm-confirm-btn-cancel">Manter</button><button class="mm-confirm-btn mm-confirm-btn-delete">Remover</button></div></div>',document.body.appendChild(_),_.querySelector(".mm-confirm-btn-cancel").addEventListener("click",function(){_.remove()}),_.querySelector(".mm-confirm-btn-delete").addEventListener("click",function(){_.remove(),window.__mmDeleteItem&&y?window.__mmDeleteItem(y,p):typeof Zord<"u"&&Zord.checkout&&Zord.checkout.deleteItem(parseInt(p))}),_.addEventListener("click",function(J){J.target===_&&_.remove()})}}},!0)}function d(){document.addEventListener("click",function(o){var l=o.target.closest(".finalizar-compra");l&&l.closest("#cart-preview-area")&&(o.preventDefault(),o.stopPropagation(),window.location.href="/checkout/identify")},!0)}function f(o,l){var p="cep=&cupom-desconto=&nenhumCreditoSelecionado=true&id="+encodeURIComponent(String(l))+"&area=main-cart";return fetch("/checkout/cart?operation="+encodeURIComponent(o),{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/x-www-form-urlencoded",Accept:"text/html,application/json,*/*","X-Requested-With":"XMLHttpRequest"},body:p}).then(function(y){if(!y.ok)throw new Error("HTTP "+y.status);return y.text()})}var u=1500,z=2e3,v="mm_cep",T="mm_cart_snapshot",A=1800*1e3;function q(){try{var o=localStorage.getItem(v)||"",l=o.replace(/\D/g,"");if(l.length===8)return l}catch{}return null}function c(o){return!o||o.length!==8?"":o.slice(0,5)+"-"+o.slice(5)}function U(o){if(!o||o.length!==8)return z;var l=parseInt(o.slice(0,2),10);return isNaN(l)?z:l>=1&&l<=39||l>=80&&l<=99?u:z}function Q(){try{var o=localStorage.getItem(T);if(!o)return null;var l=JSON.parse(o);return!l||!l.ts||Date.now()-l.ts>A?null:l}catch{return null}}function D(o){var l=[];return o.forEach(function(p){var y=(p.querySelector(".prod-nome a, .prod-nome")||{}).textContent||"",F=p.querySelector(".qty-display"),k=F?parseInt(F.textContent):parseInt(p.getAttribute("data-item-quantity"))||1;l.push(y.trim().slice(0,30)+"x"+k)}),l.sort().join("|")}function V(o){if(!o||!Array.isArray(o.items))return"";var l=o.items.map(function(p){return(p.name||"").trim().slice(0,30)+"x"+(p.quantity||1)});return l.sort().join("|")}var O='<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><polyline points="20 6 9 17 4 12"/></svg>',B='<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M18 6 6 18M6 6l12 12"/></svg>',tn=["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"];function rn(o){if(!o)return null;var l=String(o).match(/\d+/g);return!l||!l.length?null:Math.max.apply(null,l.map(Number))}function mn(o,l){for(var p=new Date(o.getTime()),y=0;y<l;){p.setDate(p.getDate()+1);var F=p.getDay();F!==0&&F!==6&&y++}return p}function pn(o){var l=new Date,p="dia "+o.getDate()+" de "+tn[o.getMonth()];return o.getFullYear()!==l.getFullYear()&&(p+=" de "+o.getFullYear()),p}function Mn(o){var l=rn(o);if(!l||l<1)return null;var p=mn(new Date,l);return"Receba até "+pn(p)}var dn={},jn=4e3,Fn={};function on(o,l){if(!o||o.length!==8)return Promise.resolve(null);if(dn[o])return dn[o];if(!l){var p=Fn[o]||0;if(Date.now()-p<jn)return Promise.resolve(null)}var y="cep="+encodeURIComponent(o.slice(0,5)+"-"+o.slice(5))+"&cupom-desconto=&nenhumCreditoSelecionado=true&area=main-cart",F=fetch("/checkout/cart?operation=atualizaValoresCarrinho",{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/x-www-form-urlencoded",Accept:"text/html,application/json,*/*","X-Requested-With":"XMLHttpRequest"},body:y}).then(function(k){if(!k.ok)throw new Error("HTTP "+k.status);return k.text()}).then(function(k){return Fn[o]=Date.now(),Bn(k)}).catch(function(){return null}).then(function(k){return delete dn[o],k});return dn[o]=F,F}function Bn(o){try{var l=new DOMParser().parseFromString(o,"text/html"),p=l.querySelector("#resumo-compra .frete-calculado")||l.querySelector(".frete-calculado");if(!p)return null;var y="",F=p.querySelector(".frete-location .city");F&&(y=F.textContent.trim());var k=null,I="",_="",J=p.querySelector(".info-frete-selec");if(J){var G=J.querySelector(".dias-entrega"),K=J.querySelector(".info-title span, .info-title");G&&(I=(G.textContent||"").trim()),K&&(_=(K.textContent||"").trim())}var t=p.querySelector(".line.valor-frete .value, .value.valor-frete")||p.querySelector(".valor-compra-frete .value");if(t){var a=(t.textContent||"").trim();if(/gr[áa]tis/i.test(a))k=0;else{var n=a.match(/[\d.,]+/);if(n){var e=parseFloat(n[0].replace(/\./g,"").replace(",","."));isNaN(e)||(k=e)}}}if(k==null){var r=p.querySelector(".servico-frete");if(r){var s=parseFloat(r.getAttribute("data-valor-frete")||"0");if(isNaN(s)||(k=s),_||(_=r.getAttribute("data-servico-frete")||""),!I){var g=r.querySelector(".dias-entrega");g&&(I=(g.textContent||"").trim())}}}if(k==null)return null;var b=null,w=l.querySelector(".total-boleto .value, .totais-valor.total-boleto .value");if(w){var E=(w.textContent||"").trim(),j=E.match(/[\d.,]+/);if(j){var L=parseFloat(j[0].replace(/\./g,"").replace(",","."));isNaN(L)||(b=L)}}return{city:y,shipping:k,shippingDeadline:I,shippingName:_,totalPix:b}}catch{return null}}function Sn(o,l,p){if(!(!o||!p)){gn(o);try{let J=function(G){for(var K=0;K<F.length;K++)if(F[K]&&F[K].name===G)return F[K];return null};var y=Q()||{};y.ts=Date.now(),y.cepValue=l.slice(0,5)+"-"+l.slice(5),y.shipping=p.shipping,y.shippingDeadline=p.shippingDeadline,y.shippingName=p.shippingName,y.shippingCity=p.city,p.totalPix!=null&&(y.subtotalPix=p.totalPix);var F=y.items&&y.items.length?y.items:[],k=o.querySelectorAll(".cart-item:not(.mm-removing)");y.items=Array.prototype.map.call(k,function(G){var K=G.querySelector(".prod-nome a, .prod-nome"),t=(K&&K.textContent||"").trim(),a=G.querySelector(".qty-display"),n=a?parseInt(a.textContent):parseInt(G.getAttribute("data-item-quantity"))||1,e=J(t);return e&&e.quantity===n&&(e.lineTotalPix>0||e.lineTotal>0||e.imgSrc)?e:{name:t,quantity:n}}),localStorage.setItem(T,JSON.stringify(y))}catch{}var I=o.querySelectorAll(".cart-item:not(.mm-removing)"),_=0;I.forEach(function(J){var G=J.querySelector(".valor");if(G){var K=wn(G.textContent);isNaN(K)||(_+=K)}}),o.querySelector(".box-total-btn")?On(o):_n(o)}}function En(o){var l=q();if(l){var p=Q(),y=V(p),F=D(o.querySelectorAll(".cart-item:not(.mm-removing)")),k=p&&p.cepValue&&p.cepValue.replace(/\D/g,"")===l,I=p&&p.shipping!=null&&!isNaN(p.shipping);p&&y===F&&k&&I||on(l).then(function(_){_&&Sn(o,l,_)})}}function un(o){return String(o||"").replace(/[&<>"']/g,function(l){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[l]})}function Cn(o){if(!o)return null;var l=o.querySelector(".box-total-btn");if(l)return{host:l,before:l.querySelector(".total")};var p=o.querySelector(".area-finalizar-compra");if(p)return{host:p,before:p.firstElementChild};var y=o.querySelector(".finalizar-compra");if(y&&y.parentElement){var F=y.parentElement;return{host:F,before:F.firstElementChild}}return null}function Xn(o){if(!o)return null;var l=o.closest(".carrinho-rapido-ctn");return l||(o.closest("#cart-preview-area")?gt():null)}function yn(o,l,p,y){if(o){var F=Cn(o);if(F){var k=F.host;o.classList.add("mm-ship-scope");var I=q(),_=Q(),J=D(o.querySelectorAll(".cart-item:not(.mm-removing)")),G=V(_),K=_&&G===J,t=U(I),a=l>=t,n=Math.max(0,t-l),e=Math.max(0,Math.min(100,l/t*100)),r=k.querySelector(".mm-cart-ship");if(!r){r=document.createElement("div"),r.className="mm-cart-ship",r.setAttribute("role","group"),r.setAttribute("aria-label","Informações de frete");var s=F.before;s&&s.parentNode===k?k.insertBefore(r,s):k.insertBefore(r,k.firstChild)}if(r.classList.toggle("is-free",a),In(r),r.dataset.mmEditing!=="1"){var g=_&&_.cepValue&&_.cepValue.replace(/\D/g,"")===I,b=I&&K&&g&&_.shippingCity,w=b?Mn(_.shippingDeadline):null,E="";if(E+='<div class="mm-cart-ship-location">',I){var j=c(I);if(b&&(j+=" · "+un(_.shippingCity)),E+='<span class="mm-cart-ship-label">Envio para</span><div class="mm-cart-ship-location-value"><span>'+j+'</span><button class="mm-cart-ship-edit" type="button" data-mm-ship="edit">Trocar</button></div>',w){var L=un(w);y&&p>0?L+=" · <strong>"+un(fn(p))+"</strong>":y&&p===0&&(L+=" · <strong>Grátis</strong>"),E+='<span class="mm-cart-ship-deadline">'+L+"</span>"}}else E+='<span class="mm-cart-ship-label">Calcule o frete</span><div class="mm-cart-ship-location-value"><span style="color:#9CA3AF;font-weight:500;">Informe seu CEP</span><button class="mm-cart-ship-edit" type="button" data-mm-ship="edit">Calcular</button></div>';E+="</div>";var Z=a?"Frete grátis desbloqueado":"Faltam "+fn(n)+" para frete grátis",W=parseFloat(o.dataset.mmShipPct||"0")||0;E+='<div class="mm-cart-ship-progress">',E+='<div class="mm-cart-ship-bar" role="progressbar" aria-valuenow="'+Math.round(l)+'" aria-valuemin="0" aria-valuemax="'+Math.round(t)+'" aria-valuetext="'+un(Z)+'" aria-label="Progresso para frete grátis"><div class="mm-cart-ship-bar-fill" style="width:'+W.toFixed(1)+'%"></div></div>',E+='<p class="mm-cart-ship-nudge'+(a?" is-free":"")+'">',a?E+=O+"Frete grátis garantido":E+="Faltam <strong>"+un(fn(n))+"</strong> para frete grátis",E+="</p>",E+="</div>",r.innerHTML=E;var N=r.querySelector(".mm-cart-ship-bar-fill");N&&requestAnimationFrame(function(){N.style.width=e.toFixed(1)+"%"});var H=o.dataset.mmShipWasFree==="1";a&&!H&&W>0&&(r.classList.remove("mm-just-unlocked"),r.offsetWidth,r.classList.add("mm-just-unlocked"),setTimeout(function(){r.classList.remove("mm-just-unlocked")},1400)),o.dataset.mmShipWasFree=a?"1":"",o.dataset.mmShipPct=e.toFixed(1)}}}}var Un='<svg class="mm-cart-ship-spinner" width="14" height="14" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><style>.mm-sp{animation:mm-spin 1.2s linear infinite;transform-origin:center}.mm-sp circle{stroke:#4B664A;stroke-width:3;fill:none;stroke-linecap:round;stroke-dasharray:44;stroke-dashoffset:16;animation:mm-sp-dash 1.2s ease-in-out infinite}@keyframes mm-spin{to{transform:rotate(360deg)}}@keyframes mm-sp-dash{0%{stroke-dashoffset:44}50%{stroke-dashoffset:8}100%{stroke-dashoffset:44}}</style><g class="mm-sp"><circle cx="12" cy="12" r="9.5"/></g></svg>';function sn(o){if(o){var l=o.querySelector(".mm-cart-ship-deadline");if(l)l.innerHTML="Recalculando frete "+Un;else{var p=o.querySelector(".mm-cart-ship-location");if(p){var y=document.createElement("span");y.className="mm-cart-ship-deadline",y.innerHTML="Recalculando frete "+Un,p.appendChild(y)}}var F=Cn(o);F&&F.host.classList.add("mm-ship-loading")}}function gn(o){if(o){var l=Cn(o);l&&l.host.classList.remove("mm-ship-loading")}}function In(o){!o||o.dataset.mmShipBound||(o.dataset.mmShipBound="1",o.addEventListener("click",function(l){var p=l.target.closest('[data-mm-ship="edit"]');if(p){l.preventDefault(),l.stopPropagation(),Ln(o);return}var y=l.target.closest('[data-mm-ship="cancel"]');if(y){l.preventDefault(),l.stopPropagation(),$n(o);return}l.target.closest(".mm-cart-ship-cep-form")&&l.stopPropagation()},!0))}function Ln(o){var l=o.querySelector(".mm-cart-ship-location");if(l){o.dataset.mmEditing="1";var p=q()||"";l.innerHTML='<span class="mm-cart-ship-label">Digite seu CEP</span><form class="mm-cart-ship-cep-form" data-mm-ship-form="1" novalidate><input class="mm-cart-ship-cep-input" type="text" inputmode="numeric" autocomplete="postal-code" placeholder="00000-000" maxlength="9" value="'+un(c(p))+'" aria-label="CEP" /><button class="mm-cart-ship-cep-save" type="submit">OK</button><button class="mm-cart-ship-cep-cancel" type="button" data-mm-ship="cancel" aria-label="Cancelar">'+B+"</button></form>";var y=l.querySelector(".mm-cart-ship-cep-input"),F=l.querySelector("form");y&&(setTimeout(function(){try{y.focus(),y.select()}catch{}},10),y.addEventListener("input",function(){y.classList.remove("is-invalid");var k=y.value.replace(/\D/g,"").slice(0,8);y.value=k.length>5?k.slice(0,5)+"-"+k.slice(5):k}),y.addEventListener("keydown",function(k){k.key==="Escape"&&(k.preventDefault(),$n(o))})),F&&F.addEventListener("submit",function(k){k.preventDefault(),k.stopPropagation(),pt(o)})}}function $n(o){o.dataset.mmEditing="";var l=Xn(o);if(l){var p=l.querySelectorAll(".cart-item:not(.mm-removing)"),y=0;p.forEach(function(F){var k=F.querySelector(".valor");if(k){var I=wn(k.textContent);isNaN(I)||(y+=I)}}),yn(l,y)}}function pt(o){var l=o.querySelector(".mm-cart-ship-cep-input");if(l){var p=l.value.replace(/\D/g,"");if(p.length!==8){l.classList.add("is-invalid"),l.focus();return}try{localStorage.setItem(v,p)}catch{}var y=o.querySelector(".mm-cart-ship-cep-save");y&&(y.disabled=!0,y.textContent="...");var F=Xn(o);try{var k=Q();k&&(k.cepValue="",localStorage.setItem(T,JSON.stringify(k)))}catch{}$n(o),on(p).then(function(I){I&&F&&Sn(F,p,I)})}}function wn(o){if(!o)return NaN;var l=String(o).replace(/\s/g,"").match(/([\d.,]+)/);return l?parseFloat(l[1].replace(/\./g,"").replace(",",".")):NaN}function fn(o){return isNaN(o)?"":"R$ "+o.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}function ln(o){if(isNaN(o))return"";var l=o.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),p=l.split(",");return"R$&nbsp;"+p[0]+'<span class="mm-cents">,'+(p[1]||"00")+"</span>"}function Qn(o){var l=0;return Array.prototype.forEach.call(o,function(p){var y=parseFloat(p.getAttribute("data-item-price"))||0,F=parseInt(p.getAttribute("data-item-quantity"));if(!F||isNaN(F)){var k=p.querySelector(".qty-display");k?F=parseInt(k.textContent)||1:F=1}l+=y*F}),l}function rt(o){var l=o.querySelectorAll(".cart-item:not(.mm-removing)");l.forEach(function(p){var y=parseFloat(p.getAttribute("data-item-price"))||0,F=parseInt(p.getAttribute("data-item-quantity"));if(!F||isNaN(F)){var k=p.querySelector(".qty-display");k?F=parseInt(k.textContent)||1:F=1}var I=p.querySelector(".valor");I&&y>0&&(I.innerHTML=ln(y*F))})}function Pn(o){if(!(!o||o.dataset.mmTotalRatio)){var l=o.querySelectorAll(".cart-item");if(l.length){var p=Qn(l),y=o.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||o.querySelector(".box-total-btn .linha-total .valor-final strong");if(y&&p>.01){var F=wn(y.textContent);isNaN(F)||(o.dataset.mmTotalRatio=String(F/p))}}}}function Yn(o,l){var p=o.querySelector(".box-total-btn .linha-total");if(p){var y=p.parentElement.querySelector(".mm-cart-savings");if(y&&y.remove(),!(!l||l<.01)){var F=document.createElement("span");F.className="mm-cart-savings",F.textContent="Você economiza "+fn(l)+" com PIX",p.nextSibling?p.parentElement.insertBefore(F,p.nextSibling):p.parentElement.appendChild(F)}}}function On(o,l){if(o){Pn(o);var p=o.querySelectorAll(".cart-item:not(.mm-removing)"),y=Qn(p),F=o.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||o.querySelector(".box-total-btn .linha-total .valor-final strong"),k=parseFloat(o.dataset.mmTotalRatio||"0.95")||.95,I=y*k,_=y-I,J=q(),G=Q(),K=V(G),t=D(p),a=G&&G.cepValue&&G.cepValue.replace(/\D/g,"")===J,n=!!(J&&G&&a&&G.shipping!=null&&!isNaN(G.shipping)),e=o.dataset.mmShipPendingFetch==="1";!n&&e&&J&&G&&G.shipping!=null&&(n=!0);var r=n?parseFloat(G.shipping):0,s=I+r,g=y+r;if(F){var b=wn(F.textContent);if(l)(isNaN(b)||Math.abs(s-b)>.005)&&(F.innerHTML=ln(s));else if(!isNaN(b)&&Math.abs(s-b)>.005){var w=o.querySelector(".box-total-btn .linha-total .valor-final");w&&(w.classList.remove("mm-pop"),w.offsetWidth,w.classList.add("mm-pop"),setTimeout(function(){w.classList.remove("mm-pop")},450)),ft(F,b,s)}else F.innerHTML=ln(s)}var E=o.querySelector(".box-total-btn .valor-final.card");if(E){var j=g/12;E.innerHTML="<span>ou em até <strong>12x</strong> de <strong>"+fn(j)+"</strong> no cartão</span>"}var L=o.querySelector(".box-total-btn .linha-total .valor-final .tipo-pagto");L&&(L.textContent="No PIX"),Yn(o,_),yn(o,y,r,n);try{var Z=0;p.forEach(function(N){var H=N.querySelector(".qty-display");H&&(Z+=parseInt(H.textContent)||0)});var W=document.getElementById("mm-h-cart-count");W&&(Z>0?(W.textContent=Z>99?"99+":String(Z),W.hidden=!1):W.hidden=!0)}catch{}}}function Jn(){var o=document.querySelector(".carrinho-rapido-ctn");if(!(!o||!o.querySelector(".box-total-btn"))){var l=o.querySelectorAll(".cart-item:not(.mm-removing)");if(l.length){var p=o.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||o.querySelector(".box-total-btn .linha-total .valor-final strong");if(p){var y=Qn(l);if(y>.01){var F=parseFloat(o.dataset.mmTotalRatio||"0.95")||.95,k=q(),I=Q(),_=I&&I.cepValue&&I.cepValue.replace(/\D/g,"")===k,J=!!(k&&I&&_&&I.shipping!=null&&!isNaN(I.shipping));!J&&o.dataset.mmShipPendingFetch==="1"&&k&&I&&I.shipping!=null&&(J=!0);var G=J?parseFloat(I.shipping):0,K=y*F+G,t=wn(p.textContent);(isNaN(t)||Math.abs(K-t)>.01)&&On(o,!0)}}}}}function gt(){var o=document.querySelector('#cart-preview-area > div[class*="z-[9999]"]');if(o)return o;var l=document.querySelector(".carrinho-rapido-ctn");return l&&!l.querySelector(".box-total-btn")&&l.querySelector(".valor-pix")?l:null}function zt(o,l){var p=o.querySelector(".installment-total");if(!(!p||!p.parentElement)){var y=p.parentElement,F=y.querySelector(".mm-cart-savings-mobile");if(!l||l<.01){F&&F.remove();return}var k="Você economiza "+fn(l)+" com PIX";if(F){F.textContent!==k&&(F.textContent=k);return}var I=document.createElement("span");I.className="mm-cart-savings-mobile",I.textContent=k,p.nextSibling?y.insertBefore(I,p.nextSibling):y.appendChild(I)}}function Dn(o,l,p){if(!(!o||!o.classList||!o.classList.contains("carrinho-rapido-ctn"))&&!o.querySelector(".box-total-btn")){var y=o.querySelector(".area-finalizar-compra");if(!(!y||!(l>0))){var F=y.querySelector(".forma-pix"),k=F?F.parentElement:null;if(k){var I=l*p,_=l/12,J=l-I;k.classList.add("mm-native-pay-hidden");var G=y.querySelector(".mm-cart-total-b");G||(G=document.createElement("div"),G.className="mm-cart-total-b",G.innerHTML='<span class="mm-tb-label">Total</span><span class="mm-tb-row"><strong class="mm-tb-value"></strong><span class="mm-tb-pix">No PIX</span></span><span class="mm-tb-savings"></span><span class="mm-tb-parcela"></span>',k.nextSibling?y.insertBefore(G,k.nextSibling):y.appendChild(G));var K=G.querySelector(".mm-tb-value"),t=fn(I);K&&K.textContent!==t&&(K.textContent=t);var a=G.querySelector(".mm-tb-savings");if(a)if(J>=.01){var n="Você economiza "+fn(J)+" com PIX";a.textContent!==n&&(a.textContent=n),a.style.display=""}else a.style.display="none";var e=G.querySelector(".mm-tb-parcela");if(e){var r="ou em até 12x de "+fn(_)+" no cartão";e.textContent!==r&&(e.textContent=r)}}}}}function _n(o){if(o=o||gt(),!(!o||o.querySelector(".box-total-btn")||!o.querySelector(".valor-pix"))){var l=o.querySelectorAll(".cart-item:not(.mm-removing)"),p=l.length,y=document.getElementById("mm-h-cart-count");if(y&&p>0){var F=p>99?"99+":String(p);(y.textContent!==F||y.hidden)&&(y.textContent=F,y.hidden=!1)}if(l.length){var k=Qn(l);if(k>0){Array.prototype.forEach.call(l,function(W){var N=parseFloat(W.getAttribute("data-item-price"))||0;if(N>0){var H=parseInt(W.getAttribute("data-item-quantity"));if(!H||isNaN(H)){var an=W.querySelector(".qty-display");H=an&&parseInt(an.textContent)||1}var en=W.querySelector(".valor");if(en){var xn=N*H,hn=en.querySelector("span")||en,zn=wn(hn.textContent);(isNaN(zn)||Math.abs(zn-xn)>.005)&&(hn.textContent=fn(xn))}}});var I=o.querySelector(".valor-pix"),_=I?I.querySelector("span")||I:null;if(!o.dataset.mmMobileRatio&&_){var J=wn(_.textContent);if(!isNaN(J)&&J>0){var G=J/k;G>.8&&G<=1.0001&&(o.dataset.mmMobileRatio=String(G))}}var K=parseFloat(o.dataset.mmMobileRatio||"0.95");if(K>.8&&K<=1.0001||(K=.95),_){var t=k*K,a=wn(_.textContent);(isNaN(a)||Math.abs(a-t)>.005)&&(_.textContent=fn(t))}var n=o.querySelector(".installment-total");if(n){var e=k/12,r=wn(n.textContent);(isNaN(r)||Math.abs(r-e)>.005)&&(n.textContent="ou 12x de "+fn(e))}var s=q(),g=Q(),b=g&&g.cepValue&&g.cepValue.replace(/\D/g,"")===s,w=!!(s&&g&&b&&g.shipping!=null&&!isNaN(g.shipping)),E=w?parseFloat(g.shipping):0,j=(s||"")+"|"+k.toFixed(2)+"|"+(w?1:0)+"|"+E,L=o.querySelector(".mm-cart-ship"),Z=L&&L.dataset.mmEditing==="1";!Z&&(!L||o.dataset.mmMobShipSig!==j)&&(o.dataset.mmMobShipSig=j,yn(o,k,E,w)),En(o),zt(o,k-k*K),Dn(o,k,K)}}}}function ft(o,l,p){if(!o||isNaN(l)||isNaN(p))return;var y=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;if(y){o.innerHTML=ln(p);return}var F=420,k=performance.now();function I(_){return 1-Math.pow(1-_,3)}(function _(J){var G=Math.min(1,(J-k)/F);o.innerHTML=ln(l+(p-l)*I(G)),G<1&&requestAnimationFrame(_)})(k)}function vt(o,l,p,y,F,k){if(!(!o||!l)){var I=parseInt(y.textContent)||1,_=I+p;if(!(_<1)){var t=o.closest(".carrinho-rapido-ctn");Pn(t),F.disabled=!0,k.disabled=!0;var J=parseFloat(o.getAttribute("data-item-price"))||0;y.textContent=_,o.setAttribute("data-item-quantity",_);var G=o.querySelector(".valor");if(G){var K=J*_;G.textContent=fn(K),G.classList.remove("mm-pop"),G.offsetWidth,G.classList.add("mm-pop"),setTimeout(function(){G.classList.remove("mm-pop")},450)}var t=o.closest(".carrinho-rapido-ctn"),a=p>0?"adicionaItem":"removeItem";q()&&t&&(t.dataset.mmShipPendingFetch="1"),On(t),_n(),q()&&sn(t),f(a,l).catch(function(){y.textContent=I,o.setAttribute("data-item-quantity",I),G&&(G.innerHTML=ln(J*I)),t&&(t.dataset.mmShipPendingFetch=""),On(t),_n()}).then(function(){F.disabled=!1,k.disabled=!1;var n=q();n&&t?(sn(t),on(n,!0).then(function(e){t.dataset.mmShipPendingFetch="",e?Sn(t,n,e):gn(t)})):t&&(t.dataset.mmShipPendingFetch="")})}}}function xt(o,l){if(!(!o||!l)){var p=o.closest(".carrinho-rapido-ctn");Pn(p);var y=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches,F=y?0:360;y||o.classList.add("mm-removing"),setTimeout(function(){o.parentNode&&o.parentNode.removeChild(o);var k=p?p.querySelectorAll(".cart-item:not(.mm-removing)"):[],I=k.length===0;if(I){if(p){p.dataset.mmShipPendingFetch="";var _=p.querySelector(".mm-cart-ship");_&&_.remove();var J=p.querySelector(".box-total-btn, .area-finalizar-compra");J&&(J.style.display="none")}typeof window.__mmForceEmptyCartState=="function"&&window.__mmForceEmptyCartState(p)}else On(p),q()&&sn(p);var G=document.getElementById("mm-h-cart-count"),K=document.getElementById("mm-h-cart"),t=k.length;G&&(G.textContent=t>99?"99+":String(t),G.hidden=t===0),K&&K.setAttribute("aria-label","Carrinho, "+t+" "+(t===1?"item":"itens")),_n()},F),f("deleteItem",l).catch(function(){}).then(function(){var k=p?p.querySelectorAll(".cart-item:not(.mm-removing)"):[],I=document.getElementById("mm-h-cart-count");if(I&&(I.textContent=k.length>99?"99+":String(k.length),I.hidden=k.length===0),k.length===0){p&&(p.dataset.mmShipPendingFetch="");return}var _=q();_&&p?(p.dataset.mmShipPendingFetch="1",sn(p),on(_,!0).then(function(J){p.dataset.mmShipPendingFetch="",J?Sn(p,_,J):gn(p)})):p&&(p.dataset.mmShipPendingFetch="")})}}window.__mmDeleteItem=xt;var qt=null,bt=new Set,mt=null;function Rn(o){if(!o)return NaN;var l=String(o).replace(/\s/g,"").match(/([\d.,]+)/);if(!l)return NaN;var p=l[1].replace(/\./g,"").replace(",","."),y=parseFloat(p);return isNaN(y)?NaN:y}function ot(o){return isNaN(o)?"":"R$ "+o.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}function At(o,l,p){if(!o||isNaN(l)||isNaN(p))return;mt&&cancelAnimationFrame(mt);var y=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;if(y){o.textContent=ot(p);return}var F=420,k=performance.now();function I(J){return 1-Math.pow(1-J,3)}function _(J){var G=J-k,K=Math.min(1,G/F),t=l+(p-l)*I(K);o.textContent=ot(t),K<1?mt=requestAnimationFrame(_):mt=null}mt=requestAnimationFrame(_)}function dt(o){return o?o.querySelector(".box-total-btn .linha-total .valor-final > .valor > strong")||o.querySelector(".box-total-btn .linha-total .valor-final strong")||o.querySelector(".valor-pix strong")||o.querySelector(".valor-pix"):null}function yt(o){return o?o.querySelector(".box-total-btn .linha-total .valor-final"):null}function Vn(o){var l=yt(o);l&&(l.classList.remove("mm-pop"),l.offsetWidth,l.classList.add("mm-pop"),setTimeout(function(){l.classList.remove("mm-pop")},450))}function Kn(){var o=document.querySelector(".carrinho-rapido-ctn");if(o){var l=o.querySelectorAll(".cart-item:not(.mm-removing)");if(l.length>0){var p=o.querySelector(".box-total-btn");p&&p.dataset.mmTotalEnhanced!=="1"&&(Pn(o),rt(o),On(o),p.dataset.mmTotalEnhanced="1",o.dataset.mmShipRendered="1",En(o))}o.querySelector(".box-total-btn")||_n(o);var y=o.querySelectorAll(".cart-item"),F=new Set;y.forEach(function(k){var I=k.id||k.getAttribute("data-item-id")||"";I&&(F.add(I),!bt.has(I)&&bt.size>0&&(k.classList.add("mm-added"),setTimeout(function(){k.classList.remove("mm-added")},500)))}),bt=F}}function it(){var o=document.querySelector(".carrinho-rapido-ctn");if(!(!o||o.dataset.mmAnimObserved)){o.dataset.mmAnimObserved="1",Kn();var l=new MutationObserver(function(){clearTimeout(it._t),it._t=setTimeout(Kn,60)});l.observe(o,{childList:!0,subtree:!0,characterData:!0})}}function ct(){x(),m(),d();var o=document.getElementById("cart-preview-area");if(o){var l=new MutationObserver(function(){setTimeout(i,100),setTimeout(it,150),setTimeout(_n,180),setTimeout(Jn,220)});l.observe(o,{childList:!0,subtree:!0})}setInterval(i,800),setInterval(it,800),setInterval(_n,800),setInterval(Jn,800),i(),it(),_n(),Jn()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ct):ct()})(),(function x(){"use strict";var P="mm_cep",R="mm_cart_snapshot",M=18e5,h="mm_onepage_draft",i=1440*60*1e3,m=2e3,d="https://magazord-public.s3.sa-east-1.amazonaws.com/madeiramania/resources/Design%20sem%20nome%20(1).svg",f="https://api.whatsapp.com/send?phone=5511915299488&text="+encodeURIComponent("Olá! Estou no checkout e gostaria de tirar uma dúvida sobre meu pedido."),u=location.pathname,z=u.indexOf("/checkout/cart")!==-1,v=u.indexOf("/checkout/identify")!==-1,T=u.indexOf("/checkout/onepage")!==-1,A=u.indexOf("/checkout/payment")!==-1,q=u.indexOf("/checkout/done")!==-1;if(q){try{localStorage.removeItem("mm_onepage_draft")}catch{}return}if(!z&&!v&&!T&&!A)return;x._retries=(x._retries||0)+1;var c=document.querySelector("#checkout-main-area");if(!c){try{var U=document.body&&document.body.textContent||"",Q=/muito tempo inativo|realize login novamente/i.test(U);if((T||v)&&(Q||x._retries>=40)&&!sessionStorage.getItem("mm_checkout_recovery")){sessionStorage.setItem("mm_checkout_recovery","1"),location.href="/checkout/cart";return}}catch{}x._retries<40&&setTimeout(x,400);return}try{sessionStorage.removeItem("mm_checkout_recovery")}catch{}function D(t){return isNaN(t)||t<0?"R$ 0,00":"R$ "+t.toFixed(2).replace(".",",").replace(/(\d)(?=(\d{3})+,)/g,"$1.")}function V(t){if(!t)return 0;var a=String(t).match(/(-?[\d.]+,\d{2})/);return a&&parseFloat(a[1].replace(/\./g,"").replace(",","."))||0}function O(t){return String(t||"").replace(/[&<>"']/g,function(a){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[a]})}var B={truck:'<svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2a3 3 0 0 0 6 0h6a3 3 0 0 0 6 0h2v-5l-3-4zM6 18.5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm12 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm.5-7H17V9.5h2.04l1.46 2-.04 0z"/></svg>',check:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>',checkCircle:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>',bolt:'<svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>',shield:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 1 3 5v6c0 5.5 3.8 10.7 9 12 5.2-1.3 9-6.5 9-12V5l-9-4zm-1.4 16L6 12.4l1.4-1.4 3.2 3.2 6.8-6.8L18.8 8.8 10.6 17z"/></svg>',lock:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM9 6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9V6z"/></svg>',card:'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>',rotate:'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.74 9.74 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>',minus:'<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M3 7h8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',plus:'<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 3v8M3 7h8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',trash:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',close:'<svg width="12" height="12" viewBox="0 0 14 14" fill="none"><path d="M10.5 3.5l-7 7M3.5 3.5l7 7" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',arrow:'<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>',tag:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.59 13.41 13.42 20.58a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>',whats:'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0 0 20.464 3.488"/></svg>',box:'<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>'},tn={get:function(t){try{return localStorage.getItem(t)}catch{return null}},set:function(t,a){try{localStorage.setItem(t,a)}catch{}},remove:function(t){try{localStorage.removeItem(t)}catch{}}};function rn(t){try{var a={ts:Date.now(),items:t.items.map(function(n){return{name:n.name,variant:n.variant,imgSrc:n.imgSrc,quantity:n.quantity,lineTotal:n.lineTotal,lineTotalPix:n.lineTotalPix,isPix:n.isPix,deposito:n.deposito}}),subtotalPix:t.subtotalPix,subtotalFull:t.subtotalFull,discount:t.discount,couponCode:t.couponCode,shipping:t.shipping,shippingDeadline:t.shippingDeadline,shippingName:t.shippingName,shippingCity:t.shippingCity,shippingOptions:t.shippingOptions,cepValue:t.cepValue};tn.set(R,JSON.stringify(a))}catch{}}function mn(){try{var t=tn.get(R);if(!t)return null;var a=JSON.parse(t);return!a||!a.ts||Date.now()-a.ts>M?null:a}catch{return null}}function pn(){try{for(var t=["mm-op-email","mm-op-nome","mm-op-cpf","mm-op-tel","mm-op-cep","mm-op-rua","mm-op-num","mm-op-comp","mm-op-bairro","mm-op-cidade","mm-op-uf"],a={ts:Date.now()},n=0,e=0;e<t.length;e++){var r=document.getElementById(t[e]);r&&r.value&&(a[t[e]]=r.value,n++)}if(n===0){window._mmDraftDebug&&console.log("[mm-draft] skip save (0 fields)");return}tn.set(h,JSON.stringify(a)),window._mmDraftDebug&&console.log("[mm-draft] saved",n,"fields",a)}catch(s){window._mmDraftDebug&&console.warn("[mm-draft] save failed",s)}}function Mn(){try{var t=tn.get(h);if(!t)return null;var a=JSON.parse(t);return!a||!a.ts?null:Date.now()-a.ts>i?(tn.remove(h),null):a}catch{return null}}function dn(){try{tn.remove(h)}catch{}}function jn(){var t=Mn();if(!t)return window._mmDraftDebug&&console.log("[mm-draft] no draft to restore"),null;for(var a=["mm-op-email","mm-op-nome","mm-op-cpf","mm-op-tel","mm-op-cep","mm-op-rua","mm-op-num","mm-op-comp","mm-op-bairro","mm-op-cidade","mm-op-uf"],n=0,e=0;e<a.length;e++){var r=document.getElementById(a[e]);if(r&&t[a[e]]){try{var s=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;s.call(r,t[a[e]])}catch{r.value=t[a[e]]}r.dispatchEvent(new Event("input",{bubbles:!0})),/^mm-op-(rua|bairro|cidade|uf)$/.test(a[e])&&(r.dataset.mmCepFill="1"),n++}}return window._mmDraftDebug&&console.log("[mm-draft] restored",n,"fields from draft",t),t}function Fn(){for(var t={items:[],subtotalPix:0,subtotalFull:0,discount:0,shipping:null,shippingRaw:"",shippingDeadline:"",shippingName:"",shippingCity:"",shippingOptions:[],couponCode:"",cepValue:"",canFinalize:!1,hasFinalizar:!1},a=c.querySelectorAll(".cart-item"),n=0;n<a.length;n++){var e=a[n],r=e.querySelector('.qtd-item[id^="item_carrinho_"]'),s=r&&r.id.match(/item_carrinho_(\d+)/),g=s?parseInt(s[1],10):null,b=e.querySelector("figure img")||e.querySelector("#product-img")||e.querySelector("img"),w=e.querySelector(".nome-produto .link")||e.querySelector("figure a"),E=e.querySelector(".column-valor-produto .valor"),j=E?E.textContent.trim():"",L=!!e.querySelector(".column-valor-produto .sub");t.items.push({dataId:g,sku:e.getAttribute("data-item-id")||"",name:e.getAttribute("data-item-name")||e.getAttribute("data-name")||"",variant:e.getAttribute("data-item-variant")||"",brand:e.getAttribute("data-item-brand")||"",category:e.getAttribute("data-item-category")||"",priceUnit:parseFloat(e.getAttribute("data-item-price")||"0"),lineTotal:parseFloat(e.getAttribute("data-valor")||"0"),quantity:parseInt(e.getAttribute("data-item-quantity")||"1",10),deposito:e.getAttribute("data-item-deposito")==="1",imgSrc:b?b.getAttribute("src")||b.currentSrc:"",href:w?w.getAttribute("href"):"",lineTotalPix:V(j),isPix:L}),t.subtotalFull+=parseFloat(e.getAttribute("data-valor")||"0")}var Z=c.querySelector("#resumo-compra .resumo-valores .value");Z&&(t.subtotalPix=V(Z.textContent)),t.subtotalPix<=0&&(t.subtotalPix=t.items.reduce(function(wt,Ft){return wt+(Ft.lineTotalPix||0)},0));var W=c.querySelector("#resumo-compra .discount-value");W&&(t.discount=V(W.textContent));var N=c.querySelector("#resumo-compra .txt-cupom, #resumo-compra .alert.alert-type-1");if(N){var H=N.textContent.match(/([A-Z0-9]{3,})/);H&&(t.couponCode=H[1])}var an=c.querySelector("#resumo-compra .frete-calculado, .frete-calculado");if(an&&an.textContent.trim()){t.shippingRaw=an.textContent.trim();var en=an.querySelector(".frete-location .city");en&&(t.shippingCity=en.textContent.trim());for(var xn=an.querySelectorAll(".servico-frete"),hn=0;hn<xn.length;hn++){var zn=xn[hn],Wn=zn.querySelector('input[type="radio"]'),S=zn.querySelector(".dias-entrega"),C=parseFloat(zn.getAttribute("data-valor-frete")||"0"),X=zn.getAttribute("data-servico-frete")||"",nn=S?S.textContent.trim().replace(/\s+/g," ").replace(/\s*-\s*$/,"").trim():"",Y=nn.match(/(\d+(?:\s*[aà]\s*\d+)?\s*dias?(?:\s*[úu]teis)?)/i),vn=Y?Y[1].replace(/\s+/g," "):nn;t.shippingOptions.push({id:Wn?Wn.value:"",name:X,deadline:vn,value:C,isFree:C===0,isSelected:Wn?Wn.checked:!1})}var bn=t.shippingOptions.filter(function(wt){return wt.isSelected})[0];if(!bn&&t.shippingOptions.length>0&&(bn=t.shippingOptions[0]),bn)t.shipping=bn.value,t.shippingName=bn.name,t.shippingDeadline=bn.deadline;else{var qn=an.querySelector(".info-frete-selec"),An=an.querySelector(".dias-entrega, .info-frete-selec .dias-entrega"),Tn=an.querySelector(".valor-frete .value, .value.valor-frete"),Zn=an.querySelector(".info-frete-selec .info-title span, .info-title span");if(Tn){var Nn=Tn.textContent.trim();if(/gr[aá]tis/i.test(Nn))t.shipping=0;else{var nt=V(Nn);nt>0&&(t.shipping=nt)}}if(An){var tt=An.textContent.match(/(\d+(?:\s*[aà]\s*\d+)?\s*dias?(?:\s*\([^)]+\))?(?:\s*[úu]teis)?)/i);tt&&(t.shippingDeadline=tt[1].replace(/\s+/g," ").replace(/\(s\)/,"").trim())}if(Zn&&(t.shippingName=Zn.textContent.trim()),t.shipping===null)if(/gr[aá]tis/i.test(t.shippingRaw))t.shipping=0;else{var ut=V(t.shippingRaw);ut>0&&(t.shipping=ut)}if(!t.shippingDeadline){var kn=t.shippingRaw.match(/(\d+(?:\s*[aà]\s*\d+)?)\s*dias?/i);kn&&(t.shippingDeadline=kn[1]+" dias úteis")}}}var ht=c.querySelector("#cep, .input-cep");return ht&&(t.cepValue=ht.value||""),t.hasFinalizar=!!c.querySelector("#finalizar-compra"),t.canFinalize=t.items.length>0,t}function on(t){t=t||"cart";var a='<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 1 3 5v6c0 5.5 3.8 10.7 9 12 5.2-1.3 9-6.5 9-12V5l-9-4zm-1.4 16L6 12.4l1.4-1.4 3.2 3.2 6.8-6.8L18.8 8.8 10.6 17z"/></svg>';function n(e,r){var s=e===t,g="mm-checkout-step"+(s?" is-active":""),b=s?' aria-current="step"':"";return'<li class="'+g+'"'+b+'><span class="mm-checkout-step-label">'+r+"</span></li>"}return'<header class="mm-checkout-header"><a class="mm-checkout-header-logo" href="/" aria-label="Madeira Mania - voltar à home"><img src="'+d+'" alt="Madeira Mania" width="180" height="44"></a><nav class="mm-checkout-steps" aria-label="Etapas do checkout"><ol>'+n("cart","Carrinho")+'<li class="mm-checkout-step-sep" aria-hidden="true">›</li>'+n("delivery","Entrega")+'<li class="mm-checkout-step-sep" aria-hidden="true">›</li>'+n("payment","Pagamento")+'</ol></nav><div class="mm-checkout-secure">'+a+'<span class="mm-checkout-secure-text"><strong>Compra 100% segura</strong><small>Site protegido por SSL</small></span></div></header>'}function Bn(t){var a=t.imgSrc?'<img src="'+O(t.imgSrc)+'" alt="'+O(t.name)+'" loading="lazy">':"",n=t.href?'<a class="mm-item-name" href="'+O(t.href)+'">'+O(t.name)+"</a>":'<span class="mm-item-name">'+O(t.name)+"</span>",e=t.variant?'<p class="mm-item-variant">'+O(t.variant)+"</p>":"",r="",s=t.quantity<=1?' disabled aria-disabled="true"':"",g;if(t.lineTotalPix>0&&t.isPix){var b='<span class="mm-item-price-sub">no PIX</span>',w=t.quantity>1?D(t.lineTotalPix/t.quantity)+" cada":"";g='<div class="mm-item-price"><span class="mm-item-price-value">'+D(t.lineTotalPix)+"</span>"+b+(w?'<span class="mm-item-price-unit">'+w+"</span>":"")+"</div>"}else{var E=t.quantity>1?D(t.priceUnit)+" cada":"";g='<div class="mm-item-price"><span class="mm-item-price-value">'+D(t.lineTotal)+"</span>"+(E?'<span class="mm-item-price-unit">'+E+"</span>":"")+"</div>"}return'<div class="mm-item" data-mm-id="'+t.dataId+'"><div class="mm-item-thumb">'+a+'</div><div class="mm-item-body">'+n+e+r+"</div>"+g+'<div class="mm-item-controls"><div class="mm-qty" role="group" aria-label="Quantidade"><button type="button" class="mm-qty-btn" data-mm-act="dec"'+s+' aria-label="Diminuir quantidade">'+B.minus+'</button><span class="mm-qty-value">'+t.quantity+'</span><button type="button" class="mm-qty-btn" data-mm-act="inc" aria-label="Aumentar quantidade">'+B.plus+'</button></div><button type="button" class="mm-item-remove" data-mm-act="remove" aria-label="Remover item" title="Remover">'+B.trash+"</button></div></div>"}function Sn(t){return t.items.length?t.items.map(Bn).join(""):'<div class="mm-empty"><div class="mm-empty-icon">'+B.box+'</div><h3 class="mm-empty-title">Seu carrinho está vazio</h3><p class="mm-empty-desc">Explore nossos móveis e encontre a peça certa para sua casa.</p><a class="mm-empty-cta" href="/">Explorar produtos '+B.arrow+'</a><p class="mm-empty-perks">Frete grátis acima de R$ 2.000 · 12x sem juros · 7% off no PIX</p></div>'}function En(t){for(var a="",n=0;n<t;n++)a+='<div class="mm-skel-item"><div class="mm-skel mm-skel-thumb"></div><div class="mm-skel-lines"><div class="mm-skel mm-skel-line w-3-4"></div><div class="mm-skel mm-skel-line w-1-2"></div><div class="mm-skel mm-skel-line w-1-3"></div></div><div class="mm-skel-lines"><div class="mm-skel mm-skel-line w-1-2"></div></div></div>';return a}function un(t){var a=t.subtotalFull>0?t.subtotalFull:t.subtotalPix,n='<div class="mm-row"><span class="mm-row-label">Subtotal</span><span class="mm-row-value">'+D(a)+"</span></div>";if(t.shipping!==null){var e;t.shipping===0?e='<span class="mm-row-value is-free">'+B.check+" Grátis</span>":e='<span class="mm-row-value">'+D(t.shipping)+"</span>";var r='<span class="mm-row-label">Frete';t.shippingName&&(r+=' <span class="mm-row-sub">· '+O(t.shippingName)+"</span>"),t.shippingDeadline&&(r+=' <span class="mm-row-sub">· '+O(t.shippingDeadline)+"</span>"),r+="</span>",n+='<div class="mm-row">'+r+e+"</div>"}t.discount>0&&(n+='<div class="mm-row"><span class="mm-row-label">Desconto</span><span class="mm-row-value is-discount">− '+D(t.discount)+"</span></div>");var s="";if(t.shipping!==null){var g=Math.max(0,a+t.shipping-t.discount),b=Math.max(0,t.subtotalPix+t.shipping-t.discount),w=g-b,E=g/12;s='<div class="mm-total"><div class="mm-total-label">Total</div><div class="mm-total-value">'+D(g)+'</div><div class="mm-total-pix"><span>'+D(b)+" à vista no PIX</span>"+(w>0?'<span class="mm-total-pix-save">economia de '+D(w)+"</span>":"")+'</div><div class="mm-total-parcela">ou 12x de '+D(E)+" sem juros no cartão</div></div>"}else s='<div class="mm-total"><div class="mm-total-label">Subtotal</div><div class="mm-total-value">'+D(t.subtotalPix)+'</div><div class="mm-total-pix"><span>à vista no PIX</span></div><div class="mm-total-pending">Informe seu CEP para ver o frete e o total final.</div></div>';var j;return t.couponCode?j='<div class="mm-coupon-applied"><span class="mm-coupon-applied-left">'+B.tag+"<span>"+O(t.couponCode)+'</span></span><button type="button" data-mm-act="coupon-remove" aria-label="Remover cupom">'+B.close+"</button></div>":j='<div class="mm-coupon"><button type="button" class="mm-coupon-toggle" data-mm-act="coupon-toggle">'+B.tag+'<span>Tenho um cupom</span></button><form class="mm-coupon-form" data-mm-act="coupon-submit"><input type="text" class="mm-input" name="mm-coupon-code" placeholder="CUPOM" autocomplete="off" autocapitalize="characters" spellcheck="false" /><button type="submit" class="mm-btn-secondary">Aplicar</button></form></div>','<div class="mm-sum-stack"><div class="mm-rows">'+n+"</div>"+j+s+"</div>"}function Cn(){var t=document.getElementById("mm-layout");if(t)return t.parentElement!==c&&c.insertBefore(t,c.firstChild),t;var a=document.createElement("div");return a.id="mm-layout",a.innerHTML=on("cart")+'<div class="mm-grid"><section class="mm-items"><h2 class="mm-h">Carrinho</h2><div class="mm-items-card"><div id="mm-item-list">'+En(2)+'</div></div></section><aside class="mm-sum"><h2 class="mm-h">Resumo</h2><div class="mm-sum-card"><div class="mm-cep"><div class="mm-cep-label"><span class="mm-cep-label-text">Calcular frete</span><a class="mm-cep-label-link" href="https://buscacepinter.correios.com.br/app/endereco/index.php" target="_blank" rel="noopener">Não sei meu CEP</a></div><div class="mm-cep-row"><input type="text" class="mm-input" id="mm-cep-input" name="mm_cep_calc" inputmode="numeric" maxlength="9" placeholder="00000-000" autocomplete="off" data-lpignore="true" data-1p-ignore="true" data-bwignore="true" data-form-type="other" aria-label="CEP" /><button type="button" class="mm-btn-secondary" data-mm-act="calc-cep">Calcular</button></div></div><div id="mm-sum-dynamic"></div><button type="button" class="mm-cta" data-mm-act="finalizar">Finalizar compra'+B.arrow+'</button><div class="mm-trust"><span class="mm-trust-item">'+B.lock+'<span>Pagamento seguro</span></span><span class="mm-trust-item">'+B.rotate+'<span>7 dias para troca</span></span><span class="mm-trust-item">'+B.card+'<span>12x sem juros</span></span></div><a class="mm-help" href="'+f+'" target="_blank" rel="noopener" data-mm-track="help-whats">'+B.whats+"<span><strong>Ficou com alguma dúvida?</strong> Fale com a gente no WhatsApp</span></a></div></aside></div>",c.insertBefore(a,c.firstChild),c.classList.add("mm-shadow-mode"),document.body.classList.add("mm-checkout-rebuild"),document.documentElement.classList.remove("mm-cart-loading"),a}function Xn(){var t=Fn(),a=document.getElementById("mm-item-list");a&&(a.innerHTML=Sn(t));var n=document.getElementById("mm-sum-dynamic");n&&(n.innerHTML=un(t));var e=document.querySelector(".mm-cta");e&&(e.disabled=!t.canFinalize,e.style.opacity=t.canFinalize?"1":"0.5",e.style.pointerEvents=t.canFinalize?"auto":"none");var r=document.getElementById("mm-cep-input");if(r&&!r.matches(":focus")){var s=tn.get(P),g=t.cepValue||s||"";g&&(r.value=yn(g))}return t.items&&t.items.length>0&&rn(t),t}function yn(t){var a=String(t||"").replace(/\D/g,"").slice(0,8);return a.length<=5?a:a.slice(0,5)+"-"+a.slice(5)}function Un(t){var a=String(t||"").replace(/\D/g,"");a.length===8&&tn.set(P,a)}function sn(t){t=t||0;var a=tn.get(P);if(!(!a||a.length!==8)){var n=c.querySelector("#cep, .input-cep");if(!n){t<12&&setTimeout(function(){sn(t+1)},350);return}var e=c.querySelector("#resumo-compra .frete-calculado");if(e&&e.textContent.trim()){var r=document.getElementById("mm-cep-input");r&&!r.value&&(r.value=yn(a));return}var s=document.getElementById("mm-cep-input");s&&!s.value&&(s.value=yn(a)),n.value=yn(a),gn(n),setTimeout(function(){In()},200)}}function gn(t){try{var a=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;a.call(t,t.value)}catch{}t.dispatchEvent(new Event("input",{bubbles:!0})),t.dispatchEvent(new Event("change",{bubbles:!0}))}function In(){try{if(window.Zord&&window.Zord.Cart&&typeof window.Zord.Cart.calculaFreteCarrinho=="function"){window.Zord.Cart.calculaFreteCarrinho();return}}catch{}var t=c.querySelector("#resumo-compra .calcula-frete > button, .area-frete button");t&&t.click()}function Ln(){var t=document.getElementById("mm-layout");!t||t._mmBound||(t._mmBound=!0,t.addEventListener("click",function(a){var n=a.target.closest("[data-mm-act]");if(n){var e=n.getAttribute("data-mm-act"),r=n.closest(".mm-item"),s=r?parseInt(r.getAttribute("data-mm-id"),10):null;switch(e){case"inc":$n(s,r,"inc");break;case"dec":$n(s,r,"dec");break;case"remove":pt(s,r);break;case"calc-cep":wn();break;case"coupon-toggle":var g=n.closest(".mm-coupon");if(g){g.classList.add("is-open");var b=g.querySelector("input");b&&setTimeout(function(){b.focus()},100)}break;case"coupon-remove":ln();break;case"finalizar":Qn();break}}}),t.addEventListener("submit",function(a){var n=a.target.closest('[data-mm-act="coupon-submit"]');if(n){a.preventDefault();var e=n.querySelector("input");e&&fn(e.value.trim())}}),t.addEventListener("input",function(a){a.target&&a.target.id==="mm-cep-input"&&(a.target.value=yn(a.target.value))}),t.addEventListener("keydown",function(a){a.key==="Enter"&&a.target&&a.target.id==="mm-cep-input"&&(a.preventDefault(),wn())}))}function $n(t,a,n){if(!(!t||!a)&&!(!window.Zord||!window.Zord.checkout)){a.classList.add("is-updating");try{n==="inc"?window.Zord.checkout.adicionaQuantidade(t):window.Zord.checkout.removeQuantidade(t)}catch(e){console.warn("[mm-cart] qty change failed",e),a.classList.remove("is-updating")}}}function pt(t,a){if(!(!t||!a)&&!(!window.Zord||!window.Zord.checkout)){a.classList.add("is-updating"),a.style.transition="opacity 200ms, transform 200ms",a.style.opacity="0",a.style.transform="translateX(-12px)";try{typeof window.Zord.checkout.deleteItem=="function"?window.Zord.checkout.deleteItem(t):window.Zord.checkout.removeQuantidade(t)}catch(n){console.warn("[mm-cart] remove failed",n),a.classList.remove("is-updating"),a.style.opacity="1",a.style.transform=""}}}function wn(){var t=document.getElementById("mm-cep-input");if(t){var a=(t.value||"").replace(/\D/g,"");if(a.length!==8){t.focus(),t.classList.add("mm-input-error"),setTimeout(function(){t.classList.remove("mm-input-error")},1200);return}Un(a);var n=c.querySelector("#cep, .input-cep");n&&(n.value=yn(a),gn(n)),In()}}function fn(t){if(t&&!(!window.Zord||!window.Zord.checkout)){var a=c.querySelector("#cupom-desconto");a&&(a.value=t.toUpperCase(),gn(a));try{window.Zord.checkout.addCupomDesconto()}catch(n){console.warn("[mm-cart] coupon apply failed",n)}}}function ln(){if(!(!window.Zord||!window.Zord.checkout))try{window.Zord.checkout.removeCupomDesconto()}catch(t){console.warn("[mm-cart] coupon remove failed",t)}}function Qn(){try{var t=Fn();t.items&&t.items.length>0&&rn(t)}catch{}var a=document.getElementById("finalizar-compra");if(a){a.click();return}location.href="/checkout/identify"}if(z){let t=function(n){if(n=n||0,n>30){a();return}var e=c.querySelectorAll(".cart-item").length>0,r=c.querySelector("#resumo-compra");e||r||n>8?a():setTimeout(function(){t(n+1)},250)},a=function(){Cn(),Ln(),Xn(),sn(),typeof jQuery<"u"&&jQuery(document).ajaxComplete(function(r,s,g){if(!(!g||!g.url)){var b=g.url;(b.indexOf("checkout/cart")!==-1||b.indexOf("atualiza")!==-1||b.indexOf("cupom")!==-1||b.indexOf("frete")!==-1||b.indexOf("removeItem")!==-1||b.indexOf("adicionaItem")!==-1)&&(setTimeout(Xn,120),setTimeout(function(){var w=Fn();w.shipping!==null&&w.cepValue&&Un(w.cepValue)},200))}});try{var n=new MutationObserver(function(r){a._mutTimer&&clearTimeout(a._mutTimer),a._mutTimer=setTimeout(Xn,200)}),e=[c.querySelector("#cart-area"),c.querySelector(".cart-area"),c.querySelector("#resumo-compra")].filter(Boolean);e.forEach(function(r){n.observe(r,{childList:!0,subtree:!0,characterData:!0})})}catch{}};if(!document.getElementById("mm-checkout-cro-done")){var rt=document.createElement("div");rt.id="mm-checkout-cro-done",rt.style.display="none",document.body.appendChild(rt)}t()}function Pn(t){if(!t||!t.items||!t.items.length)return'<aside class="mm-id-sum mm-sum"><h2 class="mm-h">Resumo</h2><div class="mm-sum-card"><div class="mm-sum-empty"><p>Não conseguimos carregar o resumo do seu pedido.</p><a class="mm-btn-secondary" href="/checkout/cart">Voltar ao carrinho</a></div></div></aside>';var a=3,n=t.items.slice(0,a),e=t.items.length-a,r=n.map(function(H){var an=H.quantity>1?'<strong class="mm-id-thumb-qty">'+H.quantity+"×</strong> ":"",en=!H.imgSrc&&!(H.lineTotalPix>0)&&!(H.lineTotal>0);if(en)return'<div class="mm-id-thumb"><div class="mm-id-thumb-img"><span class="mm-skel" style="display:block;width:100%;height:100%"></span></div><div class="mm-id-thumb-body"><p class="mm-id-thumb-name">'+an+O(H.name)+"</p>"+(H.variant?'<p class="mm-id-thumb-variant">'+O(H.variant)+"</p>":"")+'</div><div class="mm-id-thumb-price"><span class="mm-skel" style="display:inline-block;width:56px;height:15px"></span></div></div>';var xn=H.imgSrc?'<img src="'+O(H.imgSrc)+'" alt="'+O(H.name)+'" loading="lazy">':'<div class="mm-id-thumb-placeholder">'+B.box+"</div>",hn=H.lineTotalPix>0?H.lineTotalPix:H.lineTotal;return'<div class="mm-id-thumb"><div class="mm-id-thumb-img">'+xn+'</div><div class="mm-id-thumb-body"><p class="mm-id-thumb-name">'+an+O(H.name)+"</p>"+(H.variant?'<p class="mm-id-thumb-variant">'+O(H.variant)+"</p>":"")+'</div><div class="mm-id-thumb-price">'+D(hn)+"</div></div>"}).join("");e>0&&(r+='<div class="mm-id-thumb-more">+ '+e+" "+(e===1?"item":"itens")+" a mais</div>");var s=t.subtotalFull>0?t.subtotalFull:t.subtotalPix,g='<div class="mm-row"><span class="mm-row-label">Subtotal</span><span class="mm-row-value">'+D(s)+"</span></div>";if(t.shipping!==null&&t.shipping!==void 0){var b;t.shipping===0?b='<span class="mm-row-value is-free">'+B.check+" Grátis</span>":b='<span class="mm-row-value">'+D(t.shipping)+"</span>",g+='<div class="mm-row"><span class="mm-row-label">Frete'+(t.shippingDeadline?' <span class="mm-row-sub">· '+O(t.shippingDeadline)+"</span>":"")+"</span>"+b+"</div>"}t.discount>0&&(g+='<div class="mm-row"><span class="mm-row-label">Desconto'+(t.couponCode?' <span class="mm-row-sub">· '+O(t.couponCode)+"</span>":"")+'</span><span class="mm-row-value is-discount">− '+D(t.discount)+"</span></div>");var w,E=t.shipping!==null&&t.shipping!==void 0?t.shipping:0;if(t.shipping!==null&&t.shipping!==void 0){var j=Math.max(0,s+E-(t.discount||0)),L=Math.max(0,t.subtotalPix+E-(t.discount||0)),Z=j-L,W=j/12;w='<div class="mm-total"><div class="mm-total-label">Total</div><div class="mm-total-value">'+D(j)+'</div><div class="mm-total-pix"><span>'+D(L)+" à vista no PIX</span>"+(Z>0?'<span class="mm-total-pix-save">economia de '+D(Z)+"</span>":"")+'</div><div class="mm-total-parcela">ou 12x de '+D(W)+" sem juros</div></div>"}else{var N=s/12;w='<div class="mm-total"><div class="mm-total-label">Subtotal</div><div class="mm-total-value">'+D(t.subtotalPix)+'</div><div class="mm-total-pix"><span>à vista no PIX</span></div><div class="mm-total-parcela">ou 12x de '+D(N)+" sem juros</div></div>"}return'<aside class="mm-id-sum mm-sum"><h2 class="mm-h">Resumo do pedido</h2><div class="mm-sum-card"><div class="mm-id-thumbs">'+r+'</div><div class="mm-rows">'+g+"</div>"+w+'<a class="mm-id-edit-cart" href="/checkout/cart"><span>Editar carrinho</span></a></div><a class="mm-help mm-sum-help" href="'+f+'" target="_blank" rel="noopener" data-mm-track="help-whats-sum">'+B.whats+"<span><strong>Ficou com alguma dúvida?</strong> Fale com a gente no WhatsApp</span></a></aside>"}function Yn(){var t='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>',a='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',n='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><line x1="7" y1="9" x2="17" y2="9"/><line x1="7" y1="13" x2="17" y2="13"/><line x1="7" y1="17" x2="13" y2="17"/></svg>',e='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>';return'<section class="mm-id-form-col"><h2 class="mm-id-h2">Quase lá! Identifique-se</h2><p class="mm-id-sub">Informe seu e-mail para finalizar a compra de forma rápida e segura.</p><form class="mm-id-form" data-mm-act="identify-submit" novalidate><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+t+'</span><input type="email" id="mm-id-email" name="mm-email" class="mm-input" placeholder="seu@email.com" autocomplete="email" inputmode="email" required></div><p class="mm-id-microcopy">'+B.lock+'<span>Seu e-mail é seguro · Não compartilhamos com terceiros</span></p><button type="submit" class="mm-cta">Continuar'+B.arrow+'</button></form><div class="mm-id-divider"><span>ou</span></div><div class="mm-id-google-slot"></div><button type="button" class="mm-id-guest-toggle" data-mm-act="guest-go" aria-label="Continuar como visitante"><span class="mm-id-guest-icon" aria-hidden="true">'+a+'</span><span class="mm-id-guest-label">Continuar como visitante (sem criar conta)</span><span class="mm-id-guest-arrow" aria-hidden="true">'+B.arrow+'</span></button><div class="mm-trust mm-id-trust"><span class="mm-trust-item">'+B.lock+'<span>Pagamento seguro</span></span><span class="mm-trust-item">'+B.rotate+'<span>7 dias para troca</span></span><span class="mm-trust-item">'+B.shield+'<span>Garantia 12 meses</span></span></div><p class="mm-id-lgpd">Ao continuar, você concorda com nossa <a href="/politica-de-privacidade" target="_blank" rel="noopener">Política de Privacidade</a></p></section>'}function On(t){var a=document.getElementById("mm-layout");if(a)return a.parentElement!==c&&c.insertBefore(a,c.firstChild),a;var n=document.createElement("div");return n.id="mm-layout",n.classList.add("mm-id-layout"),n.innerHTML=on("delivery")+'<div class="mm-grid mm-id-grid">'+Yn()+Pn(t)+"</div>",c.insertBefore(n,c.firstChild),document.body.classList.add("mm-checkout-rebuild"),Jn(),c.classList.add("mm-shadow-mode"),document.documentElement.classList.remove("mm-cart-loading"),n}function Jn(){var t=document.querySelector(".mm-id-google-slot"),a=c.querySelector(".social-login-area");if(!(!t||!a)&&!t.contains(a))try{t.appendChild(a),t.classList.add("is-loaded")}catch{}}function gt(t){tn.set("mm_user_email",t);var a=c.querySelector("#login");if(!a)return!1;a.value=t,gn(a);var n=a.closest("form"),e=n?n.querySelector('button.button-send, button[type="submit"]'):null;return e?(e.click(),!0):n?(n.submit(),!0):!1}function zt(){var t=document.getElementById("mm-layout");!t||t._mmBound||(t._mmBound=!0,t.addEventListener("submit",function(a){var n=a.target.closest('[data-mm-act="identify-submit"]');if(n){a.preventDefault();var e=n.querySelector("#mm-id-email"),r=e?e.value.trim():"";if(!r||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(r)){e&&(e.classList.add("mm-input-error"),e.focus(),setTimeout(function(){e.classList.remove("mm-input-error")},1500));return}var s=gt(r);if(s){var g=n.querySelector(".mm-cta");g&&g.classList.add("is-loading")}return}}),t.addEventListener("click",function(a){var n=a.target.closest("[data-mm-act]");if(n){var e=n.getAttribute("data-mm-act");e==="guest-go"&&(tn.set("mm_checkout_mode","guest"),n.classList.add("is-loading"),location.href="/checkout/onepage")}}))}function Dn(t){try{var a=new DOMParser().parseFromString(t,"text/html"),n=a.querySelector("#checkout-main-area");if(!n)return null;for(var e=[],r=n.querySelectorAll(".cart-item"),s=0,g=0;g<r.length;g++){var b=r[g],w=b.querySelector("figure img")||b.querySelector("#product-img")||b.querySelector("img"),E=b.querySelector(".column-valor-produto .valor"),j=parseFloat(b.getAttribute("data-valor")||"0"),L=E?V(E.textContent):0;e.push({name:b.getAttribute("data-item-name")||b.getAttribute("data-name")||"",variant:b.getAttribute("data-item-variant")||"",imgSrc:w&&w.getAttribute("src")||"",quantity:parseInt(b.getAttribute("data-item-quantity")||"1",10),lineTotal:j,lineTotalPix:L,isPix:!!b.querySelector(".column-valor-produto .sub"),deposito:b.getAttribute("data-item-deposito")==="1"}),s+=j}if(e.length===0)return null;var Z=n.querySelector("#resumo-compra .resumo-valores .value"),W=Z?V(Z.textContent):0;W<=0&&(W=e.reduce(function(nn,Y){return nn+(Y.lineTotalPix||0)},0));var N=n.querySelector("#resumo-compra .discount-value"),H=N?V(N.textContent):0,an=n.querySelector("#resumo-compra .txt-cupom, #resumo-compra .alert.alert-type-1"),en="";if(an){var xn=an.textContent.match(/([A-Z0-9]{3,})/);xn&&(en=xn[1])}var hn=n.querySelector("#resumo-compra .frete-calculado"),zn=null,Wn="";if(hn&&hn.textContent.trim()){var S=hn.textContent.trim();if(/gr[aá]tis/i.test(S))zn=0;else{var C=V(S);C>0&&(zn=C)}var X=S.match(/(\d+)\s*dias?/i);X&&(Wn=X[1]+" dias úteis")}return{ts:Date.now(),items:e,subtotalPix:W,subtotalFull:s,discount:H,couponCode:en,shipping:zn,shippingDeadline:Wn,cepValue:""}}catch{return null}}function _n(t){try{fetch("/checkout/cart",{credentials:"include",headers:{"X-Requested-With":"XMLHttpRequest"}}).then(function(a){return a.text()}).then(function(a){var n=Dn(a);n&&tn.set(R,JSON.stringify(n)),t(n)}).catch(function(){t(null)})}catch{t(null)}}function ft(t){var a=document.querySelector("#mm-layout .mm-id-sum");if(a){var n=a.parentNode;if(n){var e=document.createElement("div");e.innerHTML=Pn(t);var r=e.firstChild;r&&n.replaceChild(r,a)}}}function vt(){_n(function(t){t&&t.items&&t.items.length>0&&ft(t)})}if(v){let t=function(n){if(n=n||0,n>30){a();return}var e=c.querySelector("#login, #login-form-etapa-01");e||n>8?a():setTimeout(function(){t(n+1)},250)},a=function(){var n=mn();On(n),zt(),Jn(),setTimeout(Jn,600),setTimeout(Jn,1500),vt(),setTimeout(function(){var e=document.getElementById("mm-id-email");e&&!("ontouchstart"in window)&&e.focus()},250)};t()}function xt(t){var a=String(t||"").replace(/\D/g,"").slice(0,11);return a.length<=3?a:a.length<=6?a.slice(0,3)+"."+a.slice(3):a.length<=9?a.slice(0,3)+"."+a.slice(3,6)+"."+a.slice(6):a.slice(0,3)+"."+a.slice(3,6)+"."+a.slice(6,9)+"-"+a.slice(9)}function qt(t){var a=String(t||"").replace(/\D/g,"").slice(0,11);return a.length<=2?a.length?"("+a:"":a.length<=6?"("+a.slice(0,2)+") "+a.slice(2):a.length<=10?"("+a.slice(0,2)+") "+a.slice(2,6)+"-"+a.slice(6):"("+a.slice(0,2)+") "+a.slice(2,7)+"-"+a.slice(7)}function bt(t){var a=String(t||"").replace(/\D/g,"").slice(0,8);return a.length<=5?a:a.slice(0,5)+"-"+a.slice(5)}function mt(t,a){var n=String(t||"").replace(/\D/g,"");if(n.length!==8){a(null);return}try{fetch("https://viacep.com.br/ws/"+n+"/json/",{headers:{Accept:"application/json"}}).then(function(e){return e.json()}).then(function(e){if(!e||e.erro){a(null);return}a({logradouro:e.logradouro||"",bairro:e.bairro||"",cidade:e.localidade||"",estado:e.uf||""})}).catch(function(){a(null)})}catch{a(null)}}var Rn={mail:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4-8 5-8-5V6l8 5 8-5v2z"/></svg>',user:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>',doc:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M14 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6zm4 18H6V4h7v5h5v11zM8 12h8v2H8zm0 4h5v2H8z"/></svg>',phone:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z"/></svg>',pin:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>',home:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>',pix:'<svg width="24" height="24" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M11.917 11.71a2.046 2.046 0 0 1-1.454-.602l-2.1-2.1a.4.4 0 0 0-.551 0l-2.108 2.108a2.044 2.044 0 0 1-1.454.602h-.414l2.66 2.66c.83.83 2.177.83 3.007 0l2.667-2.668h-.253zM4.25 4.282c.55 0 1.066.214 1.454.602l2.108 2.108a.39.39 0 0 0 .552 0l2.1-2.1a2.044 2.044 0 0 1 1.453-.602h.253L9.503 1.623a2.127 2.127 0 0 0-3.007 0l-2.66 2.66h.414z"/><path d="m14.377 6.496-1.612-1.612a.307.307 0 0 1-.114.023h-.733c-.379 0-.75.154-1.017.422l-2.1 2.1a1.005 1.005 0 0 1-1.425 0L5.268 5.32a1.448 1.448 0 0 0-1.018-.422h-.9a.306.306 0 0 1-.109-.021L1.623 6.496c-.83.83-.83 2.177 0 3.008l1.618 1.618a.305.305 0 0 1 .108-.022h.901c.38 0 .75-.153 1.018-.421L7.375 8.57a1.034 1.034 0 0 1 1.426 0l2.1 2.1c.267.268.638.421 1.017.421h.733c.04 0 .079.01.114.024l1.612-1.612c.83-.83.83-2.178 0-3.008z"/></svg>',cardBig:'<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 4H4c-1.11 0-1.99.89-1.99 2L2 18c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V6c0-1.11-.89-2-2-2zm0 14H4v-6h16v6zm0-10H4V6h16v2z"/></svg>',barcode:'<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M2 6h2v12H2zm3 0h1v12H5zm2 0h3v12H7zm4 0h1v12h-1zm3 0h2v12h-2zm3 0h1v12h-1zm2 0h3v12h-3z"/></svg>',editPencil:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a.996.996 0 0 0 0-1.41l-2.34-2.34a.996.996 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>'};function ot(t){var a=t?' value="'+O(t)+'"':"";return'<section class="mm-op-form-col"><h2 class="mm-id-h2">Falta pouco. Onde entregar?</h2><p class="mm-id-sub">Preencha seus dados e o endereço de entrega — o frete é calculado automaticamente.</p><form class="mm-op-form" data-mm-act="onepage-submit" novalidate><div class="mm-op-card"><h3 class="mm-op-card-title">'+Rn.user+'<span>Quem vai receber</span></h3><div class="mm-op-grid-2"><div class="mm-input-wrap mm-op-col-2"><span class="mm-input-icon" aria-hidden="true">'+Rn.mail+'</span><input type="email" id="mm-op-email" class="mm-input" placeholder="seu@email.com" autocomplete="email" inputmode="email" required'+a+'></div><div class="mm-input-wrap mm-op-col-2"><span class="mm-input-icon" aria-hidden="true">'+Rn.user+'</span><input type="text" id="mm-op-nome" class="mm-input" placeholder="Nome completo" autocomplete="name" required></div><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+Rn.doc+'</span><input type="tel" id="mm-op-cpf" class="mm-input" placeholder="CPF" inputmode="numeric" autocomplete="off" maxlength="14" required></div><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+Rn.phone+'</span><input type="tel" id="mm-op-tel" class="mm-input" placeholder="(11) 91234-5678" inputmode="tel" autocomplete="tel" maxlength="15" required></div></div><p class="mm-op-microcopy-soft">Usamos seus dados só pra emitir nota fiscal e te avisar da entrega.</p></div><div class="mm-op-card"><h3 class="mm-op-card-title">'+Rn.pin+'<span>Endereço de entrega</span></h3><div class="mm-op-grid-2"><div class="mm-input-wrap"><span class="mm-input-icon" aria-hidden="true">'+Rn.pin+'</span><input type="tel" id="mm-op-cep" class="mm-input" placeholder="CEP — 00000-000" inputmode="numeric" autocomplete="postal-code" maxlength="9" required><span class="mm-input-status" id="mm-op-cep-status" aria-live="polite"></span></div><a class="mm-op-cep-help" href="https://buscacepinter.correios.com.br/app/endereco/index.php" target="_blank" rel="noopener">Não sei meu CEP</a><div class="mm-input-wrap mm-op-col-2"><span class="mm-input-icon" aria-hidden="true">'+Rn.home+'</span><input type="text" id="mm-op-rua" class="mm-input" placeholder="Rua, Av, Travessa..." autocomplete="address-line1" required></div><div class="mm-input-wrap"><input type="tel" id="mm-op-num" class="mm-input mm-input-noicon" placeholder="Número" inputmode="numeric" required></div><div class="mm-input-wrap"><input type="text" id="mm-op-comp" class="mm-input mm-input-noicon" placeholder="Complemento (opcional)" autocomplete="address-line2"></div><div class="mm-input-wrap mm-op-col-2"><input type="text" id="mm-op-bairro" class="mm-input mm-input-noicon" placeholder="Bairro" autocomplete="address-level3" required></div><div class="mm-input-wrap"><input type="text" id="mm-op-cidade" class="mm-input mm-input-noicon" placeholder="Cidade" autocomplete="address-level2" required></div><div class="mm-input-wrap"><input type="text" id="mm-op-uf" class="mm-input mm-input-noicon" placeholder="UF" maxlength="2" autocomplete="address-level1" required></div></div><div class="mm-op-frete" id="mm-op-frete-slot"></div></div><button type="submit" class="mm-cta mm-op-cta">Última etapa: pagamento'+B.arrow+'</button><p class="mm-id-microcopy mm-op-cta-sub">'+B.lock+'<span>Você revisa tudo antes de finalizar</span></p></form><div class="mm-trust mm-id-trust"><span class="mm-trust-item">'+B.lock+'<span>Pagamento seguro</span></span><span class="mm-trust-item">'+B.rotate+'<span>7 dias para troca</span></span><span class="mm-trust-item">'+B.shield+"<span>Garantia 12 meses</span></span></div></section>"}function At(t,a){var n=document.getElementById("mm-layout");if(n)return n.parentElement!==c&&c.insertBefore(n,c.firstChild),n;var e=document.createElement("div");return e.id="mm-layout",e.classList.add("mm-id-layout"),e.classList.add("mm-op-layout"),e.innerHTML=on("delivery")+'<div class="mm-grid mm-id-grid mm-op-grid">'+ot(a)+Pn(t)+"</div>",c.insertBefore(e,c.firstChild),document.body.classList.add("mm-checkout-rebuild"),c.classList.add("mm-shadow-mode"),document.documentElement.classList.remove("mm-cart-loading"),e}function dt(t){var a=document.getElementById("mm-op-frete-slot");if(a){if(t==="loading"){a.innerHTML='<div class="mm-op-frete-loading"><div class="mm-op-frete-spinner"></div><span>Calculando frete...</span></div>';return}if(t==="error"){a.innerHTML='<div class="mm-op-frete-error"><span>Não conseguimos calcular o frete. Confira o CEP e tente novamente.</span></div>';return}var n=t.value===0,e=n?'<strong class="mm-op-frete-value is-free">Grátis</strong>':'<strong class="mm-op-frete-value">'+D(t.value)+"</strong>",r=t.name?'<span class="mm-op-frete-name">'+O(t.name)+"</span>":"",s=t.deadline?'<span class="mm-op-frete-deadline">Entrega em '+O(t.deadline)+"</span>":"",g=t.city?'<span class="mm-op-frete-city">para '+O(t.city)+"</span>":"",b="";if(t.options&&t.options.length>1){b='<div class="mm-op-frete-options"><button type="button" class="mm-op-frete-toggle" data-mm-act="toggle-frete-opts" aria-expanded="false">Ver outras opções ('+t.options.length+')</button><div class="mm-op-frete-options-list" hidden>';for(var w=0;w<t.options.length;w++){var E=t.options[w],j=E.isSelected?" is-selected":"",L=E.isFree?'<span class="mm-op-frete-opt-value is-free">Grátis</span>':'<span class="mm-op-frete-opt-value">'+D(E.value)+"</span>";b+='<button type="button" class="mm-op-frete-opt'+j+'" data-mm-act="op-ship-select" data-ship-id="'+O(E.id)+'" aria-pressed="'+(E.isSelected?"true":"false")+'"><span class="mm-op-frete-opt-radio" aria-hidden="true"><span></span></span><span class="mm-op-frete-opt-body"><span class="mm-op-frete-opt-name">'+O(E.name||"Padrão")+"</span>"+(E.deadline?'<span class="mm-op-frete-opt-deadline">'+O(E.deadline)+"</span>":"")+"</span>"+L+"</button>"}b+="</div></div>"}a.innerHTML='<div class="mm-op-frete-card'+(n?" is-free":"")+'"><div class="mm-op-frete-icon">'+B.truck+'</div><div class="mm-op-frete-body"><div class="mm-op-frete-row">'+r+e+"</div>"+s+g+"</div></div>"+b}}function yt(){function t(nn){if(!nn)return"";var Y=nn.match(/(\d+(?:\s*[aà]\s*\d+)?)\s*dias?(?:\s*\([^)]+\))?\s*([úu]teis)?/i);return Y?Y[1].replace(/\s+/g," ")+" dias úteis":""}function a(nn){for(var Y=[],vn=nn.querySelectorAll(".servico-frete"),bn=0;bn<vn.length;bn++){var qn=vn[bn],An=qn.querySelector('input[type="radio"]'),Tn=qn.querySelector(".dias-entrega"),Zn=parseFloat(qn.getAttribute("data-valor-frete")||"0"),Nn=qn.getAttribute("data-servico-frete")||"",nt=Tn?Tn.textContent.trim():"",tt=nt.match(/(\d+(?:\s*[aà]\s*\d+)?\s*dias?(?:\s*[úu]teis)?)/i);Y.push({id:An?An.value:"",name:Nn,deadline:tt?tt[1].replace(/\s+/g," "):nt,value:Zn,isFree:Zn===0,isSelected:An?An.checked:!1})}return Y}var n=c.querySelector(".frete-calculado");if(n&&n.textContent.trim()){var e=a(n),r=n.querySelector(".frete-location .city"),s=r?r.textContent.trim():"",g=e.filter(function(nn){return nn.isSelected})[0]||e[0];if(g)return{value:g.value,name:g.name,deadline:g.deadline,city:s,options:e};var b=n.querySelector(".info-frete-selec .info-title span, .info-title span"),w=n.querySelector(".info-frete-selec .dias-entrega, .dias-entrega"),E=n.querySelector(".value.valor-frete, .valor-frete .value"),j=n.textContent,L=null;if(E&&(/gr[aá]tis/i.test(E.textContent)?L=0:L=V(E.textContent)),L===null&&(/gr[aá]tis/i.test(j)?L=0:L=V(j)||null),L!==null)return{value:L,name:b?b.textContent.trim():"",deadline:t(w?w.textContent:j),city:s,options:[]}}var Z=c.querySelector(".line-entrega"),W=c.querySelector(".value.valor-frete, .valor-frete .value");if(Z||W){var N=((Z||W).textContent||"").trim(),H=mn(),an=H&&H.shippingName||"",en=H&&H.shippingDeadline||"",xn=H&&H.shippingCity||"",hn=H?H.shippingOptions||[]:[];if(N){var zn=(c.querySelector(".nome-servico-frete, .info-frete-selec .info-title span")||{}).textContent||"",Wn=(c.querySelector(".info-entrega, .prazo-entrega, .line-entrega .sub, .info-frete-selec .dias-entrega")||{}).textContent||"",S=t(Wn)||t(N)||en,C=zn.trim()||an;if(/gr[aá]tis/i.test(N))return{value:0,deadline:S,name:C,city:xn,options:hn};var X=V(N);if(X>0)return{value:X,deadline:S,name:C,city:xn,options:hn}}if(H&&H.shipping!==null&&H.shipping!==void 0)return{value:H.shipping,deadline:en,name:an,city:xn,options:hn}}return null}function Vn(){var t=document.getElementById("mm-op-cep");if(!t)return;var a=(t.value||"").replace(/\D/g,"");if(a.length!==8)return;if(Vn._lastCep===a){var n=document.getElementById("mm-op-frete-slot");if(n&&n.querySelector(".mm-op-frete-card"))return}Vn._lastCep=a;var e=(Vn._token||0)+1;Vn._token=e,Un(a);var r=c.querySelector("#cep, .input-cep");r&&(r.value=yn(a),gn(r)),dt("loading");try{window.Zord&&window.Zord.Cart&&typeof window.Zord.Cart.calculaFreteCarrinho=="function"&&window.Zord.Cart.calculaFreteCarrinho()}catch{}var s=0,g=18;function b(){if(Vn._token===e){s++;var w=yt();if(w){dt(w);var E=mn();E&&(E.shipping=w.value,E.shippingDeadline=w.deadline,E.shippingName=w.name||"",E.shippingCity=w.city||"",E.shippingOptions=w.options||[],tn.set(R,JSON.stringify(E)),ft(E));return}s<g?setTimeout(b,350):dt("error")}}setTimeout(b,400)}function Kn(){var t=document.getElementById("mm-layout");if(!t||t._mmOpBound)return;t._mmOpBound=!0,t.addEventListener("click",function(n){var e=n.target.closest('[data-mm-act="toggle-frete-opts"]');if(e){n.preventDefault();var r=e.parentElement.querySelector(".mm-op-frete-options-list");if(r){var s=r.hasAttribute("hidden");s?r.removeAttribute("hidden"):r.setAttribute("hidden",""),e.setAttribute("aria-expanded",s?"true":"false"),e.textContent=s?"Ocultar opções":"Ver outras opções"}return}var g=n.target.closest('[data-mm-act="op-ship-select"]');if(g){n.preventDefault();var b=g.getAttribute("data-ship-id");if(!b)return;var w=c.querySelector('.servico-frete input[type="radio"][value="'+b+'"]');if(!w){console.warn("[mm-op] modalidade não encontrada no DOM:",b);return}for(var E=t.querySelectorAll(".mm-op-frete-opt"),j=0;j<E.length;j++){var L=E[j],Z=L.getAttribute("data-ship-id")===b;L.classList.toggle("is-selected",Z),L.setAttribute("aria-pressed",Z?"true":"false")}w.checked=!0,w.click(),setTimeout(function(){var W=yt();if(W){dt(W);var N=mn();N&&(N.shipping=W.value,N.shippingDeadline=W.deadline,N.shippingName=W.name||"",N.shippingOptions=W.options||[],tn.set(R,JSON.stringify(N)),ft(N))}},700);return}}),t.addEventListener("submit",function(n){var e=n.target.closest('[data-mm-act="onepage-submit"]');if(e){n.preventDefault();var r={email:(document.getElementById("mm-op-email")||{}).value||"",nome:(document.getElementById("mm-op-nome")||{}).value||"",cpf:(document.getElementById("mm-op-cpf")||{}).value||"",tel:(document.getElementById("mm-op-tel")||{}).value||"",cep:(document.getElementById("mm-op-cep")||{}).value||"",rua:(document.getElementById("mm-op-rua")||{}).value||"",num:(document.getElementById("mm-op-num")||{}).value||"",comp:(document.getElementById("mm-op-comp")||{}).value||"",bairro:(document.getElementById("mm-op-bairro")||{}).value||"",cidade:(document.getElementById("mm-op-cidade")||{}).value||"",uf:(document.getElementById("mm-op-uf")||{}).value||""},s=[];if(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(r.email.trim())||s.push("mm-op-email"),r.nome.trim().split(/\s+/).length<2&&s.push("mm-op-nome"),r.cpf.replace(/\D/g,"").length!==11&&s.push("mm-op-cpf"),r.tel.replace(/\D/g,"").length<10&&s.push("mm-op-tel"),r.cep.replace(/\D/g,"").length!==8&&s.push("mm-op-cep"),r.rua.trim()||s.push("mm-op-rua"),r.num.trim()||s.push("mm-op-num"),r.bairro.trim()||s.push("mm-op-bairro"),r.cidade.trim()||s.push("mm-op-cidade"),r.uf.trim()||s.push("mm-op-uf"),s.length){s.forEach(function(w){var E=document.getElementById(w);E&&(E.classList.add("mm-input-error"),setTimeout(function(){E.classList.remove("mm-input-error")},1800))});var g=document.getElementById(s[0]);g&&(g.focus(),g.scrollIntoView({block:"center",behavior:"smooth"}));return}var b=e.querySelector(".mm-cta");b&&b.classList.add("is-loading"),tn.set("mm_user_email",r.email.trim()),o(r)}}),t.addEventListener("input",function(n){var e=n.target;if(e){if(e.dataset&&e.dataset.mmCepFill==="1"&&delete e.dataset.mmCepFill,e.id==="mm-op-cpf")e.value=xt(e.value);else if(e.id==="mm-op-tel")e.value=qt(e.value);else if(e.id==="mm-op-cep"){e.value=bt(e.value);var r=e.value.replace(/\D/g,"");r.length===8&&it(r)}else e.id==="mm-op-uf"&&(e.value=(e.value||"").replace(/[^A-Za-z]/g,"").toUpperCase().slice(0,2));e.id&&e.id.indexOf("mm-op-")===0&&(Kn._draftTimer&&clearTimeout(Kn._draftTimer),Kn._draftTimer=setTimeout(pn,400))}});function a(){Kn._draftTimer&&(clearTimeout(Kn._draftTimer),Kn._draftTimer=null),pn()}t.addEventListener("blur",function(n){var e=n.target;e&&e.id&&e.id.indexOf("mm-op-")===0&&a()},!0),window.addEventListener("beforeunload",a)}function it(t){var a=document.getElementById("mm-op-cep-status");a&&(a.className="mm-input-status is-loading",a.textContent="Buscando..."),mt(t,function(n){if(a&&(a.className="mm-input-status"),!n){a&&(a.className="mm-input-status is-error",a.textContent="CEP não encontrado",setTimeout(function(){a.className="mm-input-status",a.textContent=""},2500));return}a&&(a.className="mm-input-status is-ok",a.innerHTML=B.check,setTimeout(function(){a.className="mm-input-status",a.innerHTML=""},1800));var e=[["mm-op-rua",n.logradouro],["mm-op-bairro",n.bairro],["mm-op-cidade",n.cidade],["mm-op-uf",n.estado]];e.forEach(function(s){var g=document.getElementById(s[0]);!g||!s[1]||(!g.value||g.dataset.mmCepFill==="1")&&(g.value=s[1],g.dataset.mmCepFill="1")});var r=document.getElementById("mm-op-num");r&&setTimeout(function(){r.focus()},100),Vn._t&&clearTimeout(Vn._t),Vn._t=setTimeout(Vn,200)})}function ct(t){if(!document.getElementById("mm-op-overlay")){var a=document.createElement("div");a.id="mm-op-overlay",a.innerHTML='<div class="mm-op-overlay-card"><div class="mm-op-overlay-spinner"></div><p class="mm-op-overlay-text">'+O(t||"Processando...")+"</p></div>",document.body.appendChild(a)}}function o(t){var a=t.nome.trim(),n=t.email.trim(),e=t.rua.trim(),r=t.bairro.trim(),s=t.cidade.trim(),g=t.uf.trim(),b=yn(t.cep.replace(/\D/g,""));tn.set("mm_user_email",n);var w=function(E,j){var L=c.querySelector(E);if(!L)return!1;try{var Z=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;Z.call(L,j)}catch{L.value=j}return L.dispatchEvent(new Event("input",{bubbles:!0})),L.dispatchEvent(new Event("change",{bubbles:!0})),L.dispatchEvent(new Event("blur",{bubbles:!0})),!0};w("#nome-completo_2",a),w("#cpf_2",t.cpf),w("#email_3",n),w("#telefone_2",t.tel),w("#cep",b),w("#destinatario",a),w("#logradouro",e),w("#numero",t.num),w("#complemento",t.comp),w("#bairro",r),w("#cidade",s),w("#estado",g),ct("Indo para o pagamento..."),setTimeout(function(){var E=document.getElementById("mm-layout");E&&(E.style.display="none"),c.classList.remove("mm-shadow-mode");function j(N){var H=c.querySelector("#"+N);return H?H.closest("form"):null}function L(){var N=j("nome-completo_2");if(!N)return!1;var H=N.querySelector('button[type="submit"], input[type="submit"], button:not([type])');return H?(H.click(),!0):typeof N.requestSubmit=="function"?(N.requestSubmit(),!0):(N.submit(),!0)}function Z(){var N=document.querySelector(".mz-toast-popup.error, .swal2-toast.swal2-icon-error");if(N&&/inativo|realize login/i.test(N.textContent||""))return"error";for(var H=document.querySelectorAll('button, [type="submit"]'),an=0;an<H.length;an++){var en=(H[an].textContent||"").toLowerCase();if(en.indexOf("cadastrar endere")!==-1&&H[an].offsetParent!==null)return"ready"}return"wait"}function W(){var N=j("cep");if(!N)return!1;var H=N.querySelector('button[type="submit"], input[type="submit"], button:not([type])');return H?(H.click(),!0):typeof N.requestSubmit=="function"?(N.requestSubmit(),!0):(N.submit(),!0)}setTimeout(function(){L();var N=0;(function H(){N++;var an=Z();if(an==="error"){var en=document.getElementById("mm-op-overlay");en&&en.remove();return}if(an!=="ready"&&N<20){setTimeout(H,400);return}W();var xn=0;function hn(){xn++;var zn=document.querySelector('input[name="forma-pagto"], #forma-pagto-pix, #forma-pagto-cartao, #forma-pagto-boleto');if(zn&&zn.offsetParent!==null){try{p(t)}catch{var Wn=document.getElementById("mm-op-overlay");Wn&&Wn.remove();var S=document.getElementById("mm-layout");S&&(S.style.display="none")}return}if(xn<30)setTimeout(hn,250);else{var C=document.getElementById("mm-op-overlay");C&&C.remove()}}setTimeout(hn,800)})()},150)},100)}function l(){var t={pix:null,cartao:null,boleto:null},a=[];try{a=window.Zord&&window.Zord.Calculo&&window.Zord.Calculo.FormasPagamentoPedido||[]}catch{}a.forEach(function(L){var Z=L.formaPagamento&&L.formaPagamento.id,W=L.formaPagamento&&L.formaPagamento.isCartao,N=L.condicoes||[];!N.length||!W||(!t.cartao||N.length>t.cartao.condicoes.length)&&(t.cartao={formaId:Z,valorTotal:N[0].valorTotal,condicoes:N.map(function(H){return{nome:H.condicaoPagamento&&H.condicaoPagamento.nome,numParcelas:H.condicaoPagamento&&H.condicaoPagamento.numeroParcelas,valorParcela:H.valorParcela,valorTotal:H.valorTotal}})})});function n(L){if(!L)return 0;var Z=L.getAttribute&&L.getAttribute("data-valor-total");if(Z){var W=parseFloat(Z);if(W>0)return W}var N=(L.textContent||"").replace(/[^\d,.]/g,"");return N.indexOf(",")!==-1&&(N=N.replace(/\./g,"").replace(",",".")),parseFloat(N)||0}var e=document.querySelector("#valor-total-pedido-pix, .valor-total-pix[data-valor-total]"),r=n(e);if(r>0)t.pix={formaId:17,valorTotal:r};else{var s=a.find&&a.find(function(L){return L.formaPagamento&&L.formaPagamento.id===17});s&&s.condicoes&&s.condicoes[0]&&(t.pix={formaId:17,valorTotal:s.condicoes[0].valorTotal})}var g=document.querySelector("#valor-total-pedido-boleto, .valor-total-boleto[data-valor-total]"),b=n(g);if(b>0)t.boleto={valorTotal:b};else{var w=a.find&&a.find(function(L){var Z=L.formaPagamento&&L.formaPagamento.id,W=L.formaPagamento&&L.formaPagamento.isCartao;return!W&&Z!==17&&L.condicoes&&L.condicoes.length});w&&(t.boleto={formaId:w.formaPagamento.id,valorTotal:w.condicoes[0].valorTotal})}if(!t.cartao){var E=document.querySelector(".valor-parcela-cartao");if(E){var j=n(E);j>0&&(t.cartao={valorTotal:j*12,condicoes:[]})}}return t}function p(t){var a=mn(),n=l();c.classList.add("mm-shadow-mode"),document.body.classList.add("mm-checkout-rebuild");var e=document.getElementById("mm-layout");(!e||e.parentElement!==c)&&(e&&e.parentElement&&e.parentElement.removeChild(e),e=document.createElement("div"),e.id="mm-layout",c.insertBefore(e,c.firstChild)),e.className="mm-op-layout mm-op-step3-layout",e.style.display="",e.innerHTML=y(a,t,n),document.documentElement.classList.remove("mm-cart-loading");var r=document.getElementById("mm-op-overlay");r&&r.remove();try{window.scrollTo({top:0,behavior:"smooth"})}catch{window.scrollTo(0,0)}J(t,n)}function y(t,a,n){var e=typeof renderGlobalFooter=="function"?renderGlobalFooter():"";return on("payment")+'<main class="mm-op-main"><div class="mm-op-step3-grid"><section class="mm-op-step3-left">'+k(n)+I()+'</section><aside class="mm-op-step3-sum-wrap">'+F(a)+_(t,n,"pix")+"</aside></div></main>"+e}function F(t){var a=t||{},n=O(a.nome||""),e=O(a.email||""),r=O(a.cpf||""),s=O(a.tel||""),g=O(a.rua||""),b=O(a.num||""),w=a.comp?", "+O(a.comp):"",E=O(a.bairro||""),j=O(a.cidade||""),L=O(a.uf||""),Z=O(a.cep||"");return'<div class="mm-op-step3-completed"><div class="mm-op-completed-card" data-section="dados"><div class="mm-op-completed-head"><span class="mm-op-completed-check">'+B.check+'</span><h3 class="mm-op-completed-title">Dados pessoais</h3><a href="/checkout/onepage" class="mm-op-completed-edit" data-mm-act="edit-dados" aria-label="Editar dados">'+Rn.editPencil+' Editar</a></div><dl class="mm-op-completed-body">'+(n?"<div><dt>Nome</dt><dd>"+n+"</dd></div>":"")+(e?"<div><dt>E-mail</dt><dd>"+e+"</dd></div>":"")+(r?"<div><dt>CPF</dt><dd>"+r+"</dd></div>":"")+(s?"<div><dt>Telefone</dt><dd>"+s+"</dd></div>":"")+'</dl></div><div class="mm-op-completed-card" data-section="endereco"><div class="mm-op-completed-head"><span class="mm-op-completed-check">'+B.check+'</span><h3 class="mm-op-completed-title">Endereço de entrega</h3><a href="/checkout/onepage" class="mm-op-completed-edit" data-mm-act="edit-endereco" aria-label="Editar endereço">'+Rn.editPencil+' Editar</a></div><address class="mm-op-completed-address">'+g+", "+b+w+"<br>"+E+" — "+j+"/"+L+"<br>"+(Z?"CEP "+Z:"")+"</address></div></div>"}function k(t){var a=t.pix?t.pix.valorTotal:0,n=t.cartao?t.cartao.valorTotal:0,e=t.boleto?t.boleto.valorTotal:0,r=n>a?n-a:0,s=null;t.cartao&&t.cartao.condicoes&&t.cartao.condicoes.length&&(s=t.cartao.condicoes[t.cartao.condicoes.length-1]);var g=s?"até "+s.numParcelas+"x de "+D(s.valorParcela)+" sem juros":n>0?"até 12x sem juros":"Cartão de crédito",b='<label class="mm-op-pay-radio is-active" data-forma="pix"><input type="radio" name="mm-pay" value="pix" checked><div class="mm-op-pay-head"><span class="mm-op-pay-radio-dot" aria-hidden="true"></span><span class="mm-op-pay-icon">'+Rn.pix+'</span><div class="mm-op-pay-body"><span class="mm-op-pay-title">PIX</span><span class="mm-op-pay-sub">Aprovação instantânea · em 1 minuto</span></div><div class="mm-op-pay-price">'+(r>0?'<span class="mm-op-pay-badge-save">Economize '+D(r)+"</span>":"")+'<span class="mm-op-pay-amount">'+D(a)+'</span></div></div><div class="mm-op-pay-detail"><ul class="mm-op-pay-benefits"><li>'+B.check+"<span>QR Code e Copia-e-Cola após confirmar</span></li><li>"+B.check+"<span>Pedido aprovado em até 1 minuto</span></li><li>"+B.check+"<span>Pagamento 100% seguro · sem dados de cartão</span></li></ul></div></label>",w='<div class="mm-op-pay-brands"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-visa.svg" alt="Visa" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-mastercard.svg" alt="Mastercard" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-american-express.svg" alt="American Express" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-elo.svg" alt="Elo" width="32" height="20"><img src="https://public-resources.zordcdn.com.br/assets/global/payment-vector/cartao-hipercard.svg" alt="Hipercard" width="32" height="20"></div>',E='<span class="mm-op-req" aria-hidden="true">*</span>';function j(N){return'<span class="mm-op-field-err" id="'+N+'-err" role="alert" aria-live="polite"></span>'}var L='<div class="mm-op-card-form"><div class="mm-op-card-field mm-op-card-field-full"><label for="mm-pay-card-num">Número do cartão'+E+'</label><div class="mm-input-wrap mm-input-wrap-card"><span class="mm-input-icon" aria-hidden="true">'+B.card+'</span><input id="mm-pay-card-num" type="tel" class="mm-input" inputmode="numeric" autocomplete="off" data-mmac="cc-number" placeholder="0000 0000 0000 0000" maxlength="23" aria-describedby="mm-pay-card-num-err" aria-required="true" disabled><span class="mm-op-card-brand-detected" aria-live="polite"></span></div>'+j("mm-pay-card-num")+'</div><div class="mm-op-card-field mm-op-card-field-full"><label for="mm-pay-card-name">Nome impresso no cartão'+E+'</label><input id="mm-pay-card-name" type="text" class="mm-input mm-input-noicon" autocomplete="off" data-mmac="cc-name" placeholder="Como aparece no cartão" maxlength="100" aria-describedby="mm-pay-card-name-err" aria-required="true" disabled>'+j("mm-pay-card-name")+'</div><div class="mm-op-card-field mm-op-card-field-half"><label for="mm-pay-card-exp">Validade'+E+'</label><input id="mm-pay-card-exp" type="tel" class="mm-input mm-input-noicon" inputmode="numeric" autocomplete="off" data-mmac="cc-exp" placeholder="MM/AA" maxlength="5" aria-describedby="mm-pay-card-exp-err" aria-required="true" disabled>'+j("mm-pay-card-exp")+'</div><div class="mm-op-card-field mm-op-card-field-half"><label for="mm-pay-card-cvv">CVV'+E+'</label><input id="mm-pay-card-cvv" type="tel" class="mm-input mm-input-noicon" inputmode="numeric" autocomplete="off" data-mmac="cc-csc" placeholder="000" maxlength="4" aria-describedby="mm-pay-card-cvv-err" aria-required="true" disabled>'+j("mm-pay-card-cvv")+'</div><div class="mm-op-card-field mm-op-card-field-full"><label for="mm-pay-card-installments">Condições de pagamento'+E+'</label><select id="mm-pay-card-installments" class="mm-input mm-input-noicon mm-op-card-installments" aria-describedby="mm-pay-card-installments-err" aria-required="true" disabled><option value="">Digite o cartão pra ver as condições</option></select>'+j("mm-pay-card-installments")+"</div></div>",Z='<label class="mm-op-pay-radio" data-forma="cartao"><input type="radio" name="mm-pay" value="cartao"><div class="mm-op-pay-head"><span class="mm-op-pay-radio-dot" aria-hidden="true"></span><span class="mm-op-pay-icon">'+Rn.cardBig+'</span><div class="mm-op-pay-body"><span class="mm-op-pay-title">Cartão de Crédito</span><span class="mm-op-pay-sub">'+O(g)+'</span></div><div class="mm-op-pay-price"><span class="mm-op-pay-amount">'+D(n)+'</span></div></div><div class="mm-op-pay-detail">'+w+L+"</div></label>",W='<label class="mm-op-pay-radio" data-forma="boleto"><input type="radio" name="mm-pay" value="boleto"><div class="mm-op-pay-head"><span class="mm-op-pay-radio-dot" aria-hidden="true"></span><span class="mm-op-pay-icon">'+Rn.barcode+'</span><div class="mm-op-pay-body"><span class="mm-op-pay-title">Boleto Bancário</span><span class="mm-op-pay-sub">Aprovação em 1 a 3 dias úteis</span></div><div class="mm-op-pay-price"><span class="mm-op-pay-amount">'+D(e)+'</span></div></div><div class="mm-op-pay-detail"><ul class="mm-op-pay-benefits"><li>'+B.check+"<span>Código de barras enviado por e-mail</span></li><li>"+B.check+"<span>Pagamento em banco, lotérica ou app</span></li><li>"+B.check+"<span>Vencimento em 3 dias úteis</span></li></ul></div></label>";return'<section class="mm-op-step3-card" aria-labelledby="mm-op-step3-h"><div class="mm-op-step3-heading"><h2 id="mm-op-step3-h" class="mm-h">Como você quer pagar?</h2><p class="mm-op-step3-sub">Última etapa · escolha sua forma de pagamento '+(r>0?"· PIX tem desconto de "+D(r):"")+'</p></div><div class="mm-op-pay-radios" role="radiogroup" aria-label="Forma de pagamento">'+b+Z+W+'</div><button type="button" class="mm-cta mm-op-cta mm-op-finalizar" data-mm-act="finalizar-compra">'+B.lock+'<span class="mm-op-finalizar-label">Finalizar compra · '+D(a)+'</span></button><p class="mm-op-finalizar-sub">'+B.shield+"<span>Pagamento seguro · criptografia SSL-256 · seus dados não são armazenados</span></p></section>"}function I(){return'<div class="mm-op-trust-payment"><div class="mm-op-trust-payment-row"><span class="mm-trust-item">'+B.lock+'<span>Site 100% seguro</span></span><span class="mm-trust-item">'+B.shield+'<span>Garantia 12 meses</span></span><span class="mm-trust-item">'+B.rotate+'<span>7 dias pra trocar</span></span><span class="mm-trust-item">'+B.whats+'<span>Atendimento humano</span></span></div><p class="mm-op-trust-payment-note">Ao finalizar, você concorda com os <a href="/termos-de-uso" target="_blank" rel="noopener">termos de compra</a> e a <a href="/politica-privacidade" target="_blank" rel="noopener">política de privacidade</a>.</p></div>'}function _(t,a,n){if(!t||!t.items||!t.items.length)return'<aside class="mm-id-sum mm-sum mm-op-step3-sum"><h2 class="mm-h">Resumo</h2><div class="mm-sum-card"><p class="mm-sum-empty">Não conseguimos carregar o resumo do seu pedido.</p></div></aside>';var e=a.pix?a.pix.valorTotal:0,r=a.cartao?a.cartao.valorTotal:0,s=a.boleto?a.boleto.valorTotal:0,g=r>e?r-e:0,b=n==="pix"?e:n==="boleto"?s:r,w=n==="pix"?"no PIX":n==="boleto"?"no boleto":"no cartão",E=3,j=t.items.slice(0,E),L=t.items.length-E,Z=j.map(function(en){var xn=en.imgSrc?'<img src="'+O(en.imgSrc)+'" alt="'+O(en.name)+'" loading="lazy">':'<div class="mm-id-thumb-placeholder">'+B.box+"</div>",hn=en.quantity>1?'<strong class="mm-id-thumb-qty">'+en.quantity+"×</strong> ":"",zn=en.lineTotalPix>0?en.lineTotalPix:en.lineTotal;return'<div class="mm-id-thumb"><div class="mm-id-thumb-img">'+xn+'</div><div class="mm-id-thumb-body"><p class="mm-id-thumb-name">'+hn+O(en.name)+"</p>"+(en.variant?'<p class="mm-id-thumb-variant">'+O(en.variant)+"</p>":"")+'</div><div class="mm-id-thumb-price">'+D(zn)+"</div></div>"}).join("");L>0&&(Z+='<div class="mm-id-thumb-more">+ '+L+" "+(L===1?"item":"itens")+" a mais</div>");var W=t.subtotalFull>0?t.subtotalFull:t.subtotalPix,N='<div class="mm-row"><span class="mm-row-label">Subtotal</span><span class="mm-row-value">'+D(W)+"</span></div>";if(t.shipping!==null&&t.shipping!==void 0){var H=t.shipping===0?'<span class="mm-row-value is-free">'+B.check+" Grátis</span>":'<span class="mm-row-value">'+D(t.shipping)+"</span>";N+='<div class="mm-row"><span class="mm-row-label">Frete'+(t.shippingDeadline?' <span class="mm-row-sub">· '+O(t.shippingDeadline)+"</span>":"")+"</span>"+H+"</div>"}t.discount>0&&(N+='<div class="mm-row"><span class="mm-row-label">Cupom'+(t.couponCode?' <span class="mm-row-sub">· '+O(t.couponCode)+"</span>":"")+'</span><span class="mm-row-value is-discount">− '+D(t.discount)+"</span></div>"),g>0&&n==="pix"&&(N+='<div class="mm-row mm-row-pix-discount"><span class="mm-row-label">'+B.bolt+' Desconto PIX</span><span class="mm-row-value is-discount">− '+D(g)+"</span></div>");var an='<div class="mm-total"><div class="mm-total-label">Total</div><div class="mm-total-value" data-mm-total>'+D(b)+'</div><div class="mm-total-pix" data-mm-total-sub><span>'+w+"</span></div></div>";return'<aside class="mm-id-sum mm-sum mm-op-step3-sum" data-active-forma="'+O(n)+'"><h2 class="mm-h">Resumo do pedido</h2><div class="mm-sum-card"><div class="mm-id-thumbs">'+Z+'</div><div class="mm-sum-rows">'+N+"</div>"+an+"</div></aside>"}function J(t,a){var n=document.getElementById("mm-layout");if(!n||n._mmStep3Bound)return;n._mmStep3Bound=!0;var e=mn(),r={activeForma:"pix",cardNumValid:!1,cardBrand:null,cardInstallments:null,submitting:!1};n.addEventListener("click",function(S){var C=S.target.closest(".mm-op-pay-radio");if(C){var X=C.querySelector('input[type="radio"]');X&&!X.checked&&(S.preventDefault(),X.checked=!0,E(C.getAttribute("data-forma")));return}if(S.target.closest('[data-mm-act="finalizar-compra"]')){S.preventDefault(),Wn();return}var nn=S.target.closest('[data-mm-act="edit-dados"], [data-mm-act="edit-endereco"]');if(nn){S.preventDefault(),location.reload();return}}),n.addEventListener("input",function(S){var C=S.target;!C||!C.id||(C.id==="mm-pay-card-num"?N(C):C.id==="mm-pay-card-exp"?H(C):C.id==="mm-pay-card-cvv"&&(C.value=(C.value||"").replace(/\D/g,"").slice(0,4)))}),n.addEventListener("change",function(S){if(S.target&&S.target.id==="mm-pay-card-installments"){var C=S.target,X=C.options[C.selectedIndex];X&&X.value&&(r.cardInstallments={numero:parseInt(X.value,10),label:X.textContent||""},xn(X.value),g("mm-pay-card-installments"))}}),n.addEventListener("blur",function(S){var C=S.target;if(!(!C||!C.id)){var X=["mm-pay-card-num","mm-pay-card-name","mm-pay-card-exp","mm-pay-card-cvv"];X.indexOf(C.id)!==-1&&b(C.id)}},!0),n.addEventListener("focus",function(S){var C=S.target;!C||!C.id||/^mm-pay-card-/.test(C.id)&&g(C.id)},!0);function s(S,C){var X=document.getElementById(S),nn=document.getElementById(S+"-err");X&&(X.classList.add("mm-input-error"),X.setAttribute("aria-invalid","true")),nn&&(nn.textContent=C,nn.classList.add("is-visible"))}function g(S){var C=document.getElementById(S),X=document.getElementById(S+"-err");C&&(C.classList.remove("mm-input-error"),C.removeAttribute("aria-invalid")),X&&(X.textContent="",X.classList.remove("is-visible"))}function b(S){var C=document.getElementById(S);if(!C)return!0;var X=(C.value||"").trim();if(S==="mm-pay-card-num"){var nn=X.replace(/\D/g,"");return nn?nn.length<13?(s(S,"Número do cartão incompleto"),!1):w(nn)?(g(S),!0):(s(S,"Número do cartão inválido — confira os dígitos"),!1):(s(S,"Informe o número do cartão"),!1)}if(S==="mm-pay-card-name")return X?X.split(/\s+/).length<2?(s(S,"Use o nome completo como aparece no cartão"),!1):(g(S),!0):(s(S,"Informe o nome impresso no cartão"),!1);if(S==="mm-pay-card-exp"){var Y=X.replace(/\D/g,"");if(Y.length!==4)return s(S,"Informe a validade no formato MM/AA"),!1;var vn=parseInt(Y.slice(0,2),10),bn=parseInt(Y.slice(2),10);if(vn<1||vn>12)return s(S,"Mês inválido (01 a 12)"),!1;var qn=new Date,An=qn.getFullYear()%100,Tn=qn.getMonth()+1;return bn<An||bn===An&&vn<Tn?(s(S,"Cartão expirado"),!1):(g(S),!0)}if(S==="mm-pay-card-cvv"){var Zn=X.replace(/\D/g,"");return Zn.length<3?(s(S,"CVV deve ter 3 ou 4 dígitos"),!1):(g(S),!0)}return S==="mm-pay-card-installments"?X?(g(S),!0):(s(S,"Selecione o número de parcelas"),!1):!0}function w(S){for(var C=0,X=!1,nn=S.length-1;nn>=0;nn--){var Y=parseInt(S.charAt(nn),10);X&&(Y*=2,Y>9&&(Y-=9)),C+=Y,X=!X}return C%10===0}function E(S){if(!(!S||r.activeForma===S)){r.activeForma=S,n.querySelectorAll(".mm-op-pay-radio").forEach(function(Y){Y.classList.toggle("is-active",Y.getAttribute("data-forma")===S)});var C=document.getElementById("forma-pagto-"+S);if(C&&!C.checked)try{C.click()}catch{}var X=n.querySelectorAll(".mm-op-card-form input, .mm-op-card-form select"),nn=S==="cartao";X.forEach(function(Y){Y.disabled=!nn,nn?Y.dataset.mmac&&Y.setAttribute("autocomplete",Y.dataset.mmac):Y.setAttribute("autocomplete","off")}),j(S),S==="cartao"&&setTimeout(function(){var Y=document.getElementById("mm-pay-card-num");Y&&!Y.value&&Y.focus()},250)}}function j(S){var C=n.querySelector(".mm-op-step3-sum");if(C){C.setAttribute("data-active-forma",S);var X=a.pix?a.pix.valorTotal:0,nn=a.cartao?a.cartao.valorTotal:0,Y=a.boleto?a.boleto.valorTotal:0,vn=nn>X?nn-X:0,bn=S==="pix"?X:S==="boleto"?Y:nn,qn=S==="pix"?"no PIX":S==="boleto"?"no boleto":"no cartão",An=C.querySelector("[data-mm-total]");if(An){var Tn=An.textContent||"",Zn=V(Tn);Zn!==bn?L(An,Zn,bn,360):An.textContent=D(bn)}var Nn=C.querySelector("[data-mm-total-sub] span");Nn&&Nn.textContent!==qn&&(Nn.textContent=qn);var nt=C.querySelector(".mm-sum-rows"),tt=nt?nt.querySelector(".mm-row-pix-discount"):null;if(S==="pix"&&vn>0){if(!tt&&nt){var ut=document.createElement("div");ut.innerHTML='<div class="mm-row mm-row-pix-discount"><span class="mm-row-label">'+B.bolt+' Desconto PIX</span><span class="mm-row-value is-discount">− '+D(vn)+"</span></div>",nt.appendChild(ut.firstChild)}}else tt&&tt.remove();Z(S)}}function L(S,C,X,nn){S._mmAnimToken&&cancelAnimationFrame(S._mmAnimToken);var Y=null,vn=X-C;function bn(qn){Y||(Y=qn);var An=qn-Y,Tn=Math.min(1,An/nn),Zn=1-Math.pow(1-Tn,3),Nn=C+vn*Zn;S.textContent=D(Nn),Tn<1?S._mmAnimToken=requestAnimationFrame(bn):(S.textContent=D(X),S._mmAnimToken=null)}S._mmAnimToken=requestAnimationFrame(bn)}function Z(S){var C=n.querySelector(".mm-op-finalizar-label");if(C){var X=S==="pix"?a.pix&&a.pix.valorTotal:S==="boleto"?a.boleto&&a.boleto.valorTotal:a.cartao&&a.cartao.valorTotal;C.textContent="Finalizar compra · "+D(X||0)}}function W(S){var C=(S||"").replace(/\D/g,"");return C?/^4/.test(C)?"visa":/^(5[1-5]|22[2-9]|2[3-6]|27[01]|2720)/.test(C)?"mastercard":/^3[47]/.test(C)?"amex":/^(4011|4312|4389|4514|4573|5041|5066|5067|509|6277|6362|6363|650|6516|6550)/.test(C)?"elo":/^(606282|384100|384140|384160|606|637095|637568|637599|637609|637612)/.test(C)?"hipercard":null:null}function N(S){var C=(S.value||"").replace(/\D/g,"").slice(0,19),X=C.replace(/(\d{4})(?=\d)/g,"$1 ");if(X!==S.value){var nn=S.selectionStart;S.value=X;try{S.setSelectionRange(nn+1,nn+1)}catch{}}var Y=W(C);r.cardBrand=Y,r.cardNumValid=C.length>=13;var vn=n.querySelector(".mm-op-card-brand-detected");vn&&(vn.className="mm-op-card-brand-detected"+(Y?" is-"+Y:""),vn.textContent=Y?Y.toUpperCase():""),C.length>=6&&(an(C),zn())}function H(S){var C=(S.value||"").replace(/\D/g,"").slice(0,4),X=C.length>2?C.slice(0,2)+"/"+C.slice(2):C;if(S.value=X,C.length===4){var nn=C.slice(0,2),Y="20"+C.slice(2);en("pag-cartao-mes-validade",String(parseInt(nn,10))),en("pag-cartao-ano-validade",Y)}}function an(S){var C=document.getElementById("pag-cartao-numero");if(C){try{var X=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;X.call(C,S)}catch{C.value=S}C.dispatchEvent(new Event("input",{bubbles:!0})),C.dispatchEvent(new Event("change",{bubbles:!0})),C.dispatchEvent(new Event("blur",{bubbles:!0}))}}function en(S,C){var X=document.getElementById(S);if(X){try{var nn=Object.getOwnPropertyDescriptor(window.HTMLSelectElement.prototype,"value").set;nn.call(X,C)}catch{X.value=C}X.dispatchEvent(new Event("change",{bubbles:!0}))}}function xn(S){en("pag-cartao-parcela",S)}var hn=null;function zn(){if(hn)return;var S=document.getElementById("pag-cartao-parcela");if(!S)return;var C=document.getElementById("mm-pay-card-installments");if(!C)return;function X(){var nn=S.querySelectorAll("option");if(!(nn.length<=1)){var Y="";nn.forEach(function(vn){if(!vn.value){Y+='<option value="">Selecione as parcelas</option>';return}Y+='<option value="'+O(vn.value)+'">'+O(vn.textContent.trim())+"</option>"}),C.innerHTML=Y,C.options.length>1&&(C.selectedIndex=1,r.cardInstallments={numero:parseInt(C.options[1].value,10)||1,label:C.options[1].textContent},xn(C.options[1].value))}}X(),hn=new MutationObserver(X),hn.observe(S,{childList:!0,subtree:!0})}function Wn(){if(console.log("[mm-checkout] submitFinalizar() entry",{submitting:r.submitting,activeForma:r.activeForma}),r.submitting)return;var S=r.activeForma,C=n.querySelector(".mm-op-finalizar");if(!C){console.log("[mm-checkout] submitFinalizar: no .mm-op-finalizar btn");return}if(S==="cartao"){var X=["mm-pay-card-num","mm-pay-card-name","mm-pay-card-exp","mm-pay-card-cvv","mm-pay-card-installments"],nn=X.filter(function(cn){return!b(cn)});if(console.log("[mm-checkout] validation",{errorCount:nn.length,errors:nn}),nn.length){var Y=document.getElementById(nn[0]);if(Y){Y.focus();try{Y.scrollIntoView({block:"center",behavior:"smooth"})}catch{}}return}var vn=document.getElementById("mm-pay-card-name"),bn=document.getElementById("mm-pay-card-cvv"),qn=document.getElementById("pag-cartao-titular");if(qn){try{var An=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;An.call(qn,vn.value.trim())}catch{qn.value=vn.value.trim()}qn.dispatchEvent(new Event("input",{bubbles:!0})),qn.dispatchEvent(new Event("blur",{bubbles:!0}))}var Tn=document.getElementById("pag-cartao-vericacao");if(Tn){try{var Zn=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;Zn.call(Tn,bn.value.replace(/\D/g,""))}catch{Tn.value=bn.value.replace(/\D/g,"")}Tn.dispatchEvent(new Event("input",{bubbles:!0})),Tn.dispatchEvent(new Event("blur",{bubbles:!0}))}var Nn=document.getElementById("pag-cartao-cpf");if(Nn&&t&&t.cpf){try{var nt=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;nt.call(Nn,t.cpf)}catch{Nn.value=t.cpf}Nn.dispatchEvent(new Event("input",{bubbles:!0})),Nn.dispatchEvent(new Event("blur",{bubbles:!0}))}}r.submitting=!0,C.classList.add("is-loading"),C.setAttribute("aria-busy","true");var tt=C.querySelector(".mm-op-finalizar-label");if(tt&&(tt.textContent="Processando pagamento..."),ct("Finalizando seu pedido..."),S==="cartao"){var ut=document.getElementById("mm-pay-card-installments");ut&&ut.value&&xn(ut.value)}window.__mmCheckoutDebug=window.__mmCheckoutDebug||[];function kn(cn,Hn){var Gn={t:new Date().toISOString(),msg:cn,data:Hn};window.__mmCheckoutDebug.push(Gn),console.log("[mm-checkout]",cn,Hn||"")}function ht(){if(kn("doSubmit() called",{forma:S}),S==="cartao"){var cn=document.getElementById("aceito-termos-bcash-one-card");cn&&!cn.checked&&(cn.checked=!0,cn.dispatchEvent(new Event("change",{bubbles:!0}))),kn("terms",{checked:cn?.checked})}var Hn=S==="pix"?"form-pag-pix":S==="boleto"?"form-pag-boleto":"form-pag-cartao",Gn=document.getElementById(Hn);if(!Gn){kn("ERROR: form not found",{formId:Hn}),alert("Erro interno: formulário de pagamento não encontrado. Recarregue a página."),r.submitting=!1,C.classList.remove("is-loading");var et=document.getElementById("mm-op-overlay");et&&et.remove();return}S==="cartao"&&kn("pre-submit cartao state",{numero:document.getElementById("pag-cartao-numero")?.value,bandeira:document.getElementById("pag-cartao-bandeira")?.value,parcela:document.getElementById("pag-cartao-parcela")?.value,titular:document.getElementById("pag-cartao-titular")?.value,mes:document.getElementById("pag-cartao-mes-validade")?.value,ano:document.getElementById("pag-cartao-ano-validade")?.value,cvv:document.getElementById("pag-cartao-vericacao")?.value?.length,cpf:document.getElementById("pag-cartao-cpf")?.value,tokenMp:document.getElementById("pag-cartao-token-mp")?.value?.substring(0,20)});var at=Gn.querySelector('button.button-success, button[type="submit"], input[type="submit"]');if(at)kn("clicking native button",{text:at.textContent?.trim()}),at.click();else if(typeof Gn.requestSubmit=="function"){kn("no native btn, using requestSubmit");try{Gn.requestSubmit()}catch(st){kn("requestSubmit error",st.message),Gn.submit()}}else kn("no native btn, using form.submit()"),Gn.submit();setTimeout(function(){if(r.submitting&&location.pathname.indexOf("/onepage")!==-1){kn("8s failsafe: still on /onepage, removing overlay"),r.submitting=!1,C.classList.remove("is-loading");var st=document.getElementById("mm-op-overlay");st&&st.remove(),c.classList.remove("mm-shadow-mode"),n&&(n.style.display="none")}},8e3),setTimeout(function(){c.classList.remove("mm-shadow-mode"),n&&(n.style.display="none")},600)}function wt(){r.submitting=!1,C.classList.remove("is-loading"),C.removeAttribute("aria-busy");var cn=C.querySelector(".mm-op-finalizar-label");cn&&(cn.textContent="Finalizar compra");var Hn=document.getElementById("mm-op-overlay");Hn&&Hn.remove()}function Ft(){var cn=Date.now(),Hn=1e4;(function Gn(){var et=document.getElementById("pag-cartao-token-mp"),at=et?(et.value||"").trim():"",st=at&&at!=="loading..."&&at.length>10;if(st){kn("fallback: token ready"),ht();return}if(Date.now()-cn>=Hn){kn("fallback: timeout",{lastVal:at}),ht();return}setTimeout(Gn,200)})()}function It(){if(kn("generateMpTokenAndSubmit() start"),typeof Mercadopago>"u"){kn("Mercadopago global missing, falling back to wait strategy"),Ft();return}var cn=document.getElementById("pag-cartao-token-mp"),Hn=cn?(cn.value||"").trim():"";if(Hn&&Hn!=="loading..."&&Hn.length>10){kn("token already present, submitting",{len:Hn.length}),ht();return}var Gn=(document.getElementById("mm-pay-card-num")?.value||"").replace(/\D/g,""),et=(document.getElementById("mm-pay-card-exp")?.value||"").replace(/\D/g,""),at=(document.getElementById("mm-pay-card-cvv")?.value||"").replace(/\D/g,""),st=(document.getElementById("mm-pay-card-name")?.value||"").trim(),Tt=(t&&t.cpf||document.getElementById("mm-op-cpf")?.value||"").replace(/\D/g,"");if(!Gn||!et||!at||!st||!Tt){kn("missing card fields",{num:Gn.length,exp:et.length,cvv:at.length,name:!!st,doc:Tt.length}),alert("Preencha todos os dados do cartão antes de finalizar."),wt();return}var Mt=et.slice(0,2),Lt=et.length===4?"20"+et.slice(2):et.slice(2);kn("calling Mercadopago.createToken",{numLen:Gn.length,month:Mt,year:Lt});try{Mercadopago.createToken({cardNumber:Gn,securityCode:at,cardExpirationMonth:Mt,cardExpirationYear:Lt,cardholderName:st,docType:"CPF",docNumber:Tt},function(kt,lt){if(kn("createToken callback",{status:kt,hasId:!!(lt&&lt.id),err:lt&&lt.error}),kt===200||kt===201){if(cn){var Pt=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,"value").set;Pt.call(cn,lt.id),cn.dispatchEvent(new Event("input",{bubbles:!0})),cn.dispatchEvent(new Event("change",{bubbles:!0}))}ht()}else{var Bt="Não foi possível validar os dados do cartão.";lt&&lt.cause&&lt.cause[0]&&lt.cause[0].description&&(Bt=lt.cause[0].description),alert(Bt),wt()}})}catch(kt){kn("createToken exception",kt.message),Ft()}}S==="cartao"?It():setTimeout(ht,500)}}if(T){let t=function(n){if(n=n||0,n>30){a();return}var e=c.querySelector("#cep, .box-area-dados, #nome-completo_2");e||n>8?a():setTimeout(function(){t(n+1)},250)},a=function(){var n=mn(),e=tn.get("mm_user_email")||"";tn.remove("mm_checkout_mode"),typeof window._mmDraftDebug>"u"&&(window._mmDraftDebug=!1),At(n,e),Kn();var r=jn(),s=tn.get(P),g=s&&String(s).replace(/\D/g,"").length===8;g?dt("loading"):n&&typeof n.shipping=="number"&&n.shipping>0&&dt({value:n.shipping,name:n.shippingName||"",deadline:n.shippingDeadline||"",city:n.shippingCity||"",options:n.shippingOptions||[]});try{var b=Array.from(c.querySelectorAll("a, button")).find(function(j){var L=(j.textContent||"").toLowerCase();return L.indexOf("sem cadastro")!==-1&&j.offsetParent!==null});b&&!b.classList.contains("active")&&b.click()}catch{}vt();var w=tn.get(P);if(w&&w.length===8){var E=document.getElementById("mm-op-cep");E&&(E.value=yn(w),setTimeout(function(){it(w)},400))}setTimeout(function(){if(!("ontouchstart"in window))for(var j=["mm-op-email","mm-op-nome","mm-op-cpf","mm-op-tel","mm-op-cep"],L=0;L<j.length;L++){var Z=document.getElementById(j[L]);if(Z&&!Z.value){Z.focus();break}}},350)};t()}if(A){document.documentElement.classList.remove("mm-cart-loading");var G=c.querySelector('input[placeholder*="numero do cart" i]');G&&(G.inputMode="numeric");var K=c.querySelector('input[placeholder*="000" i]');K&&(!K.maxLength||K.maxLength<=4)&&(K.inputMode="numeric")}setTimeout(function(){document.documentElement.classList.contains("mm-cart-loading")&&(console.warn("[mm-cart] failsafe: removing mm-cart-loading after 2s timeout"),document.documentElement.classList.remove("mm-cart-loading"))},2e3)})(),(function(){var P=window.location.pathname,R=/^\/login\/?$/.test(P),M=P.indexOf("/cliente/pedidos")===0;if(!R&&!M)return;var h="5511915299488",i='<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>';function m(v){return"https://api.whatsapp.com/send?phone="+h+"&text="+encodeURIComponent(v)}function d(v){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",v):v()}function f(){function v(q){var c=q.slice(0,14);if(c.length<=11)return c.length>9?c.slice(0,3)+"."+c.slice(3,6)+"."+c.slice(6,9)+"-"+c.slice(9):c.length>6?c.slice(0,3)+"."+c.slice(3,6)+"."+c.slice(6):c.length>3?c.slice(0,3)+"."+c.slice(3):c;var U=c.slice(0,2)+"."+c.slice(2,5)+"."+c.slice(5,8)+"/"+c.slice(8,12);return c.length>12&&(U+="-"+c.slice(12)),U}function T(q){if(!(!q||q.getAttribute("data-mm-mask"))){q.setAttribute("data-mm-mask","1"),q.setAttribute("maxlength","18"),q.setAttribute("inputmode","numeric"),q.addEventListener("input",function(){var U=v(this.value.replace(/\D/g,""));this.value!==U&&(this.value=U)});var c=q.form;c&&!c.getAttribute("data-mm-mask")&&(c.setAttribute("data-mm-mask","1"),c.addEventListener("submit",function(){q.value=v(q.value.replace(/\D/g,""))}))}}var A=0;(function q(){var c=document.getElementById("form-consulta-pedido"),U=document.getElementById("cpfcnpj");if(!c||!U)return++A<20?void setTimeout(q,250):void 0;T(U);var Q=c.querySelector(".title-area h2");if(Q&&!c.querySelector(".mm-cp-eyebrow")){var D=document.createElement("span");D.className="mm-cp-eyebrow",D.textContent="Acompanhe sua compra",Q.insertAdjacentElement("beforebegin",D)}function V(sn,gn){if(sn){var In=sn.closest(".line")||sn.parentElement;if(!In.querySelector(".mm-cp-label")){var Ln=document.createElement("label");Ln.className="mm-cp-label",Ln.textContent=gn,sn.id&&Ln.setAttribute("for",sn.id),In.insertAdjacentElement("afterbegin",Ln)}}}V(U,"CPF ou CNPJ"),V(document.getElementById("numero-pedido"),"Nº do pedido"),U.placeholder="000.000.000-00";var O=document.getElementById("numero-pedido");O&&(O.placeholder="Ex.: 0012606865731");var B=c.querySelector("button.button-login");if(B)for(var tn=0;tn<B.childNodes.length;tn++){var rn=B.childNodes[tn];rn.nodeType===3&&/consultar/i.test(rn.textContent)&&(rn.textContent=" Consultar meu pedido ")}var mn=c.querySelector(".cancel-consulta span");mn&&(mn.textContent="Voltar para login");var pn=document.getElementById("numero-pedido");if(pn&&!c.querySelector(".mm-cp-hint")){var Mn=document.createElement("span");Mn.className="mm-cp-hint",Mn.textContent="O número do pedido está no e‑mail de confirmação da compra.";var dn=pn.closest(".line")||pn.parentElement;dn.appendChild(Mn)}var jn=c.querySelector("form");if(jn&&!c.querySelector(".mm-cp-wa")){var Fn=document.createElement("div");Fn.className="mm-cp-wa",Fn.innerHTML='Não encontra os dados? <a href="'+m("Olá! Preciso de ajuda para consultar o meu pedido.")+'" target="_blank" rel="noopener">'+i+" Fale com a gente</a>",jn.insertAdjacentElement("afterend",Fn)}document.documentElement.classList.add("mm-consulta-on");var on=document.querySelector(".login-holder"),Bn=document.querySelector(".page.page-login");if(on&&Bn){var Sn=on.querySelector(".login-header");if(Sn&&!Sn.querySelector(".mm-lg-eyebrow")){var En=document.createElement("span");En.className="mm-lg-eyebrow",En.textContent="Área do cliente";var un=Sn.querySelector("h2");un&&un.insertAdjacentElement("beforebegin",En)}var Cn=on.querySelector(".social-login-area");if(Cn&&!on.querySelector(".mm-lg-ou")){var Xn=document.createElement("div");Xn.className="mm-lg-ou",Xn.textContent="ou",Cn.insertAdjacentElement("beforebegin",Xn)}setTimeout(function(){try{var sn=on.querySelector(".social-login-area .render-button");sn&&window.google&&google.accounts&&google.accounts.id&&(sn.innerHTML="",google.accounts.id.renderButton(sn,{theme:"outline",size:"large",shape:"pill",width:320,text:"continue_with",logo_alignment:"center"}))}catch{}setTimeout(function(){var gn=on.querySelector(".social-login-area"),In=on.querySelector(".mm-lg-ou"),Ln=gn&&gn.offsetParent!==null&&gn.querySelector("iframe");Ln||(In&&(In.style.display="none"),gn&&(gn.style.display="none"))},2e3)},1200);var yn=[].filter.call(on.querySelectorAll("span, div, button"),function(sn){return/pessoa jur/i.test(sn.textContent)&&sn.textContent.length<60&&sn.children.length===0})[0];yn&&yn.classList.add("mm-lg-link");var Un=document.querySelector(".footer-login");Un&&[].forEach.call(Un.querySelectorAll("p"),function(sn){/nunca postaremos/i.test(sn.textContent)&&sn.classList.add("mm-lg-nopost")}),document.documentElement.classList.add("mm-login-on")}})(),/#consultar?-?pedido/i.test(window.location.hash)&&setTimeout(function(){var q=document.getElementById("form-consulta-pedido"),c=document.getElementById("cpfcnpj");q&&q.scrollIntoView({behavior:"smooth",block:"center"}),c&&setTimeout(function(){c.focus()},500)},600)}var u={"recebimento-pedido":"Pedido recebido","confirmacao-pagamento":"Pagamento confirmado","emissao-nota":"Nota fiscal emitida",transporte:"Em transporte",entrega:"Entregue"};function z(){var v=document.querySelector(".detalhes-pedido");if(!(!v||v.getAttribute("data-mm-ped"))){v.setAttribute("data-mm-ped","1");var T=v.querySelector(".numero-pedido"),A=T?(T.textContent.match(/[\d]+/)||[""])[0]:"",q=v.querySelector(".resumo-data strong"),c=q?q.textContent.trim().split(" ")[0]:"",U=v.querySelectorAll(".item-historico.status-waiting");U.length&&U[0].classList.add("mm-atual");for(var Q='viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"',D={"recebimento-pedido":"<svg "+Q+'><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><path d="M3 6h18"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>',"confirmacao-pagamento":"<svg "+Q+'><rect width="20" height="14" x="2" y="5" rx="2"/><line x1="2" x2="22" y1="10" y2="10"/></svg>',"emissao-nota":"<svg "+Q+'><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7z"/><path d="M15 2v5h5"/><path d="M8 13h8"/><path d="M8 17h5"/></svg>',transporte:"<svg "+Q+'><path d="M14 18V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v11a1 1 0 0 0 1 1h2"/><path d="M15 18H9"/><path d="M19 18h2a1 1 0 0 0 1-1v-3.65a1 1 0 0 0-.22-.62l-3.48-4.35A1 1 0 0 0 17.52 8H14"/><circle cx="17" cy="18" r="2"/><circle cx="7" cy="18" r="2"/></svg>',entrega:"<svg "+Q+'><path d="M3 10.18v8.82a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-8.82"/><path d="M2.5 10.5 12 3l9.5 7.5"/><path d="m9 15 2 2 4-4"/></svg>'},V=v.querySelectorAll(".item-historico"),O=0;O<V.length;O++){var B=V[O];if(!B.querySelector(".mm-step-dot")){var tn="";for(var rn in D)if(B.classList.contains(rn)){tn=D[rn];break}var mn=document.createElement("span");mn.className="mm-step-dot",mn.innerHTML=tn,B.insertAdjacentElement("afterbegin",mn);var pn=document.createElement("span");pn.className="mm-step-line",B.appendChild(pn)}}var Mn=v.querySelectorAll(".item-historico.status-success"),dn="Pedido recebido";if(Mn.length){var jn=Mn[Mn.length-1];for(var Fn in u)if(jn.classList.contains(Fn)){dn=u[Fn];break}}if(!document.getElementById("mm-ped-hero")&&A){var on=document.createElement("div");on.id="mm-ped-hero",on.innerHTML='<div class="mm-ped-hero-info"><span class="mm-ped-eyebrow">Acompanhe sua compra</span><h1 class="mm-ped-num">Pedido #'+A+' <button type="button" class="mm-ped-copy" aria-label="Copiar número do pedido">copiar</button></h1><div class="mm-ped-meta">'+(c?"<span>Feito em "+c+"</span>":"")+'<span class="mm-ped-badge">'+dn+'</span></div></div><a class="mm-ped-hero-wa" target="_blank" rel="noopener" href="'+m("Olá! Gostaria de saber sobre o meu pedido #"+A+".")+'">'+i+" Falar sobre este pedido</a>",v.insertAdjacentElement("afterbegin",on);var Bn=v.querySelector(".resumo-pagamento .resumo-total > span:first-child");Bn&&/resumo do pedido/i.test(Bn.textContent)&&(Bn.textContent="Total");var Sn=v.querySelector(".title-itens-pedido h3");Sn&&/itens do pedido/i.test(Sn.textContent)&&(Sn.textContent=" Itens do pedido ");for(var En=document.querySelectorAll(".main-content h1, .main-content h2, .main-content .title-user-area, .main-content > div"),un=0;un<En.length;un++){var Cn=En[un];if(!Cn.contains(v)&&/meus pedidos/i.test(Cn.textContent||"")&&(Cn.textContent||"").trim().length<40){Cn.classList.add("mm-ped-native-title");break}}var Xn=on.querySelector(".mm-ped-copy");Xn.addEventListener("click",function(){var On=this;try{navigator.clipboard.writeText(A).then(function(){On.textContent="copiado ✓",On.classList.add("mm-copiado"),setTimeout(function(){On.textContent="copiar",On.classList.remove("mm-copiado")},2e3)})}catch{}})}var yn=v.querySelector(".rastreio-area"),Un=yn?yn.querySelector(".previsao-entrega .previsao"):null,sn=Un?Un.textContent.trim():"";if(sn){var gn=v.querySelector(".item-historico.entrega");if(gn&&!gn.querySelector(".mm-step-prev")){var In=document.createElement("span");In.className="mm-step-prev",In.textContent="Previsão: "+sn,gn.appendChild(In)}}var Ln=v.querySelector(".status-pagamento-pedido");if(Ln&&!yn&&!document.getElementById("mm-ped-entrega")){var $n=document.createElement("div");$n.id="mm-ped-entrega";for(var pt=null,wn=v.querySelectorAll("a[href]"),fn=0;fn<wn.length;fn++){var un=(wn[fn].textContent||"")+" "+wn[fn].href;if(/rastre/i.test(un)&&!/politica/i.test(wn[fn].href)){pt=wn[fn];break}}var ln='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#4b664a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14 18V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v11a1 1 0 0 0 1 1h2"/><path d="M15 18H9"/><path d="M19 18h2a1 1 0 0 0 1-1v-3.65a1 1 0 0 0-.22-.62l-3.48-4.35A1 1 0 0 0 17.52 8H14"/><circle cx="17" cy="18" r="2"/><circle cx="7" cy="18" r="2"/></svg>';pt?($n.classList.add("mm-tem-rastreio"),$n.innerHTML=ln+'<span>Seu pedido está a caminho — <a href="'+pt.href+'" target="_blank" rel="noopener">acompanhe o rastreio aqui</a>.</span>'):$n.innerHTML=ln+'<span>O código de rastreio aparece aqui assim que o pedido for despachado. Enquanto isso, acompanhe as etapas acima ou veja nossa <a href="/politica-de-entrega" target="_blank" rel="noopener">política de entrega</a>.</span>',Ln.insertAdjacentElement("afterend",$n)}var Qn=document.querySelector(".btn-comprar-novamente"),rt=document.querySelector(".btn-ajuda-pedido"),Pn=Qn||rt?(Qn||rt).parentElement:null;Pn&&!Pn.classList.contains("mm-ped-acoes")&&(Pn.classList.add("mm-ped-acoes"),Pn.parentElement!==v&&v.appendChild(Pn));var Yn=document.querySelector("main.central-cliente");Yn&&Yn.children.length===1&&Yn.classList.add("mm-ped-center"),document.documentElement.classList.add("mm-ped-on")}}d(function(){try{R&&f(),M&&z()}catch(v){window.console&&console.warn&&console.warn("[mm-pedidos]",v)}})})()})();
